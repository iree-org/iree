...
function(iree_single_backend_e2e_runner_test)
  ...
  foreach(_COMPILER_FLAG IN LISTS _RULE_COMPILER_FLAGS)
    ...
    if (_COMPILER_FLAG MATCHES "${_CPU_FEATURES_REGEX}")
      ...
      # Compile variants may include base ISA letters (+m,+a,+f,+d,+c) needed by
      # LLVM/ld; the runtime cpu feature check only knows real extensions.
      string(REPLACE "," ";" _REQ_LIST "${_CPU_FEATURES}")
      set(_REQ_FILTERED "")
      foreach(_F IN LISTS _REQ_LIST)
        if(_F MATCHES "^\\+[mafdc]$")
          continue()
        endif()
        list(APPEND _REQ_FILTERED "${_F}")
      endforeach()
      list(JOIN _REQ_FILTERED "," _CPU_FEATURES)
