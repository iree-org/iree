...
# CTS2 test suite infrastructure for HAL drivers.
#
# Two functions:
#
# 1. iree_hal_cts2_testdata() - Compiles CTS2 test executables for one format
#    and creates a testdata registration library. MLIR sources are discovered
#    via file(GLOB) from the testdata directory.
#
# 2. iree_hal_cts2_test_suite() - Creates CTS2 test binaries for a driver,
#    linking against pre-built testdata libraries from iree_hal_cts2_testdata().

# Template for generated testdata registration .cc files.
set(_IREE_CTS2_TESTDATA_TEMPLATE [=[
// Auto-generated executable format registration for CTS2.
...
function(_iree_cts2_camel_case INPUT OUTPUT_VAR)
...
#
# Compiles CTS2 test executables for one format and creates a testdata
...
function(iree_hal_cts2_testdata)
  ...
  if(NOT DEFINED _RULE_TESTDATA_DIR)
    message(SEND_ERROR "iree_hal_cts2_testdata requires TESTDATA_DIR")
  endif()
  ...
  _iree_cts2_camel_case("${_RULE_FORMAT_NAME}" _FUNC_NAME)
  ...
  string(CONFIGURE "${_IREE_CTS2_TESTDATA_TEMPLATE}" _GENERATED_CC @ONLY)
...
#
# Creates CTS2 test binaries for a HAL driver. Non-executable tests (buffer,
...
#   TESTDATA_LIBS: Testdata library targets from iree_hal_cts2_testdata().
...
function(iree_hal_cts2_test_suite)
  cmake_parse_arguments(
    _RULE
    ""
    "BACKENDS_LIB;NAME"
  ...
  if(NOT DEFINED _RULE_BACKENDS_LIB)
    message(SEND_ERROR "iree_hal_cts2_test_suite requires BACKENDS_LIB")
  endif()
  ...
  set(_COMMON_DEPS
    ${_RULE_BACKENDS_LIB}
    iree::hal::cts2::util::registry
    iree::hal::cts2::util::test_base
    iree::testing::gtest
  )

  set(_TEST_MAIN
    "${PROJECT_SOURCE_DIR}/runtime/src/iree/hal/cts2/util/test_main.cc"
  )
  ...
  foreach(_CATEGORY buffer command_buffer core file queue)
    iree_cc_test(
      ...
      DEPS
        ${_COMMON_DEPS}
        "iree::hal::cts2::${_CATEGORY}::all_tests"
      ${_ARGS_BLOCK}
      ${_LABELS_BLOCK}
    )
  endforeach()
  ...
  if(_RULE_TESTDATA_LIBS)
    set(_EXECUTABLE_SUITES
      "dispatch_tests\;iree::hal::cts2::command_buffer::all_dispatch_tests"
      "executable_tests\;iree::hal::cts2::core::all_executable_tests"
    )
