...
"""Generates CTS2 test binaries for a HAL driver.

Usage in a driver's cts/BUILD.bazel:

    load("//build_tools/bazel:build_defs.oss.bzl", "iree_runtime_cc_library")
    load("//build_tools/bazel:iree_hal_cts2_test_suite.bzl", "iree_hal_cts2_test_suite")
    ...
    iree_hal_cts2_test_suite(
        ...
        testdata = "//runtime/src/iree/hal/cts2/testdata:executable_srcs",
    )
"""
...
_NON_EXECUTABLE_SUITES = [
    ("buffer_tests", "//runtime/src/iree/hal/cts2/buffer:all_tests"),
    ("command_buffer_tests", "//runtime/src/iree/hal/cts2/command_buffer:all_tests"),
    ("core_tests", "//runtime/src/iree/hal/cts2/core:all_tests"),
    ("file_tests", "//runtime/src/iree/hal/cts2/file:all_tests"),
    ("queue_tests", "//runtime/src/iree/hal/cts2/queue:all_tests"),
]

# Executable-dependent test categories. Each entry maps a test binary name
# suffix to the aggregate test library it links.
_EXECUTABLE_SUITES = [
    ("dispatch_tests", "//runtime/src/iree/hal/cts2/command_buffer:all_dispatch_tests"),
    ("executable_tests", "//runtime/src/iree/hal/cts2/core:all_executable_tests"),
]
...
def _cts2_testdata_gen_impl(ctx):
...
_cts2_testdata_gen = rule(
    implementation = _cts2_testdata_gen_impl,
    attrs = {
...
def iree_hal_cts2_testdata(
        ...
        testonly = True,
        **kwargs):
    """Compiles CTS2 test executables and creates a testdata registration library.

    Use this directly when multiple iree_hal_cts2_test_suite() calls need to
    share the same compiled executables (e.g., CUDA graph/stream variants).
    For single-variant drivers, use executable_formats in iree_hal_cts2_test_suite
    ...
    _cts2_testdata_gen(
        name = gen_cc_name,
        template = "//runtime/src/iree/hal/cts2/util:testdata_format.cc.tpl",
        out = gen_cc_file,
        substitutions = {
...
def iree_hal_cts2_test_suite(
    ...
    Args:
        ...
        testdata_libs: Pre-built testdata library labels for multi-variant
            drivers. When multiple iree_hal_cts2_test_suite() calls share
        ...
        flag_values: Dict mapping string_flag build setting labels to
            placeholder names. Forwarded to iree_hal_cts2_testdata when
    ...
    if executable_formats and not testdata:
        fail("iree_hal_cts2_test_suite: testdata is required when executable_formats is provided")
    ...
    for format_name, config in executable_formats.items():
        lib_label = iree_hal_cts2_testdata(
            format_name = format_name,
    ...
    common_deps = [
        backends_lib,
        "//runtime/src/iree/hal/cts2/util:registry",
        "//runtime/src/iree/hal/cts2/util:test_base",
