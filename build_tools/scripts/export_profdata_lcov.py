...
def parse_arguments(argv):
    parser = argparse.ArgumentParser(description="LCOV coverage data exporter")
...
def main(args):
    ...
    source_files = [
        ...
        and os.path.splitext(file)[1] == ".profraw"
    ]
    if not source_files:
    ...
    subprocess.check_call(
        ...
        + [os.path.join(source_dir, file) for file in source_files]
    )
    ...
    for source_file in source_files:
        target_name = os.path.basename(source_file).split(".", 1)[0]
