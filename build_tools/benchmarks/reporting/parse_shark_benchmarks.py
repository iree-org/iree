...
def _generate_table(df_iree, df_shark, df_baseline, title):
    ...
    for dialect in dialects:
      ...
      for data_type in data_types:
        ...
        for device in device_types:
          ...
          iree_vs_baseline = format_latency_comparison(iree_latency,
                                                       baseline_latency)
          ...
          if df_shark is not None:
            ...
            shark_vs_baseline = format_latency_comparison(
                shark_latency, baseline_latency)
            iree_vs_shark = format_latency_comparison(iree_latency,
                                                      shark_latency)
          else:
  ...
  st = summary.style.set_table_styles(get_table_css())
  st = st.hide(axis='index')
  if df_shark is None:
    st = st.hide_columns(
        subset=[_SHARK_LATENCY, _SHARK_VS_BASELINE, _IREE_VS_SHARK])
  st = st.set_caption(title)
  st = st.applymap(style_performance, subset=_PERF_COLUMNS)
...
def main(args):
  ...
  verison_html = verison_html + '<i>last updated: ' + datetime.today().strftime(
      '%Y-%m-%d') + '</i><br/><br/>'
  html = generate_header_and_legend(verison_html)
