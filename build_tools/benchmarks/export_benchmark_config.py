...
from benchmark_suites.iree import (
    benchmark_collections as iree_benchmark_collections,
    benchmark_presets as iree_benchmark_presets,
)
...
def filter_and_group_run_configs(
  ...
  for run_config in run_configs:
    ...
    if (target_device_names is not None and
      ...
      continue
    if not (preset_set is None or preset_set.intersection(run_config.presets)):
      continue
...
def _export_execution_handler(
    benchmark_presets: Optional[Sequence[str]] = None,
    target_device_names: Optional[Sequence[str]] = None,
    **_unused_args):
  _, all_run_configs = iree_benchmark_collections.generate_benchmarks()
...
def _export_compilation_handler(
    benchmark_presets: Optional[Sequence[str]] = None, **_unused_args):
  all_gen_configs, _ = iree_benchmark_collections.generate_benchmarks()
  # Include all compilation benchmark presets if `presets`` is not set.
  preset_set = set(iree_benchmark_presets.COMPILATION_PRESETS
                   if benchmark_presets is None else benchmark_presets)
  compile_stats_gen_configs = [
      config for config in all_gen_configs
      if preset_set.intersection(config.presets)
  ]
...
def _parse_benchmark_presets(
    arg: str,
    supported_presets: Sequence[str],
) -> List[str]:
  presets = []
