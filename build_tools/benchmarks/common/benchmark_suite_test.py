...
class BenchmarkSuiteTest(unittest.TestCase):
  ...
  def test_load_from_run_configs(self):
    ...
    device_spec_b = common_definitions.DeviceSpec(
        ...
        host_environment=common_definitions.HostEnvironment.LINUX_X86_64,
        device_parameters=[])
    run_config_a = iree_definitions.E2EModelRunConfig.composite(
        module_generation_config=iree_definitions.ModuleGenerationConfig.
        composite(imported_model=iree_definitions.ImportedModel.from_model(
            model_tflite),
                  compile_config=iree_definitions.CompileConfig(
                      id="1", tags=[], compile_targets=[])),
        module_execution_config=exec_config_a,
        target_device_spec=device_spec_a,
        input_data=common_definitions.ZEROS_MODEL_INPUT_DATA)
    run_config_b = iree_definitions.E2EModelRunConfig.composite(
        module_generation_config=iree_definitions.ModuleGenerationConfig.
        composite(imported_model=iree_definitions.ImportedModel.from_model(
            model_tflite),
                  compile_config=iree_definitions.CompileConfig(
                      id="2", tags=[], compile_targets=[])),
        module_execution_config=exec_config_b,
        target_device_spec=device_spec_b,
        input_data=common_definitions.ZEROS_MODEL_INPUT_DATA)
    run_config_c = iree_definitions.E2EModelRunConfig.composite(
        module_generation_config=iree_definitions.ModuleGenerationConfig.
        composite(
