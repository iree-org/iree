...
class Android_Mali_Benchmarks(object):
  ...
  def generate(
    ...
    presets = [benchmark_presets.ANDROID_GPU, benchmark_presets.DEFAULT]
  ...
  def _get_module_generation_configs(
      ...
      presets: Sequence[str]) -> List[iree_definitions.ModuleGenerationConfig]:
    ...
    return [
        iree_definitions.ModuleGenerationConfig.build(
            ...
            presets=presets) for model in fp32_models
    ] + [
        iree_definitions.ModuleGenerationConfig.build(
            compile_config=demote_compile_config,
            imported_model=iree_definitions.ImportedModel.from_model(model),
            presets=presets) for model in fp16_models
    ] + [
        iree_definitions.ModuleGenerationConfig.build(
            compile_config=compile_config,
            imported_model=iree_definitions.ImportedModel.from_model(model),
            presets=presets) for model in quant_models
    ]
