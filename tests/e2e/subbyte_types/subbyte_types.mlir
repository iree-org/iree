...
func.func @i1_type() {
  ...
  %lhs = flow.tensor.bitcast %input1 : tensor<1xi8> -> tensor<8xi1, #encoding>
  %rhs = flow.tensor.bitcast %input2 : tensor<1xi8> -> tensor<8xi1, #encoding>
  %empty = tensor.empty() : tensor<8xi1, #encoding>
  %res = linalg.generic
        {indexing_maps = [affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>], iterator_types = ["parallel"]}
        ins(%lhs, %rhs : tensor<8xi1, #encoding>, tensor<8xi1, #encoding>) outs(%empty: tensor<8xi1, #encoding>) {
  ^bb0(%inlhs: i1, %inrhs: i1, %out: i1):
    %inres = arith.xori %inlhs, %inrhs: i1
    linalg.yield %inres : i1
  } -> tensor<8xi1, #encoding>
  %tensor_res = flow.tensor.bitcast %res : tensor<8xi1, #encoding> -> tensor<1xi8>
...
func.func @i1_type_slice() {
  ...
  %flat_input_all = flow.tensor.bitcast %input : tensor<3xi8> -> tensor<24xi1, #encoding>
  %slice = tensor.extract_slice %flat_input_all[8][8][1] : tensor<24xi1, #encoding> to tensor<8xi1, #encoding>
  %tensor_res = flow.tensor.bitcast %slice : tensor<8xi1, #encoding> -> tensor<1xi8>
  check.expect_eq_const(%tensor_res, dense<[255]> : tensor<1xi8>) : tensor<1xi8>
  return
}
