...
func.func @demote() {
  ...
  %accum = linalg.generic {indexing_maps = [#map0, #map1], iterator_types = [#linalg.iterator_type<reduction>]} ins(%input : tensor<8388608xf32>) outs(%init : tensor<1xf64>) {
  ...
  %result = linalg.generic {indexing_maps = [#map0, #map0], iterator_types = [#linalg.iterator_type<parallel>]} ins(%accum : tensor<1xf64>) outs(%init2 : tensor<1xf32>) {
