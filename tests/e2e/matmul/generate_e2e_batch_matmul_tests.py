...
def get_batch_test_shapes(shapes_id: str) -> list:
    if shapes_id not in BATCH_BLOCK_SHAPES:
        raise ValueError(f"Unknown batch shapes id: {shapes_id}. Valid: {list(BATCH_BLOCK_SHAPES)}")
...
def _generate_random_matrix_3d(name: str, shape: list, element_type: MatrixElemTypeId, increment_seed=True):
...
def generate_batch_call(
    ...
    op += _generate_random_matrix_3d("acc_copy", [b, m, n], acc_type, increment_seed=False)
