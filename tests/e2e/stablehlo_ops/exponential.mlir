func.func @tensor() {
  ...
  check.expect_almost_eq_const(%result, dense<[1.0, 2.7183, 7.3891, 54.5981]> : tensor<4xf32>, 1.0e-4) : tensor<4xf32>
  return
}
...
func.func @scalar() {
  ...
  check.expect_almost_eq_const(%result, dense<2.7183> : tensor<f32>, 1.0e-4) : tensor<f32>
  return
}
...
func.func @double() {
  ...
  check.expect_almost_eq_const(%result, dense<2.7183> : tensor<f64>, 1.0e-4) : tensor<f64>
  return
}
...
func.func @negative() {
  %input = util.unfoldable_constant dense<-1.0> : tensor<f32>
  %result = "stablehlo.exponential"(%input) : (tensor<f32>) -> tensor<f32>
  check.expect_almost_eq_const(%result, dense<0.367879> : tensor<f32>, 1.0e-4) : tensor<f32>
  return
}
