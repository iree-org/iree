...
^bb1(%variant_op: !pdl.operation):
  ...
  %fill0, %fill1 = transform.split_handles %fill in [2] : (!pdl.operation) -> (!pdl.operation, !pdl.operation)
  %matmul0, %matmul1 = transform.split_handles %matmul in [2] : (!pdl.operation) -> (!pdl.operation, !pdl.operation)

  // Step 2. Tile the matmul and fuse the fill
  // ===========================================================================
  %forall_grid, %grid_reduction =
  transform.structured.tile_to_forall_op %matmul1 tile_sizes [16] ( mapping = [#gpu.block<x>] )
