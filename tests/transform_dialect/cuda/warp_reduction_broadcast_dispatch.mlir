// iree-transform-compile   /usr/local/google/home/ntv/github/iree/tests/transform_dialect/cuda/warp_reduction_broadcast_dispatch.mlir -b cuda        -- --iree-hal-benchmark-dispatch-repeat-count=5 |   /usr/local/cuda-11.4/bin/nvprof  --print-gpu-trace  iree-run-module --entry_function=warp_reduction_broadcast_dispatch --device=cuda --function_input="512x10240xf32=1"
// Tested to run in 172us (i.e. 122GB/s on an RTX2080Ti on which https://github.com/NVIDIA/cuda-samples/tree/master/Samples/1_Utilities/bandwidthTest says 520GB/s)

!in_tensor_t = tensor<512x10240xf32>
!out_tensor_t = tensor<512xf32>

func.func @warp_reduction_broadcast_dispatch(%arg : !in_tensor_t) -> (!in_tensor_t) {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %cst = arith.constant -0.000000e+00 : f32
  
  %d0 = tensor.dim %arg, %c0 : !in_tensor_t
  %d1 = tensor.dim %arg, %c1 : !in_tensor_t
  ...
  %i = tensor.empty() : !in_tensor_t
  // %i = tensor.empty(%d0, %d1) : !in_tensor_t
  %3 = linalg.generic {
    indexing_maps = [affine_map<(d0, d1) -> (d0)>, affine_map<(d0, d1) -> (d0, d1)>],
    iterator_types = ["parallel", "parallel"]}
    ins(%2 : !out_tensor_t) outs(%i : !in_tensor_t) {
      ^bb0(%arg0: f32, %arg1: f32):
        %12 = arith.divf %arg0, %cst_0 : f32
        linalg.yield %12 : f32
      } -> !in_tensor_t

  return %3 : !in_tensor_t
}
