...
// RUN: iree-opt %s \
// RUN:     --iree-preloaded-transforms=%p/softmax_transform_spec.mlir \
// RUN:     --iree-hal-target-backends=cuda \
// RUN:     --iree-abi-transformation-pipeline \
// RUN:     --iree-flow-transformation-pipeline  \
// RUN:     --iree-stream-transformation-pipeline \
// RUN:     --iree-hal-configuration-pipeline | \
// RUN: iree-opt --pass-pipeline='builtin.module(hal.executable(hal.executable.variant(transform-preload-library{transform-library-paths=%p/softmax_transform_spec.mlir},iree-llvmgpu-lower-executable-target)))' \
