...
- `IREE_COMPILER_PLUGINS`: Select statically linked compiler plugins
    (configure-time, default: `all` default buildable plugins)
    - `IREE_COMPILER_PLUGINS=all`: Enable every default plugin whose
      prerequisites are available
    - `IREE_COMPILER_PLUGINS=all,-input_torch`: Enable all default buildable
      plugins except `input_torch`
    - `IREE_COMPILER_PLUGINS=hal_target_llvm_cpu,input_tosa`: Enable only the
      listed plugins
    - `IREE_TARGET_BACKEND_WEBGPU_SPIRV=ON`: Include WebGPU SPIR-V/Tint support
      in `all`
...
Plugins loaded at run time resolve against a renamed copy of the compiler's
symbols, so the tools must link the compiler as a shared library. Bazel does
this by default, through
`--//compiler/src/iree/compiler/API:link_shared`; setting it to `False` links
the compiler statically, and a plugin cannot then load. See
