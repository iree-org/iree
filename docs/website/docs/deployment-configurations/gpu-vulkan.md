...
Note that a target triple is needed to compile towards your GPU architecture.
It's of the form `<vendor/arch>-<product>-<os>`.
Otherwise `cpu-swiftshader-unknown` will be used, which is fairly limiting (but
good for our testing purpose).
