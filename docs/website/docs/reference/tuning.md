...
Tuning in IREE is used to generate transform dialect (TD) specs that guide the
IREE compiler in selecting optimal values for configurable parameters during
...
many users, the default specs will provide sufficient performance, but are far
from perfect.
...
specs can be made manually or generated using our dispatch tuner, [SHARK Tuner](https://github.com/nod-ai/shark-ai/tree/main/sharktuner).

A dispatch tuner is simply a tuner which performs tuning on units of 
compuitation called a "dispatch". Dispatches are blocks of computation
generated by IREE to allow a program to be divided into subdivisions capable of
running concurrently. For more information on dispatches, see this [explanation](../../../../samples/custom_dispatch/README.md).
...
SHARK Tuner can produce improved specs for dispatches which can be (and have
been) used in IREE to enhance performance.
