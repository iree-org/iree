...
applications. It consists of attempting greater performance through the
exploration of new specs (specifications) for tunable "knobs" within a program,
such as tiling sizes or subgroup sizes.
...
A dispatch tuner is simply a tuner which performs tuning on units of compute
called a "dispatch". Dispatches are blocks of compute generated by IREE to
allow a program to be divided into subdivisions capable of running
concurrently.
