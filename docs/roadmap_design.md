...
- Inserting transpose, reshape, and broadcast operations to allow for more
  optimal memory access patterns (such as transposing gather input to allow for
  memcpy-like transfers instead of column-wise cache-unfriendly accesses).
...
useful for when target capabilities may vary at runtime, such as the [number of
available storage buffer
bindings](https://vulkan.gpuinfo.org/displaydevicelimit.php?name=maxPerStageDescriptorStorageBuffers&platform=android)
