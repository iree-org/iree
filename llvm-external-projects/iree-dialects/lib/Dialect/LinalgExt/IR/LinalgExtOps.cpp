...
UnPackOp::getTiledImplementation(OpBuilder &builder,
  ...
  for (auto dim : llvm::seq<int64_t>(0, outputRank)) {
    if (dimAndTileMapping.count(dim)) {
      ...
      // size of output shape. In incomplete tile cases, the input could have
      // larger shape, it is safe to extend the boundary because they are
      // pre-padded.
      ...
      if (!failed(cstSize) && cstInnerSize &&
        ...
        // Assume that offset[dim] must be 0 or a multiple of sizes[dim]. This
        // is true if the unpack op is untiled or generated by its tiling
        // implementation. In this context, the input size is always
        // (sizes[dim] / innerSize) in this case.
