...
func.func @set_encoding_ops(%arg0: tensor<?x?xf32>) -> tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = LHS>> {
  // CHECK: iree_linalg_ext.set_encoding %[[ARG0]] : tensor<?x?xf32> -> tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = LHS>>
  %0 = iree_linalg_ext.set_encoding %arg0 : tensor<?x?xf32> -> tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = LHS>>
...
func.func @set_encoding_ops_mixed_dynamic_static(%arg0: tensor<?x10xf32>) -> tensor<20x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = LHS>> {
  // CHECK: iree_linalg_ext.set_encoding %[[ARG0]] : tensor<?x10xf32> -> tensor<20x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = LHS>>
  %0 = iree_linalg_ext.set_encoding %arg0 : tensor<?x10xf32> -> tensor<20x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = LHS>>
  return %0 : tensor<20x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = LHS>>
}

// -----

// CHECK: @unset_encoding_ops(%[[ARG0:.+]]: tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RHS>>)
func.func @unset_encoding_ops(%arg0: tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RHS>>) -> tensor<?x?xf32> {
  // CHECK: iree_linalg_ext.unset_encoding %[[ARG0]] : tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RHS>> -> tensor<?x?xf32>
  %0 = iree_linalg_ext.unset_encoding %arg0 : tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RHS>> -> tensor<?x?xf32>
  return %0 : tensor<?x?xf32>
}

// -----

// CHECK: @unset_encoding_ops_mixed_dynamic_static(%[[ARG0:.+]]: tensor<10x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RHS>>)
func.func @unset_encoding_ops_mixed_dynamic_static(%arg0: tensor<10x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RHS>>) -> tensor<?x20xf32> {
  // CHECK: iree_linalg_ext.unset_encoding %[[ARG0]] : tensor<10x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RHS>>
  %0 = iree_linalg_ext.unset_encoding %arg0 : tensor<10x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RHS>> -> tensor<?x20xf32>
...
func.func @encoding_tensors_with_ops(%arg0 : tensor<?x?xf32>,
  ...
  %0 = iree_linalg_ext.set_encoding %arg0 : tensor<?x?xf32> -> tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = LHS>>
  %1 = iree_linalg_ext.set_encoding %arg1 : tensor<?x?xf32> -> tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RHS>>
  %2 = iree_linalg_ext.set_encoding %arg2 : tensor<?x?xf32> -> tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RESULT>>
  %3 = linalg.matmul
      ins(%0, %1 : tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = LHS>>, tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RHS>>)
      outs(%2 : tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RESULT>>)
      -> tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RESULT>>
  %4 = iree_linalg_ext.unset_encoding %3 : tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RESULT>> -> tensor<?x?xf32>
...
//  CHECK-SAME:       tensor<?x?xf32> -> tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = LHS>>
//       CHECK:   %[[RHS:.+]] = iree_linalg_ext.set_encoding %[[ARG1]]
//  CHECK-SAME:       tensor<?x?xf32> -> tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RHS>>
//       CHECK:   %[[OUT:.+]] = iree_linalg_ext.set_encoding %[[ARG2]]
//  CHECK-SAME:       tensor<?x?xf32> -> tensor<?x?xf32, #iree_linalg_ext.encoding<op_kind = MATMUL_F32F32F32, role = RESULT>>
