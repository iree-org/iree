...
#layoutv2_1 = #iree_vector_ext.layoutv2<
...
#layoutv2_2 = #iree_vector_ext.layoutv2<
  subgroups_per_workgroup = [1, 1],
...
func.func @specify_layoutv2(%lhs: memref<32x32xf16>) -> vector<32x32xf16> {
  %cst_0 = arith.constant 0.0 : f16
  %c0 = arith.constant 0 : index
  %result = vector.transfer_read %lhs[%c0, %c0], %cst_0 {in_bounds = [true, true]} : memref<32x32xf16>, vector<32x32xf16>
  %2 = iree_vector_ext.layout_conflict_resolution %result {sourceLayout = #layoutv2_1, desiredLayout = #layoutv2_2} : vector<32x32xf16> -> vector<32x32xf16>
  return %2 : vector<32x32xf16>
}

// CHECK-DAG: #[[LAYOUT0:.+]] = #iree_vector_ext.layoutv2<subgroups_per_workgroup = [1, 1], batches_per_subgroup = [4, 2], duplicates_per_batch = [1, 4], threads_per_duplicate = [2, 4], elements_per_thread = [4, 1], subgroup_order = [1, 0], batch_order = [1, 0], duplicate_order = [0, 1], thread_order = [1, 0], element_order = [1, 0]>
// CHECK-DAG: #[[LAYOUT1:.+]] = #iree_vector_ext.layoutv2<subgroups_per_workgroup = [1, 1], batches_per_subgroup = [2, 4], duplicates_per_batch = [4, 1], threads_per_duplicate = [4, 2], elements_per_thread = [1, 4], subgroup_order = [0, 1], batch_order = [0, 1], duplicate_order = [1, 0], thread_order = [0, 1], element_order = [0, 1]>
// CHECK-LABEL: func.func @specify_layoutv2
// CHECK:      iree_vector_ext.layout_conflict_resolution
// CHECK-SAME:         desiredLayout = #[[LAYOUT0]]
// CHECK-SAME:         sourceLayout = #[[LAYOUT1]]
