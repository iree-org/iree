...
import iree.compiler.api.tf

if not iree.compiler.api.tf.is_available():
...
class TfCompilerTest(tf.test.TestCase):
  ...
  def testImportSavedModel(self):
    import_mlir = iree.compiler.api.tf.compile_saved_model(
  ...
  def testCompileSavedModel(self):
    binary = iree.compiler.api.tf.compile_saved_model(
        self.smdir,
        target_backends=iree.compiler.api.tf.DEFAULT_TESTING_BACKENDS)
  ...
  def testCompileModule(self):
    binary = iree.compiler.api.tf.compile_module(
        self.m, target_backends=iree.compiler.api.tf.DEFAULT_TESTING_BACKENDS)
    logging.info("Compiled len: %d", len(binary))
    self.assertIn(b"simple_matmul", binary)
    self.assertIn(b"simple_mul", binary)
