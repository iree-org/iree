...
class CompilerTest(unittest.TestCase):
  ...
  def testNoTargetBackends(self):
    with self.assertRaisesRegex(
      ...
      binary = iree.compiler.api.compile_str(SIMPLE_MUL_ASM)

  def testCompileStr(self):
    binary = iree.compiler.api.compile_str(
        SIMPLE_MUL_ASM,
        input_type="mhlo",
        target_backends=iree.compiler.api.DEFAULT_TESTING_BACKENDS)
  ...
  def testCompileInputFile(self):
    with tempfile.NamedTemporaryFile("wt", delete=False) as f:
      try:
        ...
        f.close()
        binary = iree.compiler.api.compile_file(
            f.name,
            input_type="mhlo",
            target_backends=iree.compiler.api.DEFAULT_TESTING_BACKENDS)
      finally:
  ...
  def testCompileOutputFile(self):
    with tempfile.NamedTemporaryFile("wt", delete=False) as f:
      try:
        f.close()
        output = iree.compiler.api.compile_str(
            SIMPLE_MUL_ASM,
            input_type="mhlo",
            output_file=f.name,
            target_backends=iree.compiler.api.DEFAULT_TESTING_BACKENDS)
  ...
  def testOutputFbText(self):
    text = iree.compiler.api.compile_str(
        SIMPLE_MUL_ASM,
        input_type="mhlo",
        output_format=iree.compiler.api.OutputFormat.FLATBUFFER_TEXT,
        target_backends=iree.compiler.api.DEFAULT_TESTING_BACKENDS).decode(
            "utf-8")
  ...
  def testBadInputType(self):
    with self.assertRaisesRegex(
        ...
        "NONE, MHLO, TOSA"):
      _ = iree.compiler.api.compile_str(
          SIMPLE_MUL_ASM,
  ...
  def testOutputMlirText(self):
    text = iree.compiler.api.compile_str(
        SIMPLE_MUL_ASM,
        input_type="mhlo",
        output_format=iree.compiler.api.OutputFormat.MLIR_TEXT,
        target_backends=iree.compiler.api.DEFAULT_TESTING_BACKENDS).decode(
            "utf-8")
  ...
  def testException(self):
    with self.assertRaisesRegex(iree.compiler.api.CompilerToolError,
                                "Invoked with"):
      _ = iree.compiler.api.compile_str(
  ...
  def testExplicitTempFileSaver(self):
    ...
    output_file.close()
    with iree.compiler.api.TempFileSaver(temp_dir.name):
      output = iree.compiler.api.compile_str(
          SIMPLE_MUL_ASM,
          input_type="mhlo",
  ...
  def testEnvTempFileSaver(self):
    ...
    with iree.compiler.api.TempFileSaver() as tfs:
