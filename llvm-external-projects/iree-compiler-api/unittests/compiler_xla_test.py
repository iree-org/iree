...
import iree.compiler.api.xla

if not iree.compiler.api.xla.is_available():
...
class CompilerTest(unittest.TestCase):
  ...
  def testImportBinaryPbFile(self):
    ...
    text = iree.compiler.api.xla.compile_file(path,
  ...
  def testCompileBinaryPbFile(self):
    path = os.path.join(os.path.dirname(__file__), "testdata", "xla_sample.pb")
    binary = iree.compiler.api.xla.compile_file(
        path, target_backends=iree.compiler.api.xla.DEFAULT_TESTING_BACKENDS)
  ...
  def testImportBinaryPbFileOutputFile(self):
    ...
    with tempfile.NamedTemporaryFile("wt", delete=False) as f:
      try:
        f.close()
        output = iree.compiler.api.xla.compile_file(path,
                                                    import_only=True,
                                                    output_file=f.name)
  ...
  def testCompileBinaryPbFileOutputFile(self):
    ...
    with tempfile.NamedTemporaryFile("wt", delete=False) as f:
      try:
        f.close()
        output = iree.compiler.api.xla.compile_file(
            path,
            output_file=f.name,
  ...
  def testImportBinaryPbBytes(self):
    ...
    with open(path, "rb") as f:
      content = f.read()
    text = iree.compiler.api.xla.compile_str(content,
                                             import_only=True).decode("utf-8")
    logging.info("%s", text)
    self.assertIn("linalg.generic", text)
  ...
  def testCompileBinaryPbBytes(self):
    ...
    with open(path, "rb") as f:
      content = f.read()
    binary = iree.compiler.api.xla.compile_str(
        content, target_backends=iree.compiler.api.xla.DEFAULT_TESTING_BACKENDS)
    logging.info("Binary length = %d", len(binary))
    self.assertIn(b"main", binary)

  def testImportHloTextFile(self):
    path = os.path.join(os.path.dirname(__file__), "testdata", "xla_sample.hlo")
    text = iree.compiler.api.xla.compile_file(
  ...
  def testImportHloTextStr(self):
    ...
    with open(path, "rt") as f:
      content = f.read()
    text = iree.compiler.api.xla.compile_str(
