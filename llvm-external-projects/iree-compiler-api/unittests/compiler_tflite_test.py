...
import iree.compiler.api.tflite

if not iree.compiler.api.tflite.is_available():
...
class CompilerTest(unittest.TestCase):
  ...
  def testImportBinaryPbFile(self):
    path = os.path.join(os.path.dirname(__file__), "testdata",
                        "tflite_sample.fb")
    text = iree.compiler.api.tflite.compile_file(
  ...
  def testCompileBinaryPbFile(self):
    path = os.path.join(os.path.dirname(__file__), "testdata",
                        "tflite_sample.fb")
    binary = iree.compiler.api.tflite.compile_file(
        path, target_backends=iree.compiler.api.tflite.DEFAULT_TESTING_BACKENDS)
  ...
  def testImportBinaryPbFileOutputFile(self):
    ...
    with tempfile.NamedTemporaryFile("wt", delete=False) as f:
      try:
        f.close()
        output = iree.compiler.api.tflite.compile_file(path,
                                                       import_only=True,
                                                       output_file=f.name)
  ...
  def testCompileBinaryPbFileOutputFile(self):
    ...
    with tempfile.NamedTemporaryFile("wt", delete=False) as f:
      try:
        f.close()
        output = iree.compiler.api.tflite.compile_file(
            path,
            output_file=f.name,
            target_backends=iree.compiler.api.tflite.DEFAULT_TESTING_BACKENDS)
        self.assertIsNone(output)
  ...
  def testImportBinaryPbBytes(self):
    ...
    with open(path, "rb") as f:
      content = f.read()
    text = iree.compiler.api.tflite.compile_str(
  ...
  def testCompileBinaryPbBytes(self):
    ...
    with open(path, "rb") as f:
      content = f.read()
    binary = iree.compiler.api.tflite.compile_str(
        content,
        target_backends=iree.compiler.api.tflite.DEFAULT_TESTING_BACKENDS)
    logging.info("Binary length = %d", len(binary))
    self.assertIn(b"main", binary)
