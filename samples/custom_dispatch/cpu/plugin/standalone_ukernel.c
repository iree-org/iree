...
// Demonstrates a standalone plugin calling a `simple_mul_workgroup`
// micro-kernel function. `standalone_ukernel.mlir` shows the input
// IR to `iree-compile`. The function `ukernel_example` there is
// compiled to `llvm-cpu` which in turn calls the `simple_mul_workgroup`
// function defined here. This models calling micro kernels from within
// dispatches. This standalone plugin mechanism is used here (and barrowed from
// standalone_plugin.c).
...
iree_hal_executable_plugin_query(
  ...
  static const iree_hal_executable_plugin_header_t header = {
      ...
      .name = "sample_standalone_ukernel",
      .description =
          "standalone plugin sample "
          "(custom_dispatch/cpu/plugin/standalone_ukernel.c)",
