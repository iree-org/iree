...
void Internal(int s, void* out, void** in) {
  ...
  memcpy(&m, inPtr, sizeof(intptr_t));
  inPtr += sizeof(intptr_t);
  memcpy(&n, inPtr, sizeof(intptr_t));
  inPtr += sizeof(intptr_t);
  int32_t* inBuffer;
  memcpy(&inBuffer, inPtr, sizeof(int32_t*));
  intptr_t totalCount = m*n;
  printf("CustomCall times: %d m: %ld n: %ld\n", s, m, n);
  for (intptr_t i = 0; i < totalCount; i++) {
    printf("CustomCall input times: %d, index: %ld, inBuffer: %d, outBuffer: %d\n", s, i, inBuffer[i], outBuffer[i]);
    outBuffer[i] = inBuffer[i]*s;
    printf("CustomCall output times: %d, index: %ld, inBuffer: %d, outBuffer: %d\n", s, i, inBuffer[i], outBuffer[i]);
  }
}
...
int main(int argc, char** argv) {
  ...
  const int32_t input_data[6] = {1, 2, 3, 4, 5, 6};
  const iree_hal_dim_t shape[2] = {2, 3};
