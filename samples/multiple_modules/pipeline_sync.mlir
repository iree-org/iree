// RUN: (iree-compile --iree-hal-target-device=local --iree-hal-local-target-device-backends=vmvx %p/module_a.mlir -o=%t.module_a.vmfb && \
// RUN:  iree-compile --iree-hal-target-device=local --iree-hal-local-target-device-backends=vmvx %p/module_b.mlir -o=%t.module_b.vmfb && \
