...
template <typename T>
const char* GetZoneName(const tracy::Worker& worker, const Zone<T>& zone) {
  const auto& srcloc = worker.GetSourceLocation(zone.source_location_id);
...
void PrintWorker(const tracy::Worker& worker) {
  ...
  for (const auto& z : cpu_zones) {
    std::cout << "[TRACY]         " << GetZoneName(worker, z)
