...
from lit_tools.core import lit_wrapper, test_file
...
class TestCaseRunLineReinjection(unittest.TestCase):
    ...
    def test_case_local_run_lines_positions(self):
        # Build a pseudo-file content with a case that includes a RUN in-body.
        body = "\n".join(
            [
                "// CHECK-LABEL: @foo",
                "// CHECK: A",
        ...
        # Case starts at line 10 in the file
        blank_prefix = "\n" * 9
        file_text = blank_prefix + body
        # Write to a temp file to drive extract_case_run_lines
        ...
        try:
            ...
            header = []
            case_runs = test_file.extract_case_run_lines(p, case)
            # Strip and re-inject into synthesized content (with blank prefix)
            stripped = test_file._strip_run_lines_preserve_line_numbers(body)
            content = ("\n" * (case.start_line - 1)) + stripped
            rebuilt = test_file.inject_run_lines_with_case(content, header, case_runs)
            lines = rebuilt.split("\n")
            # RUN should be at blank_prefix + original relative position (3rd line of body)
            idx = (case.start_line - 1) + 2
            self.assertTrue(lines[idx].startswith("// RUN:"))
            self.assertIn("iree-opt --canonicalize", lines[idx])
        finally:
            p.unlink()
    ...
    def test_case_local_run_lines_continuation(self):
        ...
        with NamedTemporaryFile("w", suffix=".mlir", delete=False) as tmp:
            ...
            tmp.write(file_text)
        try:
            case = test_file.TestCase(1, "foo", body, 5, 9, 5, 1)
            case_runs = test_file.extract_case_run_lines(p, case)
            stripped = test_file._strip_run_lines_preserve_line_numbers(body)
            content = ("\n" * (case.start_line - 1)) + stripped
            rebuilt = test_file.inject_run_lines_with_case(content, [], case_runs)
            lines = rebuilt.split("\n")
            idx = (case.start_line - 1) + 2
            self.assertTrue(lines[idx].startswith("// RUN:"))
            self.assertIn("--canonicalize", lines[idx])
        finally:
            p.unlink()
