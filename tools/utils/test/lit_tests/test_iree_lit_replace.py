...
from lit_tools.core import test_file, verification
...
"""
    ...
    def test_round_trip_extract_replace(self):
        ...
        try:
            # Extract case 2
            original_cases = test_file.parse_test_file(tmp_path)
            ...
            # Extract again
            new_cases = test_file.parse_test_file(tmp_path)
...
class TestIdempotency(unittest.TestCase):
    ...
    def test_byte_for_byte_idempotency(self):
        ...
        try:
            ...
            cases = test_file.parse_test_file(tmp_path)
            header_runs = test_file.extract_run_lines(tmp_path, raw=True)
...
"""
            ...
            new_runs = test_file.extract_run_lines(tmp_path, raw=False)
...
class TestJSONMode(unittest.TestCase):
    ...
    def test_json_case_local_runs_match_original(self):
        ...
        try:
            # Replacement includes the original case-local RUN line (should succeed)
            json_input = json.dumps(
                [
                    {
                        "file": str(tmp_path),
...
class TestMultiFileReplacements(unittest.TestCase):
    ...
    def test_json_multi_file_commit_and_verify(self):
        ...
        try:
            ...
            # Use test_file as tf for brevity in this test
            tf = test_file
            ca = tf.parse_test_file(a)
            cb = tf.parse_test_file(b)
