...
class TestStripRunLines(unittest.TestCase):
    ...
    def test_single_run_line(self):
        ...
        result = test_file._strip_run_lines_preserve_line_numbers(content)
        lines = result.split("\n")
...
class TestInjectRunLines(unittest.TestCase):
    ...
    def test_case1_scenario(self):
        ...
        result = test_file.inject_run_lines(content, run_lines)
        lines = result.split("\n")
    ...
    def test_case2_scenario(self):
        ...
        result = test_file.inject_run_lines(content_with_blanks, run_lines)
        lines = result.split("\n")
    ...
    def test_no_run_lines(self):
        ...
        content = "\n\n// CHECK: foo"
        result = test_file.inject_run_lines(content, [])
    ...
    def test_no_run_fixture_header_and_body(self):
        ...
        header_runs = test_file.extract_run_lines(fixture)
        ...
        stripped = test_file._strip_run_lines_preserve_line_numbers(case.content)
        synthesized = ("\n" * (case.start_line - 1)) + stripped
        rebuilt = test_file.inject_run_lines_with_case(
            synthesized, header_runs, test_file.extract_case_run_lines(fixture, case)
        )
...
class TestIntegrationWithParse(unittest.TestCase):
    ...
    def test_all_cases_preserve_line_numbers(self):
        ...
        cases = test_file.parse_test_file(self.fixture_path)
        run_lines = test_file.extract_run_lines(self.fixture_path)

        # Original file has CHECK-LABEL at these lines
        expected_check_lines = {
            1: 5,  # first_case at line 5
            2: 15,  # second_case at line 15
            3: 27,  # third_case at line 27
        }

        for case in cases:
            # Simulate lit_wrapper processing
            blank_prefix = "\n" * (case.start_line - 1)
            content_with_blanks = blank_prefix + case.content
            result = test_file.inject_run_lines(content_with_blanks, run_lines)
