...
class TestFetcherWithMocks(unittest.TestCase):
    ...
    def test_fetch_single_run(self, mock_run):
        ...
        # Mock gh run view for job logs.
        log_response = "Sample log content\nERROR: test failure\n"

        mock_run.side_effect = [
            MagicMock(stdout=run_response, returncode=0),  # get_run
...
class TestClassifierWithMocks(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.corpus.save_log(
            "12345", "111", "ERROR: undefined reference to `missing_symbol`\n"
        )
    ...
    @patch("subprocess.run")
    def test_classify_recognized_failure(self, mock_run):
        """Test classification with recognized pattern."""
        # Mock iree-ci-triage --json output.
        ...
        # Mock which command (find iree-ci-triage).
        which_response = "/path/to/iree-ci-triage\n"

        mock_run.side_effect = [
            MagicMock(stdout=which_response, returncode=0),  # which iree-ci-triage
            MagicMock(stdout=triage_response, returncode=0),  # iree-ci-triage --json
        ]
        ...
        self.assertIn("linker_error", result.categories)

    @patch("subprocess.run")
    def test_classify_unrecognized_failure(self, mock_run):
        """Test classification with unrecognized pattern."""
        # Mock iree-ci-triage returning no root causes.
        triage_response = json.dumps({"jobs": [{"root_causes": []}]})

        which_response = "/path/to/iree-ci-triage\n"

        mock_run.side_effect = [
            MagicMock(stdout=which_response, returncode=0),
            MagicMock(stdout=triage_response, returncode=0),
        ]
...
class TestStatusCommand(unittest.TestCase):
    ...
    def test_status_json_output(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            result = run_python_module(
                "ci.iree_ci_garden",
                ["status", "--corpus-dir", temp_dir, "--json"],
                capture_output=True,
                text=True,
            )
            self.assertEqual(result.returncode, 0)
