...
class TestDetectBuildDir(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Create build directory: /tmp/xxx/iree-loom-build/
    ...
    def test_fallback_to_iree_build(self):
        """Test fallback to ../iree-build for main repo."""
        # Remove default build directory
        self.build_dir.rmdir()

        # Create ../iree-build
        iree_build = self.root / "iree-build"
        iree_build.mkdir()

        result = build_detection.detect_build_dir(cwd=self.worktree)
        self.assertEqual(result, iree_build)
