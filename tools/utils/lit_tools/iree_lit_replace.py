...
def build_file_content(header_runs: list[str], cases: list[test_file.TestCase]) -> str:
    ...
    for i, case in enumerate(cases):
        ...
        # Add case content (already has embedded newlines)
        result += content

        # Add trailing newline after case content if not last case
        # (parsed content doesn't include the blank line after each case)
        if i < len(cases) - 1:
            result += "\n"
...
def _validate_text_mode_args(args: argparse.Namespace) -> Optional[int]:
...
def _load_and_parse_test_file(
    ...
    Returns:
        Tuple of (file_path, all_cases, original_stat, original_hash, error_code).
...
def _validate_replacement_content_checks(
    ...
    target_case: test_file.TestCase,
    args: argparse.Namespace,
) -> Optional[int]:
    ...
    if args.verify:
        # Skip verification for content with expected-error directives.
        if verification.should_skip_verification_for_content(replacement_content_clean):
            if not args.quiet:
                console.note(
                    f"Skipping verification for case {target_case.number}: "
                    "replacement contains expected-error directives",
                    args=args,
                )
        else:
            case_info = f"Case {target_case.number}"
            if target_case.name:
                case_info += f" (@{target_case.name})"

            valid, error_msg = verification.verify_ir(
                replacement_content_clean, case_info, args, timeout=args.verify_timeout
            )
            if not valid:
                console.error(error_msg, args=args)
                return exit_codes.ERROR
...
def _handle_unchanged_case(
    target_case: test_file.TestCase, file_path: Path, args: argparse.Namespace
) -> int:
...
def handle_text_mode(replacement_content: str, args: argparse.Namespace) -> int:
    ...
    try:
        header_runs_raw = test_file.extract_run_lines(file_path, raw=True)
        header_runs_normalized = test_file.extract_run_lines(file_path, raw=False)
    except (OSError, UnicodeDecodeError, ValueError) as e:
    ...
    original_case_runs_absolute = test_file.extract_case_run_lines(
        file_path, target_case
    )
    ...
    # Filter both raw and normalized header runs using normalized comparison.
    filtered_raw = []
    filtered_normalized = []
    for raw, normalized in zip(header_runs_raw, header_runs_normalized):
        if normalized not in case_run_cmds_set:
            filtered_raw.append(raw)
            filtered_normalized.append(normalized)

    header_runs_raw = filtered_raw
    header_runs_normalized = filtered_normalized
    ...
    target_case.content = final_content
    ...
    if args.replace_run_lines and replacement_runs:
        ...
        new_content = build_file_content(new_header_runs_raw, all_cases)
    else:
        new_content = build_file_content(header_runs_raw, all_cases)
...
def _find_case_by_number(
    all_cases: list, case_num: int
) -> Optional[test_file.TestCase]:
    """Find case by number.
...
def _find_cases_by_name(all_cases: list, case_name: str) -> list[test_file.TestCase]:
...
def _resolve_and_check_duplicates(
    ...
    """
    all_errors = []
    resolved_replacements = {}  # (file_path, case_num) -> (repl_idx, repl, all_cases)
    ...
    for file_path, replacements_for_file in file_groups.items():
        ...
        for repl_idx, repl in replacements_for_file:
            ...
            if canonical_key in resolved_replacements:
                ...
                resolved_replacements[canonical_key] = (repl_idx, repl, all_cases)
...
def _validate_single_replacement_content(
    ...
    original_replace_flag = args.replace_run_lines
    original_allow_empty = args.allow_empty
    original_require_label = args.require_label

    args.replace_run_lines = replace_run_lines
    args.allow_empty = allow_empty
    args.require_label = require_label
    ...
    args.replace_run_lines = original_replace_flag
    args.allow_empty = original_allow_empty
    args.require_label = original_require_label
    ...
    if replacement_runs and replace_run_lines:
    ...
    replacement_data = {
        "case": target_case,
        "content": replacement_content_clean,
...
def _execute_batch_file_writes(
    ...
    for file_path, results in validation_results.items():
        ...
        # Apply replacements to cases.
        for repl_data in replacements_data:
            target_case = repl_data["case"]
            target_case.content = repl_data["content"]
        ...
        new_content = build_file_content(header_runs, all_cases)
