...
def _ensure_lit_importable() -> None:
    ...
    repo_root = build_detection._find_repo_root()
    if repo_root:
...
def run_lit_on_case(
    ...
    Args:
        ...
        test_file: Original .mlir file path
    ...
    blank_lines = "\n" * (case.start_line - 1)
    test_content = blank_lines + case.content

    # 2. Extract and inject RUN lines from original test file.
    header_runs = test_file_module.extract_run_lines(test_file)
    case_runs = test_file_module.extract_case_run_lines(test_file, case)
    test_content = test_file_module.inject_run_lines_with_case(
    ...
    try:
        ...
        # Find real suite config from original test directory.
        ...
        if not real_cfg_path:
            ...
            for parent in [test_file.parent] + list(test_file.parent.parents):
        ...
        if not real_cfg_path:
            raise FileNotFoundError(
                f"Unable to locate lit.cfg.py for {test_file.parent}"
            )

        # Create stub config in temp_root and map it to real config via config_map.
        stub_cfg = temp_root / "lit.cfg.py"
        stub_cfg.write_text("# stub config mapped via config_map\n")
        ...
        lit_config = LitConfig.LitConfig(
            ...
            order=None,
            params={
                # Map the stub config path to the real suite config.
                "config_map": {os.path.normcase(str(stub_cfg)): str(real_cfg_path)}
            },
            config_prefix=None,
        ...
        if not keep_temps and temp_file.exists():
            ...
            from lit_tools.core import (  # noqa: PLC0415
                console as _console,
