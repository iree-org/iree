...
from lit_tools.core import formatting
...
def _is_pretty(args: Any | None, pretty: bool | None) -> bool:
    ...
    # Only use color if: pretty enabled AND stdout is TTY AND NO_COLOR not set.
    return bool(
        pretty_flag and sys.stdout.isatty() and os.environ.get("NO_COLOR") is None
    )
