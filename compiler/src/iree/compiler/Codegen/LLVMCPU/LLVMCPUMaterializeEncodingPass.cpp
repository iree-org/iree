...
struct LLVMCPUMaterializeEncodingPaddingSizePass
    : public LLVMCPUMaterializeEncodingBase<
          LLVMCPUMaterializeEncodingPaddingSizePass> {
  void getDependentDialects(DialectRegistry &registry) const override {
...
// EncodingPaddingSizeOp, in top-level functions that are not part of one HAL
...
void LLVMCPUMaterializeEncodingPaddingSizePass::runOnOperation() {
  MLIRContext *context = &getContext();
  auto operation = getOperation();
  auto targetAttrs =
  ...
  populateMaterializeEncodingPaddingSizePatterns(patterns,
                                                 materializeEncodingFn);
  if (failed(applyPatternsAndFoldGreedily(operation, std::move(patterns)))) {
...
createLLVMCPUMaterializeEncodingPaddingSizePass() {
  return std::make_unique<LLVMCPUMaterializeEncodingPaddingSizePass>();
}
