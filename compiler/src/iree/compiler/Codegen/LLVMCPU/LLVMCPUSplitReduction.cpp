...
LogicalResult splitReductionImpl(Operation *op, SmallVector<int64_t> tileSizes,
  ...
  for (auto r : tileResFirst->replacements) {
    LLVM_DEBUG(llvm::dbgs() << "tileResFirst: " << r << "\n");
  }
  ...
  LLVM_DEBUG(llvm::dbgs() << "fillOp: " << splitRes->fillOp << "\n");
  LLVM_DEBUG(llvm::dbgs() << "splitLinalgOp: " << splitRes->splitLinalgOp
                          << "\n");
  LLVM_DEBUG(llvm::dbgs() << "resultCombiningLinalgOp: "
...
void LLVMCPUSplitReductionPass::runOnOperation() {
  ...
  for (auto genericOp : candidates) {
    ...
    if (failed(splitReductionImpl(genericOp, reductionSizes, rewriter))) {
