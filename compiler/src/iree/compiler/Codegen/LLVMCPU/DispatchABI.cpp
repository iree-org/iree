...
Value HALDispatchABI::loadProcessorData(Operation *forOp, OpBuilder &builder) {
  ...
  //
  // `forOp` here may be a call inside a loop, so if we built these ops at the
  // caller's insertion point we'd re-run the stack allocation on every loop
  // iteration with nothing to ever pop it back off, overflowing the stack for
  // large iteration counts (#24744). Build them in the enclosing function's
  // entry block instead, where they only run once.
  auto funcOp = forOp->getParentOfType<LLVM::LLVMFuncOp>();
  assert(funcOp && "usage requires an enclosing LLVMFuncOp");
