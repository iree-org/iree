...
static llvm::cl::opt<bool> clEnableReductionReordering(
    "iree-llvmcpu-enable-reduction-reordering",
    llvm::cl::desc("Enables reordering reductions for floats that are not"
                   "(strictly) associative for addition or multiplication"),
    llvm::cl::init(true));
...
void addMultiTilingExpertPassPipeline(OpPassManager &passManager,
  ...
  nestedModulePM.addNestedPass<func::FuncOp>(
      createLinalgSplitReductionPass(clEnableReductionReordering));
  {
    LinalgFusePassOptions options;
