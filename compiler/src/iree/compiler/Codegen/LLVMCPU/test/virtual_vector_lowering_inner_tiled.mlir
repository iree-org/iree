// RUN: iree-opt %s --pass-pipeline="builtin.module(func.func(iree-llvmcpu-virtual-vector-lowering))" --split-input-file | FileCheck %s

// Focused test for the `iree_codegen.inner_tiled` lowering in
// `iree-llvmcpu-virtual-vector-lowering`. That pass runs three pattern sets
// in sequence on the `inner_tiled` op:
//   1. unroll any non-unit iter dims to unit,
//   2. drop the (now-unit) iter domain,
//   3. lower the iter-free vector inner_tiled to llvm.call_intrinsic via the
//      kind's `buildUnderlyingOperations`,
// and ends with `IREE::Util::eliminateHoistableConversions` to cancel the
// inverse conversion pairs the lowering patterns leave around the ACC.
