...
// RUN:          --iree-codegen-hoist-redundant-vector-transfers %s | FileCheck --implicit-check-not=mask %s
...
//    * ? = vscale * 16
// Hence, masking is not really needed. Vector mask canonicalisation are
// capable of recognising that and remove the masks inserted by the Linalg
// vectoriser.
//
// Separately, the hoisting logic recognises that there's no need to store/load
// the output tensor and insted hoists it so that it becomes a loop variable.
// This is highly desirable as the opposite leads to very poor performance.
...
// CHECK-NEXT:      %[[OUT_VEC:.*]] = vector.transfer_read %[[OUT_SLICE_1]]{{.*}} : tensor<8x?xf32>, vector<8x[16]xf32>
