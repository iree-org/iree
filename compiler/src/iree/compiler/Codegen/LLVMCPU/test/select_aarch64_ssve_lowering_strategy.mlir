...
// opts into scalable (SSVE) tile sizes here. Note the preprocessing strategy
// (Masking vs Peeling) is unaffected by +sme/streaming: it stays keyed off
// +sve alone (see getVectorPreProcStrategy() in KernelDispatch.cpp), so this
// always goes through Peeling here, same as the plain NEON case would.
