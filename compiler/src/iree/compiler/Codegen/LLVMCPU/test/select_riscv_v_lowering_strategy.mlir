...
// RUN:   --iree-llvmcpu-enable-scalable-vectorization=true --split-input-file \
...
#executable_target_riscv64 = #hal.executable.target<"llvm-cpu", "embedded-elf-riscv_64", {cpu_features = "+v", data_layout = "e-m:e-p:64:64-i64:64-i128:128-n32:64-S128", native_vector_size = 16 : index, target_triple = "riscv64-unknown-unknown-eabi-elf", vscale_range = [1, 16]}>
...
#executable_target_riscv64 = #hal.executable.target<"llvm-cpu", "embedded-elf-riscv_64", {cpu_features = "+zvfh,+v", data_layout = "e-m:e-p:64:64-i64:64-i128:128-n32:64-S128", native_vector_size = 16 : index, target_triple = "riscv64-unknown-unknown-eabi-elf", vscale_range = [1, 16]}>
...
// RISC-V +v target with no vscale range: strategy selection is expected to fail.
...
// expected-error @+1 {{scalable vectorization is enabled but no vscale range is available}}
func.func @matmul_no_vscale_range(%a: tensor<?x?xf32>, %b: tensor<?x?xf32>, %c: tensor<?x?xf32>) -> tensor<?x?xf32> attributes {hal.executable.target = #executable_target_riscv64} {
