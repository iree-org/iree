...
func.func @test_mod_mixed_sign_int32_dispatch_0_elementwise_6_i32() attributes {translation_info = #iree_codegen.translation_info<CPUDoubleTilingExpert, {enable_loop_peeling}>} {
  ...
  %3 = flow.dispatch.tensor.load %2, offsets = [0], sizes = [6], strides = [1] : !flow.dispatch.tensor<writeonly:tensor<6xi32>> -> tensor<6xi32>
  %4 = flow.dispatch.tensor.load %0, offsets = [0], sizes = [6], strides = [1] : !flow.dispatch.tensor<readonly:tensor<6xi32>> -> tensor<6xi32>
  %5 = flow.dispatch.tensor.load %1, offsets = [0], sizes = [6], strides = [1] : !flow.dispatch.tensor<readonly:tensor<6xi32>> -> tensor<6xi32>
  %6 = scf.for %arg0 = %c0 to %c6 step %c4 iter_args(%arg1 = %3) -> (tensor<6xi32>) {
    %7 = affine.min affine_map<(d0) -> (-d0 + 6, 4)>(%arg0)
    %extracted_slice = tensor.extract_slice %4[%arg0] [%7] [1] : tensor<6xi32> to tensor<?xi32>
    %extracted_slice_0 = tensor.extract_slice %5[%arg0] [%7] [1] : tensor<6xi32> to tensor<?xi32>
    %extracted_slice_1 = tensor.extract_slice %arg1[%arg0] [%7] [1] : tensor<6xi32> to tensor<?xi32>
    %8 = linalg.generic {indexing_maps = [affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>, affine_map<(d0) -> (d0)>], iterator_types = ["parallel"]} ins(%extracted_slice, %extracted_slice_0 : tensor<?xi32>, tensor<?xi32>) outs(%extracted_slice_1 : tensor<?xi32>) attrs =  {lowering_config = #iree_codegen.lowering_config<tile_sizes = [[6], [4], [0], [0]]>} {
    ^bb0(%in: i32, %in_2: i32, %out: i32):
      %9 = arith.remsi %in, %in_2 : i32
      linalg.yield %9 : i32
    } -> tensor<?xi32>
    %inserted_slice = tensor.insert_slice %8 into %arg1[%arg0] [%7] [1] : tensor<?xi32> into tensor<6xi32>
    scf.yield %inserted_slice : tensor<6xi32>
  }
  flow.dispatch.tensor.store %6, %2, offsets = [0], sizes = [6], strides = [1] : tensor<6xi32> -> !flow.dispatch.tensor<writeonly:tensor<6xi32>>
  return
}

// CHECK-LABEL: func.func @test_mod_mixed_sign_int32_dispatch_0_elementwise_6_i32

// CHECK: linalg.generic
// CHECK: %[[RESULT1:.*]] = arith.remsi
// CHECK-NEXT: linalg.yield %[[RESULT1]]
// CHECK-NEXT: } -> tensor<4xi32>

// CHECK: linalg.generic
// CHECK: %[[RESULT2:.*]] = arith.remsi
// CHECK: linalg.yield %[[RESULT2]]
// CHECK: } -> tensor<2xi32>
