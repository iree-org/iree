...
// RUN: iree-opt --iree-llvmcpu-riscv-aggressive-distribution=true --pass-pipeline='builtin.module(iree-llvmcpu-select-lowering-strategy, func.func(iree-llvmcpu-lower-executable-target))' --split-input-file %s | FileCheck %s -check-prefixes=CHECK-RISCV
...
// CHECK-RISCV-LABEL: func.func @f32_rvv_matmul(
// CHECK-RISCV-DAG:     %[[c1:.+]] = arith.constant 1 : index
// CHECK-RISCV-DAG:     %[[c7:.+]] = arith.constant 7 : index
// CHECK-RISCV-DAG:     %[[c128:.+]] = arith.constant 128 : index
// CHECK-RISCV-DAG:     %[[c256:.+]] = arith.constant 256 : index
// CHECK-RISCV-DAG:     %[[c512:.+]] = arith.constant 512 : index
// CHECK-RISCV:       scf.for {{.*}} step %[[c7]]
// CHECK-RISCV:         scf.for {{.*}} step %[[c128]]
// CHECK-RISCV:           scf.for {{.*}} step %[[c1]]
// CHECK-RISCV-COUNT-7:     vector.fma
// CHECK-RISCV-COUNT-7:   vector.store
// CHECK-RISCV:       scf.for {{.*}} step %[[c128]]
// CHECK-RISCV:           scf.for {{.*}} step %[[c1]]
// CHECK-RISCV-COUNT-4:     vector.fma
// CHECK-RISCV-COUNT-4:   vector.store
