...
static void rewriteParallelInsertSlices(
    ...
    LinalgExt::PerformConcurrentlyOp performConcurrentlyOp, Block &block,
...
/// Pattern to rewrite a InParallelOp into a Flow::DispatchWorkGroupsOp.
/// This rewrite proceeds in a few steps:
///   - Step 1: Compute the result types and their result dynamic dim operands.
///     This first step takes advantage of the ops contained in the InParallelOp
///     terminator and that are tied to the results.
...
///   - Step 6: Move the body of inParallel to the dispatchOp.
///   - Step 7: Set up bvm for RAUWIf. In particular, tensor operands become
///     flow dispatch tensor bbArgs and need to be flow.dispatch.tensor.load'ed.
...
///   - Step 11: Drop the inParallel terminator and replace inParallel.
// TODO: n-D InParallelOp
struct InParallelOpToFlowDispatchWorkgroupsRewriter
...
InParallelOpToFlowDispatchWorkgroupsRewriter::returningMatchAndRewrite(
    LinalgExt::InParallelOp inParallelOp, PatternRewriter &rewriter) const {
  // Entry point start just before the inParallelOp.
  ...
  assert(resultTensorOperands.size() == inParallelOp.getNumResults() &&
         "Expected as many resultTensorOperands as results of inParallelOp");

  // Step 2. Get values defined above and separate them between non-tensors,
  // tensors and introduce appropriate tensor dims.
  // Tensors that have already been recorded as resultTensorOperands are omitted
  // to avoid duplications.
  llvm::SetVector<Value> valuesDefinedAbove;
  mlir::getUsedValuesDefinedAbove(inParallelOp.region(), valuesDefinedAbove);
  ...
  SmallVector<Value> workgroupCount{inParallelOp.num_threads()};
  ...
  // InParallel tensors inserted into are tied to results and translate to the
  // tied operands of the dispatch.
  ...
  auto dispatchOp = rewriter.create<Flow::DispatchWorkgroupsOp>(
      ...
      /*resultTypes=*/inParallelOp.getResultTypes(),
  ...
  //    trailing bbArgs.
  //    Another possibility would be to interleave (tensor, tensor dynamic dims)
  //    but unless proven wrong, the trailing indices convention is quite
  //    simpler to implement: if bugs surface, these should be fixed or a real
  //    convention + verification should be adopted on the op + builder.
  ...
  // insert an explicit Flow::DispatchTensorLoadOp to get back a proper tensor.
  // Save the tensor operand -> flow tensor bbArg mapping in `tensorToFlowBvm`.
  ...
  bvm.map(inParallelOp.getThreadIndex(), idXOp);
  bvm.map(inParallelOp.num_threads(), countXOp);

  // Step 9. Rewrite tensor::ExtractSlice and ParallelInsert ops to the relevant
  // Flow DispatchTensorLoad/Store version.
  rewriteParallelInsertSlices(
      rewriter, terminator, *block, resultTensorOperands.getArrayRef(),
      resultTensorsDynamicDims.getArrayRef(), tensorToFlowBvm);
    ...
    if (state.getTopLevel()
            ->walk<WalkOrder::PostOrder>([&](LinalgExt::InParallelOp op) {
              if (failed(functional::applyReturningPatternAt(
                      InParallelOpToFlowDispatchWorkgroupsRewriter(
                          getContext()),
                      op)))
                return WalkResult::interrupt();
              return WalkResult::advance();
