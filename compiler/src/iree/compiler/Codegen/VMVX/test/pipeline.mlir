  ...
  hal.executable.variant public @vmvx_bytecode_fb target(<"vmvx", "vmvx-bytecode-fb", {ukernels = true}>) {
    ...
    builtin.module {
      func.func @mmt4d_i8() {
        ...
        %0:2 = iree_codegen.query_tile_sizes tensor<16x16xi8, #iree_linalg_ext.encoding<user = MATMUL, role = LHS, elementTypes = [i8, i8, i32]>> -> index, index
        ...
        %4:2 = iree_codegen.query_tile_sizes tensor<16x16xi8, #iree_linalg_ext.encoding<user = MATMUL, role = RHS, elementTypes = [i8, i8, i32]>> -> index, index
        ...
        %8:2 = iree_codegen.query_tile_sizes tensor<16x16xi32, #iree_linalg_ext.encoding<user = MATMUL, role = RESULT, elementTypes = [i8, i8, i32]>> -> index, index
