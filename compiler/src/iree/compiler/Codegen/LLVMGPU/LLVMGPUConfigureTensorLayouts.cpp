...
getContractionLayout(Operation *candidate, ArrayRef<int64_t> bounds,
  ...
  // individually.
  //
  // For block intrinsics the subgroup layout is 3D: [Block, outer, inner].
  // blockDim is the iteration-space dim consumed by the block dimension.
  ...
  // For non-block intrinsics blockDim is unused (-1 is safe because the
  // non-block branch never indexes with it).
  VectorLayoutInterface lhs =
