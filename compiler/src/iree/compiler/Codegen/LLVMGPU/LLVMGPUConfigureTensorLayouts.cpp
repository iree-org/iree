...
static LogicalResult setDerivedThreadConfigLayout(
  ...
  // Project the layout to create a layout mapping for each operand.
  Location loc = linalgOp.getLoc();
  // rewriter.setInsertionPoint(linalgOp);
  // for (OpOperand &operand : linalgOp->getOpOperands()) {
  //   VectorLayoutInterface operandLayout =
  //       getLayoutForMap(layout, linalgOp.getMatchingIndexingMap(&operand));
  //   auto toLayout = rewriter.create<ToLayoutOp>(
  //       loc, operand.get(), operandLayout);
