...
static void prefetchSharedMemory(FunctionOpInterface funcOp, unsigned numStages) {
  IRRewriter rewriter(funcOp.getContext());
  SmallVector<scf::ForOp> loops;
  funcOp.walk([&loops](scf::ForOp forOp) { loops.push_back(forOp); });

  for (scf::ForOp forOp : loops) {
    FailureOr<scf::ForOp> newLoop = prefetchSharedMemoryCopy(rewriter, forOp, numStages);
...
struct ROCDLPrefetchSharedMemoryPass final
  ...
  using impl::ROCDLPrefetchSharedMemoryPassBase<
      ROCDLPrefetchSharedMemoryPass>::ROCDLPrefetchSharedMemoryPassBase;
