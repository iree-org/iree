...
setVectorDistributionConfig(mlir::FunctionOpInterface entryPoint,
  ...
  // targetInfo.supportedSubgroupSizes;
  ...
  // HACK: need proper heuristics for workgroup size.
  tileSizes.push_back(workgroupTileSizes);
  workgroupSize[0] = 64;

  // This pipeline is currently unaware of the subgroup size.
...
static LogicalResult setRootConfig(mlir::FunctionOpInterface entryPointFn,
  ...
  if (auto linalgOp = dyn_cast<linalg::LinalgOp>(computeOp)) {
    if (succeeded(
            setVectorDistributionConfig(entryPointFn, linalgOp, targetInfo))) {
      return success();
    }
