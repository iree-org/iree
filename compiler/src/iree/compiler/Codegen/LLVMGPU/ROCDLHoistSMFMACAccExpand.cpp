...
/// For the loop boundary case, hoist the first expand (from iter_arg) before
/// the loop and the last collapse (to iter_arg) after the loop. This changes
/// the relevant elements in the packed iter_arg from 2-element to 4-element.
///
/// This is a more invasive transformation that modifies the loop iter_arg
/// packing structure.
///
/// TODO(#HACK): This handles the loop-carried dependency case. The iter_arg
/// is a packed multi-tile vector (e.g., vector<1x2x2x1xf32>). Individual
/// 2-element accumulators are extracted, expanded, processed, collapsed, and
/// packed back. We change the packing to carry 4-element accumulators and
/// only collapse after the loop.
static LogicalResult tryHoistLoopBoundary(IRRewriter &rewriter,
                                          scf::ForOp forOp) {
  // For now, we rely on the peephole to handle intra-iteration pairs.
  // The loop boundary hoisting would require changing the packed iter_arg
  // structure, which is significantly more complex. This is left as a future
  // enhancement.
  //
  // The peephole eliminates the K-unroll collapse→expand pairs. The remaining
  // expand (from iter_arg) and collapse (to iter_arg) at the loop boundary
  // are one per N-tile per iteration.
  return failure();
}
