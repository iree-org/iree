...
struct ContractToChainFMA final : OpRewritePattern<vector::ContractionOp> {
  ...
  LogicalResult matchAndRewrite(vector::ContractionOp op,
    ...
    Value lhs = op.getLhs();
    Value rhs = op.getRhs();
    Type accElemType = accVecType.getElementType();
    ...
    // This follows ContractOpToElementwise from upstream MLIR.
    ...
    for (unsigned i = 0; i < numParallelDims; i++) {
      ...
      if (rhsDim) {
        ...
        // Missing parallel dimension - needs broadcast
...
struct LLVMGPUVectorLoweringPass final
  ...
  void runOnOperation() override {
    ...
    // {
    //   RewritePatternSet patterns(ctx);
    //   patterns.add<ContractToChainFMA>(ctx);
    //   if (failed(applyPatternsGreedily(funcOp, std::move(patterns)))) {
    //     return signalPassFailure();
    //   }
    // }
