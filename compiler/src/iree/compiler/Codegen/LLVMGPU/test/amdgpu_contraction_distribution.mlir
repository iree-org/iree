...
builtin.module attributes { transform.with_named_sequence } {
  ...
  transform.named_sequence @__transform_main(%variant_op: !transform.any_op {transform.readonly}) {
    ...
    transform.iree.test_amdgpu_contraction_distribution %top_level_func {m = 16, n = 16, k = 16}: !transform.any_op
    transform.yield
  }
}
...
#layout_c = #iree_vector_ext.layout<#row_layout2, #col_layout2>
builtin.module attributes { transform.with_named_sequence } {
  func.func @distribute_mfma_32x32x8_mmt(%a : vector<32x8xf16>, %b : vector<32x8xf16>, %c : vector<32x32xf32>) -> vector<32x32xf32> {
    // CHECK-LABEL: distribute_mfma_32x32x8_mmt
    // CHECK-SAME: %[[ARG0:.+]]: vector<32x8xf16>, %[[ARG1:.+]]: vector<32x8xf16>, %[[ARG2:.+]]: vector<32x32xf32>
    ...
    // CHECK-DAG: %[[AV:.+]] = vector.extract %[[A]][0, 0] : vector<4xf16> from vector<1x1x4xf16>
    // CHECK-DAG: %[[B:.+]] = iree_vector_ext.to_simt %[[ARG1]] : vector<32x8xf16> -> vector<1x1x4xf16>
    // CHECK-DAG: %[[BV:.+]] = vector.extract %[[B]][0, 0] : vector<4xf16> from vector<1x1x4xf16>
    ...
    %output = vector.contract {indexing_maps = [#map1, #map2, #map3], iterator_types = ["parallel", "parallel", "reduction"],
                               ...
                               "__vector_layout_test_anchor_result_0" = #layout_c
                                ...
                                %a, %b, %c : vector<32x8xf16>, vector<32x8xf16> into vector<32x32xf32>
    return %output : vector<32x32xf32>
  }
  transform.named_sequence @__transform_main(%variant_op: !transform.any_op {transform.readonly}) {
    %top_level_func = transform.structured.match ops{["func.func"]} in %variant_op : (!transform.any_op) -> !transform.any_op
    transform.iree.test_amdgpu_contraction_distribution %top_level_func {m = 32, n = 32, k = 8}: !transform.any_op
    transform.yield
  }
}
