// RUN: iree-opt -pass-pipeline="builtin.module(func.func(iree-llvmgpu-prefetch-shared-memory),cse,canonicalize)" %s --split-input-file --debug-only=iree-codegen-llvmgpu-prefetch-shared-memory-copy | FileCheck %s
