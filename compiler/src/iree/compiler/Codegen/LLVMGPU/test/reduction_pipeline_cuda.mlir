...
//         CHECK:    %[[R0:.+]] = scf.for %{{.*}} = %c0 to %c10240 step %c1024 iter_args(%[[A0:.+]] = %[[CST]]) -> (vector<1x1x1x1x1x4xf32>) {
//         CHECK:      %[[V:.+]] = vector.transfer_read {{.*}} : memref<512x10240xf32, #hal.descriptor_type<storage_buffer>>, vector<1x4xf32>
//         CHECK:      %[[STRIDED:.+]] = vector.insert_strided_slice %[[V]], {{.*}} : vector<1x4xf32> into vector<1x1x1x1x1x4xf32>
//         CHECK:      %[[ADD:.+]] = arith.addf %[[STRIDED]], %[[A0]] : vector<1x1x1x1x1x4xf32>
//         CHECK:      scf.yield %[[ADD]] : vector<1x1x1x1x1x4xf32>
//         CHECK:    }
...
//         CHECK:    vector.transfer_write %{{.*}}, %[[ALLOC]]{{.*}} : vector<1x1xf32>
//         CHECK:    gpu.barrier
...
//    CHECK-SAME:      translation_info = #[[TRANSLATION_INFO]]
//         CHECK:    scf.for {{.*}} -> (vector<1x1x1x1x1x4xf32>) {
//         CHECK:      vector.transfer_read {{.*}} : memref<512x10240xf32,
//         CHECK:      arith.addf {{.*}} : vector<1x1x1x1x1x4xf32>
//         CHECK:      scf.yield
//         CHECK:    gpu.subgroup_reduce
//         CHECK:    vector.transfer_write {{.*}} : vector<1x1xf32
//         CHECK:    gpu.subgroup_reduce
//         CHECK:    arith.divf {{.*}} : vector<1x1x1xf32>
//         CHECK:    vector.transfer_write {{.*}} : vector<1x4xf32>, {{.*}}
//         CHECK:    return
...
//    CHECK-SAME:      translation_info = #[[TRANSLATION_INFO]]
//         CHECK:    scf.for {{.*}} -> (vector<1x1x1x1x1x1x1x1x4xf32>) {
//         CHECK:      vector.transfer_read {{.*}} : memref<12x128x40960xf32,
//         CHECK:      arith.maxnumf {{.*}} : vector<1x1x1x1x1x1x1x1x4xf32>
//         CHECK:      scf.yield
...
//         CHECK:    gpu.barrier
//         CHECK:    gpu.subgroup_reduce  maxnumf
//         CHECK:    vector.broadcast %{{.*}} : f32 to vector<1x1x1x1x1x1x1x1x4xf32>
//         CHECK:    scf.for {{.*}} -> (vector<1x1x1x1x1x1x1x1x4xf32>) {
...
//     CHECK-DAG:   %[[CST:.+]] = arith.constant dense<0.000000e+00> : vector<1x1x1x1x1x1x1x1x4xf16>
//         CHECK:   scf.for %{{.+}} = %[[C0]] to %[[C32]] step %[[C1]] iter_args(%{{.*}} = %[[CST]]) -> (vector<1x1x1x1x1x1x1x1x4xf16>)
//         CHECK:     arith.mulf %{{.*}}, %{{.*}} : vector<1x1x1x1x1x1x1x1x4xf16>
//         CHECK:     arith.addf %{{.*}}, %{{.*}} : vector<1x1x1x1x1x1x1x1x4xf16>

//         CHECK:   vector.multi_reduction <add>, %{{.*}}, %{{.*}} [1, 2, 4, 5, 7, 8] : vector<1x1x1x1x1x1x1x1x4xf16> to vector<1x1x1xf16>
