...
func.func @sort2D_static_shape() {
  ...
  %1 = flow.dispatch.tensor.load %0, offsets = [0, 0], sizes = [2000, 30000], strides = [1, 1] : !flow.dispatch.tensor<readwrite:tensor<2000x30000xi32>> -> tensor<2000x30000xi32>
...
//       CHECK:     translation_info = #[[$TRANSLATION]]
//       CHECK:   iree_linalg_ext.sort {
//  CHECK-SAME:       lowering_config = #iree_gpu.lowering_config<{thread = [1], workgroup = [1]}>
...
func.func @sort5D_static_shape() {
  %c0 = arith.constant 0 : index
  %0 = hal.interface.binding.subspan layout(<bindings = [#hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) binding(0) alignment(64) offset(%c0) flags(Indirect) {iree_gpu.use_rocdl_buffer_instructions} : !flow.dispatch.tensor<readwrite:tensor<4x100x100x200x300xi32>>
  %1 = flow.dispatch.tensor.load %0, offsets = [0, 0, 0, 0, 0], sizes = [4, 100, 100, 200, 300], strides = [1, 1, 1, 1, 1] : !flow.dispatch.tensor<readwrite:tensor<4x100x100x200x300xi32>> -> tensor<4x100x100x200x300xi32>
  ...
  flow.dispatch.tensor.store %2, %0, offsets = [0, 0, 0, 0, 0], sizes = [4, 100, 100, 200, 300], strides = [1, 1, 1, 1, 1] : tensor<4x100x100x200x300xi32> -> !flow.dispatch.tensor<readwrite:tensor<4x100x100x200x300xi32>>
  return
}
...
//       CHECK:     translation_info = #[[$TRANSLATION]]
//       CHECK:   iree_linalg_ext.sort {
//  CHECK-SAME:       lowering_config = #iree_gpu.lowering_config<{thread = [0, 0, 1, 1, 1], workgroup = [0, 0, 1, 1, 1]}>
