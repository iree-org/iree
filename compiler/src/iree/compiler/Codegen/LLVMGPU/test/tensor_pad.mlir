...
func.func @transpose_no_align_dispatch_0_generic_48x32() {
  ...
  %0 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c0) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x48xf32>>
  %1 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c0) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<48x32xf32>>
  ...
  scf.for %arg0 = %2 to %c48 step %3 {
    ...
    scf.for %arg1 = %5 to %c32 step %6 {
      %7 = iree_tensor_ext.dispatch.tensor.load %1, offsets = [%arg0, %arg1], sizes = [%4, 32], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<48x32xf32>> -> tensor<?x32xf32>
      %8 = iree_tensor_ext.dispatch.tensor.load %0, offsets = [%arg1, %arg0], sizes = [32, %4], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x48xf32>> -> tensor<32x?xf32>
      ...
      iree_tensor_ext.dispatch.tensor.store %9, %1, offsets = [%arg0, %arg1], sizes = [%4, 32], strides = [1, 1] : tensor<?x32xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<48x32xf32>>
    }
  }
  return
}
...
//       CHECK:  %[[D0:.*]] = hal.interface.binding.subspan layout({{.+}}) binding(0) alignment(64) offset(%[[C0]]) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x48xf32>>
//       CHECK:  %[[D1:.*]] = hal.interface.binding.subspan layout({{.+}}) binding(1) alignment(64) offset(%[[C0]]) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<48x32xf32>>
...
//       CHECK:      %[[D7:.*]] = iree_tensor_ext.dispatch.tensor.load %[[D1]], offsets = {{\[}}%[[ARG0]], %[[ARG1]]], sizes = {{\[}}%[[D4]], 32], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<48x32xf32>> -> tensor<?x32xf32>
//       CHECK:      %[[D8:.*]] = iree_tensor_ext.dispatch.tensor.load %[[D0]], offsets = {{\[}}%[[ARG1]], %[[ARG0]]], sizes = [32, %[[D4]]], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<32x48xf32>> -> tensor<32x?xf32>
...
//       CHECK:      iree_tensor_ext.dispatch.tensor.store %[[D13]], %[[D1]], offsets = {{\[}}%[[ARG0]], %[[ARG1]]], sizes = {{\[}}%[[D4]], 32], strides = [1, 1] : tensor<?x32xf32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<48x32xf32>>
...
func.func @unpack_dynamic() {
  ...
  %8 = hal.interface.binding.subspan layout(#pipeline_layout) binding(0) alignment(64) offset(%c64) flags(ReadOnly) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?x2x2xi32>>{%4, %5}
  %9 = hal.interface.binding.subspan layout(#pipeline_layout) binding(1) alignment(64) offset(%c0) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<?x?xi32>>{%6, %7}
  %workgroup_id_x = hal.interface.workgroup.id[0] : index
  %workgroup_count_x = hal.interface.workgroup.count[0] : index
  %workgroup_id_y = hal.interface.workgroup.id[1] : index
  %workgroup_count_y = hal.interface.workgroup.count[1] : index
  ...
  scf.for %arg0 = %10 to %6 step %11 {
    ...
    scf.for %arg1 = %13 to %7 step %14 {
      ...
      %16 = iree_tensor_ext.dispatch.tensor.load %9, offsets = [%arg0, %arg1], sizes = [%12, %15], strides = [1, 1] : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<?x?xi32>>{%6, %7} -> tensor<?x?xi32>
      %17 = affine.apply #map2(%12)
      %18 = affine.apply #map2(%15)
      %19 = affine.apply #map3(%arg0)
      %20 = affine.apply #map3(%arg1)
      %21 = iree_tensor_ext.dispatch.tensor.load %8, offsets = [%19, %20, 0, 0], sizes = [%17, %18, 2, 2], strides = [1, 1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<?x?x2x2xi32>>{%4, %5} -> tensor<?x?x2x2xi32>
      ...
      iree_tensor_ext.dispatch.tensor.store %unpack, %9, offsets = [%arg0, %arg1], sizes = [%12, %15], strides = [1, 1] : tensor<?x?xi32> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<?x?xi32>>{%6, %7}
    }
  }
  return
}
...
// CHECK:           %[[LOAD:.+]] = iree_tensor_ext.dispatch.tensor.load %[[DEST_BUF]]
// CHECK:           %[[PAD:.+]] = tensor.pad %[[LOAD]]
// CHECK:           %[[SLICE:.+]] = tensor.extract_slice %[[PAD]]
// CHECK:           %[[UNPACK:.+]] = linalg.unpack {{.+}} into %[[SLICE]]
// CHECK:           iree_tensor_ext.dispatch.tensor.store %[[UNPACK]], %[[DEST_BUF]]
