// RUN: iree-opt --split-input-file --pass-pipeline='hal.executable(hal.executable.variant(iree-llvmgpu-lower-executable-target-pass, fold-memref-alias-ops, canonicalize, cse))' %s | FileCheck %s
