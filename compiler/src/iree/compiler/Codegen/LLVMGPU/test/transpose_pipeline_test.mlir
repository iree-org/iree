...
//       CHECK:   %[[D9:.*]] = vector.transfer_read %[[D4]]{{\[}}%{{.*}}, %[[D7]], %[[D8]]], %[[CST]] {in_bounds = [true]} : memref<10x2048x768xf32>, vector<4xf32>
//       CHECK:   %[[D10:.*]] = affine.apply #{{.*}}(){{\[}}%[[D0]], %[[D1]], %[[D2]]]
//       CHECK:   %[[D11:.*]] = affine.apply #{{.*}}(){{\[}}%[[D0]]]
//       CHECK:   vector.transfer_write %[[D9]], %[[D3]]{{\[}}%[[C0]], %[[D10]], %[[D11]]] {in_bounds = [true]} : vector<4xf32>, memref<1x32x33xf32, 3>
//       CHECK:   gpu.barrier
      ...
      builtin.module {
        ...
        scf.for %arg0 = %workgroup_id_z to %c10 step %workgroup_count_z {
            ...
            scf.for %arg2 = %3 to %c2048 step %4 {
                ...
                flow.dispatch.tensor.store %11, %2, offsets = [%arg0, %arg1, %arg2], sizes = [1, 1, %c256], strides = [1, 1, 1] : tensor<1x1x?xf32> -> !flow.dispatch.tensor<writeonly:10x768x2048xf32>
            }
            }
        }
        return
