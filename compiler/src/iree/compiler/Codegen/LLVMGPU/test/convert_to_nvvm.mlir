// RUN: iree-opt --pass-pipeline="builtin.module(hal.executable(hal.executable.variant(builtin.module(iree-convert-to-nvvm))))" --iree-codegen-test-target=sm_60 --split-input-file %s | FileCheck %s
