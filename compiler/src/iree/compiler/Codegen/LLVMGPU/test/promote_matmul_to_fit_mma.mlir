...
func.func @batch_matmul_f16() {
  ...
  %7 = iree_tensor_ext.dispatch.tensor.load %2, offsets = [%workgroup_id_z, %3, %4], sizes = [1, %5, %6], strides = [1, 1, 1] : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<64x968x1281xf16>> -> tensor<1x?x?xf16>
  %8 = iree_tensor_ext.dispatch.tensor.load %0, offsets = [%workgroup_id_z, %3, 0], sizes = [1, %5, 1281], strides = [1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x968x1281xf16>> -> tensor<1x?x1281xf16>
  %9 = iree_tensor_ext.dispatch.tensor.load %1, offsets = [%workgroup_id_z, 0, %4], sizes = [1, 1281, %6], strides = [1, 1, 1] : !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x1281x1281xf16>> -> tensor<1x1281x?xf16>
  ...
  iree_tensor_ext.dispatch.tensor.store %11, %2, offsets = [%workgroup_id_z, %3, %4], sizes = [1, %5, %6], strides = [1, 1, 1] : tensor<1x?x?xf16> -> !iree_tensor_ext.dispatch.tensor<writeonly:tensor<64x968x1281xf16>>
  return
}
// CHECK-LABEL:     func.func @batch_matmul_f16
// CHECK:             %[[LHS_HANDLE:.+]] = hal.interface.binding.subspan layout({{.+}}) binding(0) alignment(64) offset(%c0) flags(ReadOnly) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x968x1281xf16>>
// CHECK:             %[[RHS_HANDLE:.+]] = hal.interface.binding.subspan layout({{.+}}) binding(1) alignment(64) offset(%c0) flags(ReadOnly) : !iree_tensor_ext.dispatch.tensor<readonly:tensor<64x1281x1281xf16>>
// CHECK:             %[[OUT_HANDLE:.+]] = hal.interface.binding.subspan layout({{.+}}) binding(2) alignment(64) offset(%c0) : !iree_tensor_ext.dispatch.tensor<writeonly:tensor<64x968x1281xf16>>
// CHECK-DAG:         %[[LHS:.+]] = iree_tensor_ext.dispatch.tensor.load %[[LHS_HANDLE]]
// CHECK-DAG:         %[[RHS:.+]] = iree_tensor_ext.dispatch.tensor.load %[[RHS_HANDLE]]
...
// CHECK:             iree_tensor_ext.dispatch.tensor.store %[[OUT_SLICE]], %[[OUT_HANDLE]]
