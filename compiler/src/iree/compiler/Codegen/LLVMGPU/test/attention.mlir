...
transform.sequence failures(propagate) {
  ...
  transform.iree.apply_patterns %top_level_func { canonicalization, cse } : (!transform.any_op) -> ()
  ...
  transform.iree.apply_patterns %func { cse } : (!transform.any_op) -> ()
  ...
  transform.iree.apply_patterns %func_7 { fold_memref_aliases, licm, canonicalization, cse } : (!transform.any_op) -> ()
  %func_8 = transform.structured.hoist_redundant_vector_transfers %memref_func
  : (!transform.any_op) -> !transform.any_op
  transform.iree.apply_patterns %func_8 { canonicalization, cse } : (!transform.any_op) -> ()
...
// CHECK:          %[[D30:.+]] = vector.broadcast %[[D23]] : vector<32xf32> to vector<64x32xf32>
// CHECK:          %[[D31:.+]] = vector.broadcast %[[D25]] : vector<32xf32> to vector<64x32xf32>
// CHECK:          %[[D32:.+]] = arith.mulf %[[D30]], %[[D31]] : vector<64x32xf32>
// CHECK:          %[[D33:.+]] = vector.transpose %[[D32]], [1, 0] : vector<64x32xf32> to vector<32x64xf32>
