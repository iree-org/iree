...
func.func @unwrap_masked_matmul(
    %lhs: vector<8x16xf32>,
    %rhs: vector<16x8xf32>,
    %acc: vector<8x8xf32>,
  ...
  // CHECK: %[[BCAST:.+]] = vector.broadcast %[[EXT]]
  // CHECK: %[[RHS_MASK:.+]] = vector.transpose %[[BCAST]], [1, 0]
  // CHECK: %[[LHS_MASKED:.+]] = arith.select %[[BCAST]], %[[LHS]], %[[IDENTITY_LHS]]
...
func.func @unwrap_masked_matmul_transposed_rhs(%lhs: vector<8x16xf32>, %rhs: vector<8x16xf32>, %acc: vector<8x8xf32>, %m: index, %n: index, %k: index) -> vector<8x8xf32> {
  ...
  // After CSE, only one step/broadcast/cmpi sequence is produced.
  ...
  // CHECK: %[[LHS_MASKED:.+]] = arith.select %[[MASK]], %[[LHS]], %[[IDENTITY]]
  // CHECK: %[[RHS_MASKED:.+]] = arith.select %[[MASK]], %[[RHS]], %[[IDENTITY]]
  // CHECK: vector.contract {{.+}} %[[LHS_MASKED]], %[[RHS_MASKED]], %[[ACC]]
...
// CHECK-LABEL: func.func @unwrap_masked_matmul_two_reduction_dims
...
func.func @unwrap_masked_matmul_two_reduction_dims(
...
// CHECK-LABEL: func.func @unwrap_masked_multi_reduction
// CHECK-SAME: %[[SRC:[a-zA-Z0-9]+]]: vector<16x64xf32>
// CHECK-SAME: %[[ACC:[a-zA-Z0-9]+]]: vector<16xf32>
// CHECK-SAME: %[[DIM:[a-zA-Z0-9]+]]: index
func.func @unwrap_masked_multi_reduction(%src: vector<16x64xf32>, %acc: vector<16xf32>, %dim: index) -> vector<16xf32> {
...
func.func @masked_reduction_add(%src : vector<16xf32>, %acc : f32, %mask : vector<16xi1>) -> f32 {
