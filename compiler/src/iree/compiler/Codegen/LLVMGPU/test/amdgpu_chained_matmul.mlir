...
builtin.module {
  ...
  func.func @chained_matmul(%lhs : vector<32x8xf16>, %rhs : vector<16x8xf16>, %acc : vector<32x16xf16>,
    ...
    // CHECK: %[[RES:.*]] = vector.contract {indexing_maps = [#[[MAP]], #[[MAP1]], #[[MAP2]]], iterator_types = ["parallel", "parallel", "reduction"], kind = #vector.kind<add>}
    // CHECK-SAME: %[[RHS]], %[[LHS]], %[[ACC]] : vector<16x8xf16>, vector<32x8xf16> into vector<32x16xf16>
    ...
    // CHECK: %[[RES2:.*]] = vector.contract {indexing_maps = [#[[MAP]], #[[MAP1]], #[[MAP2]]], iterator_types = ["parallel", "parallel", "reduction"], kind = #vector.kind<add>}
    // CHECK-SAME: %[[RHS2]], %[[EXP]], %[[ACC2]] : vector<8x16xf16>, vector<32x16xf16> into vector<32x8xf16>
...
builtin.module {
  func.func @chained_matmul_mmt_mm(%lhs : vector<32x8xf16>, %rhs : vector<16x8xf16>, %acc : vector<32x16xf16>,
    ...
    // CHECK: %[[EXP:.*]] = math.exp2 %[[RES]] : vector<32x16xf16>
    %exp = math.exp2 %result : vector<32x16xf16>
    // CHECK: %[[RES2:.*]] = vector.contract {indexing_maps = [#[[MAP3]], #[[MAP1]], #[[MAP2]]], iterator_types = ["parallel", "parallel", "reduction"], kind = #vector.kind<add>}
    // CHECK-SAME: %[[RHS2]], %[[EXP]], %[[ACC2]] : vector<16x8xf16>, vector<16x32xf16> into vector<32x8xf16>
