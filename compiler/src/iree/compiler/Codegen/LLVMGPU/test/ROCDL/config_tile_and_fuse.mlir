...
// RUN: iree-opt --mlir-print-local-scope --split-input-file --iree-gpu-test-target=gfx942 \
// RUN: --iree-codegen-llvmgpu-use-tile-and-fuse-matmul=false  --iree-codegen-llvmgpu-use-igemm=false \
...
// LATE-SAME:    #iree_codegen.translation_info<pipeline = LLVMGPUTileAndFuse workgroup_size = [64, 16, 1] subgroup_size = 64
// LATE-SAME:    {gpu_pipeline_options = #iree_gpu.pipeline_options<prefetch_shared_memory = false, no_reduce_shared_memory_bank_conflicts = true, use_igemm_convolution = false>}
//      LATE:    linalg.batch_matmul {{.*}}lowering_config = #iree_gpu.lowering_config
//  LATE-SAME:     reduction = [0, 0, 0, 1]
//  LATE-SAME:     thread = [1, 1, 8, 0]
//  LATE-SAME:     workgroup = [1, 64, 128, 1]
...
// LATE-SAME:  {translation_info = #iree_codegen.translation_info<pipeline = LLVMGPUTileAndFuse workgroup_size = [64, 16, 1] subgroup_size = 64
// LATE:       linalg.generic
// LATE-SAME:  {lowering_config = #iree_gpu.lowering_config<{
// LATE-SAME:  reduction = [0, 0, 0, 1],
// LATE-SAME:  thread = [1, 8, 0, 0],
// LATE-SAME:  workgroup = [64, 0, 1, 1]}
