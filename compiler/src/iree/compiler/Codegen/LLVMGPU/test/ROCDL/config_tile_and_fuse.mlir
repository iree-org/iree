...
func.func @skinny_gemm_m8_small_k_f16(%lhs: tensor<8x64xf16>, %rhs: tensor<64x4096xf16>) -> tensor<8x4096xf32> {
  %cst = arith.constant 0.000000e+00 : f32
  %init = tensor.empty() : tensor<8x4096xf32>
  %fill = linalg.fill ins(%cst : f32) outs(%init : tensor<8x4096xf32>) -> tensor<8x4096xf32>
  %result = linalg.generic {
    indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d2)>,
                     ...
                     affine_map<(d0, d1, d2) -> (d0, d1)>],
    iterator_types = ["parallel", "parallel", "reduction"]}
    ins(%lhs, %rhs : tensor<8x64xf16>, tensor<64x4096xf16>) outs(%fill : tensor<8x4096xf32>) {
  ^bb0(%in: f16, %in_0: f16, %out: f32):
    %0 = arith.extf %in : f16 to f32
    %1 = arith.extf %in_0 : f16 to f32
    %2 = arith.mulf %0, %1 : f32
