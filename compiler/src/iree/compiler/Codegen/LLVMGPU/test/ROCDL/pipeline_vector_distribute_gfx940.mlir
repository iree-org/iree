...
#config = #iree_codegen.lowering_config<tile_sizes = [[1, 1, 1, 512]]>
#translation = #iree_codegen.translation_info<LLVMGPUVectorDistribute workgroup_size = [64, 1, 1] subgroup_size = 64>

hal.executable private @main_reduction {
  hal.executable.variant public @rocm_hsaco_fb target(<"rocm", "rocm-hsaco-fb">) {
    hal.executable.export public @main_reduction_2x32x10x4096_f16xf32xf32xf32 ordinal(0) layout(#hal.pipeline.layout<bindings = [#hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) {
    ^bb0(%arg0: !hal.device):
      %x, %y, %z = flow.dispatch.workgroup_count_from_slice
      hal.return %x, %y, %z : index, index, index
    }
    builtin.module {
      func.func @main_reduction_2x32x10x4096_f16xf32xf32xf32() attributes {translation_info = #translation} {
        %cst = arith.constant 0.000000e+00 : f32
        ...
        %0 = hal.interface.binding.subspan layout(<bindings = [#hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) binding(0) alignment(64) offset(%c69524992) flags("ReadOnly|Indirect") : !flow.dispatch.tensor<readonly:tensor<2x32x10x4096xf16>>
        %1 = hal.interface.binding.subspan layout(<bindings = [#hal.pipeline.binding<storage_buffer, "ReadOnly|Indirect">, #hal.pipeline.binding<storage_buffer, Indirect>], flags = Indirect>) binding(1) alignment(64) offset(%c74767872) flags(Indirect) : !flow.dispatch.tensor<writeonly:tensor<2x32xf32>>
        ...
        %5 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%2 : tensor<2x32x10x4096xf16>) outs(%3 : tensor<2x32x10x4096xf32>) attrs =  {lowering_config = #config} {
        ^bb0(%in: f16, %out: f32):
          %11 = arith.extf %in : f16 to f32
          linalg.yield %11 : f32
        } -> tensor<2x32x10x4096xf32>
        %6 = linalg.fill {lowering_config = #config} ins(%cst : f32) outs(%4 : tensor<2x32xf32>) -> tensor<2x32xf32>
        %7 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1)>], iterator_types = ["parallel", "parallel", "reduction", "reduction"]} ins(%5 : tensor<2x32x10x4096xf32>) outs(%6 : tensor<2x32xf32>) attrs =  {lowering_config = #config} {
        ^bb0(%in: f32, %out: f32):
          %11 = arith.addf %in, %out : f32
          linalg.yield %11 : f32
        } -> tensor<2x32xf32>
        %8 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%7 : tensor<2x32xf32>) outs(%4 : tensor<2x32xf32>) attrs =  {lowering_config = #config} {
        ^bb0(%in: f32, %out: f32):
...
// CHECK-LABEL: func.func @main_reduction_2x32x10x4096_f16xf32xf32xf32()
// CHECK:     vector.multi_reduction <add>, {{.+}} : vector<1x1x8xf32> to f32
// CHECK:     scf.yield
// CHECK:     vector.transfer_write {{.+}} : vector<f32>, memref<2x32xf32, strided<[32, 1], offset: 18691968>, #hal.descriptor_type<storage_buffer>>
