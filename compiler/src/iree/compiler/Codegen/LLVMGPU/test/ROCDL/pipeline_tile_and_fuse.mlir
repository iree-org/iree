...
//       CHECK:   %[[B:.+]] = amdgpu.fat_raw_buffer_cast %[[BIND]]
  ...
  hal.executable.variant public @rocm_hsaco_fb target(<"rocm", "rocm-hsaco-fb">) {
    ...
    builtin.module {
      func.func @multi_mma_data_tiled_unrolled_MFMA_F32_16x16x4_F32()
        ...
        %2 = hal.interface.binding.subspan layout(#pipeline_layout) binding(2) alignment(64) offset(%c0) flags(Indirect) : !flow.dispatch.tensor<readwrite:tensor<4x4x4x8x2x4x16x4xf32>>
        ...
        %5 = flow.dispatch.tensor.load %2, offsets = [0, 0, 0, 0, 0, 0, 0, 0], sizes = [4, 4, 4, 8, 2, 4, 16, 4], strides = [1, 1, 1, 1, 1, 1, 1, 1] : !flow.dispatch.tensor<readwrite:tensor<4x4x4x8x2x4x16x4xf32>> -> tensor<4x4x4x8x2x4x16x4xf32>
        ...
        flow.dispatch.tensor.store %6, %2, offsets = [0, 0, 0, 0, 0, 0, 0, 0], sizes = [4, 4, 4, 8, 2, 4, 16, 4], strides = [1, 1, 1, 1, 1, 1, 1, 1] : tensor<4x4x4x8x2x4x16x4xf32> -> !flow.dispatch.tensor<readwrite:tensor<4x4x4x8x2x4x16x4xf32>>
        return
      }
    }
  }
...
// CHECK-DAG:  %[[BUFFER_A:.+]] = amdgpu.fat_raw_buffer_cast %[[BINDING_A]]
// CHECK-DAG:  %[[BINDING_B:.+]] = hal.interface.binding.subspan {{.*}} binding(1)
// CHECK-DAG:  %[[BUFFER_B:.+]] = amdgpu.fat_raw_buffer_cast %[[BINDING_B]]
// CHECK-DAG:  %[[BINDING_C:.+]] = hal.interface.binding.subspan {{.*}} binding(2)
// CHECK-DAG:  %[[BUFFER_C:.+]] = amdgpu.fat_raw_buffer_cast %[[BINDING_C]]
...
// CHECK-DAG:         %[[INPUT_SUBVIEW:.+]] = memref.subview {{.*}} : memref<100x250xi32, #hal.descriptor_type<storage_buffer>> to memref<?x?xi32, strided<[250, 1], offset: ?>, #hal.descriptor_type<storage_buffer>>
...
// CHECK-DAG:         memref.copy %[[INPUT_SUBVIEW]], %[[ALLOCA_SUBVIEW]] : memref<?x?xi32, strided<[250, 1], offset: ?>, #hal.descriptor_type<storage_buffer>> to memref<?x?xi32, strided<[1, 1]>, #gpu.address_space<private>>
// CHECK-DAG:         %[[READ:.+]] = vector.transfer_read{{.*}}: memref<1x4xi32, strided<[4, 1]>, #gpu.address_space<private>>, vector<4xi32>
// CHECK-DAG:         vector.transfer_write %[[READ]]{{.*}}: vector<4xi32>, memref<16x4x16x32xi32, #hal.descriptor_type<storage_buffer>>
