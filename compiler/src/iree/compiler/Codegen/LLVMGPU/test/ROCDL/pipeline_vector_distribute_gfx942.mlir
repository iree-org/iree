...
// CHECK:   scf.for {{.*}} = %c0 to %c256 step %c128 iter_args(%[[ACC:.+]] = {{.*}}) -> (vector<16xf32>)
...
// CHECK:   scf.for {{.*}} = %c0 to %c256 step %c128 iter_args(%[[ARG:.+]] = {{.*}}) -> (vector<4xf32>)
...
func.func @attention_gather_k() attributes {hal.executable.target = #executable_target_rocm, translation_info = #translation} {
  ...
  ^bb0(%in: i64, %out: f16):
    ...
    %16 = arith.index_cast %in : i64 to index
    %17 = linalg.index 3 : index
    %extracted = tensor.extract %5[%14, %15, %16, %17] : tensor<2x10x4096x64xf16>
    linalg.yield %extracted : f16
