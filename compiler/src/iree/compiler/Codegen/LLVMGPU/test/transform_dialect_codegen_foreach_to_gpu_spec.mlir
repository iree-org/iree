...
^bb1(%variant_op: !pdl.operation):
  ...
  %foreach_thread, %tiled_fill = transform.structured.tile_to_forall_op %0 num_threads [5, 1] 
  ( mapping = [#gpu.thread<y>, #gpu.thread<x>] )

  %1 = transform.structured.match ops{["linalg.matmul"]} in %variant_op : (!pdl.operation) -> !pdl.operation
  %foreach_thread_2, %tiled_matmul = transform.structured.tile_to_forall_op %1 num_threads [7, 9]
