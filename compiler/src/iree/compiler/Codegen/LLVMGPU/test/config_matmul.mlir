...
func.func @matmul_32_32_32(%arg0: !TA, %arg1: !TB, %arg2: !TC, %arg3: !DTC) {
  ...
  // Sanity check that generalize/specialize works. This is the only place where this test file should use this prefix.
  // The pipeline/config should be the same irrespective if the op is specialized/generalized.
  // them so here-on in we only use CHECK (the prefix for both)
...
// CHECK;         workgroup_size = [64, 16, 1] subgroup_size = 64, {gpu_pipeline_options = #iree_gpu.pipeline_options<prefetch_shared_memory = false,
// CHECK:         no_reduce_shared_memory_bank_conflicts = true, use_igemm_convolution = false>}>
...
// CHECK-SPECIALIZED: LLVMGPUWarpReduction
// CHECK-GENERALIZED: LLVMGPUVectorDistribute
