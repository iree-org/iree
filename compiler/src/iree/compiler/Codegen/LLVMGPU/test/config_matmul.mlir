...
//      CHECK:         #iree_gpu.lowering_config<{expand_dims = {{\[}}[0], [1], [2, 3]{{\]}},
// CHECK-SAME:         lane_basis = {{\[}}[1, 1, 64, 1], [0, 1, 2, 3]{{\]}},
// CHECK-SAME:         partial_reduction = [0, 0, 1024, 0], subgroup_basis = {{\[}}[1, 1, 16, 1], [0, 1, 2, 3]{{\]}}, thread = [0, 0, 1, 4], workgroup = [1, 1, 0, 0]}
  %0 = linalg.matmul ins(%arg0, %arg1 : !TA, !TB) outs(%arg2 : !TC) -> !TC
...
func.func @matmul_DYN_1_4096(%arg0: !TA, %arg1: !TB, %arg2: !TC, %arg3: !DTC, %arg4 : index, %arg5 : index, %arg6 : index) {
  ...
  //      CHECK:     {expand_dims = {{\[}}[0], [1], [2, 3]{{\]}},
  // CHECK-SAME:     lane_basis = {{\[}}[1, 1, 64, 1], [0, 1, 2, 3]{{\]}},
  // CHECK-SAME:     partial_reduction = [0, 0, 1024, 0], subgroup_basis = {{\[}}[1, 1, 16, 1], [0, 1, 2, 3]{{\]}},
  // CHECK-SAME:     thread = [0, 0, 1, 4], workgroup = [1, 1, 0, 0]}
  %0 = linalg.matmul ins(%arg0, %arg1 : !TA, !TB) outs(%arg2 : !TC) -> !TC
