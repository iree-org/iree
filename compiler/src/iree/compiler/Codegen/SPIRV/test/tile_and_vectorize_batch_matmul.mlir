// Note: upstream hoisting on tensors does not canonicalize scf.for anymore, run -canonicalize to make this test happy

// RUN: iree-opt --split-input-file --pass-pipeline='builtin.module(hal.executable(hal.executable.variant(builtin.module(func.func(iree-spirv-tile,iree-spirv-vectorize,canonicalize,cse)))))' %s | FileCheck %s
