      ...
      func.func @static_scatter_update_slice() {
        ...
        scf.for %arg0 = %workgroup_id_y to %c40 step %workgroup_count_y {
          ...
          scf.for %arg1 = %3 to %c500 step %4 {
            ...
            iree_linalg_ext.scatter {lowering_config = #config} dimension_map(dense<0> : tensor<1xi64>) unique_indices(true) ins(%7, %9 : memref<?x?xi32, affine_map<(d0, d1)[s0] -> (d0 * 500 + s0 + d1)>>, memref<?x1xi32, affine_map<(d0, d1)[s0] -> (d0 + s0 + d1)>>) outs(%10 : memref<100x?xi32, affine_map<(d0, d1)[s0] -> (d0 * 500 + s0 + d1)>>)  {
