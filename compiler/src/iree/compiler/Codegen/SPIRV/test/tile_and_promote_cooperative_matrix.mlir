  ...
  hal.executable.variant public @vulkan_spirv_fb, target = <"vulkan-spirv", "vulkan-spirv-fb", {
    ...
    builtin.module {
      func.func @matmul_f16_128x262144x2304() {
        ...
        %1 = hal.interface.binding.subspan set(0) binding(0) type(storage_buffer) offset(%c96565312) alignment(64) : memref<128x2304xf16>
        %2 = hal.interface.binding.subspan set(0) binding(1) type(storage_buffer) offset(%c806357120) alignment(64) : memref<2304x262144xf16>
        %3 = hal.interface.binding.subspan set(0) binding(2) type(storage_buffer) offset(%c134217728) alignment(64) : memref<128x262144xf16>
        %workgroup_id_x = hal.interface.workgroup.id[0] : index
        %workgroup_count_x = hal.interface.workgroup.count[0] : index
        %workgroup_id_y = hal.interface.workgroup.id[1] : index
        %workgroup_count_y = hal.interface.workgroup.count[1] : index
