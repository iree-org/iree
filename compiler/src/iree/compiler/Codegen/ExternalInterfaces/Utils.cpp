...
void adjustTileSizesForBitcast(RankedTensorType type,
  ...
  assert(originalType.isIntOrFloat() && storageType.isIntOrFloat() &&
         "expected int or float element types in encoding");
  ...
  if (innermostTileSize == ShapedType::kDynamic) {
    return;
  }
