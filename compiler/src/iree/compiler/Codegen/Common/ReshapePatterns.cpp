...
/// Utility to return the inner product sizes for each dimension of the
/// provided sizes array. For example, this returns [12, 3, 1] for [2, 4, 3].
static SmallVector<OpFoldResult> getInnerSizes(ArrayRef<OpFoldResult> sizes,
                                               RewriterBase &rewriter) {
  AffineExpr d0, d1;
  bindDims(rewriter.getContext(), d0, d1);
  AffineExpr mul = d0 * d1;
  OpFoldResult res = rewriter.getIndexAttr(1);
  SmallVector<OpFoldResult> innerSizes;
  innerSizes.reserve(sizes.size());
  for (OpFoldResult size : llvm::reverse(llvm::drop_begin(sizes))) {
    innerSizes.push_back(res);
    res = affine::makeComposedAffineApply(rewriter, rewriter.getUnknownLoc(),
                                          mul, {res, size})
              .getResult();
  }
  innerSizes.push_back(res);
  std::reverse(innerSizes.begin(), innerSizes.end());
  return innerSizes;
}
