...
struct FoldCollapseShapeIntoInterfaceTensorStore
  ...
  LogicalResult matchAndRewrite(IREE::TensorExt::DispatchTensorStoreOp storeOp,
    ...
    for (auto &&[i, subspanSize, group, size] : llvm::enumerate(
    ...
    for (auto [subspanSize, group, offset, size] : llvm::zip_equal(
      ...
      // A potential non-zero store offset will be assigned to the last
      // dimension and this is accounted for by making the other offsets
      // for the expanded store zero.
      std::fill_n(std::back_inserter(expandedOffsets), innerDimSizes.size() - 1,
                  zero);
      expandedOffsets.push_back(offset);
