...
struct TileAndDistributeToWorkgroupsPass
  ...
  TileAndDistributeToWorkgroupsPass() : customPatternsFn(std::nullopt) {}
  TileAndDistributeToWorkgroupsPass(CustomTileAndDistributePatternsFn fn)
      : customPatternsFn(fn) {}
  ...
  std::optional<CustomTileAndDistributePatternsFn> customPatternsFn;
};
}  // namespace
  ...
  for (func::FuncOp funcOp : innerModule.getOps<func::FuncOp>()) {
    ...
    // Lower the workgroup count ops.
    {
      ...
      patterns.insert<LowerDispatchWorkgroupCountForDagRootOp>(
          ...
          partitionableLoops);
      if (customPatternsFn) {
        (*customPatternsFn)(patterns, dispatchRootOp);
      }
...
createTileAndDistributeToWorkgroupsPass(
  ...
  return std::make_unique<TileAndDistributeToWorkgroupsPass>(fn);
}
