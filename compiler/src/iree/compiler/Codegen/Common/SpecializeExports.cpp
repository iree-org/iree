...
getIterationDomainAsWorkload(TilingInterface specializationRoot) {
  ...
  for (auto [i, range] : llvm::enumerate(iterationSpace)) {
    ...
    auto assumption =
        workloadOrdinal.getOperand().getDefiningOp<IREE::Util::AssumeIntOp>();
    // TODO: Make the assumption optional and construct a replacement if not
    // found.
    if (!assumption) {
      LLVM_DEBUG({
    ...
    workloadAssumptions[i].assumption =
  ...
  for (auto specializationRange : specializationRanges) {
    ...
    for (auto [range, assumedSize] :
      ...
      if (!ShapedType::isDynamic(assumedSize.staticSize)) {
        ...
        auto assumeOp = llvm::cast<IREE::Util::AssumeIntOp>(
            assumedSize.assumption.getOwner());
        std::pair<std::optional<int64_t>, std::optional<int64_t>> dynamicRange =
            assumeOp.getUnionedUnsignedRange(
                assumedSize.assumption.getResultNumber());
    ...
    currentExport.setConditionFallback(newFunctionName);
    {
      ...
      for (auto [range, assumedSize] :
        ...
        auto originalAssumeOp = llvm::cast<IREE::Util::AssumeIntOp>(
            assumedSize.assumption.getOwner());
        auto clonedAssumeOp =
        ...
        newAssumptionsLists[assumedSize.assumption.getResultNumber()] =
