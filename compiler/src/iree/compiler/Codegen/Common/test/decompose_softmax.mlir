...
// CHECK:          {{.+}} = arith.maxnumf %[[IN]], %[[OUT]] : f32
// CHECK:          linalg.yield
...
// CHECK:          %[[SUB:.+]] = arith.subf %[[IN]], %[[IN_1]] : f32
// CHECK:          %[[EXP:.+]] = math.exp %[[SUB]] : f32
// CHECK:          {{.+}} = arith.addf %[[EXP]], %[[OUT]]
// CHECK:          linalg.yield
// CHECK:        } -> tensor<2x16xf32>
// CHECK:        %[[D6:.+]] = linalg.generic {indexing_maps = [#[[MAP]], #[[MAP1]], #[[MAP1]], #[[MAP]]], iterator_types =
...
// CHECK:        ^bb0(%[[IN:.+]]: f32, %[[IN_1:.+]]: f32, %[[IN_2:.+]]: f32, %[[OUT:.+]]: f32):
// CHECK:          %[[SUB:.+]] = arith.subf %[[IN]], %[[IN_1]] : f32
// CHECK:          %[[EXP:.+]] = math.exp %[[SUB]] : f32
// CHECK:          {{.+}} = arith.divf %[[EXP]], %[[IN_2]] : f32
// CHECK:          linalg.yield
// CHECK:        } -> tensor<2x16x32xf32>
// CHECK:        return %[[D6]] : tensor<2x16x32xf32>
// CHECK:      }
...
// CHECK-NO-FUSE:          {{.+}} = arith.maxnumf %[[IN]], %[[OUT]] : f32
// CHECK-NO-FUSE:          linalg.yield
// CHECK-NO-FUSE:        } -> tensor<2x16xf32>
// CHECK-NO-FUSE:        %[[D4:.+]] = linalg.generic {indexing_maps = [#[[MAP]], #[[MAP1]], #[[MAP]]], iterator_types = ["parallel",
// CHECK-NO-FUSE-SAME:     "parallel", "parallel"]} ins(%[[ARG0]], %[[D3]] : tensor<2x16x32xf32>, tensor<2x16xf32>) outs(%[[D0]] : tensor<2x16x32xf32>) {
// CHECK-NO-FUSE:        ^bb0(%[[IN:.+]]: f32, %[[IN_1:.+]]: f32, %[[OUT:.+]]: f32):
// CHECK-NO-FUSE:          %[[SUB:.+]] = arith.subf %[[IN]], %[[IN_1]] : f32
// CHECK-NO-FUSE:          {{.+}} = math.exp %[[SUB]] : f32
// CHECK-NO-FUSE:          linalg.yield
...
// CHECK-NO-FUSE:        ^bb0(%[[IN:.+]]: f32, %[[OUT:.+]]: f32):
// CHECK-NO-FUSE:          {{.+}} = arith.addf %[[IN]], %[[OUT]]
// CHECK-NO-FUSE:          linalg.yield
// CHECK-NO-FUSE:        } -> tensor<2x16xf32>
...
// CHECK-NO-FUSE:        ^bb0(%[[IN:.+]]: f32, %[[IN_1:.+]]: f32, %[[OUT:.+]]: f32):
// CHECK-NO-FUSE:          {{.+}} = arith.divf %[[IN]], %[[IN_1]] : f32
// CHECK-NO-FUSE:          linalg.yield
// CHECK-NO-FUSE:        } -> tensor<2x16x32xf32>
...
// Verify that the gather generic is not fused into the softmax decomposition.
