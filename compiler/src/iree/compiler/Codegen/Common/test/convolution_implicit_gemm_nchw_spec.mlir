...
^bb0(%arg0: !pdl.operation):
  ...
  // ===========================================================================
  %8:2 = transform.iree.promote_operands %tiled_linalg_op [0] : (!pdl.operation) -> (!pdl.operation, !pdl.operation)

  // Step 5. Tile to threads/warps.
  // ===========================================================================
  %forall_op_2, %tiled_op_3 = transform.structured.tile_to_forall_op %7   num_threads [0, 0, 32] tile_sizes [](mapping = [#gpu.thread<x>])
  transform.iree.apply_patterns %arg0 {canonicalization, cse, licm, tiling_canonicalization} : (!pdl.operation) -> ()
  %forall_op_4, %tiled_op_5 = transform.structured.tile_to_forall_op %rest_1   num_threads [0, 0, 32] tile_sizes [](mapping = [#gpu.thread<x>])
  transform.iree.apply_patterns %arg0 {canonicalization, cse, licm, tiling_canonicalization} : (!pdl.operation) -> ()
  %forall_op_6, %tiled_op_7 = transform.structured.tile_to_forall_op %5   num_threads [0, 0, 32] tile_sizes [](mapping = [#gpu.thread<x>])
  transform.iree.apply_patterns %arg0 {canonicalization, cse, licm, tiling_canonicalization} : (!pdl.operation) -> ()
  %forall_op_8, %tiled_op_9 = transform.structured.tile_to_forall_op %8#0   num_threads [0, 0, 1] tile_sizes [](mapping = [#gpu.warp<x>])
  transform.iree.apply_patterns %arg0 {canonicalization, cse, licm, tiling_canonicalization} : (!pdl.operation) -> ()
