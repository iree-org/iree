...
// The output tensor is packed as tensor<1x1x8x2x16xf32> with swizzle permutation.
// Original dim 0 (M) maps to: d3 + d2*2 (inner_m * stride + outer_m)
// Original dim 1 (N) maps to: d4 (inner_n, no outer due to size 1)
...
// CHECK-SAME:    %[[ARG0:.+]]: tensor<1x1x8x2x16xf32>
// CHECK-DAG:   %[[C2:.+]] = arith.constant 2 : index
// CHECK:       linalg.generic
// CHECK-SAME:    ins(%[[ARG0]] : tensor<1x1x8x2x16xf32>)
// CHECK:       ^bb0(%[[IN:.+]]: f32, %[[OUT:.+]]: f32):
// CHECK-DAG:     %[[D3:.+]] = linalg.index 3 : index
// CHECK-DAG:     %[[D2:.+]] = linalg.index 2 : index
// CHECK:         %[[MUL:.+]] = arith.muli %[[D2]], %[[C2]] : index
// CHECK:         %[[IDX0:.+]] = arith.addi %[[D3]], %[[MUL]] : index
// CHECK:         %[[IDX1:.+]] = linalg.index 4 : index
