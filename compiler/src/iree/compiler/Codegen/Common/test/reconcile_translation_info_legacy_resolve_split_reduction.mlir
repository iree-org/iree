// iree-opt --split-input-file --pass-pipeline="builtin.module(hal.executable(hal.executable.variant(iree-codegen-reconcile-translation-info{fold-split-reduction-loop-into-workgroup-mapping-loop}, canonicalize, cse)))" %s --verify-diagnostics --allow-unregistered-dialect | FileCheck %s
