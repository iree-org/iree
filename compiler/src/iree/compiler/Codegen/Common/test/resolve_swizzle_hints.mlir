...
func.func @swizzle_adjust_add_offset(%src: memref<?xf32>, %vec: vector<4xf32>, %offset_base: index) -> vector<4xf32> {
  ...
  %load_offset = arith.addi %offset_base, %c16 : index
  %1 = vector.load %0[%load_offset] : memref<?xf32>, vector<4xf32>
  %store_offset = arith.addi %offset_base, %c1040 : index
...
//       CHECK:   %[[APPLY_BASE:.+]] = arith.addi %[[OFFSET]], %[[GROUP_COUNT]] : index
...
//       CHECK:   %[[STORE_BASE:.+]] = arith.addi %[[OFFSET]], %[[C1040]] : index
