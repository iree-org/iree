...
struct DistributeTransferWrite final
  ...
  LogicalResult matchAndRewrite(vector::TransferWriteOp writeOp,
    ...
    //
    // TODO(newling) understand NOTE[1].
    // TODO(newling) understand NOTE[2].
    ...
    int64_t threadTileSize = std::accumulate(
        threadTile.begin(), threadTile.end(), 1LL, std::multiplies<int64_t>());
    ...
    int64_t wgTileSize =
        std::accumulate(subgroupTile.begin(), subgroupTile.end(), 1LL,
    ...
    // TODO(newling) is this needed, to zeros appear at this level? I don't
    // think so actually.
    bool containsZeros = threadTileSize == 0 || wgTileSize == 0;

    // If there are threads in a subgroup that write to the same address:
    if (!containsZeros && isDuplication) {
      ...
      auto subgroupGroupId = delinearized[0];
      auto threadGroupId = delinearized[rank + 1];
