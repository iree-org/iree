...
struct DistributeMultiReduction final
  ...
  LogicalResult matchAndRewrite(vector::MultiDimReductionOp multiReduceOp,
        ...
        loc, read, acc, reducedDims, multiReduceOp.getKind());
    {
      ...
      auto dstLayout = dyn_cast_or_null<NestedLayoutAttr>(signature[resVector]);
      if (!dstLayout) {
        return rewriter.notifyMatchFailure(
            multiReduceOp, "expected nested layout attr for dst");
      }
      ...
      ArrayAttr reduceResultsAttr =
          ArrayAttr::get(rewriter.getContext(), {dstLayout});
