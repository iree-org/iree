  ...
  auto walkResult = funcOp.walk([&](memref::AllocOp allocOp) -> WalkResult {
    ...
    unsigned bitWidth =
        allocType.getElementType().isIndex()
            ? 64 // IREE's default bitWidth for indexes
            : IREE::Util::getTypeBitWidth(allocType.getElementType());
    if (allocOp.getAlignment()) {
      int64_t alignmentInBits = *allocOp.getAlignment() * 8;
      bitWidth = llvm::divideCeil(bitWidth, alignmentInBits) * alignmentInBits;
    }
    ...
    int64_t allocSizeBytes = (bitWidth * numElements) / 8;
