...
matchArgmaxDAGForUKernel(RewriterBase &rewriter, linalg::GenericOp op) {
  ...
  IREE::Codegen::UKernelSpecAttr ukernelAttr =
      op->getAttrOfType<IREE::Codegen::UKernelSpecAttr>("ukernel");
  if (!ukernelAttr) {
...
// Helper to manually copy "hal.executable.objects" because when selecting a
// built-in ukernel, the GPUSelectUKernels pass creates this attribute on the
// source op, and we typically don't run the HoistExecutableObjects pass between
// the GPUSelectUKernels and GPULowerToUKernels passes.
static void copyExecutableObjects(Operation *destination, Operation *source) {
  const char execObjAttrName[] = "hal.executable.objects";
  if (auto execObj = source->getAttr(execObjAttrName)) {
    destination->setAttr(execObjAttrName, execObj);
  }
}
...
struct LowerArgmaxToUKernelPattern : OpRewritePattern<linalg::GenericOp> {
  ...
  LogicalResult matchAndRewrite(linalg::GenericOp op,
    ...
    copyExecutableObjects(ukernelOp->getOperation(), op.getOperation());
