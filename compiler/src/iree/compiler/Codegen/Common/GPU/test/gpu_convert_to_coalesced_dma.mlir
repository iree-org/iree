...
// Test that im2col ops with use_global_load_dma are downgraded to
// derived_thread_config when DMA conversion fails (source row not DWORD-aligned:
// 97 bf16 = 194 bytes). GPUPromoteMatmulOperands stamps use_global_load_dma
// directly on Im2colOp (no copy inserted), so the post-conversion cleanup must
// explicitly cover Im2colOp in addition to linalg.copy.
...
#gpu_target_im2col = #iree_gpu.target<arch = "gfx950", features = "", wgp = <
  ...
  dma_sizes = [32, 128]
>>
#exec_target_im2col = #hal.executable.target<"rocm", "rocm-hsaco-fb", {
  iree_codegen.target_info = #gpu_target_im2col
}>
#translation_im2col = #iree_codegen.translation_info<
  pipeline = LLVMGPUTileAndFuse workgroup_size = [512, 1, 1] subgroup_size = 64,
  {gpu_pipeline_options = #iree_gpu.pipeline_options<
    prefetch_num_stages = 2, no_reduce_shared_memory_bank_conflicts = true,
    use_igemm_convolution = true>}>

// CHECK-LABEL: func.func @im2col_dma_not_aligned
// Source row = 97 bf16 = 194 bytes (not DWORD-aligned) → DMA conversion fails.
// CHECK: iree_linalg_ext.im2col {lowering_config = #iree_gpu.derived_thread_config}
// CHECK-NOT: iree_linalg_ext.im2col {lowering_config = #iree_gpu.use_global_load_dma}
func.func @im2col_dma_not_aligned(
    %src_buf: memref<32x97x97xbf16, #amdgpu.address_space<fat_raw_buffer>>,
    %init: tensor<2x32x128xbf16>) -> tensor<2x32x128xbf16>
    attributes {
      hal.executable.target = #exec_target_im2col,
      translation_info = #translation_im2col
    } {
  %cst = arith.constant 0.0 : bf16
  %c0 = arith.constant 0 : index
  %src = iree_codegen.load_from_buffer %src_buf
      : memref<32x97x97xbf16, #amdgpu.address_space<fat_raw_buffer>> -> tensor<32x97x97xbf16>
  %padded = tensor.pad %src low[0, 0, 0] high[0, 1, 31] {
  ^bb0(%a: index, %b: index, %c: index):
    tensor.yield %cst : bf16
  } : tensor<32x97x97xbf16> to tensor<32x98x128xbf16>
  %result = iree_linalg_ext.im2col
      ...
      ins(%padded : tensor<32x98x128xbf16>)
