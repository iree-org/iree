...
// CHECK-SAME:      output_shape [16, 129, 1, 16, 1, 4, 1] : tensor<16x129x16x4xf32> into tensor<16x129x1x16x1x4x1xf32>
// CHECK:         %[[EMPTY_LHS2:.*]] = tensor.empty() : tensor<16x129x4x1x16x1x1xf32>
// CHECK:         %[[TRANSPOSE:.*]] = linalg.transpose
// CHECK-SASME:     ins(%[[EXPAND_LHS]]
// CHECK-SAME:      outs(%[[EMPTY_LHS2]]
// CHECK-SAME:      permutation = [0, 1, 5, 2, 3, 6, 4]
...
// CHECK-SAME:      output_shape [33, 64, 1, 16, 1, 4, 1] : tensor<33x64x16x4xf32> into tensor<33x64x1x16x1x4x1xf32> 
// CHECK:         %[[EMPTY2:.*]] = tensor.empty() : tensor<33x64x4x1x16x1x1xf32>
// CHECK:         %[[TRANSPOSE:.*]] = linalg.transpose
...
// CHECK:         %[[EXPAND:.*]] = tensor.expand_shape %[[PACK]]
// CHECK-SAME:      output_shape [16, 33, 1, 4, 4, 16, 1] : tensor<16x33x16x16xf32> into tensor<16x33x1x4x4x16x1xf32>
// CHECK:         %[[EMPTY2:.*]] = tensor.empty() : tensor<16x33x1x16x4x4x1xf32>
// CHECK:         %[[TRANSPOSE:.*]] = linalg.transpose
// CHECK-SAME:      ins(%[[EXPAND]]
// CHECK-SAME:      outs(%[[EMPTY2]]
...
// CHECK:         %[[UNSET_EMPTY:.*]] = tensor.empty() : tensor<16x129x1x16x1x4x1xf32>
// CHECK:         %[[UNSET_TRANSPOSE:.*]] = linalg.transpose ins(%2 : tensor<16x129x4x1x16x1x1xf32>)  
// CHECK-SAME:      outs(%[[UNSET_EMPTY]] : tensor<16x129x1x16x1x4x1xf32>) permutation = [0, 1, 3, 4, 6, 2, 5]
// CHECK:         %[[UNSET_COLLAPSE:.*]] = tensor.collapse_shape %[[UNSET_TRANSPOSE]]
// CHECK-SAME:      tensor<16x129x1x16x1x4x1xf32> into tensor<16x129x16x4xf32>
...
// CHECK:         %[[UNSET_EMPTY:.*]] = tensor.empty() : tensor<33x64x1x16x1x4x1xf32>
// CHECK:         %[[UNSET_TRANSPOSE:.*]] = linalg.transpose ins(%{{.+}} : tensor<33x64x4x1x16x1x1xf32>)
// CHECK-SAME:      outs(%[[UNSET_EMPTY]] : tensor<33x64x1x16x1x4x1xf32>) permutation = [0, 1, 3, 4, 6, 2, 5] 
// CHECK:         %[[UNSET_COLLAPSE:.*]] = tensor.collapse_shape %[[UNSET_TRANSPOSE]]
// CHECK-SAME:      tensor<33x64x1x16x1x4x1xf32> into tensor<33x64x16x4xf32>
// CHECK:         %[[UNSET_EMPTY:.*]] = tensor.empty() : tensor<255x513xf32>
...
// CHECK:         %[[UNSET_EMPTY:.*]] = tensor.empty() : tensor<16x33x1x4x4x16x1xf32>
// CHECK:         %[[UNSET_TRANSPOSE:.*]] = linalg.transpose ins(%{{.+}} : tensor<16x33x1x16x4x4x1xf32>)
// CHECK-SAME:       outs(%[[UNSET_EMPTY]] : tensor<16x33x1x4x4x16x1xf32>) permutation = [0, 1, 2, 4, 5, 3, 6]
// CHECK:         %[[UNSET_COLLAPSE:.*]] = tensor.collapse_shape %[[UNSET_TRANSPOSE]]
// CHECK-SAME:      tensor<16x33x1x4x4x16x1xf32> into tensor<16x33x16x16xf32>
// CHECK:         %[[UNSET_EMPTY:.*]] = tensor.empty() : tensor<255x513xf32>
...
// CHECK-DAG:   %[[LHS:.+]] = flow.dispatch.tensor.load %[[LHS_BINDING]]{{.+}} -> tensor<?x?x4x1x16x1x1xf32>
// CHECK-DAG:   %[[RHS:.+]] = flow.dispatch.tensor.load %[[RHS_BINDING]]{{.+}} -> tensor<?x?x4x1x16x1x1xf32>
// CHECK-DAG:   %[[ACC:.+]] = flow.dispatch.tensor.load %[[ACC_BINDING]]{{.+}} -> tensor<?x?x1x16x4x4x1xf32>
