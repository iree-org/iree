...
//       CHECK:     scf.forall
//       CHECK:       linalg.elementwise <add> {{.*}} -> tensor<1x4xf32>
...
func.func @do_not_redistribute_in_forall(%3: tensor<64x64xf32>, %4: tensor<64x64xf32>, %5: tensor<64x64xf32>) -> tensor<64x64xf32>
  ...
  %6 = scf.for %arg0 = %c0 to %c64 step %c8 iter_args(%arg1 = %5) -> (tensor<64x64xf32>) {
    ...
    %1 = scf.forall (%arg5, %arg6) = (0, 0) to (64, 8) step (1, 4) shared_outs(%arg7 = %extracted_slice_1) -> (tensor<64x8xf32>) {
      ...
      %2 = linalg.elementwise <add> ins(%extracted_slice_2, %extracted_slice_3 : tensor<1x4xf32>, tensor<1x4xf32>) outs(%extracted_slice_4 : tensor<1x4xf32>) -> tensor<1x4xf32>
...
//  CHECK-SAME:   %[[ARG2:[A-Za-z0-9_]+]]: tensor<64x64xf32>)
...
//       CHECK:       linalg.elementwise <add> {{.*}} outs(%[[DEST]] : tensor<1x4xf32>
//       CHECK:       scf.forall.in_parallel
...
//       CHECK:     %[[ADD:.+]] = linalg.elementwise <add> {{.*}} -> tensor<1x4xf32>
//       CHECK:     scf.forall.in_parallel
//       CHECK:       tensor.parallel_insert_slice %[[ADD]]
//       CHECK:   mapping = [#gpu.thread<linear_dim_1>, #gpu.thread<linear_dim_0>]
//       CHECK:   %[[T_DIST:.+]] = scf.forall
//       CHECK:     %[[FUSED_ADD:.+]] = linalg.elementwise <add> {{.*}} -> tensor<4x1xf32>
