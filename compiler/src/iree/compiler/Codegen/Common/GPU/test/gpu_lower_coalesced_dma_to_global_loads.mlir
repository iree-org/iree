...
func.func @lower_coalesced_gather_dma_basic(
  ...
  // CHECK: %[[INDICES_SLICE:.+]] = memref.subview %arg0[0] [1] [1] : memref<32xindex> to memref<1xindex, strided<[1]>>
  // CHECK-NOT: iree_gpu.coalesced_gather_dma
  // CHECK: %[[LANE_ID:.+]] = gpu.lane_id
  // CHECK: %[[SUBGROUP_SIZE:.+]] = arith.constant 32 : index
  ...
  // CHECK:   %[[DELINEAR_IDX:.+]] = affine.delinearize_index %[[IDX_POS]] into (1)
  // CHECK:   %[[LOADED_IDX:.+]] = memref.load %[[INDICES_SLICE]][%[[DELINEAR_IDX]]]
  // CHECK:   amdgpu.gather_to_lds %arg1[%[[LOADED_IDX]]], %arg2[%[[DELINEAR_DEST]]] : vector<4xf32>
  scf.forall (%arg5, %arg6) in (32, 1) {
    %indices_slice = memref.subview %indices[0] [1] [1] : memref<32xindex> to memref<1xindex, strided<[1]>>
    %1 = iree_gpu.coalesced_gather_dma %indices_slice, %source into %dest :
      memref<1xindex, strided<[1]>>,
...
// Test case where each thread loads 4xf32 per iteration across 4 iterations
// 512 f32 elements = 2048 bytes, 4 indices per thread, 32 threads = 128 total indices
// bytesPerIndexCopy = 2048/128 = 16 bytes = 4xf32
// 4 indices / 1 thread = 4 iterations per thread
...
func.func @lower_coalesced_gather_dma_multiple(
  ...
  attributes {
    hal.executable.target = #executable_target_rocm_hsaco_fb,
    translation_info = #translation_info} {
  // CHECK: %[[INDICES_SLICE:.+]] = memref.subview %arg0[0] [4] [1] : memref<128xindex> to memref<4xindex, strided<[1]>>
  // CHECK-NOT: iree_gpu.coalesced_gather_dma
  // CHECK: %[[LANE_ID:.+]] = gpu.lane_id
  // CHECK: %[[SUBGROUP_SIZE:.+]] = arith.constant 32 : index
  // CHECK: %[[C0:.+]] = arith.constant 0 : index
  ...
  // CHECK:   %[[IDX_POS:.+]] = arith.addi %[[LANE_ID]], %[[IDX_OFFSET]]
  // CHECK:   %[[DELINEAR_IDX:.+]] = affine.delinearize_index %[[IDX_POS]] into (4)
  // CHECK:   %[[LOADED_IDX:.+]] = memref.load %[[INDICES_SLICE]][%[[DELINEAR_IDX]]]
  // CHECK:   amdgpu.gather_to_lds %arg1[%[[LOADED_IDX]]], %arg2[%[[DELINEAR_DEST]]] : vector<4xf32>
  scf.forall (%arg5, %arg6) in (32, 1) {
    %indices_slice = memref.subview %indices[0] [4] [1] : memref<128xindex> to memref<4xindex, strided<[1]>>
    %1 = iree_gpu.coalesced_gather_dma %indices_slice, %source into %dest :
      memref<4xindex, strided<[1]>>,
      memref<2048xf32, #amdgpu.address_space<fat_raw_buffer>>,
...
func.func @lower_loop_nest(%arg0: memref<1024xf32, #amdgpu.address_space<fat_raw_buffer>>,
  ...
  // CHECK:   %[[INDICES_SLICE:.+]] = memref.subview %arg1[%[[WG_I]], %[[WG_J]]] [1, 1] [1, 1]
  // CHECK-SAME:   memref<32x32xindex> to memref<1x1xindex, strided<[32, 1], offset: ?>>
  ...
  // CHECK:     %[[IDX_POS:.+]] = arith.addi %[[LANE_ID]], %[[IDX_OFFSET]]
  // CHECK:     %[[DELINEAR_IDX:.+]]:2 = affine.delinearize_index %[[IDX_POS]] into (1, 1)
  ...
  scf.forall (%arg3, %arg4) = (0, 0) to (32, 32) step (1, 32) {
    %subview = memref.subview %arg1[%arg3, %arg4] [1, 1] [1, 1] : memref<32x32xindex> to memref<1x1xindex, strided<[32, 1], offset: ?>>
    ...
    scf.forall (%arg5, %arg6) in (1, 32) {
      %0 = iree_gpu.coalesced_gather_dma %subview, %arg0 into %subview_0 :
        memref<1x1xindex, strided<[32, 1], offset: ?>>,
