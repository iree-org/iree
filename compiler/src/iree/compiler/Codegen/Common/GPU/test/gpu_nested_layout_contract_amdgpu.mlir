...
// Sparse-trick VSMFMA_F32_16x16x64_BF16 (M=8 skinny GEMM via smfmac)
...
func.func @contract_to_vsmfma_bf16_8x16x64_mm(%a : vector<8x64xbf16>, %b : vector<64x16xbf16>, %c : vector<8x16xf32>) -> vector<8x16xf32> {
  %A = iree_vector_ext.to_layout %a to layout(#layout_a) : vector<8x64xbf16>
  %B = iree_vector_ext.to_layout %b to layout(#layout_b) : vector<64x16xbf16>
  %C = iree_vector_ext.to_layout %c to layout(#layout_c) : vector<8x16xf32>
  ...
  %output = vector.contract {
    ...
    iterator_types = ["parallel", "parallel", "reduction"],
    kind = #vector.kind<add>,
    iree.gpu.mma = #iree_gpu.virtual_mma_layout<VSMFMA_F32_16x16x64_BF16>
  } %A, %B, %C : vector<8x64xbf16>, vector<64x16xbf16> into vector<8x16xf32>
...
// CHECK-LABEL: func @contract_to_vsmfma_bf16_8x16x64_mm
// CHECK-SAME: (%[[A:.+]]: vector<8x64xbf16>, %[[B:.+]]: vector<64x16xbf16>, %[[C:.+]]: vector<8x16xf32>)
// CHECK-DAG:   %[[A_SIMT:.+]] = iree_vector_ext.to_simt %[[A]] : vector<8x64xbf16> -> vector<1x1x1x1x1x16xbf16>
// CHECK-DAG:   %[[B_SIMT:.+]] = iree_vector_ext.to_simt %[[B]] : vector<64x16xbf16> -> vector<1x1x1x1x16x1xbf16>
// CHECK-DAG:   %[[C_SIMT:.+]] = iree_vector_ext.to_simt %[[C]] : vector<8x16xf32> -> vector<1x1x1x1x2x1xf32>
// CHECK-DAG:   %[[A_CAST:.+]] = vector.shape_cast %[[A_SIMT]] : vector<1x1x1x1x1x16xbf16> to vector<16xbf16>
// CHECK-DAG:   %[[B_CAST:.+]] = vector.shape_cast %[[B_SIMT]] : vector<1x1x1x1x16x1xbf16> to vector<16xbf16>
//
// Acc expand: [c0, c1] -> [c0, 0, c1, 0]
// CHECK-DAG:   %[[ZERO:.+]] = arith.constant dense<0.000000e+00> : vector<4xf32>
// CHECK:       %[[C0:.+]] = vector.extract %[[C_SIMT]][0, 0, 0, 0, 0, 0] : f32 from vector<1x1x1x1x2x1xf32>
...
// CHECK:       %[[EVEN_A:.+]] = vector.shuffle %[[A_CAST]], %[[A_CAST]] [0, 1, 4, 5, 8, 9, 12, 13]
// CHECK:       %[[ODD_A:.+]] = vector.shuffle %[[A_CAST]], %[[A_CAST]] [2, 3, 6, 7, 10, 11, 14, 15]
// CHECK:       %[[SPARSE_A:.+]] = arith.select %[[IS_ODD]], %[[ODD_A]], %[[EVEN_A]] : vector<8xbf16>
//
// Sparsity index and smfmac (bf16 is 16-bit, so uses vector<4xi8> indices)
// CHECK:       %[[SPARSE_IDX:.+]] = arith.select %[[IS_ODD]], %{{.+}}, %{{.+}} : vector<4xi8>
// CHECK:       %[[SMFMA:.+]] = amdgpu.sparse_mfma 16x16x64 %[[SPARSE_A]] * %[[B_CAST]] + %[[ACC_EXP]] sparse(%[[SPARSE_IDX]] : vector<4xi8>) : vector<8xbf16>, vector<16xbf16>, vector<4xf32>
//
// Acc collapse: pairwise addition
// CHECK:       %[[R0:.+]] = vector.extract %[[SMFMA]][0]
// CHECK:       %[[R1:.+]] = vector.extract %[[SMFMA]][1]
...
// Sparse-trick VSMFMA_F32_16x16x64_F8E5M2FNUZ (M=8 skinny GEMM via smfmac)
...
func.func @contract_to_vsmfma_f8e5m2_8x16x64_mm(%a : vector<8x64xf8E5M2FNUZ>, %b : vector<64x16xf8E5M2FNUZ>, %c : vector<8x16xf32>) -> vector<8x16xf32> {
  %A = iree_vector_ext.to_layout %a to layout(#layout_a) : vector<8x64xf8E5M2FNUZ>
  %B = iree_vector_ext.to_layout %b to layout(#layout_b) : vector<64x16xf8E5M2FNUZ>
  %C = iree_vector_ext.to_layout %c to layout(#layout_c) : vector<8x16xf32>
  ...
  %output = vector.contract {
    ...
    iterator_types = ["parallel", "parallel", "reduction"],
    kind = #vector.kind<add>,
    iree.gpu.mma = #iree_gpu.virtual_mma_layout<VSMFMA_F32_16x16x64_F8E5M2FNUZ>
  } %A, %B, %C : vector<8x64xf8E5M2FNUZ>, vector<64x16xf8E5M2FNUZ> into vector<8x16xf32>
...
// CHECK-LABEL: func @contract_to_vsmfma_f8e5m2_8x16x64_mm
// CHECK-SAME: (%[[A:.+]]: vector<8x64xf8E5M2FNUZ>, %[[B:.+]]: vector<64x16xf8E5M2FNUZ>, %[[C:.+]]: vector<8x16xf32>)
// CHECK-DAG:   %[[A_SIMT:.+]] = iree_vector_ext.to_simt %[[A]] : vector<8x64xf8E5M2FNUZ> -> vector<1x1x1x1x1x16xf8E5M2FNUZ>
// CHECK-DAG:   %[[B_SIMT:.+]] = iree_vector_ext.to_simt %[[B]] : vector<64x16xf8E5M2FNUZ> -> vector<1x1x1x1x16x1xf8E5M2FNUZ>
// CHECK-DAG:   %[[C_SIMT:.+]] = iree_vector_ext.to_simt %[[C]] : vector<8x16xf32> -> vector<1x1x1x1x2x1xf32>
// CHECK-DAG:   %[[A_CAST:.+]] = vector.shape_cast %[[A_SIMT]] : vector<1x1x1x1x1x16xf8E5M2FNUZ> to vector<16xf8E5M2FNUZ>
// CHECK-DAG:   %[[B_CAST:.+]] = vector.shape_cast %[[B_SIMT]] : vector<1x1x1x1x16x1xf8E5M2FNUZ> to vector<16xf8E5M2FNUZ>
//
// Acc expand: [c0, c1] -> [c0, 0, c1, 0]
// CHECK-DAG:   %[[ZERO:.+]] = arith.constant dense<0.000000e+00> : vector<4xf32>
// CHECK:       %[[C0:.+]] = vector.extract %[[C_SIMT]][0, 0, 0, 0, 0, 0] : f32 from vector<1x1x1x1x2x1xf32>
...
// CHECK:       %[[ACC_EXP:.+]] = vector.insert %[[C1]], %[[ACC_0]] [2] : f32 into vector<4xf32>
//
// Lane parity + A shuffle
// CHECK:       %[[LANE_ID:.+]] = gpu.lane_id
// CHECK:       %[[IS_ODD:.+]] = arith.cmpi ne
// CHECK:       %[[EVEN_A:.+]] = vector.shuffle %[[A_CAST]], %[[A_CAST]] [0, 1, 4, 5, 8, 9, 12, 13]
// CHECK:       %[[ODD_A:.+]] = vector.shuffle %[[A_CAST]], %[[A_CAST]] [2, 3, 6, 7, 10, 11, 14, 15]
// CHECK:       %[[SPARSE_A:.+]] = arith.select %[[IS_ODD]], %[[ODD_A]], %[[EVEN_A]] : vector<8xf8E5M2FNUZ>
//
// Sparsity index and smfmac
// CHECK:       %[[SPARSE_IDX:.+]] = arith.select %[[IS_ODD]], %{{.+}}, %{{.+}} : vector<2xi16>
// CHECK:       %[[SMFMA:.+]] = amdgpu.sparse_mfma 16x16x64 %[[SPARSE_A]] * %[[B_CAST]] + %[[ACC_EXP]] sparse(%[[SPARSE_IDX]] : vector<2xi16>) : vector<8xf8E5M2FNUZ>, vector<16xf8E5M2FNUZ>, vector<4xf32>
//
// Acc collapse: pairwise addition
// CHECK:       %[[R0:.+]] = vector.extract %[[SMFMA]][0]
// CHECK:       %[[R1:.+]] = vector.extract %[[SMFMA]][1]
...
// Sparse-trick VSMFMA_F32_16x16x64_F8E4M3FNUZ (M=8 skinny GEMM via smfmac)
...
func.func @contract_to_vsmfma_f8e4m3_8x16x64_mm(%a : vector<8x64xf8E4M3FNUZ>, %b : vector<64x16xf8E4M3FNUZ>, %c : vector<8x16xf32>) -> vector<8x16xf32> {
  %A = iree_vector_ext.to_layout %a to layout(#layout_a) : vector<8x64xf8E4M3FNUZ>
  %B = iree_vector_ext.to_layout %b to layout(#layout_b) : vector<64x16xf8E4M3FNUZ>
  %C = iree_vector_ext.to_layout %c to layout(#layout_c) : vector<8x16xf32>
  ...
  %output = vector.contract {
    ...
    iterator_types = ["parallel", "parallel", "reduction"],
    kind = #vector.kind<add>,
    iree.gpu.mma = #iree_gpu.virtual_mma_layout<VSMFMA_F32_16x16x64_F8E4M3FNUZ>
  } %A, %B, %C : vector<8x64xf8E4M3FNUZ>, vector<64x16xf8E4M3FNUZ> into vector<8x16xf32>
...
// CHECK-LABEL: func @contract_to_vsmfma_f8e4m3_8x16x64_mm
// CHECK-SAME: (%[[A:.+]]: vector<8x64xf8E4M3FNUZ>, %[[B:.+]]: vector<64x16xf8E4M3FNUZ>, %[[C:.+]]: vector<8x16xf32>)
// CHECK-DAG:   %[[A_SIMT:.+]] = iree_vector_ext.to_simt %[[A]] : vector<8x64xf8E4M3FNUZ> -> vector<1x1x1x1x1x16xf8E4M3FNUZ>
// CHECK-DAG:   %[[B_SIMT:.+]] = iree_vector_ext.to_simt %[[B]] : vector<64x16xf8E4M3FNUZ> -> vector<1x1x1x1x16x1xf8E4M3FNUZ>
// CHECK-DAG:   %[[C_SIMT:.+]] = iree_vector_ext.to_simt %[[C]] : vector<8x16xf32> -> vector<1x1x1x1x2x1xf32>
// CHECK-DAG:   %[[A_CAST:.+]] = vector.shape_cast %[[A_SIMT]] : vector<1x1x1x1x1x16xf8E4M3FNUZ> to vector<16xf8E4M3FNUZ>
// CHECK-DAG:   %[[B_CAST:.+]] = vector.shape_cast %[[B_SIMT]] : vector<1x1x1x1x16x1xf8E4M3FNUZ> to vector<16xf8E4M3FNUZ>
//
// Acc expand
// CHECK-DAG:   %[[ZERO:.+]] = arith.constant dense<0.000000e+00> : vector<4xf32>
// CHECK:       %[[C0:.+]] = vector.extract %[[C_SIMT]][0, 0, 0, 0, 0, 0] : f32 from vector<1x1x1x1x2x1xf32>
...
// CHECK:       %[[EVEN_A:.+]] = vector.shuffle %[[A_CAST]], %[[A_CAST]] [0, 1, 4, 5, 8, 9, 12, 13]
// CHECK:       %[[ODD_A:.+]] = vector.shuffle %[[A_CAST]], %[[A_CAST]] [2, 3, 6, 7, 10, 11, 14, 15]
// CHECK:       %[[SPARSE_A:.+]] = arith.select %[[IS_ODD]], %[[ODD_A]], %[[EVEN_A]] : vector<8xf8E4M3FNUZ>
//
// Sparsity index and smfmac
// CHECK:       %[[SPARSE_IDX:.+]] = arith.select %[[IS_ODD]], %{{.+}}, %{{.+}} : vector<2xi16>
// CHECK:       %[[SMFMA:.+]] = amdgpu.sparse_mfma 16x16x64 %[[SPARSE_A]] * %[[B_CAST]] + %[[ACC_EXP]] sparse(%[[SPARSE_IDX]] : vector<2xi16>) : vector<8xf8E4M3FNUZ>, vector<16xf8E4M3FNUZ>, vector<4xf32>
//
// Acc collapse: pairwise addition
// CHECK:       %[[R0:.+]] = vector.extract %[[SMFMA]][0]
// CHECK:       %[[R1:.+]] = vector.extract %[[SMFMA]][1]
