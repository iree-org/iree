...
#define DEBUG_TYPE "iree-gpu-tile-tensor"
...
namespace {
struct GPUTileTensorPass
    : public GPUTileTensorBase<GPUTileTensorPass> {
 private:
  // Distribute the workloads to warp if true otherwise distribute to threads.
  bool distributeToWarp = false;

 public:
  GPUTileTensorPass(bool distributeToWarp)
  ...
  void runOnOperation() override {
    ...
    if (failed(tilingUtils::tileToSerialLoops(funcOp, /*onlyReduction=*/true))) {
      return signalPassFailure();
    }
...
std::unique_ptr<OperationPass<func::FuncOp>> createGPUTileTensor(
    bool distributeToWarp) {
  return std::make_unique<GPUTileTensorPass>(distributeToWarp);
}
