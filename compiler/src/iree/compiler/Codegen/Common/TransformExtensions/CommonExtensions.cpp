...
static void generateWorkgroups(RewriterBase &rewriter,
                               scf::ForeachThreadOp foreachOp,
                               SmallVectorImpl<Value> &workgroupIdOps) {
  // Step 1. Create the workgroup id and count ops.
  Location const loc = foreachOp->getLoc();
  BlockAndValueMapping bvm;
  SmallVector<Value, 8> workgroupCountOps;
  ...
  bvm.map(foreachOp.getThreadIndices(), workgroupIdOps);
  bvm.map(foreachOp.getNumThreads(), workgroupCountOps);
}

DiagnosedSilenceableFailure
transform_dialect::MapForeachThreadToWorkgroupOp::applyToOne(
  ...
  OpBuilder::InsertionGuard const guard(rewriter);
  ...
  // Using `transform.iree.tile_to_foreach_thread_and_workgroup_count_region`
  // is the preferred way to set up tiling and workgroup_count region **at the
  // same time**.
  //
  ...
  if (exportOp.getWorkgroupCount().empty()) {
    auto maybeWorkgroupCounts =
        topLevelForeachThreadOp.getPermutedNumThreads(rewriter);
    if (failed(maybeWorkgroupCounts) ||
        llvm::any_of(*maybeWorkgroupCounts, [](OpFoldResult ofr) {
          return !getConstantIntValue(ofr).has_value();
        }))
      topLevelForeachThreadOp->emitError(
          "unsupported dynamic workgroup_count atm --- need to slice out "
          "workgroup_count computation into "
          "ExecutableExport::workgroup_count. This region may require "
          "arbitrary computations and cannot magically match what the "
          "`stream.cmd.dispatch` has already imposed on us at a distance. "
    ...
    if (failed(populateWorkgroupCountComputingRegion(
            rewriter, topLevelForeachThreadOp, exportOp))) {
      topLevelForeachThreadOp->emitOpError(
  ...
  diag = mlir::transform::gpu::mapForeachToBlocksImpl(
      rewriter, topLevelForeachThreadOp, generateWorkgroups, workgroupCounts,
      transformOp);
...
void transform_dialect::MapForeachThreadToWorkgroupOp::getEffects(
    SmallVectorImpl<MemoryEffects::EffectInstance> &effects) {
  transform::consumesHandle(getTarget(), effects);
