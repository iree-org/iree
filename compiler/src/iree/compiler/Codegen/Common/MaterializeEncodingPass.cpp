...
static FailureOr<SmallVector<OpFoldResult>> getMixedDimsForDispatchTensor(
...
static FailureOr<SmallVector<Value>> getDynamicDimsForDispatchTensor(
    OpBuilder &builder, Location loc,
    IREE::LinalgExt::MaterializeEncodingTypeConverter &typeConverter,
    IREE::Flow::DispatchTensorType dispatchTensorType, ValueRange dynamicDims) {
  FailureOr<SmallVector<OpFoldResult>> convertedTargetShape =
      getMixedDimsForDispatchTensor(builder, loc, typeConverter,
...
struct MaterializeInterfaceBindingEncoding
  ...
  LogicalResult matchAndRewrite(
    ...
    FailureOr<SmallVector<Value>> convertedDynamicDims =
        getDynamicDimsForDispatchTensor(rewriter, loc, *typeConverter,
                                        resultType, subspanOp.getDynamicDims());
...
struct MaterializeFlowDispatchTensorLoadOp
  ...
  LogicalResult matchAndRewrite(
    ...
    FailureOr<SmallVector<OpFoldResult>> convertedMixedSizes =
        getMixedDimsForDispatchTensor(rewriter, loc, *typeConverter, sourceType,
...
struct MaterializeFlowDispatchTensorStoreOp
  ...
  LogicalResult matchAndRewrite(
    ...
    FailureOr<SmallVector<OpFoldResult>> convertedMixedSizes =
        getMixedDimsForDispatchTensor(rewriter, loc, *typeConverter, targetType,
...
void IREEMaterializeEncodingPass::runOnOperation() {
...

  {
    ...
    IREE::LinalgExt::populateMaterializeEncodingPatterns(patterns, target,
                                                         typeConverter);
    patterns.insert<MaterializeFlowDispatchTensorLoadOp,
                    MaterializeFlowDispatchTensorStoreOp,
                    MaterializeInterfaceBindingEncoding>(typeConverter,
                                                         context);
    if (failed(
