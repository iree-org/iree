...
/// The storage is a dictionary attribute which stores pointers of values to the
/// layout of each value. It is expected that a dictionary will not be copied
/// from one operation to another.

static DictionaryAttr getLayoutFetcherStorage(Operation *op) {
  return op->getAttrOfType<DictionaryAttr>(kVectorLayoutFetcherStorageAttrName);
}

static void setLayoutFetcherStorage(Operation *op, DictionaryAttr storage) {
  op->setAttr(kVectorLayoutFetcherStorageAttrName, storage);
}

static DictionaryAttr getOrCreateLayoutFetcherStorage(Operation *op) {
  if (DictionaryAttr dictAttr = getLayoutFetcherStorage(op)) {
    return dictAttr;
  }

  setLayoutFetcherStorage(op, DictionaryAttr::get(op->getContext(), {}));
  return getLayoutFetcherStorage(op);
}

static Operation *getParentOp(Value val) {
  // For results, the defining op is the parent.
  if (Operation *op = val.getDefiningOp()) {
    return op;
  }

  // For block arguments, the owner of the parent block is the parent.
  BlockArgument arg = val.cast<BlockArgument>();
  return arg.getOwner()->getParentOp();
}

VectorLayoutFetcher::VectorLayoutFetcher(VectorLayoutAnalysis &analysis)
    : root(analysis.getRootOperation()) {
  // Walk the root and add layout attributes to the storage.
  root->walk([&](Operation *op) {
    // Add all results of the operation to the storage.
    for (Value result : op->getResults()) {
      if (auto vectorValue = dyn_cast<TypedValue<VectorType>>(result)) {
        setLayout(vectorValue,
                  analysis.getLayout<VectorLayoutInterface>(vectorValue));
      }
    }

    // Add all block arguments in the regions of the operation to the storage.
    for (Region &region : op->getRegions()) {
      for (Block &block : region) {
        for (BlockArgument arg : block.getArguments()) {
          if (auto vectorValue = dyn_cast<TypedValue<VectorType>>(arg)) {
            setLayout(vectorValue,
                      analysis.getLayout<VectorLayoutInterface>(vectorValue));
          }
        }
...
/// The destructor cleans up the storage attributes from all operations.
VectorLayoutFetcher::~VectorLayoutFetcher() {
  // Walk the root and remove the storage attribute from all operations.
  root->walk([&](Operation *op) {
    op->removeAttr(kVectorLayoutFetcherStorageAttrName);
    op->hashProperties();
  });
}

void VectorLayoutFetcher::setLayout(TypedValue<VectorType> val,
                                    VectorLayoutInterface layout) {
  if (!layout) {
    return;
  }

  Operation *op = getParentOp(val);
  DictionaryAttr storage = getOrCreateLayoutFetcherStorage(op);
}
