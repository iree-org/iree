...
struct LowerLoopOp : public OpRewritePattern<IREE::PCF::LoopOp> {
  ...
  LogicalResult matchAndRewrite(IREE::PCF::LoopOp loopOp,
    ...
    SmallVector<OpFoldResult> lbs(workerCounts.size(),
                                  rewriter.getIndexAttr(0));
    SmallVector<OpFoldResult> steps(workerCounts.begin(), workerCounts.end());
...
WalkResult verifyOperationLegality(Operation *op) {
  if (isa<PCF::GenericOp, PCF::BranchCondReturnOp>(op)) {
