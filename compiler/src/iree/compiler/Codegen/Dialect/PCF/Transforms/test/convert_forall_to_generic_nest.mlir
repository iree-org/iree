// RUN: iree-opt %s --pass-pipeline="builtin.module(func.func(iree-pcf-test-convert-forall-to-generic-nest{scopes=sequential}))" --split-input-file | FileCheck %s --check-prefix=CHECK-1SCOPE
// RUN: iree-opt %s --pass-pipeline="builtin.module(func.func(iree-pcf-test-convert-forall-to-generic-nest{scopes=sequential,sequential}))" --split-input-file | FileCheck %s --check-prefix=CHECK-2SCOPE
...
// Test empty mapping - should be converted (treated as sequential).
