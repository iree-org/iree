...
util.func private @generic(%0: tensor<?xi32>, %1: tensor<?x?xi32>, %d0: index, %d1: index) {
  %2:4 = pcf.generic scope(#pcf.dummy_scope)
    execute(%ref = %0, %ref_1, %ref_2, %ref_3 = %1)[%id: index, %n: index]
         : (!pcf.sref<?xi32, #pcf.dummy_scope>, !pcf.sref<?xi32, #pcf.dummy_scope>, !pcf.sref<?x?xi32, #pcf.dummy_scope>, !pcf.sref<?x?xi32, #pcf.dummy_scope>)
        -> (tensor<?xi32>, tensor<?xi32>{%d0}, tensor<?x?xi32>{%d0, %d1}, tensor<?x?xi32>) {
    util.optimization_barrier %id, %n, %ref, %ref_1, %ref_2, %ref_3 : index, index, !pcf.sref<?xi32, #pcf.dummy_scope>, !pcf.sref<?xi32, #pcf.dummy_scope>, !pcf.sref<?x?xi32, #pcf.dummy_scope>, !pcf.sref<?x?xi32, #pcf.dummy_scope>
    pcf.return
  } {hello = "world"}
...
//  CHECK-SAME:             !pcf.sref<?x?xi32, #pcf.dummy_scope>,
//  CHECK-SAME:             !pcf.sref<?x?xi32, #pcf.dummy_scope>)
...
util.func private @generic_with_initializer(%arg0: memref<?x?xi32>) {
  %0 = pcf.generic scope(#pcf.dummy_scope) initialize {
...
//       CHECK:   pcf.generic scope(#pcf.dummy_scope) initialize {
...
util.func private @loop(%0: tensor<?xi32>, %1: tensor<?x?xi32>, %d0: index, %d1: index, %n: index) {
  %2:4 = pcf.loop scope(#pcf.dummy_scope) count(%n)
    execute(%ref = %0, %ref_1, %ref_2, %ref_3 = %1)[%id: index]
         : (!pcf.sref<?xi32, #pcf.dummy_scope>, !pcf.sref<?xi32, #pcf.dummy_scope>, !pcf.sref<?x?xi32, #pcf.dummy_scope>, !pcf.sref<?x?xi32, #pcf.dummy_scope>)
        -> (tensor<?xi32>, tensor<?xi32>{%d0}, tensor<?x?xi32>{%d0, %d1}, tensor<?x?xi32>) {
    util.optimization_barrier %id, %ref, %ref_1, %ref_2, %ref_3 : index, !pcf.sref<?xi32, #pcf.dummy_scope>, !pcf.sref<?xi32, #pcf.dummy_scope>, !pcf.sref<?x?xi32, #pcf.dummy_scope>, !pcf.sref<?x?xi32, #pcf.dummy_scope>
    pcf.return
  } {hello = "world"}
  util.optimization_barrier %2#0, %2#1, %2#2, %2#3 : tensor<?xi32>, tensor<?xi32>, tensor<?x?xi32>, tensor<?x?xi32>
  util.return
...
//       CHECK:   pcf.loop scope(#pcf.dummy_scope) count(%[[N]])
//  CHECK-NEXT:     execute(%[[REF:[A-Za-z0-9_]+]] = %[[ARG0]],
//  CHECK-SAME:             %[[REF1:[A-Za-z0-9_]+]],
//  CHECK-SAME:             %[[REF2:[A-Za-z0-9_]+]],
//  CHECK-SAME:             %[[REF3:[A-Za-z0-9_]+]] = %[[ARG1]])
