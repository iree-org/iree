...
LogicalResult vectorizeGatherLikeGenericToTransferGather(
  ...
  if (!LinalgExt::isGatherlikeOp(linalgOp)) {
  ...
  // Since upstream vectorization does not support hooks to vectorize indvidual
  // operations inside a linalg.generic, we take an alternate approach here,
  // by spliting the generic into 3 operations, anchored around the first
