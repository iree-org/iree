...
//   CHECK-DAG:     %[[ROW:.+]] = iree_codegen.index_hint %[[ID]]#1 {hint = #iree_gpu.lane_constant<16>} : index
//   CHECK-DAG:     %[[COL:.+]] = iree_codegen.index_hint %[[ID]]#2 {hint = #iree_gpu.lane_increment<16>} : index
...
//   CHECK-DAG:     %[[ROW_1:.+]] = iree_codegen.index_hint %[[ID_1]]#1 {hint = #iree_gpu.lane_constant<16>} : index
...
//   CHECK-DAG:     %[[ROW_2:.+]] = iree_codegen.index_hint %[[ID_2]]#1 {hint = #iree_gpu.lane_constant<16>} : index
//   CHECK-DAG:     %[[COL_2:.+]] = iree_codegen.index_hint %[[ID_2]]#2 {hint = #iree_gpu.lane_increment<16>} : index
...
//   CHECK-DAG:     %[[ROW:.+]] = iree_codegen.index_hint %[[ID]]#1 {hint = #iree_gpu.lane_constant<16>} : index
//   CHECK-DAG:     %[[COL:.+]] = iree_codegen.index_hint %c0 {hint = #iree_gpu.lane_increment<16>} : index
...
//   CHECK-DAG:     %[[ROW_ACC:.+]] = iree_codegen.index_hint %[[ID_ACC]]#1 {hint = #iree_gpu.lane_constant<16>} : index
//   CHECK-DAG:     %[[COL_ACC:.+]] = iree_codegen.index_hint %[[ID_ACC]]#2 {hint = #iree_gpu.lane_increment<16>} : index
...
//   CHECK-DAG:     %[[ID:.+]]:3 = affine.delinearize_index %[[LANEID]] into (4, 16)
//   CHECK-DAG:     %[[ROW1:.+]] = iree_codegen.index_hint %[[ID]]#1 {hint = #iree_gpu.lane_constant<16>} : index
//   CHECK-DAG:     %[[ROW2:.+]] = iree_codegen.index_hint %[[ID]]#2 {hint = #iree_gpu.lane_constant<16>} : index
//   CHECK-DAG:     %[[LHS_SLICE:.+]] = tensor.extract_slice %[[LHS]][0, 0, 0, %[[ROW2]], %[[ROW1]], 0] [3, 5, 1, 1, 1, 32]
//   CHECK-DAG:     %[[RHS_SLICE:.+]] = tensor.extract_slice %[[RHS]][0, 0, 0, %[[ROW1]], 0, %[[ROW2]]] [5, 1, 7, 1, 32, 1]
//   CHECK-DAG:     %[[COL:.+]] = iree_codegen.index_hint %[[ID]]#2 {hint = #iree_gpu.lane_increment<16>} : index
//   CHECK-DAG:     %[[LHS_SCALE_SLICE:.+]] = tensor.extract_slice %[[LHS_SCALE]][0, 0, %[[COL]], %[[ROW1]]] [3, 5, 1, 1]
//   CHECK-DAG:     %[[RHS_SCALE_SLICE:.+]] = tensor.extract_slice %[[RHS_SCALE]][0, 0, %[[ROW1]], %[[COL]]] [5, 7, 1, 1]
//   CHECK-DAG:     %[[IDY:.+]] = affine.linearize_index disjoint [%[[ROW1]], %c0] by (4, 4)
//   CHECK-DAG:     %[[ACC_SLICE:.+]] = tensor.extract_slice %[[ACC]][0, 0, %[[IDY]], %[[COL]]] [3, 7, 4, 1]
//       CHECK:     %[[MMA:.+]] = iree_codegen.inner_tiled ins(%[[LHS_SLICE]], %[[RHS_SLICE]], %[[LHS_SCALE_SLICE]], %[[RHS_SCALE_SLICE]]) outs(%[[ACC_SLICE]])
...
//       CHECK:     tensor.parallel_insert_slice %[[MMA]] into %[[ACC]][0, 0, %[[IDY]], %[[COL]]] [3, 7, 4, 1]
//       CHECK:   mapping = [#iree_gpu.lane_id<0>]
...
//       CHECK:   scf.forall (%[[LANEID:.+]]) in (64) shared_outs(%[[ACC:.+]] = {{.*}}) -> (tensor<3x7x16x16xf32>)
//   CHECK-DAG:     iree_codegen.index_hint {{.*}} {hint = #iree_gpu.lane_constant<16>} : index
//   CHECK-DAG:     iree_codegen.index_hint {{.*}} {hint = #iree_gpu.lane_increment<16>} : index
...
//   CHECK-DAG:     %[[RHS_SCALE_SLICE:.+]] = tensor.extract_slice %[[RHS_SCALE]]{{.*}} [5, 7, 1, 1]
//   CHECK-DAG:     %[[ACC_SLICE:.+]] = tensor.extract_slice %[[ACC]]{{.*}} [3, 7, 4, 1]
//       CHECK:     %[[MMA:.+]] = iree_codegen.inner_tiled ins(%[[LHS_SLICE]], %[[RHS_SLICE]], %[[LHS_SCALE_SLICE]], %[[RHS_SCALE_SLICE]]) outs(%[[ACC_SLICE]])
...
//       CHECK:     tensor.parallel_insert_slice %[[MMA]] into %[[ACC]]
//       CHECK:   mapping = [#iree_gpu.lane_id<0>]
...
//   CHECK-DAG:     %[[LHS_SCALE_SLICE:.+]] = tensor.extract_slice %[[LHS_SCALE]]{{.*}} [3, 5, 1, 1]
//   CHECK-DAG:     %[[RHS_SCALE_SLICE:.+]] = tensor.extract_slice %[[RHS_SCALE]]{{.*}} [5, 7, 1, 1]
//   CHECK-DAG:     %[[ACC_SLICE:.+]] = tensor.extract_slice %[[ACC]]{{.*}} [3, 7, 4, 4, 1]
//       CHECK:     %[[MMA:.+]] = iree_codegen.inner_tiled ins(%[[LHS_SLICE]], %[[RHS_SLICE]], %[[LHS_SCALE_SLICE]], %[[RHS_SCALE_SLICE]]) outs(%[[ACC_SLICE]])
