// RUN: sed /^\/\/ GFX950-TESTS-BEGIN$/, %s | iree-opt - -iree-transform-dialect-interpreter -transform-dialect-drop-schedule --split-input-file | FileCheck %s
// RUN: sed -n /^\/\/ GFX950-TESTS-BEGIN$/,/^\/\/ GFX950-TESTS-END$/p %s | iree-opt - -iree-transform-dialect-interpreter -transform-dialect-drop-schedule --split-input-file --iree-gpu-test-target=gfx950 | FileCheck %s --check-prefixes=CHECK,GFX950
