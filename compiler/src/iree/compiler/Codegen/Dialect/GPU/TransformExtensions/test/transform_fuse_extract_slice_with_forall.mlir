...
//   CHECK-DAG:     %[[SIZE_CLAMPED_HIGH0:.+]] = affine.min #[[$MAP]](%[[IDX0]])[%[[EXTRACT_SIZE0]], %[[STEP0]]]
//   CHECK-DAG:     %[[SIZE_CLAMPED_LOW0:.+]] = affine.max #[[$MAP1]](%[[SIZE_CLAMPED_HIGH0]])
//   CHECK-DAG:     %[[SIZE_CLAMPED_HIGH1:.+]] = affine.min #[[$MAP]](%[[IDX1]])[%[[EXTRACT_SIZE1]], %[[STEP1]]]
//   CHECK-DAG:     %[[SIZE_CLAMPED_LOW1:.+]] = affine.max #[[$MAP1]](%[[SIZE_CLAMPED_HIGH1]])
//       CHECK:     %[[SLICED_COPY:.+]] = tensor.extract_slice %[[COPY]][0, 0] [%[[SIZE_CLAMPED_LOW0]], %[[SIZE_CLAMPED_LOW1]]] [1, 1] : tensor<?x?xf32> to tensor<?x?xf32>
//       CHECK:     scf.forall.in_parallel {
//       CHECK:       tensor.parallel_insert_slice %[[SLICED_COPY]] into %[[SLICED_BBARG]]
//  CHECK-SAME:         [%[[IDX0]], %[[IDX1]]] [%[[SIZE_CLAMPED_LOW0]], %[[SIZE_CLAMPED_LOW1]]] [1, 1] : tensor<?x?xf32> into tensor<?x?xf32>
//       CHECK:     }
...
// CHECK-DAG: #[[$MAP:.+]] = affine_map<(d0) -> (-d0 + 1, 2)>
// CHECK-DAG: #[[$MAP1:.+]] = affine_map<(d0) -> (0, d0)>
...
//   CHECK-DAG:     %[[SIZE_CLAMPED_HIGH0:.+]] = affine.min #[[$MAP]](%[[IDX0]])
//   CHECK-DAG:     %[[SIZE_CLAMPED_LOW0:.+]] = affine.max #[[$MAP1]](%[[SIZE_CLAMPED_HIGH0]])
//   CHECK-DAG:     %[[SIZE_CLAMPED_HIGH1:.+]] = affine.min #[[$MAP2]](%[[IDX1]])[%[[ARG1]]]
//   CHECK-DAG:     %[[SIZE_CLAMPED_LOW1:.+]] = affine.max #[[$MAP1]](%[[SIZE_CLAMPED_HIGH1]])
//       CHECK:     %[[SLICED_COPY:.+]] = tensor.extract_slice %[[COPY]]
//  CHECK-SAME:       [0, 0] [%[[SIZE_CLAMPED_LOW0]], %[[SIZE_CLAMPED_LOW1]]] [1, 1] : tensor<2x2xf32> to tensor<?x?xf32>
//       CHECK:     scf.forall.in_parallel {
//       CHECK:       tensor.parallel_insert_slice %[[SLICED_COPY]] into %[[SLICED_BBARG]]
//  CHECK-SAME:         [%[[IDX0]], %[[IDX1]]] [%[[SIZE_CLAMPED_LOW0]], %[[SIZE_CLAMPED_LOW1]]] [1, 1] : tensor<?x?xf32> into tensor<1x?xf32>
//       CHECK:     }
//       CHECK:   } {mapping = [#gpu.thread<y>, #gpu.thread<x>]}
...
//       CHECK:     scf.forall.in_parallel {
//       CHECK:       tensor.parallel_insert_slice %[[SLICED_COPY]] into %[[SLICED_BBARG]]
//  CHECK-SAME:         [0, %[[SLICE_IDX]]] [1, %[[SIZE_CLAMPED_LOW]]] [1, 1] : tensor<?xf32> into tensor<4x?xf32>
//       CHECK:     }
//       CHECK:   } {mapping = [#gpu.thread<x>]}
//       CHECK:   return %[[FORALL_RESULT]]
