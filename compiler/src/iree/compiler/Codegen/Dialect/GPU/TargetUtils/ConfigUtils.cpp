...
getGemmHeuristicSeeds(GemmSize gemmSize, int64_t inBitWidth) {
  switch (gemmSize) {
...
getContractionHeuristicSeeds(GPUMatmulShapeType problem, bool isGemm) {
  ...
  if (isGemm) {
    return getGemmHeuristicSeeds(gemmSize, inBitWidth);
  }
...
static std::optional<GPUMMASchedule> getMmaScheduleFromProblemAndTarget(
  ...
  int64_t bytes = (mSize * nSize * outBitWidth + nSize * kSize * inBitWidth +
                   mSize * kSize * inBitWidth) /
                  8;
  ...
  std::optional<GPUMMAHeuristicSeeds> maybeSeeds =
      getContractionHeuristicSeeds(problem, isGemm);
