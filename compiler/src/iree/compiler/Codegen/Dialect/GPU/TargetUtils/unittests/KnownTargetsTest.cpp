...
TEST(PhaseGroupsTest, CDNA4_B128_NonContiguous) {
  ...
  EXPECT_THAT(phases[0], ElementsAreArray({0, 1, 2, 3, 12, 13, 14, 15, 20, 21,
                                           22, 23, 24, 25, 26, 27}));
  // Phase 1: threads 32-35, 44-47, 52-59
  EXPECT_THAT(phases[1], ElementsAreArray({32, 33, 34, 35, 44, 45, 46, 47, 52,
                                           53, 54, 55, 56, 57, 58, 59}));
  // Phase 2: threads 4-11, 16-19, 28-31
  EXPECT_THAT(phases[2], ElementsAreArray({4, 5, 6, 7, 8, 9, 10, 11, 16, 17, 18,
                                           19, 28, 29, 30, 31}));
  // Phase 3: threads 36-43, 48-51, 60-63
  EXPECT_THAT(phases[3], ElementsAreArray({36, 37, 38, 39, 40, 41, 42, 43, 48,
                                           49, 50, 51, 60, 61, 62, 63}));
