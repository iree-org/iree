...
static VectorType getThreadVectorType(MLIRContext *context,
  ...
  // For 3D block intrinsics, preserve the rank: each dim holds
  // outer[i]*element[i] elements per thread. This matches the 3D undistributed
  // tile structure.
  if (s.outer.size() == 3) {
    return VectorType::get({s.outer[0] * s.element[0],
                            s.outer[1] * s.element[1],
    ...
    if (numElems == 1) {
      ...
      if (vecTy.getRank() > 1) {
        vec = vector::ShapeCastOp::create(
            builder, loc, VectorType::get({1}, vecTy.getElementType()), vec);
      }
      return vector::ExtractOp::create(builder, loc, vec, 0);
    }
    if (vecTy.getRank() > 1) {
      vec = vector::ShapeCastOp::create(
          builder, loc, VectorType::get({numElems}, vecTy.getElementType()),
          vec);
    }
    return vec;
  };
