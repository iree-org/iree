...
namespace mlir::iree_compiler::IREE::GPU::detail {
...
bool mlir::iree_compiler::IREE::GPU::isScaledContractionBody(
...
inferScaledContractionDimsImpl(ArrayRef<AffineMap> indexingMaps,
                               ArrayRef<utils::IteratorType> iterators) {
  auto printArray = [](ArrayRef<AffineMap> a) -> int {
    llvm::errs() << "------------\n";
    for (auto b : a)
      llvm::errs() << "\t" << b << "\n";
    return 1;
  };
  printArray(indexingMaps);
...
mlir::iree_compiler::IREE::GPU::inferScaledContractionDims(
...
mlir::iree_compiler::IREE::GPU::detail::MatchContractionResult
mlir::iree_compiler::IREE::GPU::isScaledContractionImpl(
    Operation *op, ScaledContractionDimensions *dimensions) {
  auto linalgOp = dyn_cast<linalg::LinalgOp>(op);
  llvm::errs() << "[DEBUG] - isScaledContractionImpl\n";
  if (!linalgOp)
    return mlir::iree_compiler::IREE::GPU::detail::MatchContractionResult::
        NotLinalgOp;
  ...
  if (!::isScaledContractionBody<
      ...
      *linalgOp.getBlock())) {
    return mlir::iree_compiler::IREE::GPU::detail::MatchContractionResult::NotAddMul;
  }
  // clang-format on
...
bool mlir::iree_compiler::IREE::GPU::isaScaledContractionOpInterface(
    linalg::LinalgOp linalgOp) {
  if (!linalgOp)
    return false;
  Operation *op = linalgOp.getOperation();
  return isScaledContractionImpl(op) == mlir::iree_compiler::IREE::GPU::detail::
                                            MatchContractionResult::Success;
}
