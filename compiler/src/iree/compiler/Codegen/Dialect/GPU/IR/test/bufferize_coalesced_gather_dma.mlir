...
func.func @bufferize_coalesced_gather_dma_static(%idx0: vector<64xindex>,
  ...
  %result = scf.forall (%i) in (%c1) shared_outs(%out = %dest) -> (tensor<64xf32>) {
    scf.forall.in_parallel {
      iree_gpu.coalesced_gather_dma %source[%idx0] into %out dest_size = [64] lane(%lane)
        : tensor<2048xf32>, vector<64xindex>, tensor<64xf32>, index
    }
  }
...
//       CHECK:   scf.forall
//       CHECK:     iree_gpu.coalesced_gather_dma %{{.+}}[%{{.+}}] into %{{.+}} dest_size = [64] lane(%{{.+}}) : memref<2048xf32, strided<[?], offset: ?>>, vector<64xindex>, memref<64xf32, strided<[?], offset: ?>>, index
...
func.func @bufferize_coalesced_gather_dma_f16(%idx0: vector<128xindex>,
  ...
  %result = scf.forall (%i) in (%c1) shared_outs(%out = %dest) -> (tensor<128xf16>) {
    scf.forall.in_parallel {
      iree_gpu.coalesced_gather_dma %source[%idx0] into %out dest_size = [128] lane(%lane)
        : tensor<8192xf16>, vector<128xindex>, tensor<128xf16>, index
    }
  }
...
//       CHECK:   scf.forall
//       CHECK:     iree_gpu.coalesced_gather_dma %{{.+}}[%{{.+}}] into %{{.+}} dest_size = [128] lane(%{{.+}}) : memref<8192xf16, strided<[?], offset: ?>>, vector<128xindex>, memref<128xf16, strided<[?], offset: ?>>, index
...
func.func @bufferize_coalesced_gather_dma_1d(%indices: vector<1024xindex>,
  ...
  %result = scf.forall (%i) in (%c32) shared_outs(%out = %dest) -> (tensor<1024xf32>) {
    scf.forall.in_parallel {
      iree_gpu.coalesced_gather_dma %source[%indices] into %out dest_size = [1024] lane(%lane)
        : tensor<2048xf32>, vector<1024xindex>, tensor<1024xf32>, index
    }
  }
...
//       CHECK:     iree_gpu.coalesced_gather_dma %{{.+}}[%{{.+}}] into %{{.+}} dest_size = [1024] lane(%{{.+}}) : memref<2048xf32, strided<[?], offset: ?>>, vector<1024xindex>, memref<1024xf32, strided<[?], offset: ?>>, index
...
func.func @bufferize_coalesced_gather_dma_nested(%idx0: vector<8xindex>,
  ...
  %result = scf.forall (%wg_i, %wg_j) in (16, 1) shared_outs(%wg_out = %dest) -> (tensor<8xf32>) {
    %inner_result = scf.forall (%sg_i, %sg_j) in (32, 1) shared_outs(%sg_out = %wg_out) -> (tensor<8xf32>) {
      scf.forall.in_parallel {
        iree_gpu.coalesced_gather_dma %source[%idx0] into %sg_out dest_size = [8] lane(%lane)
          : tensor<2048xf32>, vector<8xindex>, tensor<8xf32>, index
      }
...
//       CHECK:       iree_gpu.coalesced_gather_dma %{{.+}}[%{{.+}}] into %{{.+}} dest_size = [8] lane(%{{.+}}) : memref<2048xf32, strided<[?], offset: ?>>, vector<8xindex>, memref<8xf32, strided<[?], offset: ?>>, index
...
func.func @bufferize_coalesced_gather_dma_no_indices(%source: tensor<1x32xf32>,
  ...
  %result = scf.forall (%i) in (%c1) shared_outs(%out = %dest) -> (tensor<1x32xf32>) {
    scf.forall.in_parallel {
      iree_gpu.coalesced_gather_dma %source into %out dest_size = [1, 32] lane(%lane)
        : tensor<1x32xf32>, tensor<1x32xf32>, index
    }
  }
...
//       CHECK:   scf.forall
//       CHECK:     iree_gpu.coalesced_gather_dma %{{.+}} into %{{.+}} dest_size = [1, 32] lane(%{{.+}}) : memref<1x32xf32, strided<[?, ?], offset: ?>>, memref<1x32xf32, strided<[?, ?], offset: ?>>, index
