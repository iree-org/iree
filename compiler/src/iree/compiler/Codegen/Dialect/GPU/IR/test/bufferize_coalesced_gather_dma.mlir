...
func.func @bufferize_coalesced_gather_dma_static(%idx0: vector<64xindex>,
                                                  ...
                                                  %dest: tensor<64xf32> {bufferization.writable = true}) -> tensor<64xf32> {
  %c1 = arith.constant 1 : index
  %result = scf.forall (%i) in (%c1) shared_outs(%out = %dest) -> (tensor<64xf32>) {
    scf.forall.in_parallel {
      iree_gpu.coalesced_gather_dma %source[%idx0] into %out
        : tensor<2048xf32>, vector<64xindex>, tensor<64xf32> -> tensor<64xf32>
    }
  }
...
//       CHECK:   scf.forall
//       CHECK:     iree_gpu.coalesced_gather_dma %{{.+}}[%{{.+}}] into %{{.+}} : memref<2048xf32, strided<[?], offset: ?>>, vector<64xindex>, memref<64xf32, strided<[?], offset: ?>>
...
func.func @bufferize_coalesced_gather_dma_f16(%idx0: vector<128xindex>,
                                               ...
                                               %dest: tensor<128xf16>) -> tensor<128xf16> {
  %c1 = arith.constant 1 : index
...
//       CHECK:   scf.forall
//       CHECK:     iree_gpu.coalesced_gather_dma %{{.+}}[%{{.+}}] into %{{.+}} : memref<8192xf16, strided<[?], offset: ?>>, vector<128xindex>, memref<128xf16, strided<[?], offset: ?>>
...
//       CHECK:     iree_gpu.coalesced_gather_dma %{{.+}}[%{{.+}}] into %{{.+}} : memref<2048xf32, strided<[?], offset: ?>>, vector<1024xindex>, memref<1024xf32, strided<[?], offset: ?>>
...
func.func @bufferize_coalesced_gather_dma_nested(%idx0: vector<8xindex>,
                                                  %source: tensor<2048xf32>,
                                                  %dest: tensor<8xf32> {bufferization.writable = true}) -> tensor<8xf32> {
  %result = scf.forall (%wg_i, %wg_j) in (16, 1) shared_outs(%wg_out = %dest) -> (tensor<8xf32>) {
    %inner_result = scf.forall (%sg_i, %sg_j) in (32, 1) shared_outs(%sg_out = %wg_out) -> (tensor<8xf32>) {
      scf.forall.in_parallel {
        iree_gpu.coalesced_gather_dma %source[%idx0] into %sg_out
          : tensor<2048xf32>, vector<8xindex>, tensor<8xf32> -> tensor<8xf32>
      }
...
//       CHECK:       iree_gpu.coalesced_gather_dma %{{.+}}[%{{.+}}] into %{{.+}} : memref<2048xf32, strided<[?], offset: ?>>, vector<8xindex>, memref<8xf32, strided<[?], offset: ?>>
...
func.func @bufferize_coalesced_gather_dma_no_indices(%source: tensor<1x32xf32>,
                                                      %dest: tensor<1x32xf32>) -> tensor<1x32xf32> {
  %c1 = arith.constant 1 : index
...
//       CHECK:   scf.forall
//       CHECK:     iree_gpu.coalesced_gather_dma %{{.+}} into %{{.+}} : memref<1x32xf32, strided<[?, ?], offset: ?>>, memref<1x32xf32, strided<[?, ?], offset: ?>>
