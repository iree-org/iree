...
func.func @coalesced_gather_dma_static(%idx0: vector<64xindex>, %source: tensor<4096xf32>, %dest: tensor<64xf32>) -> tensor<64xf32> {
  %c1 = arith.constant 1 : index
  %result = scf.forall (%i) in (%c1) shared_outs(%out = %dest) -> (tensor<64xf32>) {
    scf.forall.in_parallel {
      iree_gpu.coalesced_gather_dma %source[%idx0] into %out
        : tensor<4096xf32>, vector<64xindex>, tensor<64xf32> -> tensor<64xf32>
    }
  }
...
//       CHECK:       iree_gpu.coalesced_gather_dma %{{.+}}[%{{.+}}] into %{{.+}} : tensor<4096xf32>, vector<64xindex>, tensor<64xf32> -> tensor<64xf32>

// -----

// Test coalesced_gather_dma with different element types
func.func @coalesced_gather_dma_f16(%idx0: vector<128xindex>, %source: tensor<8192xf16>, %dest: tensor<128xf16>) -> tensor<128xf16> {
  %c1 = arith.constant 1 : index
  %result = scf.forall (%i) in (%c1) shared_outs(%out = %dest) -> (tensor<128xf16>) {
    scf.forall.in_parallel {
      iree_gpu.coalesced_gather_dma %source[%idx0] into %out
        : tensor<8192xf16>, vector<128xindex>, tensor<128xf16> -> tensor<128xf16>
    }
  }
...
//       CHECK:   scf.forall
//       CHECK:     scf.forall.in_parallel
//       CHECK:       iree_gpu.coalesced_gather_dma %{{.+}}[%{{.+}}] into %{{.+}} : tensor<8192xf16>, vector<128xindex>, tensor<128xf16> -> tensor<128xf16>

// -----

func.func @coalesced_gather_dma_1d(%indices: vector<1024xindex>, %source: tensor<2048xf32>, %dest: tensor<1024xf32>) -> tensor<1024xf32> {
  %c32 = arith.constant 32 : index
  %result = scf.forall (%i) in (%c32) shared_outs(%out = %dest) -> (tensor<1024xf32>) {
    scf.forall.in_parallel {
      iree_gpu.coalesced_gather_dma %source[%indices] into %out
        : tensor<2048xf32>, vector<1024xindex>, tensor<1024xf32> -> tensor<1024xf32>
    }
  }
...
//       CHECK:     scf.forall.in_parallel
//       CHECK:       iree_gpu.coalesced_gather_dma %{{.+}}[%{{.+}}] into %{{.+}} : tensor<2048xf32>, vector<1024xindex>, tensor<1024xf32> -> tensor<1024xf32>

// -----

func.func @coalesced_gather_dma_copy(%source: tensor<32x128xf32>, %dest: tensor<32x128xf32>) -> tensor<32x128xf32> {
  %c1 = arith.constant 1 : index
  %result = scf.forall (%i) in (%c1) shared_outs(%out = %dest) -> (tensor<32x128xf32>) {
    scf.forall.in_parallel {
      iree_gpu.coalesced_gather_dma %source into %out
...
//       CHECK:   scf.forall
//       CHECK:     scf.forall.in_parallel
//       CHECK:       iree_gpu.coalesced_gather_dma %{{.+}} into %{{.+}} : tensor<32x128xf32>, tensor<32x128xf32> -> tensor<32x128xf32>

// -----

func.func @coalesced_gather_dma_copy_memref(%source: memref<1x32xf32, strided<[128, 1], offset: ?>>, %dest: memref<1x32xf32, strided<[128, 1], offset: ?>, #gpu.address_space<workgroup>>) {
  iree_gpu.coalesced_gather_dma %source into %dest
    : memref<1x32xf32, strided<[128, 1], offset: ?>>, memref<1x32xf32, strided<[128, 1], offset: ?>, #gpu.address_space<workgroup>>
  return
}

// CHECK-LABEL: func @coalesced_gather_dma_copy_memref
//       CHECK:   iree_gpu.coalesced_gather_dma %{{.+}} into %{{.+}} : memref<1x32xf32, strided<[128, 1], offset: ?>>, memref<1x32xf32, strided<[128, 1], offset: ?>, #gpu.address_space<workgroup>>

// -----

func.func @coalesced_gather_dma_tensor_indices(%idx0: tensor<64xindex>, %source: tensor<4096xf32>, %dest: tensor<64xf32>) -> tensor<64xf32> {
  %c1 = arith.constant 1 : index
  %result = scf.forall (%i) in (%c1) shared_outs(%out = %dest) -> (tensor<64xf32>) {
    scf.forall.in_parallel {
      iree_gpu.coalesced_gather_dma %source[%idx0] into %out
        : tensor<4096xf32>, tensor<64xindex>, tensor<64xf32> -> tensor<64xf32>
    }
  }
  return %result : tensor<64xf32>
}
...
//       CHECK:   scf.forall
//       CHECK:     scf.forall.in_parallel
//       CHECK:       iree_gpu.coalesced_gather_dma %{{.+}}[%{{.+}}] into %{{.+}} : tensor<4096xf32>, tensor<64xindex>, tensor<64xf32> -> tensor<64xf32>
