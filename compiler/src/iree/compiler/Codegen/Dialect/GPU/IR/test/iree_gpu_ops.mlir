...
//       CHECK:   scf.forall
//       CHECK:     %[[GATHERED:.+]] = iree_gpu.coalesced_gather_dma %[[INDICES]], %[[SOURCE]] into %{{.+}} : tensor<64x32xindex>, tensor<1024x64xf32>, tensor<64x32xf32> -> tensor<64x32xf32>
...
//       CHECK:   scf.forall
//       CHECK:     %[[GATHERED:.+]] = iree_gpu.coalesced_gather_dma %[[INDICES]], %[[SOURCE]] into %{{.+}} : tensor<?x?xindex>, tensor<?x?xf32>, tensor<?x?xf32> -> tensor<?x?xf32>
//       CHECK:     scf.forall.in_parallel
...
//       CHECK:   scf.forall
//       CHECK:     %[[GATHERED:.+]] = iree_gpu.coalesced_gather_dma %[[INDICES]], %[[SOURCE]] into %{{.+}} : tensor<128x64xindex>, tensor<2048x64xf16>, tensor<128x64xf16> -> tensor<128x64xf16>
//       CHECK:     scf.forall.in_parallel
...
func.func @coalesced_gather_dma_1d(%indices: tensor<1024xindex>, %source: tensor<1024xf32>, %dest: tensor<256xf32>) -> tensor<256xf32> {
  %c1 = arith.constant 1 : index
  %result = scf.forall (%i) in (%c1) shared_outs(%out = %dest) -> (tensor<256xf32>) {
    %gathered = iree_gpu.coalesced_gather_dma %indices, %source into %out
      : tensor<1024xindex>, tensor<1024xf32>, tensor<256xf32> -> tensor<256xf32>
    scf.forall.in_parallel {
...
//       CHECK:   scf.forall
//       CHECK:     %[[GATHERED:.+]] = iree_gpu.coalesced_gather_dma %[[INDICES]], %[[SOURCE]] into %{{.+}} : tensor<1024xindex>, tensor<1024xf32>, tensor<256xf32> -> tensor<256xf32>
//       CHECK:     scf.forall.in_parallel
...
func.func @coalesced_gather_dma_in_forall(%indices: tensor<1024x64xindex>, %source: tensor<2048x64xf32>, %dest: tensor<256x16xf32>) -> tensor<256x16xf32> {
  ...
  %result = scf.forall (%i) in (%c4) shared_outs(%out = %dest) -> (tensor<256x16xf32>) {
    ...
    %gathered = iree_gpu.coalesced_gather_dma %indices_slice, %source into %dest_slice
      : tensor<64x16xindex>, tensor<2048x64xf32>, tensor<64x16xf32> -> tensor<64x16xf32>
...
//       CHECK:     %[[GATHERED:.+]] = iree_gpu.coalesced_gather_dma %[[INDICES_SLICE]], %[[SOURCE]] into %[[DEST_SLICE]] : tensor<64x16xindex>, tensor<2048x64xf32>, tensor<64x16xf32> -> tensor<64x16xf32>
//       CHECK:     scf.forall.in_parallel
