...
// nullopt if it is dynamic / absent. Ported from the ROCM ukernel support
// (`getCInnermostStaticCrossIntrinsicDim`), using the CPU DataTiledMMA
// swizzle. The ukernel needs this dim's stride to address each unrolled
// intrinsic's ACC fragment.
...
getAccInnermostCrossIntrinsicDim(IREE::Codegen::InnerTiledOp op,
  ...
  for (size_t e = swizzleDims.size(), i = e - 1; i < e; --i) {
...
// stride. Mirrors ROCM's `handleInnerTiledMmaUkernel` (minus the
// shared-memory / subgroup machinery, which CPU does not have).
...
handleInnerTiledMmaUkernel(RewriterBase &rewriter, StringRef name,
  ...
  // strided_dims: one entry per shaped operand (LHS, RHS, ACC). Only ACC gets
  // a stride — the innermost cross-intrinsic (N) dim — so the ukernel can
  // address each unrolled intrinsic's ACC fragment. LHS/RHS are passed as
  // base+offset and the ukernel infers their strides from the contiguous
  // data-tiled layout.
