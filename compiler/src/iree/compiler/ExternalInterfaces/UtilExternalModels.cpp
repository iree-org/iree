...
struct SCFWhileOpMutableRegionBranchOpInterface
  ...
  Operation *rebuildWithExpandedTypes(
    ...
    // Expand block arguments in the before region.
    Block &beforeBlock = newWhileOp.getBefore().front();
    for (int i = beforeBlock.getNumArguments() - 1; i >= 0; --i) {
      auto arg = beforeBlock.getArgument(i);
      SmallVector<Type> expandedTypes;
      expandTypeFn(arg.getType(), expandedTypes);
      if (expandedTypes.size() > 1) {
        // This type expands to multiple values.
        // Insert new arguments after the current one.
        for (unsigned j = 1; j < expandedTypes.size(); ++j) {
          beforeBlock.insertArgument(i + j, expandedTypes[j], arg.getLoc());
        }
      }
    }

    // Expand block arguments in the after region.
    Block &afterBlock = newWhileOp.getAfter().front();
    for (int i = afterBlock.getNumArguments() - 1; i >= 0; --i) {
      auto arg = afterBlock.getArgument(i);
      SmallVector<Type> expandedTypes;
      expandTypeFn(arg.getType(), expandedTypes);
      if (expandedTypes.size() > 1) {
        // This type expands to multiple values.
        // Insert new arguments after the current one.
        for (unsigned j = 1; j < expandedTypes.size(); ++j) {
          afterBlock.insertArgument(i + j, expandedTypes[j], arg.getLoc());
        }
      }
    }
    ...
    // Update the condition to yield expanded values.
    if (beforeBlock.mightHaveTerminator()) {
      auto *terminator = beforeBlock.getTerminator();
      if (auto condOp = dyn_cast<scf::ConditionOp>(terminator)) {
        SmallVector<Value> newCondOperands;
        builder.setInsertionPoint(condOp);
        for (Value operand : condOp.getArgs()) {
          expandOperandFn(operand, newCondOperands, builder);
        }
        scf::ConditionOp::create(builder, condOp.getLoc(),
                                 condOp.getCondition(), newCondOperands);
        condOp.erase();
      }
    }

    // Update the yield to return expanded values.
    if (afterBlock.mightHaveTerminator()) {
      auto *terminator = afterBlock.getTerminator();
      if (auto yieldOp = dyn_cast<scf::YieldOp>(terminator)) {
        SmallVector<Value> newYieldOperands;
        builder.setInsertionPoint(yieldOp);
        for (Value operand : yieldOp.getOperands()) {
