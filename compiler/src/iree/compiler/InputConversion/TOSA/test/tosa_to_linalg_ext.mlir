// RUN: iree-opt --split-input-file --iree-tosa-to-linalg-ext --verify-diagnostics %s
// -verify-diagnostics
...
func.func @scatter_static(%arg0 : tensor<1x4x5xf32>, %arg1 : tensor<1x2xi32>, %arg2 : tensor<1x2x5xf32>) ->  tensor<1x4x5xf32> {
  // CHECK: %[[EXPANDIDX:.+]] = tensor.expand_shape %arg1 [[0], [1, 2]] : tensor<1x2xi32> into tensor<1x2x1xi32>
  // CHECK: %[[EMPTY:.+]] = tensor.empty() : tensor<1x2x1xi32>
  // CHECK: %[[GENERIC:.+]] = linalg.generic 
  // CHECK-SAME: indexing_maps = [#map, #map]
  // CHECK-SAME: iterator_types = ["parallel", "parallel", "parallel"]}
  // CHECK-SAME: ins(%[[EXPANDIDX]] : tensor<1x2x1xi32>)
  // CHECK-SAME: outs(%[[EMPTY:.+]] : tensor<1x2x1xi32>) {
  ...
  // CHECK: %[[COLLAPSE_IDX:.+]] = tensor.collapse_shape %[[CONCAT]] [[0, 1], [2]] : tensor<1x2x2xi32> into tensor<2x2xi32>
  // CHECK: %[[COLLAPSE_UPD:.+]] = tensor.collapse_shape %arg2 [[0, 1], [2]] : tensor<1x2x5xf32> into tensor<2x5xf32>
...
func.func @scatter_static_batched(%arg0 : tensor<2x4x5xf32>, %arg1 : tensor<2x2xi32>, %arg2 : tensor<2x2x5xf32>) ->  tensor<2x4x5xf32> {
  // CHECK: %[[EXPANDIDX:.+]] = tensor.expand_shape %arg1 [[0], [1, 2]] : tensor<2x2xi32> into tensor<2x2x1xi32>
  ...
  // CHECK:   linalg.yield %[[CAST]] : i32
  // CHECK: %[[CONCAT:.+]] = "tosa.concat"(%[[GENERIC]], %[[EXPANDIDX]]) {axis = 2 : i64}
  // CHECK: %[[COLLAPSE_IDX:.+]] = tensor.collapse_shape %[[CONCAT]] [[0, 1], [2]] : tensor<2x2x2xi32> into tensor<4x2xi32>
  // CHECK: %[[COLLAPSE_UPD:.+]] = tensor.collapse_shape %arg2 [[0, 1], [2]] : tensor<2x2x5xf32> into tensor<4x5xf32>
  // CHECK: %[[SCATTER:.+]] = iree_linalg_ext.scatter dimension_map = [0, 1] unique_indices(true)
...
func.func @scatter_dynamic(%arg0 : tensor<?x?x?xf32>, %arg1 : tensor<?x?xi32>, %arg2 : tensor<?x?x?xf32>) ->  tensor<?x?x?xf32> {
  ...
  // CHECK: %[[DIM0:.+]] = tensor.dim %[[EXPAND]], %[[C0]] : tensor<?x?x1xi32>
  // CHECK: %[[DIM1:.+]] = tensor.dim %[[EXPAND]], %[[C1]] : tensor<?x?x1xi32>
  ...
  // CHECK: %[[COLLAPSE_IDX:.+]] = tensor.collapse_shape %[[GENERIC]] [[0, 1], [2]] : tensor<?x?x2xi32> into tensor<?x2xi32>
  // CHECK: %[[COLLAPSE_UPD:.+]] = tensor.collapse_shape %arg2 [[0, 1], [2]] : tensor<?x?x?xf32> into tensor<?x?xf32>
