...
struct FftOpConversion : public OpConversionPattern<mhlo::FftOp> {
  ...
  static SmallVector<Value> getBitReversalOrder(ImplicitLocOpBuilder &b,
    ...
    auto genericOp = b.create<linalg::GenericOp>(
        TypeRange{realType}, indices, empty, maps, iterTypes,
...
struct ReverseOpConversion : public OpConversionPattern<mhlo::ReverseOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    rewriter.replaceOpWithNewOp<IREE::LinalgExt::ReverseOp>(
        op, op->getResultTypes(), adaptor.getOperands(), empty,
        op.dimensions());
    return success();
  }
};
...
struct TopkOpConversion : public OpConversionPattern<chlo::TopKOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    Value emptyOutputValues = rewriter.create<mlir::tensor::EmptyOp>(
        loc, outputValuesType.getShape(), valueElementType, dynSizes);
    Value emptyOutputIndices = rewriter.create<mlir::tensor::EmptyOp>(
    ...
    Value negInfTensor =
        rewriter.create<linalg::FillOp>(loc, negInf, emptyOutputValues)
            .result();
    Value posInfTensor =
        rewriter.create<linalg::FillOp>(loc, posInf, emptyOutputIndices)
            .result();
