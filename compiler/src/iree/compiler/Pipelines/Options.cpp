...
void ParameterOptions::bindOptions(OptionsBinder &binder) {
  ...
  binder.opt<std::string>(
      "iree-parameter-encoder-target", encoderTarget,
      llvm::cl::desc("Device target name for encoder (empty = unspecified). "
                     "Allows encoding on a different target than compilation."),
      llvm::cl::cat(category));
