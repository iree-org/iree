...
createImportWrapperFunc(IREE::ABI::InvocationModel invocationModel,
  ...
  if (!defaultAffinityAttr) {
    defaultAffinityAttr = importOp->getAttr("stream.affinity");
    importOp->removeAttr("stream.affinity");
  }
...
createExportWrapperFunc(IREE::ABI::InvocationModel invocationModel,
  ...
  // Fetch and normalize any explicitly assigned affinity.
  Attribute defaultAffinityAttr = exportOp->getAttr("iree.abi.affinity");
  if (!defaultAffinityAttr) {
    defaultAffinityAttr = exportOp->getAttr("stream.affinity");
    exportOp->removeAttr("stream.affinity");
  }
