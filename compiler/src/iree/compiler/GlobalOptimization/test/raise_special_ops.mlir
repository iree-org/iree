...
util.func public @jit_eval_168(%arg0: tensor<1024x7x7x2xi8>) -> tensor<1024x7x7xi8> {
  ...
  %3 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d1, d2)>, affine_map<(d0, d1, d2) -> (d0, d1, d2)>], iterator_types = ["parallel", "parallel", "parallel"]} ins(%extracted_slice : tensor<1024x7x7xf32>) outs(%2 : tensor<1024x7x7xi8>) {
  ^bb0(%in: f32, %out: i8):
...
// CHECK-SAME:      tensor<2x1024x7x7xf32> to tensor<1024x7x7xf32>
