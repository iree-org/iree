// RUN: iree-opt --iree-stream-replicate-globals-per-affinity="use-transfers=true" --split-input-file %s | FileCheck %s
...
util.func private @unknown_global_device(%arg0: tensor<10xf32>) -> (tensor<10xf32>, tensor<10xf32>) {
  ...
  // CHECK-DAG: util.global.load immutable @[[$GLOBAL]]
  // CHECK-DAG: %[[LOAD_A:.+]] = util.global.load immutable @[[$GLOBAL]]
  // CHECK-DAG: %[[TRANSFER_A:.+]] = flow.tensor.transfer %[[LOAD_A]] {{.+}} to #hal.device.affinity<@[[$DEVICE_A]]>
  %global = util.global.load immutable @global : tensor<10xf32>

  // CHECK: flow.dispatch @dispatch(%[[OPERAND_A]], %[[TRANSFER_A]])
  ...
  // CHECK-DAG: %[[LOAD_B:.+]] = util.global.load immutable @[[$GLOBAL]]
  // CHECK-DAG: %[[TRANSFER_B:.+]] = flow.tensor.transfer %[[LOAD_B]] {{.+}} to #hal.device.affinity<@[[$DEVICE_B]]>
  // CHECK: flow.dispatch @dispatch(%[[OPERAND_B]], %[[TRANSFER_B]])
...
util.func private @ambiguous_indirect_global_load(%arg0: tensor<f32>, %arg1: tensor<f32>) -> (tensor<f32>, tensor<f32>) {
  ...
  // CHECK-DAG: %[[LOAD:.+]] = util.global.load immutable @[[$GLOBAL]]
  // CHECK-DAG: %[[TRANSFER_A:.+]] = flow.tensor.transfer %[[LOAD]] {{.+}} to #hal.device.affinity<@[[$DEVICE_A]]>
  // CHECK-DAG: %[[TRANSFER_B:.+]] = flow.tensor.transfer %[[LOAD]] {{.+}} to #hal.device.affinity<@[[$DEVICE_B]]>
  // CHECK-DAG: %[[LHS_A:.+]] = flow.dispatch @dispatch0(%[[TRANSFER_A]])
  // CHECK-DAG: %[[LHS_B:.+]] = flow.dispatch @dispatch0(%[[TRANSFER_B]])
...
// CHECK-NOT: util.global private @{{.+}}_{{.+}} : tensor<2x5xf32>
util.global private @foo : tensor<2x5xf32>

// CHECK: util.global private @[[$BAR:.+]] : tensor<10xf32>
// CHECK-NOT: util.global private @{{.+}}_{{.+}} : tensor<10xf32>
...
util.func private @ambiguous_indirect_global_load_multi_levels(%arg0: tensor<10xf32>, %arg1: tensor<10xf32>) -> (tensor<10xf32>, tensor<10xf32>) {
  ...
  // CHECK-DAG: %[[LOAD_FOO:.+]] = util.global.load immutable @[[$FOO]]
  // CHECK-DAG: %[[TRANSFER_FOO_A:.+]] = flow.tensor.transfer %[[LOAD_FOO]] {{.+}} to #hal.device.affinity<@[[$DEVICE_A]]>
  // CHECK-DAG: %[[TRANSFER_FOO_B:.+]] = flow.tensor.transfer %[[LOAD_FOO]] {{.+}} to #hal.device.affinity<@[[$DEVICE_B]]>
  // CHECK-DAG: %[[VAL_A:.+]] = flow.dispatch @dispatch0(%[[TRANSFER_FOO_A]])
  // CHECK-DAG: %[[RESHAPE_A:.+]] = flow.tensor.reshape %[[VAL_A]] : tensor<2x5xf32> -> tensor<10xf32>
  // CHECK-DAG: %[[VAL_B:.+]] = flow.dispatch @dispatch0(%[[TRANSFER_FOO_B]])
  ...
  // CHECK-DAG: %[[LOAD_BAR:.+]] = util.global.load immutable @[[$BAR]]
  // CHECK-DAG: %[[TRANSFER_BAR_A:.+]] = flow.tensor.transfer %[[LOAD_BAR]] {{.+}} to #hal.device.affinity<@[[$DEVICE_A]]>
  // CHECK-DAG: %[[TRANSFER_BAR_B:.+]] = flow.tensor.transfer %[[LOAD_BAR]] {{.+}} to #hal.device.affinity<@[[$DEVICE_B]]>
  // CHECK-DAG: %[[LHS_A:.+]] = flow.dispatch @dispatch1(%[[RESHAPE_A]], %[[TRANSFER_BAR_A]])
  // CHECK-DAG: %[[LHS_B:.+]] = flow.dispatch @dispatch1(%[[RESHAPE_B]], %[[TRANSFER_BAR_B]])
...
util.func private @launch_on_device_a(%arg0: tensor<f32>, %arg1: tensor<f32>) -> tensor<f32> {
  ...
  // CHECK: %[[TRANSFER:.+]] = flow.tensor.transfer %[[LOAD]] {{.+}} to #hal.device.affinity<@[[$DEVICE_A]]>
  // CHECK: flow.dispatch @dispatch(%[[OPERAND0]], %[[TRANSFER]])
...
util.func private @launch_on_device_b(%arg0: tensor<f32>, %arg1: tensor<f32>) -> tensor<f32> {
  ...
  // CHECK: %[[LOAD:.+]] = util.global.load immutable @[[$GLOBAL]]
  // CHECK: %[[TRANSFER:.+]] = flow.tensor.transfer %[[LOAD]] {{.+}} to #hal.device.affinity<@[[$DEVICE_B]]>
  // CHECK: flow.dispatch @dispatch(%[[OPERAND0]], %[[TRANSFER]])
  %1 = flow.dispatch @dispatch(%0, %arg1) : (tensor<f32>, tensor<f32>) -> tensor<f32>
  util.return %1 : tensor<f32>
}
