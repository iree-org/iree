...
// A pre-existing encoding in the target attribute should be materialized in the encoding attribute
...
module {
  util.global private @device_a = #device_target_local_0_
  util.func public @existing_layout_encoding_in_target(%arg0: index, %arg1: index) -> index {
    %0 = stream.tensor.sizeof on(#hal.device.affinity<@device_a>) tensor<?x?xf32, #encoding>{%arg0, %arg1} : index
    util.return %0 : index
  }
}
// CHECK:       #[[$ENCODING:.+]] = #iree_encoding.encoding
// CHECK-SAME:    #iree_gpu.gpu_encoding_layout
// CHECK-SAME:    encoding_info = {innerDimsPos = [0, 1], innerTileSizes = [128, 32], outerDimsPerm = [0, 1]{{.+}}}
// CHECK-LABEL: util.func public @existing_layout_encoding_in_target
// CHECK:         %[[RES:.+]] = stream.tensor.sizeof {{.+}} tensor<?x?xf32, #[[$ENCODING]]>
// CHECK:         return %[[RES]]
