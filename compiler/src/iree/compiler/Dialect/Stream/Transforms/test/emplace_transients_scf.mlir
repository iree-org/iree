...
util.func public @scf_if_transient_in_then(
  ...
  %final_tp = scf.if %cond -> !stream.timepoint {
    ...
    // CHECK: %[[C512_CONSERVATIVE:.+]] = arith.constant 512 : index
    ...
    // CHECK-NEXT:   [0, 0] = %[[C512_CONSERVATIVE]]
...
util.func public @scf_if_transient_in_both(
  ...
  %final_tp = scf.if %cond -> !stream.timepoint {
    ...
    // CHECK: %[[THEN_SUBVIEW:.+]] = stream.resource.subview %[[STORAGE_SUBVIEW]][%[[PACK]]#1] : !stream.resource<transient>{%[[PACK]]#0} -> !stream.resource<transient>{%[[C512]]}
    %t0, %tp0 = stream.resource.alloca uninitialized await(%input_timepoint) => !stream.resource<transient>{%c512} => !stream.timepoint
  ...
  // CHECK: } else {
    ...
    // CHECK: %[[ELSE_SUBVIEW:.+]] = stream.resource.subview %[[STORAGE_SUBVIEW]][%[[PACK]]#1] : !stream.resource<transient>{%[[PACK]]#0} -> !stream.resource<transient>{%[[C256]]}
...
util.func public @scf_for_with_transient(
  ...
  %final_tp = scf.for %iv = %lb to %ub step %step iter_args(%iter_tp = %input_timepoint) -> !stream.timepoint {
    ...
    // CHECK: %[[TRANSIENT_SUBVIEW:.+]] = stream.resource.subview %[[STORAGE_VIEW]][%[[PACK]]#1] : !stream.resource<transient>{%[[PACK]]#0} -> !stream.resource<transient>{%c128{{(_[0-9]+)?}}}
    ...
    // CHECK: %[[EXEC_TP:.+]] = stream.cmd.execute await(%[[JOINED]]) => with(%[[TRANSIENT_SUBVIEW]] as %[[CAPTURE:.+]]: !stream.resource<transient>{%c128{{(_[0-9]+)?}}}) {
    // CHECK:   stream.cmd.fill %[[C0_I32]], %[[CAPTURE]][%[[C0]] for %c128{{(_[0-9]+)?}}] : i32 -> !stream.resource<transient>{%c128{{(_[0-9]+)?}}}
    // CHECK: } => !stream.timepoint
...
util.func public @alloca_in_scf_for_size_outside(
  ...
  %final_tp = scf.for %iv = %lb to %ub step %step iter_args(%iter_tp = %input_timepoint) -> !stream.timepoint {
    ...
    // CHECK: %[[LOOP_SIZE_INNER:.+]] = arith.muli %[[ARG0_SIZE]], %c4{{(_[0-9]+)?}} : index
    ...
    // CHECK:   stream.cmd.fill %c0_i32, %[[CAPTURE]][%c0 for %[[LOOP_SIZE_INNER]]] : i32 -> !stream.resource<transient>{%[[LOOP_SIZE_INNER]]}
    // CHECK: } => !stream.timepoint
...
util.func public @scf_for_with_nested_if(
  ...
  %final_tp = scf.for %iv = %lb to %ub step %step iter_args(%iter_tp = %input_timepoint) -> !stream.timepoint {
    ...
    // CHECK: %[[MAX:.+]] = arith.maxui %[[MAX0]], %{{.+}} : index
    // CHECK: %[[PACK:.+]]:2 = stream.resource.pack slices({
    // CHECK-NEXT:   [0, 0] = %[[MAX]]
    // CHECK-NEXT: }) : index attributes {stream.experimental.transients}
    // CHECK: %[[C0_0:.+]] = arith.constant 0 : index
...
util.func public @scf_if_multiple_allocations_per_branch(
  ...
  // Conservative max computations interleaved with constant definitions.
  ...
  // CHECK: %[[SLOT1_MAX:.+]] = arith.maxui %[[SLOT1_MAX0]], %{{.+}}
  //
