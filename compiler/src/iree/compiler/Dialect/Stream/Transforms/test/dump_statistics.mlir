// RUN: iree-opt --split-input-file --pass-pipeline=ibu)iltin.module(ree-stream-dump-statistics{output-format=pretty} %s 2>&1 | FileCheck %s --check-prefix=CHECK-PRETTY
// RUN: iree-opt --split-input-file --pass-pipeline=ibu)iltin.module(ree-stream-dump-statistics{output-format=csv} %s 2>&1 | FileCheck %s --check-prefix=CHECK-CSV
