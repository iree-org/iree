...
// Test that bindings are not fused when the entry point does not represent
// them explicitly in the function signature.

// CHECK-LABEL: @noFuseBindingsEx
stream.executable private @noFuseBindingsEx {
  stream.executable.export public @dispatch
  builtin.module  {
    // CHECK: func.func @dispatch(
    // CHECK-SAME:           %[[OPERAND:[a-zA-Z0-9]+]]: index)
    func.func @dispatch(%operand : index) {
      // CHECK: util.optimization_barrier %[[OPERAND]] : index
      util.optimization_barrier %operand : index
      return
    }
  }
}
// CHECK: func.func @noFuseBindings(%[[OPERAND:.+]]: index)
func.func @noFuseBindings(%operand: index) {
  %c0 = arith.constant 0 : index
  %c1 = arith.constant 1 : index
  %c20 = arith.constant 20 : index
  %c40 = arith.constant 40 : index
  ...
  %result_timepoint = stream.cmd.execute
      ...
      with(%alloc0 as %capture0: !stream.resource<transient>{%c200},
           %alloc1 as %capture1: !stream.resource<transient>{%c200}) {
    // CHECK: stream.cmd.dispatch {{.+}}(%[[OPERAND]] : index)
    stream.cmd.dispatch @noFuseBindingsEx::@dispatch[%c1, %c1, %c1](%operand : index) {
      // CHECK-NEXT: rw %[[CAPTURE0]][%c40
      rw %capture0[%c40 for %c20] : !stream.resource<transient>{%c200},
      // CHECK-NEXT: rw %[[CAPTURE1]][%c80
      rw %capture1[%c80 for %c20] : !stream.resource<transient>{%c200}
    }
    // CHECK: stream.cmd.dispatch {{.+}}(%[[OPERAND]] : index)
    stream.cmd.dispatch @noFuseBindingsEx::@dispatch[%c1, %c1, %c1](%operand : index) {
      // CHECK-NEXT: rw %[[CAPTURE0]][%c120
      rw %capture0[%c120 for %c20] : !stream.resource<transient>{%c200},
      // CHECK-NEXT: ro %[[CAPTURE1]][%c160
      ro %capture1[%c160 for %c20] : !stream.resource<transient>{%c200}
    }
  } => !stream.timepoint
  return
