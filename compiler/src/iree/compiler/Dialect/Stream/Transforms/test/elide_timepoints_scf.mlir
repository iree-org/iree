...
// Tests multiple uses of scf.if result in the same block.
// Both uses should be able to fold without dominance issues.

// CHECK-LABEL: @scfIfMultipleUsesOfResult
util.func public @scfIfMultipleUsesOfResult(%cond: i1, %arg0: !stream.resource<external>) -> (!stream.resource<external>, !stream.resource<external>) {
  %c0 = arith.constant 0 : index
  %c0_i32 = arith.constant 0 : i32
  %c1_i32 = arith.constant 1 : i32
  %c4 = arith.constant 4 : index
  ...
  %if_result = scf.if %cond -> !stream.resource<external> {
    // CHECK: stream.async.execute await(%[[TP]])
  ...
  // Multiple uses of if_result - both should work without dominance violations.
  // The ScheduleExecution pass fuses the two clones into a single execute.
  // CHECK: %[[RESULTS:.+]]:2, %[[TP:.+]] = stream.async.execute
  // CHECK-SAME: with(%[[IF_RESULT]] as %{{.+}}: !stream.resource<external>
  // CHECK: %{{.+}} = stream.async.clone
  // CHECK: %{{.+}} = stream.async.clone
  // CHECK: stream.yield
  %use1 = stream.async.clone %if_result : !stream.resource<external>{%c4} -> !stream.resource<external>{%c4}
  %use2 = stream.async.clone %if_result : !stream.resource<external>{%c4} -> !stream.resource<external>{%c4}

  // CHECK: %{{.+}}:2 = stream.timepoint.await %[[TP]] => %[[RESULTS]]#0, %[[RESULTS]]#1
  util.return %use1, %use2 : !stream.resource<external>, !stream.resource<external>
}

// -----

// Tests that values defined in one branch cannot be used in another branch.

// CHECK-LABEL: @scfIfCrossBranchDominance
util.func public @scfIfCrossBranchDominance(%cond: i1, %arg0: !stream.resource<external>) -> !stream.resource<external> {
  %c0 = arith.constant 0 : index
  %c0_i32 = arith.constant 0 : i32
  %c4 = arith.constant 4 : index
  %c8 = arith.constant 8 : index
  ...
  // CHECK: %{{.+}} = scf.if
  %result = scf.if %cond -> !stream.resource<external> {
    // This branch produces a value that should NOT be substituted into the else branch.
    // CHECK: stream.async.execute await(%[[TP]])
    %slice = stream.async.slice %clone[%c0 to %c4] : !stream.resource<external>{%c8} -> !stream.resource<external>{%c4}
    scf.yield %slice : !stream.resource<external>
  } else {
    // This branch is independent and should not see values from the then branch.
    // CHECK: stream.async.execute
    %splat = stream.async.splat %c0_i32 : i32 -> !stream.resource<external>{%c4}
    scf.yield %splat : !stream.resource<external>
  }
...
// Tests nested regions with multiple timepoints at different nesting levels.
...
util.func public @scfIfMultipleTimepointsNested(%outer: i1, %inner: i1, %arg0: !stream.resource<external>) -> !stream.resource<external> {
  ...
  %outer_result = scf.if %outer -> !stream.resource<external> {
    ...
    %clone2 = stream.async.clone %clone1 : !stream.resource<external>{%c8} -> !stream.resource<external>{%c8}

    // CHECK: %{{.+}} = scf.if
    %inner_result = scf.if %inner -> !stream.resource<external> {
      // Uses clone2 from parent region.
      // CHECK: stream.async.execute await(%[[TP2]])
      %slice = stream.async.slice %clone2[%c0 to %c4] : !stream.resource<external>{%c8} -> !stream.resource<external>{%c4}
      scf.yield %slice : !stream.resource<external>
    } else {
      // CHECK: stream.async.execute
      %splat = stream.async.splat %c0_i32 : i32 -> !stream.resource<external>{%c4}
    ...
    %slice = stream.async.slice %clone1[%c0 to %c4] : !stream.resource<external>{%c8} -> !stream.resource<external>{%c4}
    scf.yield %slice : !stream.resource<external>
  }
...
// Tests that region results (yielded values) are handled correctly
// and not confused with block arguments.

// CHECK-LABEL: @scfIfRegionResultVsBlockArgument
util.func public @scfIfRegionResultVsBlockArgument(%cond: i1, %arg0: !stream.resource<external>) -> !stream.resource<external> {
  %c0 = arith.constant 0 : index
  %c0_i32 = arith.constant 0 : i32
  %c4 = arith.constant 4 : index
  %c8 = arith.constant 8 : index
  ...
  // CHECK: %[[IF_RESULT:.+]] = scf.if
  %if_result = scf.if %cond -> !stream.resource<external> {
    // This value is yielded and becomes the if result - it should NOT dominate uses inside this branch.
    // CHECK: stream.async.execute await(%[[TP]])
    %slice = stream.async.slice %clone[%c0 to %c4] : !stream.resource<external>{%c8} -> !stream.resource<external>{%c4}
    scf.yield %slice : !stream.resource<external>
  } else {
  ...
  // The if_result value dominates uses here (outside the scf.if).
  // CHECK: stream.async.execute
  // CHECK-SAME: with(%[[IF_RESULT]]
  %final = stream.async.clone %if_result : !stream.resource<external>{%c4} -> !stream.resource<external>{%c4}
...
// Tests scf.if result used directly as an operand to await.

// CHECK-LABEL: @scfIfResultAsAwaitOperand
util.func public @scfIfResultAsAwaitOperand(%cond: i1, %arg0: !stream.resource<external>) -> !stream.resource<external> {
  %c0 = arith.constant 0 : index
  %c0_i32 = arith.constant 0 : i32
  %c4 = arith.constant 4 : index
  %c8 = arith.constant 8 : index
  ...
  // scf.if produces a resource that will be used in execute.
  // CHECK: %[[IF_RESULT:.+]] = scf.if
  %if_result = scf.if %cond -> !stream.resource<external> {
    // CHECK: stream.async.execute await(%[[TP]])
    %slice = stream.async.slice %clone[%c0 to %c4] : !stream.resource<external>{%c8} -> !stream.resource<external>{%c4}
  ...
  // Use if_result directly in an execute - should not cause dominance violations.
  // CHECK: stream.async.execute
  // CHECK-SAME: with(%[[IF_RESULT]]
  %final = stream.async.clone %if_result : !stream.resource<external>{%c4} -> !stream.resource<external>{%c4}
...
// Tests scf.index_switch where the result is used in subsequent operations
// and dominance must be preserved.

// CHECK-LABEL: @scfIndexSwitchResultDependency
util.func public @scfIndexSwitchResultDependency(%selector: index, %arg0: !stream.resource<external>) -> !stream.resource<external> {
  %c0 = arith.constant 0 : index
  %c0_i32 = arith.constant 0 : i32
  %c1_i32 = arith.constant 1 : i32
  %c4 = arith.constant 4 : index
  ...
  // CHECK: %[[SWITCH_RESULT:.+]] = scf.index_switch
  %switch_result = scf.index_switch %selector -> !stream.resource<external>
  case 0 {
    // CHECK: stream.async.execute await(%[[TP]])
    %slice = stream.async.slice %clone[%c0 to %c4] : !stream.resource<external>{%c8} -> !stream.resource<external>{%c4}
    scf.yield %slice : !stream.resource<external>
  ...
  default {
    // CHECK: stream.async.execute
    %splat = stream.async.splat %c1_i32 : i32 -> !stream.resource<external>{%c4}
    scf.yield %splat : !stream.resource<external>
  }

  // Use switch_result in a subsequent async op - result must dominate this use.
  // CHECK: stream.async.execute
  // CHECK-SAME: with(%[[SWITCH_RESULT]]
  %final = stream.async.clone %switch_result : !stream.resource<external>{%c4} -> !stream.resource<external>{%c4}
