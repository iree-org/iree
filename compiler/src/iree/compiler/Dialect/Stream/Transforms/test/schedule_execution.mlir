// RUN: iree-opt --split-input-file --iree-stream-schedule-execution %s | FileCheck %s
...
util.func public @transferWithPinnedConsumer(%arg0: !hal.buffer_view, %arg1: !hal.fence) -> !hal.buffer_view {
  ...
  // CHECK-NEXT: %{{.+}} = stream.async.transfer %{{.+}} : !stream.resource<transient>{%c16} -> !stream.resource<external>{%c16}
  // CHECK-NEXT: stream.yield %{{.+}} : !stream.resource<external>{%c16}
  // CHECK-NEXT: } => !stream.timepoint
  %transfer_to_a = stream.async.transfer %dispatch_b_result : !stream.resource<transient>{%c16} from(#hal.device.affinity<@device_b>) -> to(#hal.device.affinity<@device_a>) !stream.resource<external>{%c16}
  %result, %result_timepoint = stream.timepoint.barrier on(#hal.device.affinity<@device_a>) %transfer_to_a : !stream.resource<external>{%c16} => !stream.timepoint
  stream.timepoint.chain_external on(#hal.device.affinity<@device_a>) %result_timepoint => (%arg1 : !hal.fence)
  // CHECK: %{{.+}} = stream.tensor.export on(#hal.device.affinity<@device_a>) %{{.+}}
  %export_result = stream.tensor.export on(#hal.device.affinity<@device_a>) %result : tensor<4xf32> in !stream.resource<external>{%c16} -> !hal.buffer_view
  util.return %export_result : !hal.buffer_view
}
