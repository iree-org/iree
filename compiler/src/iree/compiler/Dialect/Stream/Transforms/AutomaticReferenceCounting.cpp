...
struct LastUseSet {
  ...
  void produce(Value resource, Value fromTimepoint) {
    ...
    assert(!map.contains(baseResource) && "produced values must be unique");
    assert(fromTimepoint);
    map[baseResource].insert(fromTimepoint);
    baseResourceOrder.push_back(baseResource);
  }
...
static void processFuncOp(FunctionOpInterface funcOp) {
  ...
  for (auto [blockIndex, block] : llvm::enumerate(funcOp.getBlocks())) {
    ...
    for (auto &op : block) {
      ...
      // Bail on any resource which has lifetime management ops. They should not
      // have been inserted yet and their presence likely indicates we've
      // already run the pass on this input.
