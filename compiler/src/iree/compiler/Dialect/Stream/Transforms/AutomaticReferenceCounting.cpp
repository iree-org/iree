    ...
    if (auto yieldOp = dyn_cast<scf::YieldOp>(terminator)) {
      for (Value operand : yieldOp.getOperands()) {
        if (isa<IREE::Stream::ResourceType>(operand.getType())) {
          ...
          // Consume in parent LastUseSet to track its lifetime properly.
          // The resource's lifetime extends to the loop's result timepoint.
          lastUseSet.consume(operand, loopResultTimepoint);
          LLVM_DEBUG({
            llvm::dbgs() << "[arc]   yielded resource lifetime extended to "
                            "loop result\n";
          });
        }
      }
      ...
      if (auto yieldOp = dyn_cast<scf::YieldOp>(terminator)) {
        for (Value operand : yieldOp.getOperands()) {
          if (isa<IREE::Stream::ResourceType>(operand.getType())) {
            ...
            // Consume in parent LastUseSet to track its lifetime properly.
            // The resource's lifetime extends to the if operation's result
            // timepoint.
            lastUseSet.consume(operand, ifResultTp);
            LLVM_DEBUG({
              llvm::dbgs() << "[arc]   yielded resource lifetime extended to "
                              "if result\n";
            });
...
static bool analyzeWhileOp(scf::WhileOp whileOp, AsmState *asmState,
  ...
  auto analyzeRegion = [&](Region &region) -> bool {
    for (Block &block : region) {
      ...
      // - scf.condition in "before" region
      // - scf.yield in "after" region
      auto *terminator = block.getTerminator();
      if (auto conditionOp = dyn_cast<scf::ConditionOp>(terminator)) {
        ...
        for (Value operand : conditionOp.getArgs()) {
          if (isa<IREE::Stream::ResourceType>(operand.getType())) {
            handledResources.insert(operand);
            lastUseSet.consume(operand, whileResultTimepoint);
            LLVM_DEBUG({
              llvm::dbgs() << "[arc]   resource yielded via scf.condition; "
        ...
        for (Value operand : yieldOp.getOperands()) {
          if (isa<IREE::Stream::ResourceType>(operand.getType())) {
            ...
            lastUseSet.consume(operand, whileResultTimepoint);
            LLVM_DEBUG({
              llvm::dbgs() << "[arc]   resource yielded from while body; "
                              "preventing local deallocation\n";
            });
          }
        }
