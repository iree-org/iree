...
static bool isSafeToElideUpdateOp(IREE::Stream::AsyncUpdateOp updateOp,
  ...
  // Check if target is a block argument with reference semantics.
  // If passed by-reference, mutation might be observable to caller.
  Value target = updateOp.getTarget();
  if (auto arg = llvm::dyn_cast<BlockArgument>(target)) {
    if (!analysis.isArgMoved(arg)) {
      LLVM_DEBUG(llvm::dbgs()
                 << "  - update target is a by-ref arg; cannot elide\n");
      return false;
    }
    LLVM_DEBUG(llvm::dbgs()
               << "  ? update target is a by-value arg; may elide\n");
  }
  ...
  for (auto user : result.getUsers()) {
    ...
    // TODO(benvanik): use transitive analysis to track reads through chains.
