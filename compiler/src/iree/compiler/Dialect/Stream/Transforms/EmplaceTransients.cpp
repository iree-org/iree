...
gatherTransientResources(FunctionOpInterface funcOp, Explorer &explorer,
  ...
  funcOp.walk([&](IREE::Stream::ResourceTransientsOp op) {
    ...
    if (!uniqueStorage) {
      ...
      LLVM_DEBUG({
        llvm::dbgs() << "[emplace-transients] found storage buffer: ";
    ...
    LLVM_DEBUG({
      llvm::dbgs() << "[emplace-transients] found transients op: ";
  ...
  LLVM_DEBUG(llvm::dbgs() << "[emplace-transients] found "
                          << transientsOps.size() << " transients op(s)\n");

  LLVM_DEBUG(
      llvm::dbgs() << "[emplace-transients] starting timeline traversal\n");
  ...
  for (auto transientsOp : transientsOps) {
    if (auto awaitTp = transientsOp.getAwaitTimepoint()) {
      if (visitedTimepoints.insert(awaitTp).second) {
        ...
        LLVM_DEBUG({
          llvm::dbgs() << "[emplace-transients] seeding worklist with: ";
  ...
  while (!worklist.empty()) {
    ...
    LLVM_DEBUG({
      llvm::dbgs() << "[emplace-transients] processing timepoint: ";
    ...
    auto result = explorer.walkDefiningOps(
        ...
        [&](OpResult definingResult) -> WalkResult {
          ...
          LLVM_DEBUG({
            llvm::dbgs() << "[emplace-transients]   defining op: ";
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients] timeline traversal complete: found "
                 << resource.allocaOps.size() << " alloca(s) and "
                 << resource.deallocaOps.size() << " dealloca(s)\n";
    for (auto allocaOp : resource.allocaOps) {
      llvm::dbgs() << "[emplace-transients]   alloca: ";
      allocaOp->print(llvm::dbgs(), explorer.getAsmState());
      llvm::dbgs() << "\n";
    }
    for (auto deallocaOp : resource.deallocaOps) {
      llvm::dbgs() << "[emplace-transients]   dealloca: ";
...
static Operation *findDominantInsertionPoint(Operation *startOp,
  ...
  while (parentOp && !isa<FunctionOpInterface>(parentOp)) {
    ...
    if (isa<RegionBranchOpInterface>(parentOp)) {
      ...
      LLVM_DEBUG({
        llvm::dbgs() << "[emplace-transients] op is nested in region branch "
                        "op, inserting before: ";
...
// Finds the dominant alloca and insertion point for a set of allocas.
// Returns a tuple of (insertionPointOp, dominantAllocaOp, hasMultipleBranches).
// If allocas are in mutually exclusive branches, hasMultipleBranches is true
// and the insertion point is the common dominator block's first operation.
static std::tuple<Operation *, IREE::Stream::ResourceAllocaOp, bool>
findDominantAllocaAndInsertionPoint(
    DominanceInfo &domInfo, ArrayRef<IREE::Stream::ResourceAllocaOp> allocaOps,
    FunctionOpInterface funcOp, Explorer &explorer) {
  IREE::Stream::ResourceAllocaOp dominantAllocaOp;
  Operation *insertionPointOp = nullptr;
  bool hasMultipleBranches = false;

  // First, try to find an alloca that dominates all others.
  for (auto allocaOp : allocaOps) {
    bool dominatesAll = true;
    for (auto otherAllocaOp : allocaOps) {
      if (allocaOp == otherAllocaOp) {
        continue;
      }
      if (!domInfo.dominates(allocaOp.getOperation(),
                             otherAllocaOp.getOperation())) {
        dominatesAll = false;
        break;
      }
    }
    if (dominatesAll) {
      dominantAllocaOp = allocaOp;
      insertionPointOp = dominantAllocaOp.getOperation();
      LLVM_DEBUG({
        llvm::dbgs() << "[emplace-transients] found dominant alloca: ";
        dominantAllocaOp->print(llvm::dbgs(), explorer.getAsmState());
        llvm::dbgs() << "\n";
      });
      return std::make_tuple(insertionPointOp, dominantAllocaOp,
                             hasMultipleBranches);
    }
  }

  // If no single alloca dominates all, find the common dominator.
  // This handles cases like allocas in different branches of scf.if.
  ...
  for (size_t i = 1; i < allocaOpPtrs.size(); ++i) {
    ...
    if (!commonBlock) {
      // This should never happen - all operations in a function have a common
      // dominator.
      funcOp.emitError(
          "cannot find common dominator for allocas - internal error");
      return std::make_tuple(nullptr, IREE::Stream::ResourceAllocaOp(), false);
    }
  }

  // Use the first operation in the common dominator block as insertion point.
  // This ensures we dominate all allocas in the function.
  insertionPointOp = &commonBlock->front();

  // Use the first alloca for validation purposes (arbitrary choice).
  // However, skip validation since allocas are in mutually exclusive branches.
  dominantAllocaOp = allocaOps.front();
  hasMultipleBranches = true;

  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients] no single dominant alloca, using "
                    "common dominator: ";
...
static LogicalResult computeSizeSliceInRegion(Value sizeValue, Region *region,
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients] computeSizeSliceInRegion for value: ";
    sizeValue.printAsOperand(llvm::dbgs(), explorer.getAsmState());
    llvm::dbgs() << "\n";
    llvm::dbgs() << "[emplace-transients]   definingOp parentRegion: "
                 << definingRegion << "\n";
    llvm::dbgs() << "[emplace-transients]   target region: " << region << "\n";
    llvm::dbgs() << "[emplace-transients]   same? "
  ...
  if (definingRegion == region) {
    ...
    LLVM_DEBUG(llvm::dbgs() << "[emplace-transients]   defined in ancestor "
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients]   backward slice found "
                 << sliceOps.size() << " op(s) for size value: ";
    sizeValue.printAsOperand(llvm::dbgs(), explorer.getAsmState());
    llvm::dbgs() << "\n";
    if (auto defOp = sizeValue.getDefiningOp()) {
      llvm::dbgs() << "[emplace-transients]   defined by: ";
  ...
  // Topologically sort the slice.
  mlir::topologicalSort(sliceOps);
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients] computed slice with "
                 << slice.opsToClone.size() << " op(s) and "
                 << slice.capturedValues.size() << " captured value(s)\n";
    for (Operation *op : slice.opsToClone) {
      llvm::dbgs() << "[emplace-transients]   op: ";
      op->print(llvm::dbgs(), explorer.getAsmState());
      llvm::dbgs() << "\n";
    }
    for (Value captured : slice.capturedValues) {
      llvm::dbgs() << "[emplace-transients]   captured: ";
...
static Value computeConservativeMax(OpBuilder &builder, Location loc,
  ...
  for (auto [i, potentialSize] : llvm::enumerate(potentialSizes)) {
    currentMax = builder.create<arith::MaxUIOp>(loc, currentMax, potentialSize);
  }
...
static DenseMap<Value, Value> cloneSliceOps(ArrayRef<Operation *> opsToClone,
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients] IRMapping has "
                 << capturedValues.size() << " captured value(s)\n";
    for (Value captured : capturedValues) {
      llvm::dbgs() << "[emplace-transients]   mapped: ";
      captured.printAsOperand(llvm::dbgs(), explorer.getAsmState());
      llvm::dbgs() << " -> ";
  ...
  for (Operation *op : opsToClone) {
    LLVM_DEBUG({
      llvm::dbgs() << "[emplace-transients] cloning op: ";
      op->print(llvm::dbgs(), explorer.getAsmState());
      llvm::dbgs() << "\n";
    });
...
hoistSizesConservatively(ArrayRef<SizeComputationSlice> slices,
  ...
  LLVM_DEBUG(
      llvm::dbgs() << "[emplace-transients] hoisting sizes conservatively for "
                   << slices.size() << " slice(s)\n");
  ...
  if (latestCapturedDefOp) {
    ...
    LLVM_DEBUG({
      llvm::dbgs() << "[emplace-transients]   setting insertion point after: ";
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients]   deduped: " << allOpsToClone.size()
                 << " unique op(s) to clone, " << allCapturedValues.size()
                 << " unique captured value(s)\n";
    for (Operation *op : allOpsToClone) {
      llvm::dbgs() << "[emplace-transients]     op to clone: ";
      op->print(llvm::dbgs(), explorer.getAsmState());
      llvm::dbgs() << "\n";
    }
    for (Value val : allCapturedValues) {
      llvm::dbgs() << "[emplace-transients]     captured value: ";
  ...
  for (const auto &slice : slices) {
    LLVM_DEBUG({
      llvm::dbgs() << "[emplace-transients]   root size value: ";
    ...
    if (valueMap.contains(slice.rootSizeValue)) {
      ...
      LLVM_DEBUG({
        llvm::dbgs() << "[emplace-transients]   using cloned value: ";
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients]   conservative max: ";
...
computeAllocationSlots(ArrayRef<IREE::Stream::ResourceAllocaOp> allocaOps,
  ...
  LLVM_DEBUG(
      llvm::dbgs() << "[emplace-transients] computing allocation slots for "
  ...
  for (auto [region, allocations] : allocationsByRegion) {
    ...
    if (auto branchOp = dyn_cast<RegionBranchOpInterface>(parentOp)) {
      ...
      if (siblingRegions.size() > 1) {
        ...
        LLVM_DEBUG({
          llvm::dbgs() << "[emplace-transients]   found mutually exclusive "
                          "group with "
  ...
  for (auto &exclusiveGroup : exclusiveRegionGroups) {
    ...
    LLVM_DEBUG({
      llvm::dbgs() << "[emplace-transients]   exclusive group needs "
                   << maxAllocationsInRegion << " slot(s)\n";
    });
    ...
    for (size_t position = 0; position < maxAllocationsInRegion; ++position) {
      ...
      for (Region *region : exclusiveGroup) {
        ...
        if (position < allocations.size()) {
          ...
          LLVM_DEBUG({
            llvm::dbgs() << "[emplace-transients]     assigning slot " << slotId
                         << " to alloca at position " << position
                         << " in region: ";
  ...
  for (auto [region, allocations] : allocationsByRegion) {
    ...
    for (auto allocaOp : allocations) {
      ...
      LLVM_DEBUG({
        llvm::dbgs() << "[emplace-transients]   assigning unique slot "
                     << slotId << " to non-exclusive alloca: ";
        allocaOp->print(llvm::dbgs(), explorer.getAsmState());
        llvm::dbgs() << "\n";
      });
    }
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients] computed " << slots.size()
                 << " slot(s) for " << allocaOps.size() << " allocation(s)\n";
    for (auto &slot : slots) {
      llvm::dbgs() << "[emplace-transients]   slot " << slot.slotId
                   << " contains " << slot.allocations.size()
                   << " allocation(s)\n";
    }
  });
...
static FailureOr<Value> computeConservativeSizeForSlot(
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients] computing conservative size for slot "
                    "with "
...
createPackedAllocation(OpBuilder &builder, Location loc,
  ...
  // being used for external transient allocation.
...
static void replaceAllocasWithSubviews(
  ...
  for (auto allocaOp : allocaOps) {
    ...
    auto subviewOp = IREE::Stream::ResourceSubviewOp::create(
        ...
        size);
    LLVM_DEBUG({
      llvm::dbgs() << "[emplace-transients] replacing alloca with subview: ";
    ...
    if (auto awaitTimepoint = allocaOp.getAwaitTimepoint()) {
      ...
      auto immediateTp =
          builder.create<IREE::Stream::TimepointImmediateOp>(allocaOp.getLoc());
...
hoistSizeComputations(FunctionOpInterface funcOp,
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients] hoisting size computations to: ";
    insertionPoint->print(llvm::dbgs(), explorer.getAsmState());
    llvm::dbgs() << "\n";
  });
  ...
  for (auto allocaOp : allocaOps) {
    ...
    LLVM_DEBUG({
      llvm::dbgs() << "[emplace-transients]   size value: ";
  ...
  if (sizeOps.empty()) {
    LLVM_DEBUG(llvm::dbgs() << "[emplace-transients] all sizes are block args, "
                               "no hoisting needed\n");
  ...
  LLVM_DEBUG(
      llvm::dbgs() << "[emplace-transients] calling moveOperandDefs with "
                   << sizeOps.size() << " size op(s)\n");
  ...
  if (!skipValidation) {
    ...
    if (insertionPoint == dominantAllocaOpPtr) {
      ...
      while (currentOp && currentOp != dominantAllocaOpPtr) {
        LLVM_DEBUG({
          llvm::dbgs() << "[emplace-transients]   checking op: ";
    ...
    LLVM_DEBUG(llvm::dbgs() << "[emplace-transients] skipping validation "
...
static LogicalResult computeConservativeSizesForSlots(
  ...
  LLVM_DEBUG(llvm::dbgs() << "[emplace-transients] computing conservative "
                             "sizes for "
                          << allocaOps.size() << " allocation(s)\n");
  ...
  for (auto allocaOp : allocaOps) {
    ...
    LLVM_DEBUG({
      llvm::dbgs() << "[emplace-transients] processing alloca: ";
      allocaOp->print(llvm::dbgs(), explorer.getAsmState());
      llvm::dbgs() << "\n";
      llvm::dbgs() << "[emplace-transients]   alloca parentRegion: "
                   << allocaRegion << "\n";
      llvm::dbgs() << "[emplace-transients]   size value: ";
      sizeValue.printAsOperand(llvm::dbgs(), explorer.getAsmState());
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients] using conservative max for all "
                 << allocaOps.size() << " allocation(s): ";
    conservativeMax.printAsOperand(llvm::dbgs(), explorer.getAsmState());
    llvm::dbgs() << "\n";
  });
...
processTransientResource(FunctionOpInterface funcOp,
  ...
  if (transientResource.allocaOps.empty()) {
    ...
    // annotation).
    LLVM_DEBUG(llvm::dbgs() << "[emplace-transients] no allocations found, "
                               "nothing to pack\n");
    return success();
  }

  // Find the dominant insertion point for size computations.
  // This handles both simple cases (single dominant alloca) and complex cases
  // (allocations in mutually exclusive branches).
  DominanceInfo domInfo(funcOp);
  auto [insertionPointOp, dominantAllocaOp, hasMultipleBranches] =
      findDominantAllocaAndInsertionPoint(domInfo, transientResource.allocaOps,
                                          funcOp, explorer);
  ...
  // We create the pack after the conservative max computation, ensuring all
  // sizes dominate the pack operation.
  OpBuilder builder(funcOp.getContext());

  // Set insertion point after the latest slot size's defining operation.
  Operation *latestDefiningOp = nullptr;
  for (auto &slot : slots) {
  ...
  if (latestDefiningOp) {
    ...
    // Fallback: all sizes are block arguments, use original insertion point.
    builder.setInsertionPoint(insertionPointOp);
  }

  auto packOp = createPackedAllocation(builder, insertionPointOp->getLoc(),
  ...
  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients] created pack operation: ";
    packOp->print(llvm::dbgs(), explorer.getAsmState());
    llvm::dbgs() << "\n";
    llvm::dbgs() << "[emplace-transients] total packed size: ";
  ...
  // transients.
  builder.setInsertionPointAfter(packOp);
  Value zero = builder.create<arith::ConstantIndexOp>(packOp.getLoc(), 0);
  auto storageSubviewOp = IREE::Stream::ResourceSubviewOp::create(
      builder, packOp.getLoc(), storageResource, storageSize, zero,
      packTotalSize);
  Value backingResource = storageSubviewOp.getResult();
  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients] created storage subview: ";
    storageSubviewOp->print(llvm::dbgs(), explorer.getAsmState());
    llvm::dbgs() << "\n";
  });

  // Build mapping from allocations to their slot offsets.
  ...
  for (auto &slot : slots) {
    ...
    for (auto allocaOp : slot.allocations) {
      ...
      // All allocations in a slot use the slot's conservative size.
      // This is the hoisted max size that dominates all uses.
      allocaToSize[allocaOp] = slot.conservativeSize;
    }
  }

  // Replace each alloca with a subview at its slot's offset.
  // All subviews are created from the storage subview (backingResource).
  replaceAllocasWithSubviews(builder, transientResource.allocaOps,
                             backingResource, packTotalSize, allocaToOffset,
                             allocaToSize, explorer);
  LLVM_DEBUG(llvm::dbgs() << "[emplace-transients] replaced "
                          << transientResource.allocaOps.size()
  ...
  for (auto deallocaOp : transientResource.deallocaOps) {
    ...
    LLVM_DEBUG({
      llvm::dbgs() << "[emplace-transients] removing dealloca: ";
      deallocaOp->print(llvm::dbgs(), explorer.getAsmState());
      llvm::dbgs() << "\n";
    });
    deallocaOp.erase();
  }
  LLVM_DEBUG(llvm::dbgs() << "[emplace-transients] removed "
                          << transientResource.deallocaOps.size()
                          << " dealloca op(s)\n");
  ...
  for (auto transientsOp : transientResource.transientsOps) {
    ...
    if (auto awaitTimepoint = transientsOp.getAwaitTimepoint()) {
      ...
      // If no await timepoint, create an immediate timepoint.
      auto immediateTp = builder.create<IREE::Stream::TimepointImmediateOp>(
          transientsOp.getLoc());
    ...
    LLVM_DEBUG({
      llvm::dbgs() << "[emplace-transients] removing transients op: ";
  ...
  LLVM_DEBUG(llvm::dbgs() << "[emplace-transients] removed "
                          << transientResource.transientsOps.size()
                          << " transients op(s)\n");
...
static LogicalResult emplaceTransientsInFuncOp(FunctionOpInterface funcOp,
                                               Explorer &explorer) {
  LLVM_DEBUG({
    llvm::dbgs() << "[emplace-transients] processing function: ";
  ...
  funcOp.walk([&](CallOpInterface callOp) {
    LLVM_DEBUG({
      llvm::dbgs() << "[emplace-transients] found unsupported call op: ";
