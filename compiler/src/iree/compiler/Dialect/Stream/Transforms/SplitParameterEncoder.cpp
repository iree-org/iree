...
// Configuration for encoding expression analysis.
...
static std::optional<uint64_t> tryEvaluateConstantSize(Value sizeValue) {
  ...
  if (auto forOp = sizeValue.getDefiningOp<scf::ForOp>()) {
    ...
    for (unsigned i = 0; i < forOp.getNumResults(); ++i) {
      if (forOp.getResult(i) == sizeValue) {
        resultIdx = i;
        break;
      }
    }
    ...
    Value yieldedValue = yieldOp.getOperand(resultIdx);
...
static EncodingExprSet gatherEncodingExprSet(mlir::ModuleOp moduleOp,
  ...
  for (auto storeOp : constantStoreOps) {
    ...
    if (needsSort) {
      SmallVector<Operation *> opsVec(slice.begin(), slice.end());
      llvm::sort(opsVec, [](Operation *a, Operation *b) {
        // Topological sort: a comes before b if b transitively uses a.
        // Simple dominance-based check: if a dominates b, a comes first.
        return a->isBeforeInBlock(b);
      });
      slice.clear();
      slice.insert(opsVec.begin(), opsVec.end());
    }
...
// An entry in the parameter index populated by an EncodingExpr.
// Note that a single EncodingExpr may populated more than one entry.
...
// Targets may be specializing for multiple devices simultaneously if the
// configuration is for heterogeneous execution and may produce multiple
// parameter indices.
//
// TODO: add information required to detect this target. For now we only support
// a single target.
struct TargetPlan {
  ...
  void appendExpr(const EncodingExpr *expr,
    ...
    step.globalByteLength = std::accumulate(
        expr->outputs.begin(), expr->outputs.end(), 0,
...
static void replaceEncodedExprs(ArrayRef<TargetPlan> targetPlans) {
  ...
  for (auto &step : targetPlan.steps) {
    for (auto &output : step.expr->outputs) {
      ...
      // subrange (if any).
      // TODO(benvanik): properly thread await_timepoint from the cloned
      // operations (AsyncConstantOp, AsyncParameterLoadOp, or
      // AsyncParameterGatherOp). There may be multiple operations in a single
      // expression. Need helper to switch/handle each.
      ...
      Value constantValue = IREE::Stream::AsyncConstantOp::create(
          ...
          /*await_timepoint=*/Value(), parameterAttr, storageSizeValue,
