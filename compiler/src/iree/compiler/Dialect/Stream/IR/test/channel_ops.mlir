...
func.func @channel_create(%rank: index) {
  // CHECK: %channel = stream.channel.create on(#hal.affinity.queue<[0, 1]>) rank(%[[RANK]]) : !stream.channel
  %channel = stream.channel.create on(#hal.affinity.queue<[0, 1]>) rank(%rank) : !stream.channel
  return
}
