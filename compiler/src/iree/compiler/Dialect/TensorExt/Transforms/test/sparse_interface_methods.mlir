// RUN: iree-opt --pass-pipeline='builtin.module(func.func(iree-tensor-ext-test-sparse-op-interface-methods{test-lower-loop-range=true}, cse))' %s --split-input-file --allow-unregistered-dialect --mlir-print-local-scope | FileCheck %s --check-prefixes=LOWER-LOOP-RANGE,CHECK-ALL
// RUN: iree-opt --pass-pipeline='builtin.module(func.func(iree-tensor-ext-test-sparse-op-interface-methods{test-get-estimated-loop-range=true}, cse))' %s --split-input-file --allow-unregistered-dialect --mlir-print-local-scope | FileCheck %s --check-prefixes=GET-ESTIMATED-LOOP-RANGE,CHECK-ALL
...
func.func public @simpleTest(%source : memref<?x?xf32>, %column_lengths: memref<?xi32>,
  ...
  %0 = iree_tensor_ext.cast_to_ragged_shape %source
      ...
      -> memref<?x?x?xf32, #iree_tensor_ext.ragged_tensor<0>>
  %d0 = memref.dim %0, %c0 : memref<?x?x?xf32, #iree_tensor_ext.ragged_tensor<0>>
  %d1 = memref.dim %0, %c1 : memref<?x?x?xf32, #iree_tensor_ext.ragged_tensor<0>>
...
// LOWER-LOOP-RANGE:       "some_op"(%[[IV0]], %[[IV1]]) : (index, index) -> ()
...
// GET-ESTIMATED-LOOP-RANGE:   "some_op"(%[[IV0]], %[[IV1]]) : (index, index) -> ()
