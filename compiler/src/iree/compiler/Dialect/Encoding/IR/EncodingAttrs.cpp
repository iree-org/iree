...
static FailureOr<ArrayAttr> parseDynamicI64ArrayAttr(AsmParser &p) {
  ...
  if (failed(p.parseCommaSeparatedList([&] {
        ...
        integerVals.push_back(IntegerAttr::get(
            IntegerType::get(p.getContext(), 64), APInt(64, value)));
        return success();
      }))) {
    return failure();
  }
...
static void printDynamicI64ArrayAttr(AsmPrinter &p, ArrayAttr attrs) {
  SmallVector<int64_t> integerVals =
      llvm::map_to_vector(attrs, [](Attribute attr) {
        return llvm::cast<IntegerAttr>(attr).getInt();
      });
  p << "[";
  for (auto &&[idx, value] : llvm::enumerate(integerVals)) {
    if (ShapedType::isDynamic(value)) {
      p << "?";
    } else {
      p << value;
    }
    if (idx < (integerVals.size() - 1)) {
      p << ", ";
    }
  }
  p << "]";
...
EncodingAttr::verify(function_ref<mlir::InFlightDiagnostic()> emitError,
  ...
  if (iterationSizesAttr) {
    EncodingOpType opType = opTypeAttr.getValue();
    if (opType == EncodingOpType::matmul && iterationSizesAttr.size() != 3) {
      return emitError() << "found a matmul encoding with "
                         << iterationSizesAttr.size()
                         << " iteration sizes, but expected 3";
    }
  }
  return success();
}
