...
func.func @hoistable_conversion_eager(%arg0 : vector<4xf32>) -> vector<2x2xf32> {
  ...
  // CHECK-SAME: (%[[B:.*]]: vector<4xf32> = %[[ARG0]]) : (vector<4xf32>) -> vector<2x2xf32>
  // CHECK:   vector.shape_cast %[[B]] : vector<4xf32> to vector<2x2xf32>
  %0 = util.hoistable_conversion "shape_cast_from_intrinsic" inverts("shape_cast_to_intrinsic") (%b: vector<4xf32> = %arg0) : (vector<4xf32>) -> vector<2x2xf32> {
...
func.func @hoistable_conversion_multi_io(%a : vector<4xf32>, %b : vector<8xf16>) -> (vector<2x2xf32>, vector<4x2xf16>) {
  ...
  // CHECK-SAME: (%[[BA:.*]]: vector<4xf32> = %[[A]], %[[BB:.*]]: vector<8xf16> = %[[B]])
  // CHECK-SAME: : (vector<4xf32>, vector<8xf16>) -> (vector<2x2xf32>, vector<4x2xf16>)
  %0:2 = util.hoistable_conversion "distribute" inverts("reassemble") (%ba: vector<4xf32> = %a, %bb: vector<8xf16> = %b) : (vector<4xf32>, vector<8xf16>) -> (vector<2x2xf32>, vector<4x2xf16>) {
...
func.func @hoistable_conversion_rdna3_interleave(%acc : vector<8xf16>) -> vector<16xf16> {
  ...
  // CHECK-SAME: (%[[B:.*]]: vector<8xf16> = %[[ACC]]) : (vector<8xf16>) -> vector<16xf16>
  %0 = util.hoistable_conversion "rdna3_interleave_acc" inverts("rdna3_deinterleave_acc") (%b: vector<8xf16> = %acc) : (vector<8xf16>) -> vector<16xf16> {
