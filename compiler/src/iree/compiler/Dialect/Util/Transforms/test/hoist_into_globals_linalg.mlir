...
module @compute_hoisted {
  ...
  func.func @main() -> (tensor<5x6xf32>) {
    ...
    %1 = linalg.generic {indexing_maps = [#map0, #map1], iterator_types = [#linalg.iterator_type<parallel>, #linalg.iterator_type<parallel>]} ins(%cst_0 : tensor<f32>) outs(%0 : tensor<5x6xf32>) {
    ...
    %3 = linalg.generic {indexing_maps = [#map1, #map1, #map1], iterator_types = [#linalg.iterator_type<parallel>, #linalg.iterator_type<parallel>]} ins(%1, %1 : tensor<5x6xf32>, tensor<5x6xf32>) outs(%2 : tensor<5x6xf32>) {
