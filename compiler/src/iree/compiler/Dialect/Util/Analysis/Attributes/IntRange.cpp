...
void IntegerMinValueElement::initializeValue(Value value, DFX::Solver &solver) {
...
void IntegerMaxValueElement::initializeValue(Value value, DFX::Solver &solver) {
  if (!isIntegerType(value.getType())) {
    indicatePessimisticFixpoint();
    return;
  }
...
const std::string IntegerMaxDivisorElement::getAsStr(AsmState &asmState) const {
