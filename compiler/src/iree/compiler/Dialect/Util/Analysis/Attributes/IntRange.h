...
class ValueGcd : public DFX::StateWrapper<GcdStateType, DFX::ValueElement> {
public:
  using BaseType = DFX::StateWrapper<GcdStateType, DFX::ValueElement>;

  static ValueGcd &createForPosition(const Position &pos, DFX::Solver &solver) {
    return *(new (solver.getAllocator()) ValueGcd(pos));
  }

  const std::string getName() const override { return "ValueGcd"; }
...
private:
  explicit ValueGcd(const Position &pos) : BaseType(pos) {}
...
// public:
//   using BaseType = DFX::StateWrapper<IntegerMaxDivisorState,
//   DFX::ValueElement>; using BaseType::BaseType;
