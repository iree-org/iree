...
/// Patterns to fold (fill)+ -> concat to a fill of the concat result shape.
struct FoldConcatsOfFill : public OpRewritePattern<tensor::ConcatOp> {
  ...
  LogicalResult matchAndRewrite(tensor::ConcatOp concatOp,
    ...
    auto isDefinedByCompatibleFillOp = [&](Value v) -> bool {
      ...
      return fillOp.getDpsInputOperand(0)->get() ==
             firstFillOp.getDpsInputOperand(0)->get();
    };
    ...
    auto empty = rewriter.create<tensor::EmptyOp>(
        concatOp.getLoc(), resultShape[0],
        concatOp.getResultType().getElementType());
    rewriter.replaceOpWithNewOp<linalg::FillOp>(
        concatOp, firstFillOp.getDpsInputOperand(0)->get(), empty.getResult());
    return success();
  }
};

/// Patterns to fold (empty)+ -> concat to a fill of the concat result shape.
struct FoldConcatsOfEmpty : public OpRewritePattern<tensor::ConcatOp> {
  using OpRewritePattern::OpRewritePattern;
...
void FuseHorizontalContractionsPass::runOnOperation() {
...

  {
    ...
    foldPatterns.insert<FoldConcatsOfEmpty, FoldConcatsOfFill>(context);
    if (failed(applyPatternsAndFoldGreedily(getOperation(),
