...
static FailureOr<unsigned> fuseMultiUseProducers(Operation *funcOp,
  ...
  // Cleanup these duplicate input operands to avoid a consumer using a producer
  // many times in the input operands, which blocks the further fusion (see the
  // condition check in getFusableUse).
  linalg::populateEraseUnnecessaryInputsPatterns(fusionPatterns);
...
struct FusionOfTensorOpsPass
  ...
  void runOnOperation() override {
...

    {
      ...
      // Duplicate input operands are generated when fusing two ops who take
      // the output from the same producer. Cleanup these duplicate input
      // operands to reduce the multi-use producers and increases the
    ...
    // Run some patterns that fold away and cleanup a few operations.
    {
      RewritePatternSet cleanupPatterns(&getContext());
      tensor::populateFoldTensorEmptyPatterns(cleanupPatterns);
      // Remove duplicate input operands generated by collapsing.
      linalg::populateEraseUnnecessaryInputsPatterns(cleanupPatterns);
      if (failed(applyPatternsAndFoldGreedily(funcOp,
                                              std::move(cleanupPatterns)))) {
        funcOp->emitError("failed to apply cleanup patterns");
        return signalPassFailure();
      }
    }
