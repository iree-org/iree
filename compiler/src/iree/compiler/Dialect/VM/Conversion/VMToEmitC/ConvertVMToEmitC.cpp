...
public:
  ImportOpConverter(IREE::VM::EmitCTypeConverter &typeConverter,
                    SmallVector<std::string> &importShims, Operation *scopeOp)
      : typeConverter(typeConverter), importShims(importShims),
        dataLayout(DataLayout::closest(scopeOp)) {}
...
private:
  ...
  size_t getTypeAlignment(Type type) const {
    if (auto opaqueType = dyn_cast<emitc::OpaqueType>(type)) {
      if (opaqueType.getValue() == "iree_vm_ref_t") {
        ...
        return static_cast<size_t>(dataLayout.getTypeABIAlignment(indexType));
      }
      // Opaque C types do not carry MLIR data layout information. Import shims
      // only use this path for pointer-like VM ABI values.
      auto indexType = IndexType::get(type.getContext());
      return static_cast<size_t>(dataLayout.getTypeABIAlignment(indexType));
    }
    // Import shims materialize VM ABI buffers as C structs. Use the preferred
    // alignment for scalar C fields here so the generated byte packing follows
    // the target C layout even when the MLIR ABI alignment is lower than the C
    // object alignment used by the runtime structs.
    return static_cast<size_t>(dataLayout.getTypePreferredAlignment(type));
  }

  Value alignSizeForType(OpBuilder &builder, Location loc, Value size,
  ...
  Value alignPtrForType(OpBuilder &builder, Location loc, Value ptr,
                        Type valueType) const {
    size_t alignment = getTypeAlignment(valueType);
    if (alignment > 1) {
  ...
  LogicalResult packArgumentBuffer(ArrayRef<Type> inputTypes,
    ...
    for (size_t i = 0; i < inputTypes.size(); i++) {
      ...
      // Align pointer before accessing this type.
