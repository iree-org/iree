...
vm.module @my_module {
  ...
  vm.func @list_alloc(%arg0: i32) -> !vm.list<i32> {
    ...
    // CHECK: %[[LIST_PTR:.+]] = address_of %3 : !emitc.lvalue<!emitc.ptr<!emitc.opaque<"iree_vm_list_t">>>
...
vm.module @my_module {
  ...
  vm.func @list_reserve(%arg0: !vm.list<i32>, %arg1: i32) {
    // CHECK: %[[LIST_DEREF_IDX:.+]] = literal "0" : !emitc.opaque<"iree_host_size_t">
    // CHECK: %[[LIST_DEREF_LVAL:.*]] = subscript %arg3[%[[LIST_DEREF_IDX]]] : (!emitc.ptr<!emitc.opaque<"iree_vm_ref_t">>, !emitc.opaque<"iree_host_size_t">) -> !emitc.lvalue<!emitc.opaque<"iree_vm_ref_t">>
    // CHECK-NEXT: %[[LIST_DEREF:.*]] = load %[[LIST_DEREF_LVAL]] : <!emitc.opaque<"iree_vm_ref_t">>
