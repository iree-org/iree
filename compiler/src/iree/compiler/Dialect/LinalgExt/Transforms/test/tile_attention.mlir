...
// TILING:          %[[ATT:.+]]:3 = iree_linalg_ext.attention 
// TILING-SAME:                                           ins(%[[Q_S]], %[[K_S]], %[[V_S]], %[[CST_1]]
// TILING-SAME:                                           outs(%[[ARG4]], %[[ARG5]], %[[ARG6]]
// TILING:          scf.yield %[[ATT]]#0, %[[ATT]]#1, %[[ATT]]#2
// TILING:        }
// TILING:        %[[D7:.+]] = linalg.generic 
// TILING-SAME:                {indexing_maps = [#[[$MAP1]], #[[$MAP]]], 
// TILING-SAME:                 iterator_types = ["parallel", "parallel"]} 
// TILING-SAME:               ins(%[[D6]]#2 : tensor<1024xf32>) 
// TILING-SAME:               outs(%[[D6]]#0 : tensor<1024x64xf32>) {
// TILING:        ^bb0(%[[IN:.+]]: f32, %[[OUT:.+]]: f32):
// TILING-DAG:      %[[CST_1:.+]] = arith.constant 1.000000e+00
// TILING:          %[[D8:.+]] = arith.divf %[[CST_1]], %[[IN]]
// TILING:          %[[D9:.+]] = arith.mulf %[[D8]], %[[OUT]]
// TILING:          linalg.yield %[[D9]]
...
// TILING:          %[[K_S:.+]] = tensor.extract_slice %[[KEY]][0, %[[ARG6]], 0] [1, %[[DIM]], %[[DIM_0]]] [1, 1, 1]
// TILING:          %[[V_S:.+]] = tensor.extract_slice %[[VALUE]][0, %[[ARG6]], 0] [1, %[[DIM]], %[[DIM_0]]] [1, 1, 1]
// TILING:          %[[Q_S:.+]] = tensor.extract_slice %[[QUERY]][0, 0, 0] [1, %[[DIM]], %[[DIM_0]]] [1, 1, 1]
// TILING:          %[[ATT:.+]]:3 = iree_linalg_ext.attention ins(%[[Q_S]], %[[K_S]], %[[V_S]], %{{[a-z0-1]+}}
...
// TILING:        ^bb0(%[[IN:.+]]: f32, %[[OUT:.+]]: f32):
// TILING-DAG:      %[[CST_3:.+]] = arith.constant 1.000000e+00
// TILING:          %[[D8:.+]] = arith.divf %[[CST_3]], %[[IN]]
// TILING:          %[[D9:.+]] = arith.mulf %[[D8]], %[[OUT]]
// TILING:          linalg.yield %[[D9]]
...
// TILING-LABEL:  @attention_f16

// TILING:        scf.for
// TILING:          iree_linalg_ext.attention ins(%{{.*}}, %{{.*}}, %{{.*}}, %{{.*}} : tensor<1024x64xf16>, tensor<1024x64xf16>, tensor<1024x64xf16>, f16
// TILING-SAME:                                           outs(%{{.*}}, %{{.*}}, %{{.*}} :
// TILING-SAME:                                           -> tensor<1024x64xf32>, tensor<1024xf32>, tensor<1024xf32>
// TILING:        scf.yield

// TILING:        linalg.generic

// TILING:        %[[TRUNCED:.+]] = linalg.generic
// TILING:          arith.truncf
// TILING:        } -> tensor<1024x64xf16>

// TILING:        %[[INSERTED_SLICE:.+]] = tensor.insert_slice %[[TRUNCED]]
// TILING:        return %[[INSERTED_SLICE]]
// TILING:      }
...
// TILING:          scf.for
// TILING:            iree_linalg_ext.attention {transpose_v = true}
// TILING:          scf.yield 
