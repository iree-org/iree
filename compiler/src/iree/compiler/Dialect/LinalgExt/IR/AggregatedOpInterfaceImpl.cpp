...
Value computeQKElementwiseMatmul(
    Location loc, OpBuilder &b, Value query, Value key, Value scale,
    std::optional<Value> mask, AffineMap qMap, AffineMap kMap, AffineMap sMap,
    std::optional<AffineMap> maskMap, SmallVector<OpFoldResult> iterationDomain,
    Type outputElementType, Region &elementwiseRegion, bool lowPrecision) {
  ...
  // S = Q @ K
  // SMap = QMap @ KMap
  Value emptyS = b.create<tensor::EmptyOp>(loc, sSizes, outputElementType);
  Value sZero =
      b.create<arith::ConstantOp>(loc, b.getZeroAttr(outputElementType));
  ...
  if (lowPrecision) {
    ...
    Value offset = b.create<arith::ConstantOp>(
        loc, b.getFloatAttr(outputElementType, clAttentionSoftmaxMax / mx));
...
FailureOr<SmallVector<Value>> AttentionOp::decomposeOperation(OpBuilder &b) {
  ...
  Value s = computeQKElementwiseMatmul(loc, b, query, key, getScale(), mask,
                                       qMap, kMap, sMap, getMaskMap(), sizes,
                                       elementType, getRegion(), lowPrecision);

  // ---- Softmax ----

  AffineMap accMap = getOutputMap();
  AffineMap maxMap = sMap.dropResults(opInfo.getK2Dims());
  ...
  Value acc = b.create<tensor::EmptyOp>(loc, accSize, f32Type);
  Value rowRedEmpty = b.create<tensor::EmptyOp>(loc, rowRedSize, f32Type);

  Value accInit = arith::getIdentityValue(arith::AtomicRMWKind::addf, f32Type,
  ...
  Value accFill =
      b.create<linalg::FillOp>(loc, ValueRange{accInit}, acc).getResult(0);
  Value maxFill =
  ...
  // TODO: We shouldn't be relying on such attributes. We need a better
  // mechanism to identify attention matmuls.
  result.getDefiningOp()->setAttr("attention_pv_matmul", b.getUnitAttr());
