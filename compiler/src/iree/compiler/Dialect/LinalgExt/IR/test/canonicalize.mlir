...
func.func public @staticize_attention_from_operand(%arg0: tensor<?x4096x16xf16>, %arg1: tensor<20x1024x16xf16>, %arg2: tensor<20x1024x64xf16>) -> tensor<20x4096x64xf16> {
  %0 = tensor.empty() : tensor<20x4096x64xf16>
  %1 = iree_linalg_ext.attention {indexing_maps = [affine_map<(d0, d1, d2, d3, d4) -> (d0, d1, d2)>, affine_map<(d0, d1, d2, d3, d4) -> (d0, d3, d2)>, affine_map<(d0, d1, d2, d3, d4) -> (d0, d3, d4)>, affine_map<(d0, d1, d2, d3, d4) -> (d0, d1, d4)>]} ins(%arg0, %arg1, %arg2 : tensor<?x4096x16xf16>, tensor<20x1024x16xf16>, tensor<20x1024x64xf16>) outs(%0 : tensor<20x4096x64xf16>) {
  ^bb0(%arg4: f16):
...
// CHECK-SAME:       ins(%[[CAST]], %[[ARG1]], %[[ARG2]] :
//      CHECK:   return %[[ATTENTION]]

// -----

func.func public @staticize_attention_from_cast(%arg0: tensor<?x4096x16xf16>, %arg1: tensor<20x?x16xf16>, %arg2: tensor<20x?x64xf16>) -> tensor<20x4096x64xf16> {
  %0 = tensor.empty() : tensor<20x4096x64xf16>
  ...
  %1 = iree_linalg_ext.attention {indexing_maps = [affine_map<(d0, d1, d2, d3, d4) -> (d0, d1, d2)>, affine_map<(d0, d1, d2, d3, d4) -> (d0, d3, d2)>, affine_map<(d0, d1, d2, d3, d4) -> (d0, d3, d4)>, affine_map<(d0, d1, d2, d3, d4) -> (d0, d1, d4)>]} ins(%arg0, %cast0, %cast1 : tensor<?x4096x16xf16>, tensor<?x?x16xf16>, tensor<?x?x64xf16>) outs(%0 : tensor<20x4096x64xf16>) {
  ^bb0(%arg4: f16):
    iree_linalg_ext.yield %arg4 : f16
  } -> tensor<20x4096x64xf16>
  return %1 : tensor<20x4096x64xf16>
...
func.func public @staticize_online_attention_from_cast(%arg0: tensor<?x4096x16xf16>, %arg1: tensor<20x1024x16xf16>, %arg2: tensor<20x1024x64xf16>) -> tensor<20x4096x64xf16> {
  %0 = tensor.empty() : tensor<20x4096x64xf16>
