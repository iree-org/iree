...
func.func @sort_keep_comparator_result(%keys : tensor<?x10xf32>,
    %indices : tensor<?x10xi64>) -> tensor<?x10xi64> {
  ...
  ^bb0(%lhs_key: f32, %rhs_key: f32, %lhs_index: i64, %rhs_index: i64):
    %take_lhs = arith.cmpf oge, %lhs_key, %rhs_key : f32
    iree_linalg_ext.yield %take_lhs : i1
  } -> tensor<?x10xf32>, tensor<?x10xi64>
  return %sorted#1 : tensor<?x10xi64>
}
// CHECK-LABEL: func.func @sort_keep_comparator_result
//       CHECK:   %[[SORT:.+]]:2 = iree_linalg_ext.sort
//  CHECK-SAME:       outs(%{{.+}}, %{{.+}} : tensor<?x10xf32>, tensor<?x10xi64>)
//       CHECK:   return %[[SORT]]#1 : tensor<?x10xi64>
