...
unsigned CustomOp::getNumNonLoopDimensions() {
  for (auto map : llvm::map_range(getIndexingMaps(), [](Attribute attr) {
...
//===---------------------------------------------------------------------===//
// Implementation of `TilingInterface`
//===---------------------------------------------------------------------===//
...
appendOffsetAndSizeForSymbolDimensions(OpBuilder &builder, CustomOp customOp,
  ...
  appendedOffsets.resize(appendedOffsets.size() + numSymbols,
...
SmallVector<std::optional<linalg::SliceParameters>>
