...
getIGEMMGenericConvDetails(linalg::LinalgOp linalgOp) {
  ...
  if (canCollapseMDims) {
    SmallVector<int64_t> oiPositions;
    for (unsigned oiDim : convDims.outputImage) {
      ...
      oiPositions.push_back(pos.value());
    }
    // Verify outputImage positions are contiguous in the output tensor.
    for (size_t i = 1; i < oiPositions.size() && canCollapseMDims; ++i) {
      if (oiPositions[i] != oiPositions[i - 1] + 1) {
        canCollapseMDims = false;
      }
    }
  }
