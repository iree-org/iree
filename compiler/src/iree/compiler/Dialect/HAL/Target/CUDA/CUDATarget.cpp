...
static llvm::cl::opt<std::string> clUsePTXASfrom(
    ...
    llvm::cl::desc(
        "It uses the provided ptxas compiler, compiles the generated PTX code "
        "with it, puts the cubin binary generated by ptxas into the "
        "executable. '--iree-hal-cuda-llvm-target-arch' is used as the target "
        "GPU. If passing additional parameters to ptxas is desired, the "
        "parameters flag can be used (e.g.'--iree-hal-use-ptxas-params=-v')."));
...
static FailureOr<std::string> compileWithPTXAS(std::string ptxas_exe,
  ...
  if (!llvm::sys::fs::exists(ptxas_exe)) return failure();
  ...
  if (commandLine.find_first_of("-v")) {
    llvm::ErrorOr<std::unique_ptr<llvm::MemoryBuffer>> maybeFlog =
        llvm::MemoryBuffer::getFile(fname_log);
    if (maybeFlog) {
      llvm::errs() << maybeFlog->get()->getBuffer();
    }
  }
  ...
  llvm::ErrorOr<std::unique_ptr<llvm::MemoryBuffer>> maybeFcubin =
      llvm::MemoryBuffer::getFile(fname_cubin);
  if (!maybeFcubin) return failure();
...
class CUDATargetBackend final : public TargetBackend {
 public:
  ...
  LogicalResult serializeExecutable(const SerializationOptions &options,
    ...
    // Attempt compiling the generated PTX with ptxas. If it fails for some
    // reason fallback to runtime compilation.
    if (!clUsePTXASfrom.empty()) {
      FailureOr<std::string> maybePtxImage = compileWithPTXAS(
          clUsePTXASfrom, clTargetChip, clUsePTXASparams, ptxImage);
