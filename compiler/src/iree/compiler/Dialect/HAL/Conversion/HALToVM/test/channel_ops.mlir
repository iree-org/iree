...
//  CHECK-SAME: (%[[DEVICE:.+]]: !vm.ref<!hal.device>, %[[AFFINITY:.+]]: i64, %[[RANK:.+]]: i32) -> !vm.ref<!hal.channel>
func.func @channel_create(%device: !hal.device, %affinity: i64, %rank: i32) -> !hal.channel {
  //      CHECK: %[[CHANNEL:.+]] = vm.call @hal.channel.create(%[[DEVICE]], %[[AFFINITY]], %[[RANK]])
  %channel = hal.channel.create device(%device : !hal.device)
                              affinity(%affinity)
                                  rank(%rank) : !hal.channel
