...
func.func @channel_create_split() -> !stream.channel {
  ...
  // CHECK: %[[CHANNEL_SPLIT:.+]] = hal.channel.split device(%[[DEVICE_0]] : !hal.device) affinity(%[[AFFINITY_0]]) groups(%[[GROUPS]]) %[[CHANNEL]] : !hal.channel
  %channel_default = stream.channel.default on(#hal.affinity.queue<[0, 1]>) : !stream.channel
  %channel_split = stream.channel.split on(#hal.affinity.queue<[0, 1]>) groups("(0),(1)") %channel_default : !stream.channel
