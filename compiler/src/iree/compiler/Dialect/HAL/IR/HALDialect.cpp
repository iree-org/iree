...
public:
  ...
  // `encoding` is not present, NopEncodingAttr is returned.
  ...
  makeLayoutAttrResolver(ModuleOp moduleOp) const {
    return [=](ArrayRef<IREE::Stream::AffinityAndOpPair> batchQueries,
      ...
      std::optional<IREE::Encoding::NopEncodingAttr> defaultAttr;
      auto getDefaultAttr = [&]() {
        if (defaultAttr) {
          return defaultAttr.value();
        }
        defaultAttr = IREE::Encoding::NopEncodingAttr::get(ctx);
        return defaultAttr.value();
      };
