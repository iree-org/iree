...
module @linalg_tensor_jit {
  ...
  util.initializer {
    ...
    %1 = linalg.generic {indexing_maps = [#map0, #map1], iterator_types = [#linalg.iterator_type<parallel>, #linalg.iterator_type<parallel>]} ins(%cst : tensor<f32>) outs(%0 : tensor<5x6xf32>) {
