...
// Move tensor.expand_shape/collapse_shape above barrier.start
struct MoveReshapeAboveBarrierStart : public RewritePattern {
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto barrierStartOp =
        op->getOperand(0).getDefiningOp<IREE::TensorExt::BarrierStartOp>();
    if (!barrierStartOp) {
      return failure();
    }

    // Update reshape's operand to use barrier.start's input
    rewriter.modifyOpInPlace(
        op, [&]() { op->getOpOperand(0).set(barrierStartOp.getValue()); });

    rewriter.setInsertionPointAfter(op);
    Value reshapeResult = op->getResult(0);
    auto newBarrier = IREE::TensorExt::BarrierStartOp::create(
...
// Move tensor.expand_shape/collapse_shape below barrier.end
struct MoveReshapeBelowBarrierEnd
    : public OpRewritePattern<IREE::TensorExt::BarrierEndOp> {
  using OpRewritePattern::OpRewritePattern;

  LogicalResult matchAndRewrite(IREE::TensorExt::BarrierEndOp barrierEndOp,
                                PatternRewriter &rewriter) const override {
    ...
    // Create a new barrier.end before the reshape with the reshape's source
    // type
    rewriter.setInsertionPoint(reshapeOp);
    auto newBarrier = IREE::TensorExt::BarrierEndOp::create(
