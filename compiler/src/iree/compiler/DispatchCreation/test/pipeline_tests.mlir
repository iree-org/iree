// RUN: iree-opt --pass-pipeline="builtin.module(iree-util-attr-based-pipeline{pipeline-name=\"preprocessing\"}, iree-dispatch-creation-fold-unit-extent-dims, iree-dispatch-creation-pipeline)" --split-input-file --mlir-print-local-scope %s | FileCheck %s
...
util.func public @make_single_dispatch(%arg0: tensor<16x8x32x2048xbf16>, %arg1: tensor<16x8x32x4096xbf16>) -> tensor<4096x1x1x2048xf32>
 attributes {util.pipelines = {preprocessing = #util.pipeline<"iree-preprocessing-make-single-dispatch">}} {
    %cst = arith.constant 0.000000e+00 : f32
