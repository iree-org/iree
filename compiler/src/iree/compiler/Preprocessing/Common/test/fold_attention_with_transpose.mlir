...
util.func public @fuse_attention_expand_transpose(
  %arg0: tensor<?x?x?xf16>, %arg1 : tensor<?x?x?xf16>, %arg2 : tensor<?x?x?xf16>) -> tensor<2x?x?x?xf16> {
  ...
  %attention = iree_linalg_ext.attention {
    ...
    ins(%arg0, %arg1, %arg2 : tensor<?x?x?xf16>, tensor<?x?x?xf16>, tensor<?x?x?xf16>)
...
//   CHECK-DAG:   %[[SPLIT:.+]] = arith.divsi %[[D]], %[[C2]]
//   CHECK-DAG:   %[[EMPTY:.+]] = tensor.empty(%[[D0]], %[[SPLIT]], %[[D1]]) : tensor<2x?x?x?xf16>
...
util.func public @fuse_attention_expand_transpose_static(
      ...
      %arg2 : tensor<20x1024x64xf16>) -> tensor<2x4096x10x64xf16> {
  ...
  %attention = iree_linalg_ext.attention {
      indexing_maps = [affine_map<(d0, d1, d2, d3, d4) -> (d0, d1, d2)>,
                       ...
                       affine_map<(d0, d1, d2, d3, d4) -> (d0, d3, d4)>,
                       affine_map<(d0, d1, d2, d3, d4) -> (d0, d1, d4)>]}
      ins(%arg0, %arg1, %arg2 : tensor<20x4096x16xf16>, tensor<20x1024x16xf16>, tensor<20x1024x64xf16>)
