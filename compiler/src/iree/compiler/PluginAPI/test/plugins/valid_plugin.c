...
// Written in C, so that a plugin in plain C stays provably loadable.

#include "iree/compiler/PluginAPI/PluginEntryPoint.h"

static bool register_sample_plugin(IreeCompilerPluginRegistrar *registrar) {
  (void)registrar;
  return true;
}
...
static const IreeCompilerPluginInfo info = {
    ...
    "sample_plugin", "test", register_sample_plugin};
