...
/// True when every --iree-load-plugin that llvm::cl parsed names a loaded
/// plugin. ireeCompilerGlobalInitialize has no argv to scan, so a flag given
/// through the C API would otherwise be swallowed by the option sink.
