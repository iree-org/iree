...
`-DIREE_EXPERIMENTAL_COMPILER_DYNAMIC_PLUGINS=ON`. Each library is `dlopen()`'d
and asked for its registration through one exported symbol, which reports the
plugin's id, the API version and the hash of the API headers it was built
against; a mismatch in either is refused. Plugins are named either on the
command line or, where there is no command line, in the environment:
...
library carries on with the plugins that did load. Registration is otherwise
identical to the static case, and one source serves both through
