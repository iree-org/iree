...
// CHECK-DAG:         %[[INTM2:.*]] = torch.constant.int -2
...
// CHECK-DAG:         %[[CST:.*]] = arith.constant dense<{{.*}}> : tensor<8xi64>
// CHECK:             %[[VAR3:.*]] = linalg.generic {{.*}} ins(%[[CST]] : tensor<8xi64>) outs(%[[VAR2]] : tensor<3x16x8xf32>) {
...
// CHECK-DAG:         %[[CST_0:.*]] = arith.constant dense<{{.*}}> : tensor<3x16x8xf32>
// CHECK-DAG:         %[[C1:.*]] = arith.constant 1 : index
// CHECK-DAG:         %[[CST_1:.*]] = arith.constant dense<{{.*}}> : tensor<1xf32>
// CHECK-DAG:         %[[CST_2:.*]] = arith.constant dense<{{.*}}> : tensor<1xf32>
// CHECK:             %[[VAR4:.*]]:2 = iree_linalg_ext.fft ins(%[[C1]], %[[CST_1]], %[[CST_2]] : index, tensor<1xf32>, tensor<1xf32>) outs(%[[VAR3]], %[[CST_0]] : tensor<3x16x8xf32>, tensor<3x16x8xf32>) : tensor<3x16x8xf32>, tensor<3x16x8xf32>
// CHECK-DAG:         %[[C2:.*]] = arith.constant 2 : index
// CHECK-DAG:         %[[CST_3:.*]] = arith.constant dense<{{.*}}> : tensor<2xf32>
// CHECK-DAG:         %[[CST_4:.*]] = arith.constant dense<{{.*}}> : tensor<2xf32>
// CHECK:             %[[VAR5:.*]]:2 = iree_linalg_ext.fft ins(%[[C2]], %[[CST_3]], %[[CST_4]] : index, tensor<2xf32>, tensor<2xf32>) outs(%[[VAR4]]#0, %[[VAR4]]#1 : tensor<3x16x8xf32>, tensor<3x16x8xf32>) : tensor<3x16x8xf32>, tensor<3x16x8xf32>
// CHECK-DAG:         %[[C3:.*]] = arith.constant 3 : index
// CHECK-DAG:         %[[CST_5:.*]] = arith.constant dense<{{.*}}> : tensor<4xf32>
// CHECK-DAG:         %[[CST_6:.*]] = arith.constant dense<{{.*}}> : tensor<4xf32>
// CHECK:             %[[VAR6:.*]]:2 = iree_linalg_ext.fft ins(%[[C3]], %[[CST_5]], %[[CST_6]] : index, tensor<4xf32>, tensor<4xf32>) outs(%[[VAR5]]#0, %[[VAR5]]#1 : tensor<3x16x8xf32>, tensor<3x16x8xf32>) : tensor<3x16x8xf32>, tensor<3x16x8xf32>
...
// CHECK-DAG:         %[[INT2_8:.*]] = torch.constant.int 2
// CHECK-DAG:         %[[INT1_9:.*]] = torch.constant.int 1
// CHECK:             %[[VAR14:.*]] = torch.aten.transpose.int %[[VAR13]], %[[INT1_9]], %[[INT2_8]] : !torch.vtensor<[3,16,5],complex<f32>>, !torch.int, !torch.int -> !torch.vtensor<[3,5,16],complex<f32>>
...
// CHECK-DAG:         %[[CST:.*]] = arith.constant dense<{{.*}}> : tensor<16xi64>
// CHECK:             %[[VAR2:.*]] = linalg.generic {{.*}} ins(%[[CST]] : tensor<16xi64>) outs(%[[VAR1]] : tensor<3x8x16xf32>) {
// CHECK:             ^bb0(%in: i64, %out: f32):
...
// CHECK-DAG:         %[[CST_0:.*]] = arith.constant dense<{{.*}}> : tensor<3x8x16xf32>
// CHECK-DAG:         %[[C1:.*]] = arith.constant 1 : index
// CHECK-DAG:         %[[CST_1:.*]] = arith.constant dense<{{.*}}> : tensor<1xf32>
// CHECK-DAG:         %[[CST_2:.*]] = arith.constant dense<{{.*}}> : tensor<1xf32>
// CHECK:             %[[VAR3:.*]]:2 = iree_linalg_ext.fft ins(%[[C1]], %[[CST_1]], %[[CST_2]] : index, tensor<1xf32>, tensor<1xf32>) outs(%[[VAR2]], %[[CST_0]] : tensor<3x8x16xf32>, tensor<3x8x16xf32>) : tensor<3x8x16xf32>, tensor<3x8x16xf32>
// CHECK-DAG:         %[[C2:.*]] = arith.constant 2 : index
// CHECK-DAG:         %[[CST_3:.*]] = arith.constant dense<{{.*}}> : tensor<2xf32>
// CHECK-DAG:         %[[CST_4:.*]] = arith.constant dense<{{.*}}> : tensor<2xf32>
// CHECK:             %[[VAR4:.*]]:2 = iree_linalg_ext.fft ins(%[[C2]], %[[CST_3]], %[[CST_4]] : index, tensor<2xf32>, tensor<2xf32>) outs(%[[VAR3]]#0, %[[VAR3]]#1 : tensor<3x8x16xf32>, tensor<3x8x16xf32>) : tensor<3x8x16xf32>, tensor<3x8x16xf32>
// CHECK-DAG:         %[[C3:.*]] = arith.constant 3 : index
// CHECK-DAG:         %[[CST_5:.*]] = arith.constant dense<{{.*}}> : tensor<4xf32>
// CHECK-DAG:         %[[CST_6:.*]] = arith.constant dense<{{.*}}> : tensor<4xf32>
// CHECK:             %[[VAR5:.*]]:2 = iree_linalg_ext.fft ins(%[[C3]], %[[CST_5]], %[[CST_6]] : index, tensor<4xf32>, tensor<4xf32>) outs(%[[VAR4]]#0, %[[VAR4]]#1 : tensor<3x8x16xf32>, tensor<3x8x16xf32>) : tensor<3x8x16xf32>, tensor<3x8x16xf32>
// CHECK-DAG:         %[[C4:.*]] = arith.constant 4 : index
// CHECK-DAG:         %[[CST_7:.*]] = arith.constant dense<{{.*}}> : tensor<8xf32>
// CHECK-DAG:         %[[CST_8:.*]] = arith.constant dense<{{.*}}> : tensor<8xf32>
// CHECK:             %[[VAR6:.*]]:2 = iree_linalg_ext.fft ins(%[[C4]], %[[CST_7]], %[[CST_8]] : index, tensor<8xf32>, tensor<8xf32>) outs(%[[VAR5]]#0, %[[VAR5]]#1 : tensor<3x8x16xf32>, tensor<3x8x16xf32>) : tensor<3x8x16xf32>, tensor<3x8x16xf32>
...
// CHECK-DAG:         %[[INTM1_10:.*]] = torch.constant.int -1
// CHECK:             %[[VAR9:.*]] = torch.aten.unsqueeze %[[VAR7]], %[[INTM1_10]] : !torch.vtensor<[3,8,9],f32>, !torch.int -> !torch.vtensor<[3,8,9,1],f32>
// CHECK:             %[[VAR10:.*]] = torch.aten.unsqueeze %[[VAR8]], %[[INTM1_10]] : !torch.vtensor<[3,8,9],f32>, !torch.int -> !torch.vtensor<[3,8,9,1],f32>
...
// CHECK:             %[[VAR12:.*]] = torch.aten.cat %[[VAR11]], %[[INTM1_10]] : !torch.list<vtensor<[3,8,9,1],f32>>, !torch.int -> !torch.vtensor<[3,8,9,2],f32>
