...
// CHECK:         %[[TRANS_ARG2:.*]] = linalg.transpose ins(%[[ARG2]] : tensor<5x2x3x4xf32>) outs({{.*}} : tensor<5x2x4x3xf32>) permutation = [0, 1, 3, 2]
// CHECK:         %[[ATTN:.*]] = iree_linalg_ext.attention {indexing_maps = [#[[$MAP_Q]], #[[$MAP_K]], #[[$MAP_V]], #[[$MAP_S]], #[[$MAP_O]]]} ins(%[[ARG0]], %[[ARG1]], %[[TRANS_ARG2]], %[[SCALE]] : tensor<5x2x3x4xf32>, tensor<5x2x3x4xf32>, tensor<5x2x4x3xf32>, f32) outs(%[[EMPTY]] : tensor<5x2x3x4xf32>) {
...
// CHECK:         %[[TRANS_ARG2:.*]] = linalg.transpose ins(%[[ARG2]] : tensor<5x2x3x4xf32>) outs({{.*}} : tensor<5x2x4x3xf32>) permutation = [0, 1, 3, 2]
// CHECK:         %[[ATTN:.*]] = iree_linalg_ext.attention {indexing_maps = [#[[$MAP_Q]], #[[$MAP_K]], #[[$MAP_V]], #[[$MAP_S]], #[[$MAP_O]]]} ins(%[[ARG0]], %[[ARG1]], %[[TRANS_ARG2]], %[[SCALE]] : tensor<5x2x8x4xf32>, tensor<5x2x3x4xf32>, tensor<5x2x4x3xf32>, f32) outs(%[[EMPTY]] : tensor<5x2x8x4xf32>) {
// CHECK:    ^[[BLOCK:.+]](%[[SCORE:.+]]: f32):
// CHECK:         linalg_ext.yield %[[SCORE]]
...
// CHECK-DAG: #[[$MAP_V:.+]] = affine_map<(d0, d1, d2, d3, d4) -> (d0, d2, d4)>
...
// CHECK:         %[[TRANS_ARG2:.*]] = linalg.transpose ins(%[[ARG2]] : tensor<1x3x4xf32>) outs({{.*}} : tensor<1x4x3xf32>) permutation = [0, 2, 1]
// CHECK:         %[[ATTN:.*]] = iree_linalg_ext.attention {indexing_maps = [#[[$MAP_Q]], #[[$MAP_K]], #[[$MAP_V]], #[[$MAP_S]], #[[$MAP_O]]]} ins(%[[ARG0]], %[[ARG1]], %[[TRANS_ARG2]], %[[SCALE]] : tensor<1x3x4xf32>, tensor<1x3x4xf32>, tensor<1x4x3xf32>, f32) outs(%[[EMPTY]] : tensor<1x3x4xf32>) {
// CHECK:    ^[[BLOCK:.+]](%[[SCORE:.+]]: f32):
// CHECK:         linalg_ext.yield %[[SCORE]]
...
// CHECK:         %[[TRANS_ARG2:.*]] = linalg.transpose ins(%[[ARG2]] : tensor<?x?x4xf32>) outs({{.*}} : tensor<?x4x?xf32>) permutation = [0, 2, 1]
// CHECK:         %[[ATTN:.*]] = iree_linalg_ext.attention {indexing_maps = [#[[$MAP_Q]], #[[$MAP_K]], #[[$MAP_V]], #[[$MAP_S]], #[[$MAP_O]]]} ins(%[[ARG0]], %[[ARG1]], %[[TRANS_ARG2]], %[[SCALE]] : tensor<?x?x4xf32>, tensor<?x?x4xf32>, tensor<?x4x?xf32>, f32) outs(%[[EMPTY]] : tensor<?x?x4xf32>) {
// CHECK:    ^[[BLOCK:.+]](%[[SCORE:.+]]: f32):
// CHECK:         linalg_ext.yield %[[SCORE]]
