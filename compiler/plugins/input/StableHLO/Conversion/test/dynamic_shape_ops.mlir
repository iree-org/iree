...
// Clamp partial windows to zero before using the result as an allocation size.
// CHECK-LABEL: @dynamic_reduce_window_empty
// CHECK-DAG: %[[ZERO:.+]] = arith.constant 0 : index
// CHECK: %[[SPAN:.+]] = arith.subi
// CHECK: %[[STEPS:.+]] = arith.divsi %[[SPAN]],
// CHECK: %[[COUNT:.+]] = arith.addi %[[STEPS]],
// CHECK: %[[FITS:.+]] = arith.cmpi sge, %[[SPAN]], %[[ZERO]]
// CHECK: %[[SIZE:.+]] = arith.select %[[FITS]], %[[COUNT]], %[[ZERO]]
// CHECK: tensor.empty(%[[SIZE]]) : tensor<?xf32>
func.func @dynamic_reduce_window_empty(%a: tensor<?xf32>) -> tensor<?xf32> {
