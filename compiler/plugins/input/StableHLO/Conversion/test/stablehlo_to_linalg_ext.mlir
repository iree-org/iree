...
// CHECK-DAG:    %[[D4:.+]] = linalg.fill ins(%[[CNEG]] : i32)
// CHECK-DAG:    %[[D5:.+]] = linalg.fill ins(%[[CPOS]] : i32)
...
// CHECK-DAG:    %[[CPOS:.+]] = arith.constant 2147483647 : i32
// CHECK-DAG:    %[[D4:.+]] = linalg.fill ins(%[[CNEG]] : f32)
// CHECK-DAG:    %[[D5:.+]] = linalg.fill ins(%[[CPOS]] : i32)
// CHECK:        %[[D6:.+]]:2 = iree_linalg_ext.topk
// CHECK-SAME:     dimension(1)
// CHECK-SAME:     ins(%[[ARG0]]
...
// CHECK:       %[[FILL:.+]] = linalg.generic {indexing_maps = [#map, #map1], iterator_types = ["parallel"]} ins(%[[ARG1]] : tensor<i32>)
// CHECK:       ^bb0(%[[IN:.+]]: i32, %[[OUT:.+]]: i32):
// CHECK:         linalg.yield %[[IN]]
// CHECK:       %[[SCAN:.+]]:2 = iree_linalg_ext.scan dimension(1) inclusive(true) ins(%[[ARG0]] : tensor<7x5xi32>)
...
// CHECK:       %[[SCAN:.+]]:2 = iree_linalg_ext.scan dimension(1) inclusive(true) ins(%[[REV_IN]] : tensor<7x5xi32>)
// CHECK:       ^bb0(%[[ARG2:.+]]: i32, %[[ARG3:.+]]: i32):
// CHECK:         %[[ADD:.+]] = arith.addi %[[ARG2]], %[[ARG3]] : i32
// CHECK:         iree_linalg_ext.yield %[[ADD]]
// CHECK:       } -> tensor<7x5xi32>, tensor<7xi32>
