...
!rhs_shared_ty = memref<2x64x4x64xi8, #gpu.address_space<workgroup>>

!lhs_shared_expand_ty = memref<2x4x16x4x64xi8, #gpu.address_space<workgroup>>
!rhs_shared_expand_ty = memref<2x4x16x4x64xi8, #gpu.address_space<workgroup>>
...
!rhs_scale_shared_ty = memref<2x64x4x4xi8, #gpu.address_space<workgroup>>

!lhs_scale_shared_expand_ty = memref<2x4x16x4x4xi8, #gpu.address_space<workgroup>>
!rhs_scale_shared_expand_ty = memref<2x4x16x4x4xi8, #gpu.address_space<workgroup>>
...
util.func @scaled_matmul_f4f4f32_m64_n64_k512(
    %lhs_base: !lhs_ty,
    %rhs_base: !rhs_ty,
  ...
  %rhs_shared_expand = memref.expand_shape %rhs_shared_base [[0], [1, 2], [3], [4]]
    output_shape [2, 4, 16, 4, 64] : !rhs_shared_ty into !rhs_shared_expand_ty
  ...
  %rhs_scale_shared_expand = memref.expand_shape %rhs_scale_shared [[0], [1, 2], [3], [4]]
    output_shape [2, 4, 16, 4, 4] : !rhs_scale_shared_ty into !rhs_scale_shared_expand_ty
  ...
  %scale_inner = arith.muli %scale_ids#1, %c2 : index
  ...
  scf.forall (%unused_id) in (512) {
    ...
    // Copy scales.
    amdgpu.gather_to_lds %lhs_scale[%scale_outer, %scale_inner], %lhs_scale_shared[%c0, %scale_outer, %c0, %c0]
      : !lhs_scale_copy_vec_ty, !lhs_scale_buffer_ty, !lhs_scale_shared_ty
    amdgpu.gather_to_lds %rhs_scale[%scale_outer, %scale_inner], %rhs_scale_shared[%c0, %scale_outer, %c0, %c0]
  ...
  %1 = scf.forall (%id) in (512) shared_outs(%out = %0) -> !res_ty {
    ...
    %loop = scf.for %i = %c16 to %k step %c16 iter_args(%iter = %init) -> !acc_ty {
      ...
      %loop_scale_inner = arith.addi %scale_inner, %i : index
      amdgpu.gather_to_lds %lhs_scale[%scale_outer, %loop_scale_inner], %lhs_scale_shared[%buffer_num, %scale_outer, %c0, %c0]
        : !lhs_scale_copy_vec_ty, !lhs_scale_buffer_ty, !lhs_scale_shared_ty
      amdgpu.gather_to_lds %rhs_scale[%scale_outer, %loop_scale_inner], %rhs_scale_shared[%buffer_num, %scale_outer, %c0, %c0]
        : !rhs_scale_copy_vec_ty, !rhs_scale_buffer_ty, !rhs_scale_shared_ty
