...
!out_sref = !pcf.sref<256x256xf32, #iree_gpu.subgroup_scope>
!med_out_sref = !pcf.sref<1x128x256xf32, #iree_gpu.subgroup_scope>
!exp_out_sref = !pcf.sref<1x256x256xf32, #iree_gpu.subgroup_scope>
    ...
    pcf.generic scope(#iree_gpu.lane_scope)
      ...
      scf.for %a = %c0 to %c8 step %c1 {
        scf.for %b = %c0 to %c4 step %c1 {
          %row = affine.apply affine_map<(d0, d1, d2) -> ((d0 + d1) * 16 + d2)>(%m_outer_id, %a, %ids#3)
          %col = affine.apply affine_map<(d0, d1, d2) -> ((d0 + d1) * 16 + d2)>(%n_outer_id, %b, %inner_id)
          %tile = tensor.extract_slice %4[%a, 0, %b, 0] [1, 1, 1, 4] [1, 1, 1, 1] : tensor<8x1x4x4xf32> to tensor<1x4xf32>
          pcf.write_slice %tile into %out_ref[%row, %col] [1, 4] [1, 1] : tensor<1x4xf32> into !out_sref
        } {iree_codegen.unroll}
      } {iree_codegen.unroll}
      pcf.return
    }
...
util.func private @pingpong_medium_f16_expanded(%lhs_base: !mexp_in_ty, %rhs_base: !in_ty, %unused_acc: tensor<1x128x256xf32>) -> tensor<1x128x256xf32> {
  ...
  %result = pcf.generic scope(#iree_gpu.subgroup_scope)
    execute(%out_ref = %unused_acc)[%sg_id: index, %num_sg: index]
         : (!med_out_sref) -> (tensor<1x128x256xf32>) {
    pcf.generic scope(#iree_gpu.lane_scope)
      execute[%lane_id: index, %sg_size: index] {
      %id = affine.linearize_index disjoint [%sg_id, %lane_id] by (%num_sg, %sg_size) : index
      %ids:4 = affine.delinearize_index %id into (2, 4, 4, 16) : index, index, index, index
      ...
      scf.for %a = %c0 to %c4 step %c1 {
        scf.for %b = %c0 to %c4 step %c1 {
          %row = affine.apply affine_map<(d0, d1, d2) -> ((d0 + d1) * 16 + d2)>(%m_outer_id, %a, %ids#3)
          %col = affine.apply affine_map<(d0, d1, d2) -> ((d0 + d1) * 16 + d2)>(%n_outer_id, %b, %inner_id)
          %tile = tensor.extract_slice %4[0, %a, 0, %b, 0] [1, 1, 1, 1, 4] [1, 1, 1, 1, 1] : tensor<1x4x1x4x4xf32> to tensor<1x1x4xf32>
          pcf.write_slice %tile into %out_ref[0, %row, %col] [1, 1, 4] [1, 1, 1] : tensor<1x1x4xf32> into !med_out_sref
        } {iree_codegen.unroll}
      } {iree_codegen.unroll}
      pcf.return
    }
  ...
  util.return %result : tensor<1x128x256xf32>
}
...
util.func private @pingpong_large_f16_expanded(%lhs_base: !exp_in_ty, %rhs_base: !in_ty, %unused_acc: tensor<1x256x256xf32>) -> tensor<1x256x256xf32> {
  ...
  %result = pcf.generic scope(#iree_gpu.subgroup_scope)
    execute(%out_ref = %unused_acc)[%sg_id: index, %num_sg: index]
         : (!exp_out_sref) -> (tensor<1x256x256xf32>) {
    pcf.generic scope(#iree_gpu.lane_scope)
      execute[%lane_id: index, %sg_size: index] {
      %id = affine.linearize_index disjoint [%sg_id, %lane_id] by (%num_sg, %sg_size) : index
      %ids:4 = affine.delinearize_index %id into (2, 4, 4, 16) : index, index, index, index
      ...
      scf.for %a = %c0 to %c8 step %c1 {
        scf.for %b = %c0 to %c4 step %c1 {
          %row = affine.apply affine_map<(d0, d1, d2) -> ((d0 + d1) * 16 + d2)>(%m_outer_id, %a, %ids#3)
          %col = affine.apply affine_map<(d0, d1, d2) -> ((d0 + d1) * 16 + d2)>(%n_outer_id, %b, %inner_id)
          %tile = tensor.extract_slice %4[0, %a, 0, %b, 0] [1, 1, 1, 1, 4] [1, 1, 1, 1, 1] : tensor<1x8x1x4x4xf32> to tensor<1x1x4xf32>
          pcf.write_slice %tile into %out_ref[0, %row, %col] [1, 1, 4] [1, 1, 1] : tensor<1x1x4xf32> into !exp_out_sref
        } {iree_codegen.unroll}
      } {iree_codegen.unroll}
      pcf.return
    }
  ...
  util.return %result : tensor<1x256x256xf32>
}
