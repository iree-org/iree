...
// 8 = k split factor
// 64 = 128 / 2 = k tile / 2
!lhs_shared_ty = memref<2x64x4x64xi8, #gpu.address_space<workgroup>>
!rhs_shared_ty = memref<2x128x4x64xi8, #gpu.address_space<workgroup>>

!lhs_shared_expand_ty = memref<2x4x16x4x64xi8, #gpu.address_space<workgroup>>
!rhs_shared_expand_ty = memref<2x8x16x4x64xi8, #gpu.address_space<workgroup>>
...
// 8 = k split factor
// 4 = k tile / k block
!lhs_scale_shared_ty = memref<2x64x4x4xi8, #gpu.address_space<workgroup>>
!rhs_scale_shared_ty = memref<2x128x4x4xi8, #gpu.address_space<workgroup>>

!lhs_scale_shared_expand_ty = memref<2x4x16x4x4xi8, #gpu.address_space<workgroup>>
!rhs_scale_shared_expand_ty = memref<2x8x16x4x4xi8, #gpu.address_space<workgroup>>
...
!return_ty = tensor<64x128xf32>

// subgroup_m = 2 waves subgroup_n = 4 waves
!acc_ty = vector<2x2x4x4x1xf32>

!reduce_ty = vector<2x2x4x1xf32>
!store_ty = vector<2x4x2x1xf32>
!tensor_store_ty = tensor<2x4x2x1xf32>
...
util.func @scaled_matmul_f4f4f32_m64_n128_k512(
    %lhs_base: !lhs_ty,
    %rhs_base: !rhs_ty,
  ...
  %lhs_shared_expand = memref.expand_shape %lhs_shared_base [[0], [1, 2], [3], [4]]
    output_shape [2, 4, 16, 4, 64] : !lhs_shared_ty into !lhs_shared_expand_ty
  %rhs_shared_expand = memref.expand_shape %rhs_shared_base [[0], [1, 2], [3], [4]]
    output_shape [2, 8, 16, 4, 64] : !rhs_shared_ty into !rhs_shared_expand_ty

  %lhs_scale_shared_expand = memref.expand_shape %lhs_scale_shared [[0], [1, 2], [3], [4]]
    output_shape [2, 4, 16, 4, 4] : !lhs_scale_shared_ty into !lhs_scale_shared_expand_ty
  %rhs_scale_shared_expand = memref.expand_shape %rhs_scale_shared [[0], [1, 2], [3], [4]]
    output_shape [2, 8, 16, 4, 4] : !rhs_scale_shared_ty into !rhs_scale_shared_expand_ty
  ...
  // Input load offsets.
  %ids:2 = affine.delinearize_index %idx into (32, 16) : index, index
  %inner = arith.muli %ids#1, %c1 : index
  %outer_base = arith.muli %ids#0, %c1 : index

  // Scale load offsets.
  %lhs_scale_ids:2 = affine.delinearize_index %idx into (64, 8) : index, index
  %lhs_scale_outer = arith.muli %lhs_scale_ids#0, %c1 : index
  %lhs_scale_inner = arith.muli %lhs_scale_ids#1, %c2 : index

  %rhs_scale_ids:2 = affine.delinearize_index %idx into (128, 4) : index, index
  %rhs_scale_outer = arith.muli %rhs_scale_ids#0, %c1 : index
  %rhs_scale_inner = arith.muli %rhs_scale_ids#1, %c4 : index

  scf.forall (%unused_id) in (512) {
    // Copy inputs.
    scf.for %i = %c0 to %c64 step %c32 {
      %outer = arith.addi %outer_base, %i : index
      amdgpu.gather_to_lds %lhs[%outer, %inner, %c0], %lhs_shared_base[%c0, %outer, %c0, %c0]
        : !lhs_copy_vec_ty, !lhs_buffer_ty, !lhs_shared_ty
    }
    scf.for %i = %c0 to %c128 step %c32 {
      %outer = arith.addi %outer_base, %i : index
      amdgpu.gather_to_lds %rhs[%outer, %inner, %c0], %rhs_shared_base[%c0, %outer, %c0, %c0]
        : !rhs_copy_vec_ty, !rhs_buffer_ty, !rhs_shared_ty
    }

    // Copy scales.
    amdgpu.gather_to_lds %lhs_scale[%lhs_scale_outer, %lhs_scale_inner], %lhs_scale_shared[%c0, %lhs_scale_outer, %c0, %c0]
      : !lhs_scale_copy_vec_ty, !lhs_scale_buffer_ty, !lhs_scale_shared_ty
    amdgpu.gather_to_lds %rhs_scale[%rhs_scale_outer, %rhs_scale_inner], %rhs_scale_shared[%c0, %rhs_scale_outer, %c0, %c0]
  ...
  %1 = scf.forall (%id) in (512) shared_outs(%out = %0) -> !res_ty {
    ...
    %n_id = arith.muli %mfma_ids#1, %c2 : index
    ...
    %loop = scf.for %i = %c16 to %k step %c16 iter_args(%iter = %init) -> !acc_ty {
      %shift = arith.shrui %i, %c4 : index
      ...
      scf.for %j = %c0 to %c64 step %c32 {
        ...
        amdgpu.gather_to_lds %lhs[%outer, %loop_inner, %c0], %lhs_shared_base[%buffer_num, %outer, %c0, %c0]
          : !lhs_copy_vec_ty, !lhs_buffer_ty, !lhs_shared_ty
      }
      scf.for %j = %c0 to %c128 step %c32 {
        %outer = arith.addi %outer_base, %j : index
        amdgpu.gather_to_lds %rhs[%outer, %loop_inner, %c0], %rhs_shared_base[%buffer_num, %outer, %c0, %c0]
          : !rhs_copy_vec_ty, !rhs_buffer_ty, !rhs_shared_ty
      }

      %loop_lhs_scale_inner = arith.addi %lhs_scale_inner, %i : index
      %loop_rhs_scale_inner = arith.addi %rhs_scale_inner, %i : index
      amdgpu.gather_to_lds %lhs_scale[%lhs_scale_outer, %loop_lhs_scale_inner], %lhs_scale_shared[%buffer_num, %lhs_scale_outer, %c0, %c0]
        : !lhs_scale_copy_vec_ty, !lhs_scale_buffer_ty, !lhs_scale_shared_ty
      amdgpu.gather_to_lds %rhs_scale[%rhs_scale_outer, %loop_rhs_scale_inner], %rhs_scale_shared[%buffer_num, %rhs_scale_outer, %c0, %c0]
        : !rhs_scale_copy_vec_ty, !rhs_scale_buffer_ty, !rhs_scale_shared_ty
    ...
    %last_shift = arith.shrui %k, %c4 : index
    ...
    scf.forall.in_parallel {
      tensor.parallel_insert_slice %to_tensor into %out[%m_id, %store_outer_offset, %n_id, %store_mfma_ids#3] [2, 4, 2, 1] [1, 1, 1, 1]
