...
!return_ty = tensor<1x1x2x4x2x2x4x16x4xf32>

// subgroup_m = 2 waves subgroup_n = 4 waves
!acc_ty = vector<2x2x4x4x1xf32>

!reduce_ty = vector<2x2x4x1xf32>
!store_ty = vector<1x1x1x1x2x2x1x1x4xf32>
!tensor_store_ty = tensor<1x1x1x1x2x2x1x1x4xf32>
...
util.func @dt_scaled_matmul_f4f4f32_m64_n128_k512(
  ...
  scf.forall (%base_id) in (512) {
    ...
    scf.for %i = %c0 to %c4 step %c1 {
      amdgpu.gather_to_lds %rhs[%c0, %c0, %c0, %i, %ids#0, %ids#1, %ids#2, %ids#3, %c0], %rhs_shared_base[%c0, %i, %ids#0, %ids#1, %ids#2, %ids#3, %c0]
        : !rhs_copy_vec_ty, !rhs_buffer_ty, !rhs_shared_ty
    }

    // Copy scales.
    %lhs_scale_ids:5 = affine.delinearize_index %base_id into (2, 2, 2, 4, 16) : index, index, index, index, index
    %cmp0 = arith.cmpi slt, %base_id, %c256 : index
    scf.if %cmp0 {
      amdgpu.gather_to_lds %lhs_scale[%c0, %c0, %lhs_scale_ids#1, %lhs_scale_ids#2, %lhs_scale_ids#3, %lhs_scale_ids#4, %c0], %lhs_scale_shared[%c0, %lhs_scale_ids#1, %lhs_scale_ids#2, %lhs_scale_ids#3, %lhs_scale_ids#4, %c0]
        : !lhs_scale_copy_vec_ty, !lhs_scale_buffer_ty, !lhs_scale_shared_ty
    }
    %rhs_scale_ids:4 = affine.delinearize_index %base_id into (4, 2, 4, 16) : index, index, index, index
    amdgpu.gather_to_lds %rhs_scale[%c0, %c0, %rhs_scale_ids#0, %rhs_scale_ids#1, %rhs_scale_ids#2, %rhs_scale_ids#3, %c0], %rhs_scale_shared[%c0, %rhs_scale_ids#0, %rhs_scale_ids#1, %rhs_scale_ids#2, %rhs_scale_ids#3, %c0]
  ...
  %1 = scf.forall (%id) in (512) shared_outs(%out = %0) -> !return_ty {
    ...
    %lhs_scale_ids:5 = affine.delinearize_index %id into (2, 2, 2, 4, 16) : index, index, index, index, index
    %rhs_scale_ids:4 = affine.delinearize_index %id into (4, 2, 4, 16) : index, index, index, index
    ...
    %loop = scf.for %i = %c1 to %k step %c1 iter_args(%iter = %init) -> !acc_ty {
      ...
      scf.for %j = %c0 to %c4 step %c1 {
        amdgpu.gather_to_lds %rhs[%c0, %i, %c0, %j, %subgroup_ids#0, %subgroup_ids#1, %ids#1, %ids#2, %c0], %rhs_shared_base[%buffer_num, %j, %subgroup_ids#0, %subgroup_ids#1, %ids#1, %ids#2, %c0]
          : !rhs_copy_vec_ty, !rhs_buffer_ty, !rhs_shared_ty
      }

      scf.if %cmp0 {
        amdgpu.gather_to_lds %lhs_scale[%c0, %i, %lhs_scale_ids#1, %lhs_scale_ids#2, %lhs_scale_ids#3, %lhs_scale_ids#4, %c0], %lhs_scale_shared[%buffer_num, %lhs_scale_ids#1, %lhs_scale_ids#2, %lhs_scale_ids#3, %lhs_scale_ids#4, %c0]
          : !lhs_scale_copy_vec_ty, !lhs_scale_buffer_ty, !lhs_scale_shared_ty
      }
      amdgpu.gather_to_lds %rhs_scale[%c0, %i, %rhs_scale_ids#0, %rhs_scale_ids#1, %rhs_scale_ids#2, %rhs_scale_ids#3, %c0], %rhs_scale_shared[%buffer_num, %rhs_scale_ids#0, %rhs_scale_ids#1, %rhs_scale_ids#2, %rhs_scale_ids#3, %c0]
        : !rhs_scale_copy_vec_ty, !rhs_scale_buffer_ty, !rhs_scale_shared_ty
      ...
      %rhs_byte_vec = vector.transfer_read %rhs_shared_base[%curr_buffer_num, %subgroup_ids#1, %c0, %c0, %ids#1, %ids#2, %c0],
      ...
      %rhs_scale_byte_vec = vector.transfer_read %rhs_scale_shared[%curr_buffer_num, %subgroup_ids#1, %c0, %ids#1, %ids#2, %c0],
    ...
    %lhs_byte_vec_t = vector.shape_cast %lhs_byte_vec : vector<1x1x2x4x1x1x16xi8> to !lhs_byte_vec_ty
    %lhs_vec = vector.bitcast %lhs_byte_vec_t : !lhs_byte_vec_ty to !lhs_vec_ty
    %rhs_byte_vec = vector.transfer_read %rhs_shared_base[%final_buffer_num, %subgroup_ids#1, %c0, %c0, %ids#1, %ids#2, %c0],
      %cst_rhs {in_bounds = [true, true, true, true, true, true, true]} : !rhs_shared_ty, vector<1x1x2x4x1x1x16xi8>
    %rhs_byte_vec_t = vector.shape_cast %rhs_byte_vec : vector<1x1x2x4x1x1x16xi8> to !rhs_byte_vec_ty
    %rhs_vec = vector.bitcast %rhs_byte_vec_t : !rhs_byte_vec_ty to !rhs_vec_ty
    ...
    %lhs_scale_byte_vec_t = vector.shape_cast %lhs_scale_byte_vec : vector<1x1x2x1x1x4xi8> to !lhs_scale_byte_vec_ty
    %lhs_scale_vec = vector.bitcast %lhs_scale_byte_vec_t : !lhs_scale_byte_vec_ty to !lhs_scale_vec_ty
    %rhs_scale_byte_vec = vector.transfer_read %rhs_scale_shared[%final_buffer_num, %subgroup_ids#1, %c0, %ids#1, %ids#2, %c0],
        %cst_scale {in_bounds = [true, true, true, true, true, true]} : !rhs_scale_shared_ty, vector<1x1x2x1x1x4xi8>
    %rhs_scale_byte_vec_t = vector.shape_cast %rhs_scale_byte_vec : vector<1x1x2x1x1x4xi8> to !rhs_scale_byte_vec_ty
    %rhs_scale_vec = vector.bitcast %rhs_scale_byte_vec_t : !rhs_scale_byte_vec_ty to !rhs_scale_vec_ty
    ...
    scf.forall.in_parallel {
      tensor.parallel_insert_slice %to_tensor into %out[%c0, %c0, %subgroup_ids#0, %subgroup_ids#1, %c0, %c0, %ids#1, %ids#2, %c0] [1, 1, 1, 1, 2, 2, 1, 1, 4] [1, 1, 1, 1, 1, 1, 1, 1, 1]
