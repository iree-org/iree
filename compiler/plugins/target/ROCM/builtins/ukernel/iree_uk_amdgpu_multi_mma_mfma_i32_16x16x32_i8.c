...
[[clang::always_inline]] void iree_uk_amdgpu_multi_mma_mfma_i32_16x16x32_i8(
  ...
  */
  ...
  // Arithmetic loop.
  const int64x2_t *a_global = (const int64x2_t *)(a_buffer + a_offset);
  const int64x2_t *b_global = (const int64x2_t *)(b_buffer + b_offset);
  for (int k_outer = 0; k_outer < k_size; ++k_outer) {
    for (int m = 0; m < unroll_m; ++m) {
      int64x2_t a = a_global[64 * m];
      for (int n = 0; n < unroll_n; ++n) {
        int64x2_t b = b_global[64 * n];
        for (int k = 0; k < unroll_k; ++k) {
          c[m][n] = __builtin_amdgcn_mfma_i32_16x16x32_i8(a[k], b[k], c[m][n],
                                                          0, 0, 0);
        }
      }
    }
    a_global += 64 * unroll_m * subgroups_m;
    b_global += 64 * unroll_n * subgroups_n;
  }
