vm.module @conversion_ops_f32 {
  ...
  vm.func @test_bitcast_f32_i32() {
    %c1 = vm.const.f32 5.5
    %c1dno = vm.optimization_barrier %c1 : f32
  ...
  vm.func @test_cast_f32_si64_int_min() {
    ...
    %c2dno = vm.optimization_barrier %c2 : i64
    %c3 = vm.const.i64 1
  ...
  // EmitC constant folding breaks through vm.optimization_barrier, causing
  // this test to be folded to unconditional error. Excluded until EmitC is
  // removed or barrier handling is fixed.
  vm.export @test_cast_f32_ui32_int_big attributes {emitc.exclude}
  ...
  // this test to be folded to unconditional error. Excluded until EmitC is
  // removed or barrier handling is fixed.
  vm.export @test_cast_f32_ui64_int_big attributes {emitc.exclude}
