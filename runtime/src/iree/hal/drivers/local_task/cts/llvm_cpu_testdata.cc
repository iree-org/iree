...
// llvm-cpu executable format registration for the local-task CTS2 backend.
//
...
#include "runtime/src/iree/hal/drivers/local_task/cts/testdata_llvm_cpu.h"

namespace iree::hal::cts {

static iree_const_byte_span_t GetLlvmCpuExecutableData(
    iree_string_view_t file_name) {
  const iree_file_toc_t* toc = iree_cts_testdata_llvm_cpu_create();
...
static bool llvm_cpu_format_registered_ =
    (CtsRegistry::RegisterExecutableFormat(
         "local_task",
         {"llvm_cpu", "embedded-elf-" IREE_ARCH, GetLlvmCpuExecutableData}),
     true);
