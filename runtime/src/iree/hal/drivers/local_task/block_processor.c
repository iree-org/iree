...
// High bit in region_drainer_state: set while the completer owns the region
// transition and is waiting to rewrite shared .data.
#define IREE_HAL_CMD_BLOCK_PROCESSOR_REGION_TRANSITION_BIT INT32_MIN

// Low bits in region_drainer_state: active workers admitted into the region.
#define IREE_HAL_CMD_BLOCK_PROCESSOR_REGION_DRAINER_COUNT_MASK INT32_MAX
...
static inline void iree_hal_cmd_dispatch_initialize_workgroup_state(
  ...
  out_workgroup_state->processor_id = worker_context->processor_id;
  out_workgroup_state->local_memory = worker_context->local_memory.data;
  out_workgroup_state->local_memory_size =
      (uint32_t)worker_context->local_memory.data_length;
}
...
// Executes tiles for a DISPATCH command. Claims tiles by atomically advancing
// the command's shared tile_index entry, executes each tile by calling the
// kernel function, and returns the total number of tiles completed.
//
// Region transitions are protected by region_drainer_state: the completer
// closes the gate and waits for all admitted drainers to leave before resetting
// tile_index. That makes fetch-add claiming safe here without an epoch CAS in
  ...
  if (worker_count == 1) {
    ...
    for (uint32_t tile = 0; tile < tile_count; ++tile) {
      ...
      iree_hal_cmd_dispatch_initialize_workgroup_state(
          tile, workgroup_count, xy_count, worker_context, &workgroup_state);
    ...
    *out_tiles_completed = tile_count;
    ...
    // Multi-worker: claim tiles by incrementing the shared tile counter.
    // Stale drainers from older regions cannot race this reset because the
    // region-drainer gate excludes them before transitions rewrite .data.
    uint32_t completed = 0;
    (void)region_epoch;
    while (true) {
      uint32_t counter = (uint32_t)iree_atomic_fetch_add(
          tile_index, (int64_t)tiles_per_reservation,
          iree_memory_order_relaxed);
...
static bool iree_hal_cmd_transfer_claim_tile(iree_atomic_int64_t* tile_index,
  ...
  (void)region_epoch;
  uint32_t tile =
      (uint32_t)iree_atomic_fetch_add(tile_index, 1, iree_memory_order_relaxed);
...
// Executes a FILL command by claiming transfer tiles with the same fetch-add
// machinery used for dispatch workgroups.
...
// Executes a COPY command by claiming transfer tiles with the same fetch-add
// machinery used for dispatch workgroups.
...
static uint32_t iree_hal_cmd_execute_update(const iree_hal_cmd_update_t* update,
  ...
  if (worker_count > 1) {
    (void)region_epoch;
    uint32_t tile = (uint32_t)iree_atomic_fetch_add(
        tile_index, (int64_t)tile_count, iree_memory_order_relaxed);
    if (tile != 0) {
      return 0;
    }
  }
...
// dispatch_count commands following the barrier from a worker-specific rotated
// start point and claims tiles from each. Commands in an open synchronization
// scope can make progress concurrently; the rotated scan spreads workers across
// the whole region instead of stampeding the first command.
//
// |region_epoch| is the global monotonically-increasing epoch published at
// region transitions. Tile claiming does not validate it directly; the
// region-drainer gate prevents stale workers from racing .data resets.
//
...
static uint32_t iree_hal_cmd_block_processor_process_region(
  ...
  const iree_hal_cmd_header_t* first_cmd = iree_hal_cmd_next(&barrier->header);
  const uint8_t start_dispatch_index =
      (uint8_t)(worker_context->worker_index % dispatch_count);
  const uint8_t first_pass_count = dispatch_count - start_dispatch_index;
  const iree_hal_cmd_header_t* cmd = first_cmd;
  for (uint8_t i = 0; i < start_dispatch_index; ++i) {
    cmd = iree_hal_cmd_next(cmd);
  }
  ...
  for (uint8_t d = 0; d < dispatch_count; ++d) {
    ...
    if (d + 1 < dispatch_count) {
      cmd = iree_hal_cmd_next(cmd);
      if (d + 1 == first_pass_count) cmd = first_cmd;
    }
  }
...
static bool iree_hal_cmd_block_processor_region_prefers_warm_spin(
  ...
  return summary->lookahead_width_bucket >
...
// memset; only binding pointers, active_region_index, and remaining_tiles
// require non-zero initialization.
//
...
static void iree_hal_cmd_block_processor_init_block(
  ...
  // acquire this after entering the region-drainer gate to observe the freshly
  // initialized .data, and no-work warm retainers use it to detect advancement.
  int32_t block_epoch = context->next_epoch;
  context->next_epoch = block_epoch + 1;

  // Reset all tile counters. ALL slots are set (not just dispatch_count for
  // the first region) because max_region_dispatch_count covers the largest
  // region in the block.
  for (uint16_t i = 0; i < context->max_region_dispatch_count; ++i) {
    iree_atomic_store(iree_hal_cmd_block_state_tile_index(context->state, i), 0,
                      iree_memory_order_relaxed);
  }
...
// are reset to counter=0. All max_region_dispatch_count entries are set, not
// just the previous region's count, because the next region may have more
// dispatches.
//
...
static void iree_hal_cmd_block_processor_init_region(
  ...
  for (uint16_t i = 0; i < context->max_region_dispatch_count; ++i) {
    iree_atomic_store(iree_hal_cmd_block_state_tile_index(state, i), 0,
                      iree_memory_order_relaxed);
  }
...
//   - Per-dispatch:  tile_indices[] (fetch-add work stealing)
//   - Per-region:    remaining_tiles (completer election) +
//                    active_region_index (region gating) +
//                    region_drainer_state (.data rewrite exclusion)
...
// Workers claim tiles with fetch-add. The region-drainer gate is the
// correctness boundary around .data reuse: a worker must enter before reading
// the current block/region state, and the completer closes the gate and waits
// for admitted drainers to leave before resetting tile counters for the next
// region/block.
//
// REGION TRANSITIONS
//
// Workers enter region_drainer_state before sampling .data and leave it after
// all .data reads are complete. The completer sets the transition bit and waits
// until only its own drainer claim remains before initializing .data for the
// next region (reset tile_indices, set remaining_tiles). This protects
// ordinary .data reads such as binding_ptrs, binding_lengths, cached_barrier,
// and indirect dispatch parameters. Workers see the new region on their next
// drain() call via the acquired region_epoch.
//
...
// the block pointer is still from the old block. The seqlock protects
// non-region state reads such as the block pointer, region count, and
// binding_ptrs during fixup resolution.
//
...
// DEADLOCK FREEDOM
//
// The only internal wait is the elected completer waiting for already-admitted
// region drainers to leave before rewriting .data. Non-completer workers simply
// return and the caller decides whether to yield, scan other contexts, or
// retry. Workers that observe a closed region-drainer gate or an even
// block_sequence bail and retry through the caller.
//
// INVARIANTS
//
//   I1: active_region_index is monotonically non-decreasing within a block.
//   I2: tile_indices, remaining_tiles, and active_region_index are set BEFORE
//       region_epoch (the release point). Workers acquire region_epoch,
//       guaranteeing all prior writes are visible.
//   I3: Workers must enter the region-drainer gate before reading .data and
//       must leave it after all tile claims and state reads for that drain.
//   I4: The completer must close the region-drainer gate and wait for all other
//       admitted drainers to leave before rewriting .data for the next region.
...
//   I7: region_drainer_state's transition bit is set before any region or
//       block transition rewrites .data. The completer waits until the low-bit
//       drainer count is 1, meaning only the completer can still read old
//       .data, before performing the rewrite.
...
iree_hal_cmd_block_processor_update_budget(
  ...
  if (context->admission_budget_ptr) {
    intptr_t admission_budget_ptr = iree_atomic_load(
        context->admission_budget_ptr, iree_memory_order_acquire);
    if (admission_budget_ptr) {
      iree_atomic_store((iree_atomic_int32_t*)admission_budget_ptr, new_budget,
                        iree_memory_order_release);
    }
  }
...
// Attempts to enter the active-region drainer set. Once entered, the worker may
// read shared .data until it calls leave. The transition bit prevents new
// entrants while the completer is rewriting .data for the next region/block.
static bool iree_hal_cmd_block_processor_try_enter_region_drainer(
    iree_hal_cmd_block_processor_context_t* context) {
  int32_t drainer_state = iree_atomic_load(&context->region_drainer_state,
                                           iree_memory_order_acquire);
  while (true) {
    if (drainer_state & IREE_HAL_CMD_BLOCK_PROCESSOR_REGION_TRANSITION_BIT) {
      return false;
    }
    IREE_ASSERT((drainer_state &
                 IREE_HAL_CMD_BLOCK_PROCESSOR_REGION_DRAINER_COUNT_MASK) <
                IREE_HAL_CMD_BLOCK_PROCESSOR_REGION_DRAINER_COUNT_MASK);
    const int32_t desired_state = drainer_state + 1;
    if (iree_atomic_compare_exchange_weak(
            &context->region_drainer_state, &drainer_state, desired_state,
...
// Leaves the active-region drainer set after all .data reads are complete.
static void iree_hal_cmd_block_processor_leave_region_drainer(
    iree_hal_cmd_block_processor_context_t* context) {
  const int32_t previous_state = iree_atomic_fetch_sub(
      &context->region_drainer_state, 1, iree_memory_order_acq_rel);
  IREE_ASSERT((previous_state &
               IREE_HAL_CMD_BLOCK_PROCESSOR_REGION_DRAINER_COUNT_MASK) > 0);
}

// Closes the region-drainer gate and waits until only the completer remains.
static void iree_hal_cmd_block_processor_begin_region_transition(
    iree_hal_cmd_block_processor_context_t* context) {
  int32_t drainer_state = iree_atomic_load(&context->region_drainer_state,
                                           iree_memory_order_acquire);
  while (true) {
    IREE_ASSERT((drainer_state &
                 IREE_HAL_CMD_BLOCK_PROCESSOR_REGION_DRAINER_COUNT_MASK) > 0);
    IREE_ASSERT(
        !(drainer_state & IREE_HAL_CMD_BLOCK_PROCESSOR_REGION_TRANSITION_BIT));
    const int32_t desired_state =
        drainer_state | IREE_HAL_CMD_BLOCK_PROCESSOR_REGION_TRANSITION_BIT;
    if (iree_atomic_compare_exchange_weak(
            &context->region_drainer_state, &drainer_state, desired_state,
            iree_memory_order_acq_rel, iree_memory_order_acquire)) {
  ...
  while ((iree_atomic_load(&context->region_drainer_state,
                           iree_memory_order_acquire) &
          IREE_HAL_CMD_BLOCK_PROCESSOR_REGION_DRAINER_COUNT_MASK) > 1) {
    iree_thread_yield();
  }
}

// Opens the region-drainer gate after the new region/block state is published.
static void iree_hal_cmd_block_processor_end_region_transition(
    iree_hal_cmd_block_processor_context_t* context) {
  const int32_t previous_state = iree_atomic_fetch_and(
      &context->region_drainer_state,
      IREE_HAL_CMD_BLOCK_PROCESSOR_REGION_DRAINER_COUNT_MASK,
      iree_memory_order_release);
  IREE_ASSERT(previous_state &
              IREE_HAL_CMD_BLOCK_PROCESSOR_REGION_TRANSITION_BIT);
}
...
drain_start:
  ...
  // Admit this worker to read shared .data for the active region. A region
  // completer closes this gate before rewriting .data, so a worker that enters
  // here must leave on every path below.
  if (!iree_hal_cmd_block_processor_try_enter_region_drainer(context)) {
    IREE_TRACE(out_result->reason =
                   IREE_HAL_CMD_BLOCK_PROCESSOR_DRAIN_REASON_REGION_TRANSITION);
    return;
  }

  // Recheck the block seqlock after entering the region-drainer set. If a
  // block transition raced the entry, leave before reading any .data.
  ...
  // region_epoch (unordered relaxed load), then claim reset tile_indices for
  // the wrong command stream.
  int32_t region_epoch =
  ...
  uint32_t my_tiles = iree_hal_cmd_block_processor_process_region(
      barrier, region_epoch, worker_context, context);
  if (my_tiles == 0) {
  ...
  // Completer election via remaining_tiles fetch-sub.
  //
  // Workers with 0 tiles skip the election to avoid false positives.
  // Workers with tiles decrement the count. The worker whose decrement drives
  // the count to zero becomes the completer. Stale workers cannot corrupt the
  // next region's count because the region-drainer gate is closed before the
  // completer resets remaining_tiles.
  bool is_completer = false;
  if (my_tiles > 0) {
    const int64_t previous_remaining = iree_atomic_fetch_sub(
        &state->remaining_tiles, (int64_t)my_tiles, iree_memory_order_acq_rel);
    IREE_ASSERT((previous_remaining >> 32) == region_epoch);
    const uint32_t previous_count =
        (uint32_t)(previous_remaining & 0xFFFFFFFFu);
    IREE_ASSERT(previous_count >= my_tiles);
    const uint32_t new_count = previous_count - my_tiles;
    IREE_TRACE(out_result->remaining_tiles = new_count);
    is_completer = new_count == 0;
  }

  if (!is_completer) goto leave_region_drainer;
  IREE_TRACE(out_result->reason =
                 IREE_HAL_CMD_BLOCK_PROCESSOR_DRAIN_REASON_COMPLETER);

  // === COMPLETER ===
  // Close the region-drainer gate before rewriting .data. This protects tile
  // counters and ordinary shared .data reads such as binding pointers and
  // cached barriers.
  iree_hal_cmd_block_processor_begin_region_transition(context);
  ...
  iree_hal_cmd_block_processor_end_region_transition(context);
  iree_hal_cmd_block_processor_wake_additional_workers(context,
                                                       out_result->wake_delta);
  iree_hal_cmd_block_processor_leave_region_drainer(context);
