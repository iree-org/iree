...
iree_status_t iree_hal_amdgpu_resolve_buffer(
    ...
    uint64_t* out_bits) {
  // DO NOT SUBMIT
  // out_device_ref->type = IREE_HAL_AMDGPU_DEVICE_BUFFER_TYPE_PTR;
  // out_device_ref->value.ptr = NULL;
  // out_device_ref->type = IREE_HAL_AMDGPU_DEVICE_BUFFER_TYPE_HANDLE;
  // out_device_ref->value.handle = NULL;
...
iree_status_t iree_hal_amdgpu_resolve_transient_buffer(
  ...
  // DO NOT SUBMIT
  // iree_hal_amdgpu_device_buffer_type_t buffer_type = 0;
  // uint64_t buffer_bits = 0;
  // IREE_RETURN_IF_ERROR(
  //     iree_hal_amdgpu_resolve_buffer(buffer, &buffer_type, &buffer_bits));
  // if (buffer_type != IREE_HAL_AMDGPU_DEVICE_BUFFER_TYPE_HANDLE) {
  //   return iree_make_status(
  //       IREE_STATUS_INVALID_ARGUMENT,
  //       "provided buffer is not a transient allocation; only buffers
  //       allocated " "with iree_hal_device_queue_alloca can be deallocated
  //       using " "iree_hal_device_queue_dealloca");
  // }
  return iree_make_status(IREE_STATUS_UNIMPLEMENTED);
}
