...
typedef struct iree_hal_amdgpu_command_buffer_t {
  ...
  // Total number of devices in the system the command buffer was prepared for.
  // Not all devices may have a copy of the command buffer.
  iree_host_size_t device_count;
  // Sparse list of device-side copies of the command buffer allocation.
  // Only those ordinals that were specified in the original queue affinity
  // provided during creation will have copies.
  iree_hal_amdgpu_device_command_buffer_t*
      device_command_buffers[/*device_count*/];
} iree_hal_amdgpu_command_buffer_t;
...
iree_status_t iree_hal_amdgpu_command_buffer_create(
  ...
  *out_command_buffer = NULL;
  ...
  // TODO(benvanik): allocate any additional resources for managing command
...
static void iree_hal_amdgpu_command_buffer_destroy(
  ...
  // TODO(benvanik): release any implementation resources and
...
iree_status_t iree_hal_amdgpu_command_buffer_query_execution_state(
  ...
  if (IREE_UNLIKELY(device_ordinal >= command_buffer->device_count)) {
    return iree_make_status(
        IREE_STATUS_OUT_OF_RANGE,
        "device ordinal %" PRIhsz
        " out of range; command buffer was allocated for %" PRIhsz " devices",
        device_ordinal, command_buffer->device_count);
  }

  iree_hal_amdgpu_device_command_buffer_t* device_command_buffer =
      command_buffer->device_command_buffers[device_ordinal];
...
static iree_status_t iree_hal_amdgpu_command_buffer_begin(
  ...
  // TODO(benvanik): if the implementation needs to route the begin to the
...
static iree_status_t iree_hal_amdgpu_command_buffer_end(
  ...
  // TODO(benvanik): if recording requires multiple passes any fixup/linking can
...
static void iree_hal_amdgpu_command_buffer_begin_debug_group(
  ...
  // TODO(benvanik): begin a nested debug group (push) if the implementation has
  // a way to insert markers. This is informational and can be ignored.
  (void)command_buffer;
}

static void iree_hal_amdgpu_command_buffer_end_debug_group(
    iree_hal_command_buffer_t* base_command_buffer) {
  iree_hal_amdgpu_command_buffer_t* command_buffer =
      iree_hal_amdgpu_command_buffer_cast(base_command_buffer);

  // TODO(benvanik): end a nested debug group (pop). Always called 1:1 in stack
...
static iree_status_t iree_hal_amdgpu_command_buffer_execution_barrier(
  ...
  // TODO(benvanik): barriers split the execution sequence into all operations
...
static iree_status_t iree_hal_amdgpu_command_buffer_discard_buffer(
    iree_hal_command_buffer_t* base_command_buffer,
...
static iree_status_t iree_hal_amdgpu_command_buffer_fill_buffer(
  ...
  // TODO(benvanik): memset on the buffer. The pattern_length is 1, 2, or 4
...
static iree_status_t iree_hal_amdgpu_command_buffer_update_buffer(
  ...
  // TODO(benvanik): embed and copy a small (~64KB) chunk of host memory to the
...
static iree_status_t iree_hal_amdgpu_command_buffer_copy_buffer(
  ...
  // TODO(benvanik): memcpy between two buffers. The buffers must both be
...
static iree_status_t iree_hal_amdgpu_command_buffer_dispatch(
  ...
  // TODO(benvanik): dispatch the specified executable entry point with the
  // given workgroup count. The constants must be copied into the command buffer
  // as they may be mutated or freed after this call returns. Note that any of
...
static iree_status_t iree_hal_amdgpu_command_buffer_dispatch_indirect(
  ...
  // TODO(benvanik): dispatch the specified executable entry point with a
  ...
  // the dispatch. The constants must be copied into the command buffer as they
  // may be mutated or freed after this call returns. Note that any of the
  // bindings may be references to binding table slots in which case they will
  // be provided during submission to a queue.
  (void)command_buffer;
  iree_status_t status = iree_make_status(IREE_STATUS_UNIMPLEMENTED,
...
static const iree_hal_command_buffer_vtable_t
    iree_hal_amdgpu_command_buffer_vtable = {
        ...
        .discard_buffer = iree_hal_amdgpu_command_buffer_discard_buffer,
