...
TEST_F(PhysicalDeviceCapabilitiesTest,
       KeepsAqlprofileSpecializedFamiliesOnAqlPm4IbPath) {
  struct VersionCase {
    const char* name;
    iree_hal_amdgpu_gfxip_version_t version;
  };
  const std::array<VersionCase, 5> versions = {{
      {"gfx950", GfxIp(9, 5, 0)},
      {"gfx1150", GfxIp(11, 5, 0)},
      {"gfx1170", GfxIp(11, 7, 0)},
      {"gfx1201", GfxIp(12, 0, 1)},
      {"gfx1251", GfxIp(12, 5, 1)},
  }};
  ...
  for (const VersionCase& version : versions) {
    ...
    iree_hal_amdgpu_vendor_packet_capability_flags_t capabilities =
        iree_hal_amdgpu_select_vendor_packet_capabilities(version.version);
    EXPECT_TRUE(iree_any_bit_set(
