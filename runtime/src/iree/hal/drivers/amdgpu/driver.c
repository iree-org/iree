...
// TODO(benvanik): if it's possible to have more than one device use real IDs.
...
void iree_hal_amdgpu_driver_options_initialize(
  ...
  iree_hal_amdgpu_device_options_initialize(
...
IREE_API_EXPORT iree_status_t iree_hal_amdgpu_driver_create(
  ...
  *out_driver = NULL;
  ...
  // TODO(benvanik): load libraries and query driver support from the system.
...
static void iree_hal_amdgpu_driver_destroy(iree_hal_driver_t* base_driver) {
  ...
  // TODO(benvanik): if the driver loaded any libraries they should be closed
  // here.
...
static iree_status_t iree_hal_amdgpu_driver_query_available_devices(
  ...
  // TODO(benvanik): query available devices and populate the output. Note that
  ...
  static const iree_hal_device_info_t device_infos[1] = {
      {
          .device_id = IREE_HAL_AMDGPU_DEVICE_ID_DEFAULT,
...
static iree_status_t iree_hal_amdgpu_driver_dump_device_info(
  ...
  // TODO(benvanik): add useful user-level information to the string builder for
...
static iree_status_t iree_hal_amdgpu_driver_create_device_by_id(
  ...
  // TODO(benvanik): use the provided params to overwrite the default options.
  ...
  iree_hal_amdgpu_device_options_t options =
      driver->options.default_device_options;

  // TODO(benvanik): implement creation by device_id; this is mostly used as
  ...
  return iree_hal_amdgpu_device_create(driver->identifier, &options,
...
static iree_status_t iree_hal_amdgpu_driver_create_device_by_path(
  ...
  // TODO(benvanik): support parsing of the device_path. Note that a single
