...
iree_status_t iree_hal_amdgpu_executable_cache_create(
  ...
  *out_executable_cache = NULL;
  ...
  // TODO(benvanik): this default implementation is a no-op; if the
  // implementation supports caching or has prohibitively expensive executable
  // load times it is worth implementing this. Here any shared resources
  // (compiler/JIT handles, device symbols, etc) can be retained and passed to
  // all executables created from the cache.
...
static bool iree_hal_amdgpu_executable_cache_can_prepare_format(
  ...
  // TODO(benvanik): this query may be used with multiple executable_format args
  // in cases where support is conditional (versions of a format) or multiple
...
static iree_status_t iree_hal_amdgpu_executable_cache_prepare_executable(
  ...
  // TODO(benvanik): add any extra args required. Usually this will be device
