  ...
  *out_semaphore = NULL;
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_allocator_malloc(host_allocator, sizeof(*semaphore),
                                (void**)&semaphore));
  iree_hal_semaphore_initialize(&iree_hal_amdgpu_semaphore_vtable,
                                &semaphore->base);
  semaphore->host_allocator = host_allocator;

  // TODO(benvanik): implement semaphores. Note that there is some basic support
...
static iree_status_t iree_hal_amdgpu_semaphore_query(
...
static void iree_hal_amdgpu_semaphore_fail(iree_hal_semaphore_t* base_semaphore,
                                           iree_status_t status) {
  iree_hal_amdgpu_semaphore_t* semaphore =
      iree_hal_amdgpu_semaphore_cast(base_semaphore);
...
static const iree_hal_semaphore_vtable_t iree_hal_amdgpu_semaphore_vtable = {
    ...
    .fail = iree_hal_amdgpu_semaphore_fail,
