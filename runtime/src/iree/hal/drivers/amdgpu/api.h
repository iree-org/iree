...
// Parameters configuring an iree_hal_amdgpu_device_t.
// Must be initialized with iree_hal_amdgpu_device_options_initialize prior to
// use.
typedef struct iree_hal_amdgpu_device_options_t {
  // TODO(benvanik): options for initializing a device such as hardware
  // identifiers, implementation mode switches, and debugging control.
  int reserved;
} iree_hal_amdgpu_device_options_t;

// Initializes |out_params| to default values.
IREE_API_EXPORT void iree_hal_amdgpu_device_options_initialize(
    iree_hal_amdgpu_device_options_t* out_params);

// Creates a AMDGPU HAL device with the given |params|.
//
...
typedef struct iree_hal_amdgpu_driver_options_t {
  // TODO(benvanik): options for initializing the driver such as library search
  // paths, version min/max, etc.

  // Default device options when none are provided during device creation.
  iree_hal_amdgpu_device_options_t default_device_options;
