...
#include "iree/schemas/hsa_executable_def_reader.h"
#include "iree/schemas/hsa_executable_def_verifier.h"
...
static iree_status_t iree_hal_amdgpu_verify_isas_equal(
  ...
  IREE_RETURN_IF_ERROR(iree_hsa_isa_get_info_alt(
      libhsa, isa_a, HSA_ISA_INFO_NAME_LENGTH, &name_length_a));
  uint32_t name_length_b = 0;
  IREE_RETURN_IF_ERROR(iree_hsa_isa_get_info_alt(
      libhsa, isa_a, HSA_ISA_INFO_NAME_LENGTH, &name_length_b));
  char name_a[64] = {0};
  char name_b[64] = {0};
  ...
  IREE_RETURN_IF_ERROR(
      iree_hsa_isa_get_info_alt(libhsa, isa_a, HSA_ISA_INFO_NAME, &name_a));
  IREE_RETURN_IF_ERROR(
      iree_hsa_isa_get_info_alt(libhsa, isa_a, HSA_ISA_INFO_NAME, &name_b));
  if (name_length_a != name_length_b ||
      memcmp(name_a, name_b, name_length_a) != 0) {
    return iree_make_status(
        IREE_STATUS_FAILED_PRECONDITION, "ISAs do not match: `%.*s` != `%.*s`",
        (int)name_length_a, name_a, (int)name_length_b, name_b);
  }
...
iree_status_t iree_hal_amdgpu_verify_device_isa_commonality(
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hsa_agent_iterate_isas(libhsa, topology->gpu_agents[0],
  ...
  for (iree_host_size_t i = 1; i < topology->gpu_agent_count; ++i) {
    ...
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, iree_hsa_agent_iterate_isas(libhsa, topology->gpu_agents[i],
                                        iree_hal_amdgpu_iterate_agent_isa,
                                        &available_isas));
...
iree_status_t iree_hal_amdgpu_executable_format_supported(
  ...
  *out_supported = false;
  ...
  // Strip hsa-* prefix.
  if (!iree_string_view_consume_prefix(&format,
                                       iree_make_cstring_view("hsa-"))) {
    // Not HSA-like.
    *out_supported = false;
    return iree_ok_status();
  }
  ...
  IREE_RETURN_IF_ERROR(iree_hsa_agent_iterate_isas(
      libhsa, device_agent, iree_hal_amdgpu_iterate_agent_isa,
      &available_isas));
  ...
  for (iree_host_size_t i = 0; i < available_isas.count; ++i) {
    ...
    IREE_RETURN_IF_ERROR(iree_hsa_isa_get_info_alt(
        libhsa, isa, HSA_ISA_INFO_NAME_LENGTH, &isa_name_length));
    char* isa_name_buffer = iree_alloca(isa_name_length);
    IREE_RETURN_IF_ERROR(iree_hsa_isa_get_info_alt(
        libhsa, isa, HSA_ISA_INFO_NAME, isa_name_buffer));
...
static iree_status_t iree_hal_amdgpu_query_device_limits(
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0,
      iree_hsa_isa_get_info_alt(libhsa, isa, HSA_ISA_INFO_WORKGROUP_MAX_SIZE,
                                &out_limits->max_workgroup_size));
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hsa_isa_get_info_alt(libhsa, isa, HSA_ISA_INFO_WORKGROUP_MAX_DIM,
...
static iree_status_t iree_hal_amdgpu_executable_flatbuffer_verify(
  ...
  int verify_ret = iree_hal_hsa_ExecutableDef_verify_as_root(
  ...
  iree_hal_hsa_ExecutableDef_table_t executable_def =
      iree_hal_hsa_ExecutableDef_as_root(flatbuffer_data.data);

  iree_hal_hsa_ModuleDef_vec_t modules_vec =
      iree_hal_hsa_ExecutableDef_modules_get(executable_def);
  iree_host_size_t module_count = iree_hal_hsa_ModuleDef_vec_len(modules_vec);
  for (iree_host_size_t i = 0; i < module_count; ++i) {
    iree_hal_hsa_ModuleDef_table_t module_def =
        iree_hal_hsa_ModuleDef_vec_at(modules_vec, i);
    if (!module_def) {
      return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                              "modules[%" PRIhsz "] is NULL", i);
    }
    if (flatbuffers_string_len(
            iree_hal_hsa_ModuleDef_elf_image_get(module_def)) == 0) {
      return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                              "modules[%" PRIhsz "] contents are empty", i);
    }
  }

  iree_hal_hsa_ExportDef_vec_t exports_vec =
      iree_hal_hsa_ExecutableDef_exports_get(executable_def);
  for (iree_host_size_t i = 0; i < iree_hal_hsa_ExportDef_vec_len(exports_vec);
       ++i) {
    iree_hal_hsa_ExportDef_table_t export_def =
        iree_hal_hsa_ExportDef_vec_at(exports_vec, i);
    if (!export_def) continue;

    if (flatbuffers_string_len(
            iree_hal_hsa_ExportDef_kernel_name_get(export_def)) == 0) {
      return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                              "exports[%" PRIhsz "] name is empty", i);
    }

    if (iree_hal_hsa_ExportDef_workgroup_size_is_present(export_def)) {
      const iree_hal_hsa_Dims_struct_t workgroup_size =
          iree_hal_hsa_ExportDef_workgroup_size_get(export_def);
    ...
    uint32_t constant_count =
        iree_hal_hsa_ExportDef_constant_count_get(export_def);
    ...
    iree_hal_hsa_BindingBits_vec_t binding_flags_vec =
        iree_hal_hsa_ExportDef_binding_flags_get(export_def);
    if (iree_hal_hsa_BindingBits_vec_len(binding_flags_vec) >
        IREE_HAL_AMDGPU_MAX_DISPATCH_BINDING_COUNT) {
      return iree_make_status(
          IREE_STATUS_INVALID_ARGUMENT,
          "exports[%" PRIhsz "] binding_flags count %zu exceeds maximum of %u",
          i, iree_hal_hsa_BindingBits_vec_len(binding_flags_vec),
          IREE_HAL_AMDGPU_MAX_DISPATCH_BINDING_COUNT);
    }

    IREE_RETURN_IF_ERROR(iree_hal_debug_verify_export_def(
        iree_hal_hsa_ExportDef_debug_info_get(export_def)));
  }
...
static iree_status_t iree_hal_amdgpu_executable_load_modules(
    ...
    iree_hal_hsa_ModuleDef_vec_t module_defs, hsa_executable_t* out_handle) {
  IREE_TRACE_ZONE_BEGIN(z0);
  *out_handle = (hsa_executable_t){0};
  ...
  if (iree_hal_hsa_ModuleDef_vec_len(module_defs) != 1) {
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, iree_make_status(IREE_STATUS_UNIMPLEMENTED,
                             ...
                             iree_hal_hsa_ModuleDef_vec_len(module_defs)));
  }
  flatbuffers_string_t elf_image = iree_hal_hsa_ModuleDef_elf_image_get(
      iree_hal_hsa_ModuleDef_vec_at(module_defs, 0));
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hsa_code_object_reader_create_from_memory(
              libhsa, elf_image, flatbuffers_string_len(elf_image),
              &code_object_reader));
  ...
  if (iree_status_is_ok(status)) {
    for (iree_host_size_t i = 0; i < topology->gpu_agent_count; ++i) {
      status = iree_hsa_executable_load_agent_code_object(
          libhsa, handle, topology->gpu_agents[i], code_object_reader, options,
          NULL);
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hsa_executable_freeze(libhsa, handle, options);
  }
  ...
  if (iree_status_is_ok(status)) {
    *out_handle = handle;
    ...
    IREE_IGNORE_ERROR(iree_hsa_executable_destroy(libhsa, handle));
  }
  IREE_TRACE_ZONE_END(z0);
  return status;
}
...
static iree_status_t iree_hal_amdgpu_executable_resolve_kernel_args(
  ...
  const char* symbol_name = iree_hal_hsa_ExportDef_kernel_name_get(export_def);
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hsa_executable_get_symbol_by_name(
  ...
  iree_hal_hsa_BindingBits_vec_t binding_bits =
      iree_hal_hsa_ExportDef_binding_flags_get(export_def);
  out_kernel_args->binding_count =
      (uint16_t)iree_hal_hsa_BindingBits_vec_len(binding_bits);
  out_kernel_args->constant_count =
      (uint16_t)iree_hal_hsa_ExportDef_constant_count_get(export_def);
...
static iree_status_t iree_hal_amdgpu_executable_intern_trace_locs(
    iree_hal_hsa_ExportDef_vec_t export_defs, iree_allocator_t host_allocator,
    iree_hal_amdgpu_trace_src_loc_t** out_export_locs) {
  IREE_TRACE_ZONE_BEGIN(z0);

  iree_host_size_t export_count = iree_hal_hsa_ExportDef_vec_len(export_defs);
  ...
  for (iree_host_size_t i = 0; i < export_count; ++i) {
    iree_hal_hsa_ExportDef_table_t export_def =
        iree_hal_hsa_ExportDef_vec_at(export_defs, i);
  ...
  iree_status_t status =
      iree_allocator_malloc(host_allocator, total_size, (void**)&export_locs);
  IREE_LEAK_CHECK_DISABLE_POP();
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, status, "allocating persistent source location storage");
  char* char_buffer = (char*)&export_locs[export_count];

  // Populate table and fill the buffer. The only pointers used are those into
...
typedef struct iree_hal_amdgpu_find_kernel_table_memory_pool_state_t {
  const iree_hal_amdgpu_libhsa_t* libhsa;
  hsa_amd_memory_pool_t best_pool;
} iree_hal_amdgpu_find_kernel_table_memory_pool_state_t;

// Finds a memory pool appropriate for storing the kernel argument table on a
// particular agent.
static hsa_status_t iree_hal_amdgpu_find_kernel_table_memory_pool(
    hsa_amd_memory_pool_t memory_pool, void* user_data) {
  iree_hal_amdgpu_find_kernel_table_memory_pool_state_t* state =
      (iree_hal_amdgpu_find_kernel_table_memory_pool_state_t*)user_data;
  ...
  // Coarse-grained is fine as the memory is immutable once uploaded.
...
static iree_status_t iree_hal_amdgpu_executable_upload_kernel_table(
    const iree_hal_amdgpu_libhsa_t* libhsa, hsa_executable_t executable,
    iree_hal_hsa_ExportDef_vec_t export_defs, iree_host_size_t kernel_count,
    iree_hal_amdgpu_device_kernel_args_t* host_kernel_args,
    hsa_agent_t device_agent,
    const iree_hal_amdgpu_device_kernel_args_t** out_device_kernel_args) {
  IREE_TRACE_ZONE_BEGIN(z0);
  *out_device_kernel_args = NULL;
  ...
  for (iree_host_size_t kernel_ordinal = 0; kernel_ordinal < kernel_count;
       ++kernel_ordinal) {
    iree_hal_hsa_ExportDef_table_t export_def =
        iree_hal_hsa_ExportDef_vec_at(export_defs, kernel_ordinal);
    const char* symbol_name =
        iree_hal_hsa_ExportDef_kernel_name_get(export_def);

    hsa_executable_symbol_t symbol = {0};
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0,
        iree_hsa_executable_get_symbol_by_name(libhsa, executable, symbol_name,
                                               &device_agent, &symbol),
  ...
  iree_hal_amdgpu_find_kernel_table_memory_pool_state_t find_state = {
      .libhsa = libhsa,
      .best_pool = {0},
  };
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0,
      iree_hsa_amd_agent_iterate_memory_pools(
          libhsa, device_agent, iree_hal_amdgpu_find_kernel_table_memory_pool,
          &find_state),
      "finding memory pool for storing kernel arg tables");
  if (!find_state.best_pool.handle) {
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, iree_make_status(IREE_STATUS_NOT_FOUND,
                             "no memory pool for storing kernel arg tables on "
                             "the agent is available"));
  }
  hsa_amd_memory_pool_t memory_pool = find_state.best_pool;
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hsa_amd_memory_pool_allocate(
              libhsa, memory_pool, kernel_args_table_size,
              HSA_AMD_MEMORY_POOL_STANDARD_FLAG, (void**)device_kernel_args));

  // Copy the entire table to device memory.
  iree_status_t status = iree_hsa_memory_copy(
      libhsa, device_kernel_args, host_kernel_args, kernel_args_table_size);

  if (iree_status_is_ok(status)) {
    *out_device_kernel_args = device_kernel_args;
  } else {
    IREE_IGNORE_ERROR(
        iree_hsa_amd_memory_pool_free(libhsa, device_kernel_args));
  }
  IREE_TRACE_ZONE_END(z0);
  return status;
}
...
iree_status_t iree_hal_amdgpu_executable_create(
  ...
  *out_executable = NULL;
  ...
  iree_hal_hsa_ExecutableDef_table_t executable_def =
      iree_hal_hsa_ExecutableDef_as_root(
          executable_params->executable_data.data);
  iree_hal_hsa_ExportDef_vec_t export_defs =
      iree_hal_hsa_ExecutableDef_exports_get(executable_def);
  const iree_host_size_t export_count =
      iree_hal_hsa_ExportDef_vec_len(export_defs);
  ...
  iree_hal_debug_publish_source_files(
      iree_hal_hsa_ExecutableDef_source_files_get(executable_def));
...
static void iree_hal_amdgpu_executable_destroy(
  ...
  for (iree_host_size_t device_ordinal = 0;
    ...
    IREE_IGNORE_ERROR(iree_hsa_amd_memory_pool_free(
        executable->libhsa,
        (void*)executable->device_kernel_args[device_ordinal]));
  }

  IREE_IGNORE_ERROR(
      iree_hsa_executable_destroy(executable->libhsa, executable->handle));
...
iree_status_t iree_hal_amdgpu_executable_lookup_export(
  ...
  *out_host_kernel_args = &executable->host_kernel_args[entry_point];
