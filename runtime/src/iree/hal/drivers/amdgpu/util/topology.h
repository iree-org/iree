...
// Initializes |out_topology| with a set of devices indicated by the
// |device_mask| bitfield.
//
// Each device bit matches the HSA agent ordinal within the GPU devices
// enumerated from the runtime. `ROCR_VISIBLE_DEVICES` will be used to filter
// the visible devices and change the ordinal collection;
