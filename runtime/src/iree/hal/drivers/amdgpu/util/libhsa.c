...
#define IREE_HAL_AMDGPU_LIBHSA_PFN(result_type, symbol, ...) \
...
#if IREE_HAL_AMDGPU_LIBHSA_TRACING
#define IREE_HAL_AMDGPU_LIBHSA_TRACE_ZONE_BEGIN IREE_TRACE_ZONE_BEGIN
#define IREE_HAL_AMDGPU_LIBHSA_TRACE_ZONE_END IREE_TRACE_ZONE_END
#else
#define IREE_HAL_AMDGPU_LIBHSA_TRACE_ZONE_BEGIN(...)
#define IREE_HAL_AMDGPU_LIBHSA_TRACE_ZONE_END(...)
...
#define IREE_HAL_AMDGPU_LIBHSA_PFN_hsa_status_t(result_type, symbol, decl,  \
  ...
  iree_status_t iree_##symbol(const iree_hal_amdgpu_libhsa_t* IREE_RESTRICT \
    ...
    IREE_HAL_AMDGPU_LIBHSA_TRACE_ZONE_BEGIN(z0);                            \
                                                                            \
      ...
      iree_status =                                                         \
          iree_make_status(iree_hsa_status_code(hsa_status), "[%s] %s: %s", \
                           ...
                           iree_hsa_status_description(hsa_status));        \
                                                                            ...
                                                                            \
    IREE_HAL_AMDGPU_LIBHSA_TRACE_ZONE_END(z0);                              \
    return iree_status;                                                     \
  }
...
#define IREE_HAL_AMDGPU_LIBHSA_PFN_void(result_type, symbol, decl, args)  \
...
#define IREE_HAL_AMDGPU_LIBHSA_PFN_primitive(result_type, symbol, decl, args) \
