...
typedef struct iree_hal_amdgpu_block_t {
  ...
  // Initially NULL on blocks acquired from the pool.
...
// Size of the user data field in a block.
...
// A shared pool of equal-sized blocks in device memory.
...
// linked list to store the block in their own lists (iovecs, etc) we provide
...
// Initializes the block pool to allocate from the given |agent| memory
// |memory_pool|. Each block will have the same block size and multiple blocks
// may be allocated at the same time to hit the pool allocation granularity.
//
// If an initial block capacity is provided the block pool will make its initial
// growth allocation to have the given number of blocks available for use.
//
...
// All blocks must have been acquired from the pool.
...
// The pools may be configured differently in their usage based on who owns them
// but generally follow the same bucketing strategy.
...
typedef struct iree_hal_amdgpu_block_arena_t {
  ...
  // Blocks are appened in FIFO order.
  iree_hal_amdgpu_block_t* block_head;
...
// is stored in the allocating data structures and each block already has 256
// bits of user data storage available for use.
//
...
// tables). If fragmentation becomes an issue we can bucket the free list in
...
// their size class so as to avoid overallocation/underutilization: allocating 1
// byte from the large block pool would acquire an entire large block that would
// be nearly entirely unused if we didn't do that first-level filtering.
//
