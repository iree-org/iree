  ...
  *out_file_toc = NULL;
  ...
  for (iree_host_size_t i = 0; i < available_isas.count && !best_file_toc;
    ...
    iree_string_view_t isa_name =
        iree_make_string_view(isa_name_buffer, isa_name_length);
  ...
  if (best_file_toc) {
    ...
    *out_file_toc = best_file_toc;
    ...
    status = iree_status_annotate(status, iree_string_builder_view(&builder));
...
iree_status_t iree_hal_amdgpu_device_library_initialize(
  ...
  if (topology->gpu_agent_count == 0) {
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                             "must have at least 1 GPU agent"));
  }
  hsa_isa_t isa = {0};
  ...
  hsa_code_object_reader_destroy(code_object_reader);
...
iree_status_t iree_hal_amdgpu_device_library_populate_agent_code_range(
  ...
  IREE_ASSERT_ARGUMENT(library);
  IREE_ASSERT_ARGUMENT(out_code_base);
  IREE_ASSERT_ARGUMENT(out_code_size);
  IREE_TRACE_ZONE_BEGIN(z0);
  *out_code_base = 0;
  *out_code_size = 0;
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_amdgpu_find_loaded_code_object(
              library->libhsa, library->executable, device_agent,
              &loaded_code_object));
  ...
  if (hsa_status == HSA_STATUS_SUCCESS) {
    hsa_status = library->libhsa->amd_loader
                     .hsa_ven_amd_loader_loaded_code_object_get_info(
                         loaded_code_object,
...
static iree_status_t iree_hal_amdgpu_device_library_populate_kernel_args(
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hsa_executable_get_symbol_by_name(
              library->libhsa, library->executable, symbol_name, &device_agent,
              &symbol));
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hsa_executable_symbol_get_info(
              library->libhsa, symbol, HSA_EXECUTABLE_SYMBOL_INFO_KERNEL_OBJECT,
