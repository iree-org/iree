...
typedef struct IREE_AMDGPU_ALIGNAS(8) iree_hal_amdgpu_trace_zone_begin_s {
...
typedef struct IREE_AMDGPU_ALIGNAS(8) iree_hal_amdgpu_trace_zone_end_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_ZONE_END
  iree_hal_amdgpu_trace_event_type_t event_type;
  uint8_t reserved[7];  // may be uninitialized
...
typedef struct IREE_AMDGPU_ALIGNAS(8) iree_hal_amdgpu_trace_zone_value_i64_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_ZONE_VALUE_I64
  iree_hal_amdgpu_trace_event_type_t event_type;
  uint8_t reserved[7];  // may be uninitialized
...
typedef struct IREE_AMDGPU_ALIGNAS(8)
    iree_hal_amdgpu_trace_execution_zone_end_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_EXECUTION_ZONE_END
  iree_hal_amdgpu_trace_event_type_t event_type;
  // Execution trace ID used to distinguish different execution units. This is
  // assigned on the host when the execution context is configured.
...
typedef struct IREE_AMDGPU_ALIGNAS(8) iree_hal_amdgpu_trace_memory_alloc_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_MEMORY_ALLOC
  iree_hal_amdgpu_trace_event_type_t event_type;
...
typedef struct IREE_AMDGPU_ALIGNAS(8) iree_hal_amdgpu_trace_memory_free_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_MEMORY_FREE
  iree_hal_amdgpu_trace_event_type_t event_type;
  uint8_t reserved[7];
  // Pool name used as both a title for the pool and the unique ID for
...
typedef struct IREE_AMDGPU_ALIGNAS(8) iree_hal_amdgpu_trace_message_literal_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_MESSAGE_LITERAL
  iree_hal_amdgpu_trace_event_type_t event_type;
...
// The contents are embedded in the trace buffer to support dynamically
// generated values.
typedef struct IREE_AMDGPU_ALIGNAS(8) iree_hal_amdgpu_trace_message_dynamic_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_MESSAGE_DYNAMIC
  iree_hal_amdgpu_trace_event_type_t event_type;
  uint8_t reserved[3];
  // Length of the value in characters.
...
typedef struct IREE_AMDGPU_ALIGNAS(8) iree_hal_amdgpu_trace_plot_config_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_PLOT_CONFIG
  iree_hal_amdgpu_trace_event_type_t event_type;
...
typedef struct IREE_AMDGPU_ALIGNAS(8) iree_hal_amdgpu_trace_plot_value_i64_s {
  // IREE_HAL_AMDGPU_TRACE_EVENT_PLOT_VALUE_I64
  iree_hal_amdgpu_trace_event_type_t event_type;
...
#define IREE_AMDGPU_TRACE_BUFFER_DEFINE(trace_buffer)                 \
...
#define IREE_AMDGPU_TRACE_BUFFER_DEFINE(...)
...
#define IREE_AMDGPU_TRACE_MESSAGE_LITERAL(level, value_literal)              \
  IREE_AMDGPU_TRACE_MESSAGE_COLORED(IREE_AMDGPU_TRACE_MESSAGE_LEVEL_##level, \
                                    (value_literal))
