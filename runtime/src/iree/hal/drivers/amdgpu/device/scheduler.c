...
static void iree_hal_amdgpu_device_queue_issue_initialize(
    iree_hal_amdgpu_device_queue_scheduler_t* IREE_AMDGPU_RESTRICT scheduler,
    iree_hal_amdgpu_device_queue_initialize_entry_t* IREE_AMDGPU_RESTRICT
        entry) {
  IREE_AMDGPU_TRACE_BUFFER_DEFINE(scheduler->trace_buffer);
...
static void iree_hal_amdgpu_device_queue_issue_deinitialize(
    iree_hal_amdgpu_device_queue_scheduler_t* IREE_AMDGPU_RESTRICT scheduler,
    iree_hal_amdgpu_device_queue_deinitialize_entry_t* IREE_AMDGPU_RESTRICT
        entry) {
  IREE_AMDGPU_TRACE_BUFFER_DEFINE(scheduler->trace_buffer);
  IREE_AMDGPU_TRACE_ZONE_BEGIN(z0);
...
static void iree_hal_amdgpu_device_queue_issue_copy(
  ...
  void* IREE_AMDGPU_RESTRICT source_ptr =
  ...
  switch (entry->type) {
    ...
    case IREE_HAL_AMDGPU_DEVICE_QUEUE_ENTRY_TYPE_INITIALIZE:
      iree_hal_amdgpu_device_queue_issue_initialize(
          scheduler, (iree_hal_amdgpu_device_queue_initialize_entry_t*)entry);
      break;
    case IREE_HAL_AMDGPU_DEVICE_QUEUE_ENTRY_TYPE_DEINITIALIZE:
      iree_hal_amdgpu_device_queue_issue_deinitialize(
          scheduler, (iree_hal_amdgpu_device_queue_deinitialize_entry_t*)entry);
      break;
...
iree_hal_amdgpu_device_queue_scheduler_tick(
  ...
  IREE_AMDGPU_TRACE_BUFFER_DEFINE(scheduler->trace_buffer);
  IREE_AMDGPU_TRACE_ZONE_BEGIN(z0);
  IREE_AMDGPU_TRACE_ZONE_APPEND_TEXT_LITERAL(z0, action_set);
...
static void iree_hal_amdgpu_device_queue_scheduler_enqueue_packet(
  ...
  iree_hsa_kernel_dispatch_packet_t* IREE_AMDGPU_RESTRICT tick_packet =
      scheduler->control_queue->base_address + (packet_id & queue_mask) * 64;
