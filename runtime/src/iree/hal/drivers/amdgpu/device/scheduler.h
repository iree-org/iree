...
// max_kernarg_capacity: 0 (unused)
typedef struct iree_hal_amdgpu_device_queue_initialize_entry_s {
  // Type: IREE_HAL_AMDGPU_DEVICE_QUEUE_ENTRY_TYPE_INITIALIZE
...
//   command_buffer->max_kernarg_capacity
typedef struct iree_hal_amdgpu_device_queue_execute_entry_s {
  // Type: IREE_HAL_AMDGPU_DEVICE_QUEUE_ENTRY_TYPE_EXECUTE
  iree_hal_amdgpu_device_queue_entry_header_t header;
  // NOTE: most fields are setup when the queue entry is issued.
  // Enqueuers only need to populate:
  //   flags
  //   command_buffer
  //   bindings
