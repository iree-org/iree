...
#include "iree/hal/drivers/amdgpu/device.h"
...
typedef struct iree_hal_amdgpu_device_t {
...
} iree_hal_amdgpu_device_t;

static const iree_hal_device_vtable_t iree_hal_amdgpu_device_vtable;

static iree_hal_amdgpu_device_t* iree_hal_amdgpu_device_cast(
    iree_hal_device_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_amdgpu_device_vtable);
  return (iree_hal_amdgpu_device_t*)base_value;
}
...
iree_status_t iree_hal_amdgpu_device_create(
    iree_string_view_t identifier,
    const iree_hal_amdgpu_device_options_t* options,
  ...
  *out_device = NULL;
  ...
  iree_status_t status = iree_hal_amdgpu_allocator_create(
...
static void iree_hal_amdgpu_device_destroy(iree_hal_device_t* base_device) {
  iree_hal_amdgpu_device_t* device = iree_hal_amdgpu_device_cast(base_device);
  iree_allocator_t host_allocator = iree_hal_device_host_allocator(base_device);
  IREE_TRACE_ZONE_BEGIN(z0);

  // TODO(benvanik): release all implementation resources here. It's expected
...
static iree_string_view_t iree_hal_amdgpu_device_id(
    iree_hal_device_t* base_device) {
  iree_hal_amdgpu_device_t* device = iree_hal_amdgpu_device_cast(base_device);
  return device->identifier;
}

static iree_allocator_t iree_hal_amdgpu_device_host_allocator(
    iree_hal_device_t* base_device) {
  iree_hal_amdgpu_device_t* device = iree_hal_amdgpu_device_cast(base_device);
  return device->host_allocator;
}

static iree_hal_allocator_t* iree_hal_amdgpu_device_allocator(
    iree_hal_device_t* base_device) {
  iree_hal_amdgpu_device_t* device = iree_hal_amdgpu_device_cast(base_device);
...
static iree_status_t iree_hal_amdgpu_device_trim(
    iree_hal_device_t* base_device) {
  iree_hal_amdgpu_device_t* device = iree_hal_amdgpu_device_cast(base_device);
...
static iree_status_t iree_hal_amdgpu_device_query_i64(
    iree_hal_device_t* base_device, iree_string_view_t category,
    iree_string_view_t key, int64_t* out_value) {
  iree_hal_amdgpu_device_t* device = iree_hal_amdgpu_device_cast(base_device);
  *out_value = 0;

  // TODO(benvanik): implement additional queries. These are stubs for common
  ...
  if (iree_string_view_equal(category, IREE_SV("hal.executable.format"))) {
    ...
    // TODO(benvanik): match a format and return true.
    *out_value = 0;
    return iree_ok_status();
  }

  // TODO(benvanik): return basic queries for concurrency to allow programs to
...
static iree_status_t iree_hal_amdgpu_device_create_channel(
...
static iree_status_t iree_hal_amdgpu_device_create_command_buffer(
  ...
  iree_hal_amdgpu_device_t* device = iree_hal_amdgpu_device_cast(base_device);
  // TODO(benvanik): pass any additional resources required to create the
  // command buffer. The implementation could pool command buffers here.
...
static iree_status_t iree_hal_amdgpu_device_create_event(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
...
static iree_status_t iree_hal_amdgpu_device_create_executable_cache(
    iree_hal_device_t* base_device, iree_string_view_t identifier,
    iree_loop_t loop, iree_hal_executable_cache_t** out_executable_cache) {
  iree_hal_amdgpu_device_t* device = iree_hal_amdgpu_device_cast(base_device);

  // TODO(benvanik): pass any additional resources required during executable
...
static iree_status_t iree_hal_amdgpu_device_import_file(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
...
static iree_status_t iree_hal_amdgpu_device_create_semaphore(
    iree_hal_device_t* base_device, uint64_t initial_value,
    iree_hal_semaphore_flags_t flags, iree_hal_semaphore_t** out_semaphore) {
  iree_hal_amdgpu_device_t* device = iree_hal_amdgpu_device_cast(base_device);

  // TODO(benvanik): pass any additional resources required to create or track
...
iree_hal_amdgpu_device_query_semaphore_compatibility(
    iree_hal_device_t* base_device, iree_hal_semaphore_t* semaphore) {
  iree_hal_amdgpu_device_t* device = iree_hal_amdgpu_device_cast(base_device);

  // TODO(benvanik): return the appropriate bits for the provided semaphore
...
static iree_status_t iree_hal_amdgpu_device_queue_alloca(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
  ...
  // TODO(benvanik): perform an allocation of a transient buffer in queue order.
...
static iree_status_t iree_hal_amdgpu_device_queue_dealloca(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
    const iree_hal_semaphore_list_t signal_semaphore_list,
  ...
  // TODO(benvanik): perform a deallocation of the transient buffer in queue
...
static iree_status_t iree_hal_amdgpu_device_queue_read(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
    const iree_hal_semaphore_list_t signal_semaphore_list,
  ...
  // TODO(benvanik): if native support for file operations are available then
...
static iree_status_t iree_hal_amdgpu_device_queue_write(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
    const iree_hal_semaphore_list_t signal_semaphore_list,
...
static iree_status_t iree_hal_amdgpu_device_queue_execute(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
    const iree_hal_semaphore_list_t signal_semaphore_list,
  ...
  // TODO(benvanik): implement a wait, execute, and signal queue operation. The
  ...
  // TODO(benvanik): optional binding tables matching 1:1 with the command
  // buffers are provided for any indirect command buffers (those who have a
  ...
  // TODO(benvanik): do this async - callers may be submitting work to multiple
...
static iree_status_t iree_hal_amdgpu_device_queue_flush(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity) {
  iree_hal_amdgpu_device_t* device = iree_hal_amdgpu_device_cast(base_device);

  // TODO(benvanik): though buffering queue operations is not recommended if any
...
static iree_status_t iree_hal_amdgpu_device_wait_semaphores(
...
static iree_status_t iree_hal_amdgpu_device_profiling_begin(
    iree_hal_device_t* base_device,
...
static iree_status_t iree_hal_amdgpu_device_profiling_flush(
    iree_hal_device_t* base_device) {
  iree_hal_amdgpu_device_t* device = iree_hal_amdgpu_device_cast(base_device);
...
static iree_status_t iree_hal_amdgpu_device_profiling_end(
    iree_hal_device_t* base_device) {
  iree_hal_amdgpu_device_t* device = iree_hal_amdgpu_device_cast(base_device);
...
static const iree_hal_device_vtable_t iree_hal_amdgpu_device_vtable = {
    .destroy = iree_hal_amdgpu_device_destroy,
    .id = iree_hal_amdgpu_device_id,
    .host_allocator = iree_hal_amdgpu_device_host_allocator,
    .device_allocator = iree_hal_amdgpu_device_allocator,
    ...
    .trim = iree_hal_amdgpu_device_trim,
    .query_i64 = iree_hal_amdgpu_device_query_i64,
    .create_channel = iree_hal_amdgpu_device_create_channel,
    .create_command_buffer = iree_hal_amdgpu_device_create_command_buffer,
    .create_event = iree_hal_amdgpu_device_create_event,
    .create_executable_cache = iree_hal_amdgpu_device_create_executable_cache,
    .import_file = iree_hal_amdgpu_device_import_file,
    .create_semaphore = iree_hal_amdgpu_device_create_semaphore,
    .query_semaphore_compatibility =
        iree_hal_amdgpu_device_query_semaphore_compatibility,
    .queue_alloca = iree_hal_amdgpu_device_queue_alloca,
    .queue_dealloca = iree_hal_amdgpu_device_queue_dealloca,
    .queue_read = iree_hal_amdgpu_device_queue_read,
    .queue_write = iree_hal_amdgpu_device_queue_write,
    .queue_execute = iree_hal_amdgpu_device_queue_execute,
    .queue_flush = iree_hal_amdgpu_device_queue_flush,
    .wait_semaphores = iree_hal_amdgpu_device_wait_semaphores,
    .profiling_begin = iree_hal_amdgpu_device_profiling_begin,
    .profiling_flush = iree_hal_amdgpu_device_profiling_flush,
    .profiling_end = iree_hal_amdgpu_device_profiling_end,
};
