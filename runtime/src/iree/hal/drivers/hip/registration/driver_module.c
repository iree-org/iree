...
IREE_FLAG(bool, hip_disable_async_caching, false,
          "Disables caching for stream-ordered allocations")
...
static const iree_string_view_t key_hip_disable_async_caching =
    iree_string_view_literal("hip_disable_async_caching");
...
static iree_status_t iree_hal_hip_driver_parse_flags(
  ...
  IREE_RETURN_IF_ERROR(iree_string_pair_builder_add_int32(
      builder, key_hip_disable_async_caching, FLAG_hip_disable_async_caching));
  IREE_RETURN_IF_ERROR(iree_string_pair_builder_add_int32(
...
static iree_status_t iree_hal_hip_driver_populate_options(
  ...
  for (iree_host_size_t i = 0; i < pairs_size; ++i) {
    ...
    } else if (iree_string_view_equal(key, key_hip_disable_async_caching)) {
      if (!iree_string_view_atoi_int32(value, &ivalue)) {
        return iree_make_status(
            IREE_STATUS_FAILED_PRECONDITION,
            "Option 'hip_disable_async_caching' expected to be int Got: '%.*s'",
            (int)value.size, value.data);
      }
      device_params->disable_async_caching = ivalue ? true : false;
