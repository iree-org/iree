...
static iree_status_t iree_hal_cuda_stream_command_buffer_collective(
  ...
  // the first and ncclGroupEnd calls on the next barrier. The channel can be
  // used as a vtable if we want to inject collective APIs - the device creation
  // function would set up the channel once and we'll receive it here each time.
  return iree_make_status(IREE_STATUS_UNIMPLEMENTED,
