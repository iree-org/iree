...
// Ready action atomic slist entry struct.
typedef struct iree_hal_cuda_atomic_slist_completion_t {
  ...
  bool created_event;
  iree_atomic_slist_intrusive_ptr_t slist_next;
} iree_hal_cuda_atomic_slist_completion_t;

// Ready action atomic slist.
IREE_TYPED_ATOMIC_SLIST_WRAPPER(iree_hal_cuda_completion,
                                iree_hal_cuda_atomic_slist_completion_t,
                                offsetof(iree_hal_cuda_atomic_slist_completion_t,
                                         slist_next));
...
static void iree_hal_cuda_completion_slist_destroy(
    iree_hal_cuda_completion_slist_t* list,
    const iree_hal_cuda_dynamic_symbols_t* symbols,
    iree_allocator_t host_allocator) {
  while (true) {
    iree_hal_cuda_atomic_slist_completion_t* entry =
        iree_hal_cuda_completion_slist_pop(list);
    if (!entry) break;
    if (entry->created_event) {
      IREE_CUDA_IGNORE_ERROR(symbols, cuEventDestroy(entry->event));
    }
    iree_allocator_free(host_allocator, entry);
  }
  iree_hal_cuda_completion_slist_deinitialize(list);
}
...
typedef struct iree_hal_cuda_completion_area_t {
  ...
  // Notification to the parent thread to indicate the worker committed exiting.
  iree_notification_t exit_notification;
  iree_hal_cuda_completion_slist_t completion_list;  // atomic
  iree_atomic_int32_t worker_state;                 // atomic
...
static void iree_hal_cuda_completion_area_initialize(
  ...
  iree_hal_cuda_completion_slist_initialize(&completion_area->completion_list);
...
static void iree_hal_cuda_completion_area_deinitialize(
    iree_hal_cuda_completion_area_t* completion_area) {
  iree_hal_cuda_completion_slist_destroy(&completion_area->completion_list,
...
iree_status_t iree_hal_cuda_pending_queue_actions_create(
  ...
  iree_hal_cuda_working_area_initialize(host_allocator, device, symbols, working_area);
...
static iree_status_t iree_hal_cuda_pending_queue_actions_issue_execution(
  ...
  if (IREE_UNLIKELY(!completion_event)) {
    IREE_CUDA_RETURN_AND_END_ZONE_IF_ERROR(
         z0, symbols,
        cuEventCreate(&completion_event, CU_EVENT_DISABLE_TIMING),
        "cuEventCreate");
    created_event = true;
  }
  ...
  IREE_CUDA_RETURN_AND_END_ZONE_IF_ERROR(
    z0, symbols,
    cuEventRecord(completion_event, action->dispatch_cu_stream),
    "cuEventRecord");
  ...
  iree_hal_cuda_atomic_slist_completion_t* entry = NULL;
  ...
  iree_hal_cuda_completion_slist_push(
...
static iree_status_t iree_hal_cuda_worker_process_completion(
    iree_hal_cuda_completion_slist_t* worklist,
    iree_hal_cuda_completion_area_t* completion_area) {
  iree_status_t status = iree_ok_status();
  while (true) {
    iree_hal_cuda_atomic_slist_completion_t* entry =
        iree_hal_cuda_completion_slist_pop(worklist);
    if (!entry) break;
    ...
    if (IREE_UNLIKELY(result != CUDA_SUCCESS)) {
      ...
      // When we know all host stream callbacks are done and not touching
      // anything.
      iree_hal_cuda_completion_slist_push(worklist, entry);
      status =
...
static int iree_hal_cuda_completion_execute(
    iree_hal_cuda_completion_area_t* completion_area) {
  iree_hal_cuda_completion_slist_t* worklist = &completion_area->completion_list;
