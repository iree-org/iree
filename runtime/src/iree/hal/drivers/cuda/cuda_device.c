...
IREE_API_EXPORT iree_status_t iree_hal_cuda_nccl_query_group_params(
  ...
  MPI_RETURN_IF_ERROR(
      ...
      "MPI_Comm_rank");
  MPI_RETURN_IF_ERROR(
      mpi_syms, MPI_Comm_size(mpi_syms->ompi_mpi_comm_world, &out_params->count),
      "MPI_Comm_size");
  ...
  MPI_RETURN_IF_ERROR(mpi_syms,
                      MPI_Bcast(id, id_storage.data_length, mpi_syms->ompi_mpi_byte,
                                0, mpi_syms->ompi_mpi_comm_world),
                      "MPI_Bcast");
