...
static iree_status_t iree_hal_sync_device_queue_host_call(
  ...
  if (!iree_status_is_ok(call_status)) {
    ...
    call_status = iree_status_from_code(IREE_STATUS_INTERNAL);
  }
  // NOTE: call_status is invalid here. We don't return it as the HAL semantics
  // require the errors be asynchronous (tagged on semaphores).
