...
kernel void fill_buffer_1byte(device uint32_t* buffer [[buffer(0)]],
                              constant FillSpec& spec [[buffer(1)]],
                              uint id [[thread_position_in_grid]]) {
  // We split the full buffer fill range into three parts:
  // 1. Left bytes: containing (0 to 3) bytes before the first 4-byte aligned
  // address
  ...
  uint32_t middle_pattern =
      metal::rotate(spec.pattern, uint32_t(8 * left_byte_count));

  // Masks for left bytes and right bytes from rotated pattern. Note that for
