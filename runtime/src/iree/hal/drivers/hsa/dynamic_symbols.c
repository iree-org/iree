...
#include "iree/hal/drivers/hsa/dynamic_symbol_tables.h"  // IWYU pragma: keep
#undef IREE_HAL_HSA_REQUIRED_PFN_DECL
#undef IREE_HAL_HSA_OPTIONAL_PFN_DECL
  return iree_ok_status();
}

static bool iree_hal_hsa_try_load_dylib(const char* file_path,
...
iree_status_t iree_hal_hsa_dynamic_symbols_initialize(
  ...
  if (hsa_lib_search_path_count == 0) {
    // If no explicit search path, then have the system try to find the library
    // by filename alone.
    for (iree_host_size_t i = 0;
         i < IREE_ARRAYSIZE(iree_hal_hsa_dylib_names) && !loaded_one; ++i) {
      if (iree_hal_hsa_try_load_dylib(
              iree_hal_hsa_dylib_names[i], IREE_DYNAMIC_LIBRARY_FLAG_NONE,
    ...
    for (iree_host_size_t i = 0; i < hsa_lib_search_path_count && !loaded_one;
         ++i) {
      iree_string_view_t path_entry = hsa_lib_search_paths[i];
      ...
      if (iree_string_view_consume_prefix(&path_entry, file_prefix)) {
        ...
        for (iree_host_size_t j = 0;
             j < IREE_ARRAYSIZE(iree_hal_hsa_dylib_names) && !loaded_one; ++j) {
          ...
          status = iree_string_builder_append_string(
              &path_builder,
              iree_make_cstring_view(iree_hal_hsa_dylib_names[j]));
          if (!iree_status_is_ok(status)) break;
  ...
  if (iree_status_is_ok(status)) {
    if (loaded_one) {
      ...
      status = iree_make_status(
          ...
          "HSA runtime library 'libhsa-runtime64.so' not available: "
...
iree_status_t iree_hal_hsa_dynamic_symbols_append_path_to_builder(
  ...
  return iree_dynamic_library_append_symbol_path_to_builder(syms->hsa_init,
                                                            out_path);
}
