...
//===----------------------------------------------------------------------===//
// iree_hal_hsa_semaphore_state_t
//===----------------------------------------------------------------------===//

void iree_hal_hsa_semaphore_state_initialize(
    iree_hal_hsa_semaphore_state_t* out_shared_state) {
  memset(out_shared_state, 0, sizeof(*out_shared_state));
  iree_notification_initialize(&out_shared_state->notification);
}

void iree_hal_hsa_semaphore_state_deinitialize(
    iree_hal_hsa_semaphore_state_t* shared_state) {
...
iree_status_t iree_hal_hsa_semaphore_create(
    iree_hal_hsa_semaphore_state_t* shared_state, uint64_t initial_value,
...
static iree_status_t iree_hal_hsa_semaphore_signal_unsafe(
    iree_hal_hsa_semaphore_t* semaphore, uint64_t new_value) {
...
static iree_status_t iree_hal_hsa_semaphore_signal(
  ...
  iree_status_t status =
      iree_hal_hsa_semaphore_signal_unsafe(semaphore, new_value);
  if (!iree_status_is_ok(status)) {
    iree_slim_mutex_unlock(&semaphore->mutex);
    return status;
  }
...
iree_status_t iree_hal_hsa_semaphore_multi_signal(
    iree_hal_hsa_semaphore_state_t* shared_state,
  ...
  for (iree_host_size_t i = 0; i < semaphore_list.count; ++i) {
    ...
    iree_slim_mutex_lock(&semaphore->mutex);
    status = iree_hal_hsa_semaphore_signal_unsafe(
...
typedef struct iree_hal_hsa_semaphore_notify_state_t {
  iree_hal_hsa_semaphore_t* semaphore;
  uint64_t value;
} iree_hal_hsa_semaphore_notify_state_t;

static bool iree_hal_hsa_semaphore_is_signaled(
    iree_hal_hsa_semaphore_notify_state_t* state) {
  iree_hal_hsa_semaphore_t* semaphore = state->semaphore;
  iree_slim_mutex_lock(&semaphore->mutex);
...
static iree_status_t iree_hal_hsa_semaphore_wait(
  ...
  iree_hal_hsa_semaphore_state_t* shared_state = semaphore->shared_state;
  iree_hal_hsa_semaphore_notify_state_t notify_state = {
      .semaphore = semaphore,
      .value = value,
  };
  iree_notification_await(
      &shared_state->notification,
      (iree_condition_fn_t)iree_hal_hsa_semaphore_is_signaled,
...
static bool iree_hal_hsa_semaphore_any_signaled(
    const iree_hal_semaphore_list_t* semaphore_list) {
  for (iree_host_size_t i = 0; i < semaphore_list->count; ++i) {
    iree_hal_hsa_semaphore_t* semaphore =
        iree_hal_hsa_semaphore_cast(semaphore_list->semaphores[i]);
...
// Used with with iree_condition_fn_t and must match that signature.
static bool iree_hal_hsa_semaphore_all_signaled(
    const iree_hal_semaphore_list_t* semaphore_list) {
  for (iree_host_size_t i = 0; i < semaphore_list->count; ++i) {
    iree_hal_hsa_semaphore_t* semaphore =
        iree_hal_hsa_semaphore_cast(semaphore_list->semaphores[i]);
...
static iree_status_t iree_hal_hsa_semaphore_result_from_state(
...
iree_status_t iree_hal_hsa_semaphore_multi_wait(
  ...
  if (iree_timeout_is_immediate(timeout)) {
    iree_status_t status =
        iree_hal_hsa_semaphore_result_from_state(wait_mode, semaphore_list);
    IREE_TRACE_ZONE_END(z0);
    return status;
  }
  ...
  iree_notification_await(
      ...
      wait_mode == IREE_HAL_WAIT_MODE_ALL
          ? (iree_condition_fn_t)iree_hal_hsa_semaphore_all_signaled
          : (iree_condition_fn_t)iree_hal_hsa_semaphore_any_signaled,
