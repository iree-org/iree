...
static iree_hal_memory_block_pool_options_t DefaultOptions() {
...
TEST(BlockPool, AllocateFree) {
  iree_hal_memory_block_pool_t* pool = NULL;
  IREE_ASSERT_OK(iree_hal_memory_block_pool_allocate(
      DefaultOptions(), iree_allocator_system(), &pool));
  iree_hal_memory_block_pool_stats_t stats;
  iree_hal_memory_block_pool_query_stats(pool, &stats);
  EXPECT_EQ(stats.block_count, 64);
  EXPECT_EQ(stats.allocation_count, 0u);
  iree_hal_memory_block_pool_free(pool);
}

TEST(BlockPool, InvalidOptionsZeroBlockSize) {
  iree_hal_memory_block_pool_options_t options = DefaultOptions();
  options.block_size = 0;
  iree_hal_memory_block_pool_t* pool = NULL;
  IREE_EXPECT_STATUS_IS(IREE_STATUS_INVALID_ARGUMENT,
                        iree_hal_memory_block_pool_allocate(
                            options, iree_allocator_system(), &pool));
}

TEST(BlockPool, InvalidOptionsZeroBlockCount) {
  iree_hal_memory_block_pool_options_t options = DefaultOptions();
...
TEST(BlockPool, InvalidOptionsExceedsMaxBlocks) {
  iree_hal_memory_block_pool_options_t options = DefaultOptions();
  options.block_count = IREE_HAL_MEMORY_BLOCK_POOL_MAX_BLOCKS + 1;
  iree_hal_memory_block_pool_t* pool = NULL;
  IREE_EXPECT_STATUS_IS(IREE_STATUS_INVALID_ARGUMENT,
                        iree_hal_memory_block_pool_allocate(
                            options, iree_allocator_system(), &pool));
...
TEST(BlockPool, InvalidOptionsOffsetRangeOverflow) {
  iree_hal_memory_block_pool_options_t options = {};
...
TEST(BlockPool, MaxBlockCountAccepted) {
  iree_hal_memory_block_pool_options_t options = {};
  options.block_size = 64;
  options.block_count = IREE_HAL_MEMORY_BLOCK_POOL_MAX_BLOCKS;
  options.frontier_capacity = 1;
  iree_hal_memory_block_pool_t* pool = NULL;
  IREE_ASSERT_OK(iree_hal_memory_block_pool_allocate(
      options, iree_allocator_system(), &pool));
  iree_hal_memory_block_pool_stats_t stats;
  iree_hal_memory_block_pool_query_stats(pool, &stats);
  EXPECT_EQ(stats.block_count, IREE_HAL_MEMORY_BLOCK_POOL_MAX_BLOCKS);
  iree_hal_memory_block_pool_free(pool);
}

TEST(BlockPool, DefaultFrontierCapacity) {
  iree_hal_memory_block_pool_options_t options = DefaultOptions();
...
TEST(BlockPool, AcquireSingle) {
  iree_hal_memory_block_pool_t* pool = NULL;
  IREE_ASSERT_OK(iree_hal_memory_block_pool_allocate(
      DefaultOptions(), iree_allocator_system(), &pool));

  iree_hal_memory_block_pool_allocation_t allocation;
  IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &allocation));
  ...
  EXPECT_EQ(allocation.block_flags, IREE_HAL_MEMORY_BLOCK_POOL_BLOCK_FLAG_NONE);

  iree_hal_memory_block_pool_stats_t stats;
  iree_hal_memory_block_pool_query_stats(pool, &stats);
  EXPECT_EQ(stats.allocation_count, 1u);

  iree_hal_memory_block_pool_release(pool, allocation.block_index, nullptr);
  iree_hal_memory_block_pool_free(pool);
}

TEST(BlockPool, BlockOffsetsAreCorrect) {
  iree_hal_memory_block_pool_options_t options = {};
  ...
  std::vector<iree_hal_memory_block_pool_allocation_t> allocations(16);
  for (int i = 0; i < 16; ++i) {
    IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &allocations[i]));
  }
  ...
  for (int i = 0; i < 16; ++i) {
    iree_hal_memory_block_pool_release(pool, allocations[i].block_index,
                                       nullptr);
  }
  iree_hal_memory_block_pool_free(pool);
}
...
TEST(BlockPool, ExhaustionReturnsResourceExhausted) {
  iree_hal_memory_block_pool_options_t options = {};
  options.block_size = 64;
  ...
  // Allocate all blocks.
  iree_hal_memory_block_pool_allocation_t allocations[4];
  for (int i = 0; i < 4; ++i) {
    IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &allocations[i]));
  }

  // Next allocation must fail.
  iree_hal_memory_block_pool_allocation_t extra;
  IREE_EXPECT_STATUS_IS(IREE_STATUS_RESOURCE_EXHAUSTED,
                        iree_hal_memory_block_pool_acquire(pool, &extra));
...
TEST(BlockPool, ReleaseAndReacquire) {
  iree_hal_memory_block_pool_options_t options = {};
  ...
  // Acquire both blocks.
  iree_hal_memory_block_pool_allocation_t a, b;
  IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &a));
  IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &b));

  // Release one, then reacquire — should succeed.
  iree_hal_memory_block_pool_release(pool, a.block_index, nullptr);

  iree_hal_memory_block_pool_allocation_t reused;
  IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &reused));
  EXPECT_EQ(reused.block_index, a.block_index);

  iree_hal_memory_block_pool_release(pool, reused.block_index, nullptr);
  iree_hal_memory_block_pool_release(pool, b.block_index, nullptr);
  iree_hal_memory_block_pool_free(pool);
}
...
TEST(BlockPool, FrontierPreservedAcrossAcquireRelease) {
  iree_hal_memory_block_pool_t* pool = NULL;
  IREE_ASSERT_OK(iree_hal_memory_block_pool_allocate(
      DefaultOptions(), iree_allocator_system(), &pool));

  // Acquire a block.
  iree_hal_memory_block_pool_allocation_t first;
  IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &first));
  ...
  iree_hal_memory_block_pool_release(pool, first.block_index, death);

  // Reacquire the same block — death frontier should be visible.
  iree_hal_memory_block_pool_allocation_t second;
  IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &second));
  ...
  EXPECT_EQ(second.block_flags, IREE_HAL_MEMORY_BLOCK_POOL_BLOCK_FLAG_NONE);

  iree_hal_memory_block_pool_release(pool, second.block_index, nullptr);
  iree_hal_memory_block_pool_free(pool);
}

TEST(BlockPool, FrontierClearedOnReleaseWithNull) {
  iree_hal_memory_block_pool_t* pool = NULL;
  IREE_ASSERT_OK(iree_hal_memory_block_pool_allocate(
      DefaultOptions(), iree_allocator_system(), &pool));
  ...
  IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &alloc));
  MAKE_FRONTIER(f, 1, E(TestQueueAxis(0), 5));
  iree_hal_memory_block_pool_release(pool, alloc.block_index, f);

  IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &alloc));
  ASSERT_NE(alloc.death_frontier, nullptr);
  // Release with null — frontier should be cleared.
  iree_hal_memory_block_pool_release(pool, alloc.block_index, nullptr);
...
TEST(BlockPool, BlockDeathFrontierAccessor) {
  iree_hal_memory_block_pool_t* pool = NULL;
  IREE_ASSERT_OK(iree_hal_memory_block_pool_allocate(
      DefaultOptions(), iree_allocator_system(), &pool));
  ...
  EXPECT_EQ(
      iree_hal_memory_block_pool_block_death_frontier(pool, alloc.block_index),
      nullptr);
...
TEST(BlockPool, OversizedFrontierCausesTaint) {
  iree_hal_memory_block_pool_options_t options = {};
  options.block_size = 256;
  options.block_count = 4;
  options.frontier_capacity = 2;
  ...
  MAKE_FRONTIER(big, 3, E(TestQueueAxis(0), 1), E(TestQueueAxis(1), 2),
                E(TestQueueAxis(2), 3));
  iree_hal_memory_block_pool_release(pool, alloc.block_index, big);

  // Reacquire — should see TAINTED flag and no frontier.
  IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &alloc));
  EXPECT_TRUE(alloc.block_flags &
              IREE_HAL_MEMORY_BLOCK_POOL_BLOCK_FLAG_TAINTED);
  EXPECT_EQ(alloc.death_frontier, nullptr);
...
TEST(BlockPool, TaintClearedOnReleaseWithFittingFrontier) {
  iree_hal_memory_block_pool_options_t options = {};
  options.block_size = 256;
  options.block_count = 4;
  options.frontier_capacity = 2;
  ...
  iree_hal_memory_block_pool_release(pool, alloc.block_index, small);

  IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &alloc));
  EXPECT_EQ(alloc.block_flags, IREE_HAL_MEMORY_BLOCK_POOL_BLOCK_FLAG_NONE);
  ASSERT_NE(alloc.death_frontier, nullptr);
...
TEST(BlockPool, BlockFlagsAccessor) {
  iree_hal_memory_block_pool_options_t options = {};
  options.block_size = 64;
  options.block_count = 2;
  options.frontier_capacity = 1;
  ...
  iree_hal_memory_block_pool_allocation_t alloc;
  IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &alloc));
  EXPECT_EQ(iree_hal_memory_block_pool_block_flags(pool, alloc.block_index),
            IREE_HAL_MEMORY_BLOCK_POOL_BLOCK_FLAG_NONE);

  // Taint it.
  MAKE_FRONTIER(big, 2, E(TestQueueAxis(0), 1), E(TestQueueAxis(1), 2));
  iree_hal_memory_block_pool_release(pool, alloc.block_index, big);
  IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &alloc));
  EXPECT_TRUE(iree_hal_memory_block_pool_block_flags(pool, alloc.block_index) &
              IREE_HAL_MEMORY_BLOCK_POOL_BLOCK_FLAG_TAINTED);
...
TEST(BlockPool, PartialWordBlockCount) {
  iree_hal_memory_block_pool_options_t options = {};
  options.block_size = 64;
  ...
  std::vector<iree_hal_memory_block_pool_allocation_t> allocations(100);
  for (int i = 0; i < 100; ++i) {
    IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &allocations[i]))
        << "allocation " << i;
  }
...
TEST(BlockPool, SingleBlockPool) {
  iree_hal_memory_block_pool_options_t options = {};
...
TEST(BlockPool, StatsTracking) {
  ...
  // Acquire 3 blocks.
  iree_hal_memory_block_pool_allocation_t a[3];
  for (int i = 0; i < 3; ++i) {
    IREE_ASSERT_OK(iree_hal_memory_block_pool_acquire(pool, &a[i]));
  }
  iree_hal_memory_block_pool_query_stats(pool, &stats);
  EXPECT_EQ(stats.allocation_count, 3u);

  // Release one.
  iree_hal_memory_block_pool_release(pool, a[1].block_index, nullptr);
  iree_hal_memory_block_pool_query_stats(pool, &stats);
  EXPECT_EQ(stats.allocation_count, 2u);

  // Release remaining.
  iree_hal_memory_block_pool_release(pool, a[0].block_index, nullptr);
  iree_hal_memory_block_pool_release(pool, a[2].block_index, nullptr);
  iree_hal_memory_block_pool_query_stats(pool, &stats);
  EXPECT_EQ(stats.allocation_count, 0u);
...
TEST(BlockPool, ConcurrentAcquireExhaustion) {
  ...
  for (int t = 0; t < kThreadCount; ++t) {
    for (uint32_t idx : results[t].block_indices) {
      iree_hal_memory_block_pool_release(pool, idx, nullptr);
    }
  }
  iree_hal_memory_block_pool_free(pool);
}

TEST(BlockPool, ConcurrentAcquireRelease) {
      ...
      for (int i = 0; i < kIterationsPerThread; ++i) {
        ...
        if (iree_status_is_ok(status)) {
          ...
          if (i % 3 == 0) {
            ...
            iree_hal_memory_block_pool_release(pool, alloc.block_index,
                                               nullptr);
          }
        } else {
          iree_status_ignore(status);
...
TEST(BlockPool, ConcurrentFrontierVisibility) {
  ...
  auto producer = [&]() {
    for (int i = 0; i < kBlockCount; ++i) {
      ...
      iree_hal_memory_block_pool_release(pool, block_indices[i], f);
    }
