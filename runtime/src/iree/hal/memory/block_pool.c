...
static inline iree_hal_memory_block_pool_block_t*
iree_hal_memory_block_pool_block_at(const iree_hal_memory_block_pool_t* pool,
                                    uint32_t block_index) {
  return (iree_hal_memory_block_pool_block_t*)(pool->block_storage +
...
iree_hal_memory_block_pool_block_frontier_at(
    const iree_hal_memory_block_pool_t* pool,
    iree_hal_memory_block_pool_block_t* block) {
...
iree_status_t iree_hal_memory_block_pool_allocate(
  ...
  *out_pool = NULL;
  ...
  if (options.block_count > IREE_HAL_MEMORY_BLOCK_POOL_MAX_BLOCKS) {
    return iree_make_status(
        IREE_STATUS_INVALID_ARGUMENT,
        "block_count (%u) exceeds IREE_HAL_MEMORY_BLOCK_POOL_MAX_BLOCKS (%u)",
        (unsigned)options.block_count,
        (unsigned)IREE_HAL_MEMORY_BLOCK_POOL_MAX_BLOCKS);
  }
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, IREE_STRUCT_LAYOUT(
              sizeof(iree_hal_memory_block_pool_block_t), &block_stride,
              IREE_STRUCT_FIELD_ALIGNED(
                  1, iree_async_frontier_t,
                  iree_alignof(iree_async_frontier_entry_t), &frontier_offset),
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0,
      IREE_STRUCT_LAYOUT(sizeof(iree_hal_memory_block_pool_t), &total_size,
                         IREE_STRUCT_ARRAY_FIELD(options.block_count,
                                                 block_stride, uint8_t, NULL)));

  // Single cache-line-aligned allocation for the entire pool.
  ...
  for (uint16_t w = 0; w < IREE_HAL_MEMORY_BLOCK_POOL_BITMAP_WORDS; ++w) {
  ...
  for (uint32_t i = 0; i < options.block_count; ++i) {
    iree_hal_memory_block_pool_block_t* block =
        iree_hal_memory_block_pool_block_at(pool, i);
    block->flags = IREE_HAL_MEMORY_BLOCK_POOL_BLOCK_FLAG_NONE;
    iree_async_frontier_t* frontier =
        iree_hal_memory_block_pool_block_frontier_at(pool, block);
...
void iree_hal_memory_block_pool_free(iree_hal_memory_block_pool_t* pool) {
  ...
  if (count > 0) {
    IREE_ASSERT(false, "block pool free with %" PRIu32 " leaked acquisitions",
                count);
  }
...
iree_status_t iree_hal_memory_block_pool_acquire(
  ...
  for (uint16_t pass = 0; pass < word_count; ++pass) {
    ...
    while (word != ~UINT64_C(0)) {
      ...
      if (!(old & bit_mask)) {
        ...
        iree_hal_memory_block_pool_block_t* block =
            iree_hal_memory_block_pool_block_at(pool, block_index);
        iree_async_frontier_t* frontier =
            iree_hal_memory_block_pool_block_frontier_at(pool, block);
  ...
  return iree_make_status(IREE_STATUS_RESOURCE_EXHAUSTED,
                          "block pool exhausted: all %u blocks are acquired",
...
void iree_hal_memory_block_pool_release(
  ...
  if (death_frontier && death_frontier->entry_count > 0) {
    if (death_frontier->entry_count <= pool->frontier_capacity) {
      ...
      block->flags |= IREE_HAL_MEMORY_BLOCK_POOL_BLOCK_FLAG_TAINTED;
    }
  } else {
    iree_async_frontier_initialize(block_frontier, 0);
  }
...
void iree_hal_memory_block_pool_query_stats(
  ...
  out_stats->allocation_count =
      iree_atomic_load(&((iree_hal_memory_block_pool_t*)pool)->allocation_count,
                       iree_memory_order_relaxed);
}
...
iree_hal_memory_block_pool_block_flags_t iree_hal_memory_block_pool_block_flags(
  ...
  IREE_ASSERT_ARGUMENT(pool);
  IREE_ASSERT(block_index < pool->block_count);
  return iree_hal_memory_block_pool_block_at(pool, block_index)->flags;
}
