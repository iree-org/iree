...
#ifndef IREE_HAL_MEMORY_BLOCK_POOL_H_
#define IREE_HAL_MEMORY_BLOCK_POOL_H_
...
//===----------------------------------------------------------------------===//
// Block Pool Offset Allocator
//===----------------------------------------------------------------------===//
//
// A lock-free fixed-size block pool that manages a contiguous range of offsets
...
// ## Why a block pool
//
// For allocation patterns where all items are the same size, a block pool is
// simpler and faster than a general-purpose allocator like TLSF:
//
...
// ## How it works
//
// The pool maintains a flat array of 64-bit bitmap words. Bit i in word j
...
// ### Capacity
//
// The bitmap is stored inline in the pool structure: up to
// IREE_HAL_MEMORY_BLOCK_POOL_BITMAP_WORDS words × 64 bits =
// IREE_HAL_MEMORY_BLOCK_POOL_MAX_BLOCKS blocks. This is sufficient for most
// block pool use cases (KV cache, MoE expert slots, signal ring entries). If
// larger pools are needed, create multiple pool instances.
//
// ### Thread safety
//
// Acquire and release are lock-free and safe to call from any thread.
// Pool allocation and freeing are NOT thread-safe and must be called from a
// single thread with no concurrent acquire/release operations.

//===----------------------------------------------------------------------===//
// Types
//===----------------------------------------------------------------------===//

// Maximum number of blocks in a single pool instance. Limited by the inline
// bitmap array size (IREE_HAL_MEMORY_BLOCK_POOL_BITMAP_WORDS words × 64 bits).
#define IREE_HAL_MEMORY_BLOCK_POOL_MAX_BLOCKS 4096
...
#define IREE_HAL_MEMORY_BLOCK_POOL_DEFAULT_FRONTIER_CAPACITY 4

// Number of 64-bit words in the inline bitmap array.
#define IREE_HAL_MEMORY_BLOCK_POOL_BITMAP_WORDS 64

// Bitfield flags for a block's current state.
typedef uint32_t iree_hal_memory_block_pool_block_flags_t;
enum iree_hal_memory_block_pool_block_flag_bits_e {
  IREE_HAL_MEMORY_BLOCK_POOL_BLOCK_FLAG_NONE = 0u,
  ...
  IREE_HAL_MEMORY_BLOCK_POOL_BLOCK_FLAG_TAINTED = 1u << 0,
};
...
typedef struct iree_hal_memory_block_pool_block_t {
  iree_hal_memory_block_pool_block_flags_t flags;
} iree_hal_memory_block_pool_block_t;

// Options for creating a block pool instance.
typedef struct iree_hal_memory_block_pool_options_t {
  // Size of each block in bytes. Must be > 0. All allocations return offsets
  // aligned to this size (offset = block_index * block_size). The block pool
  ...
  // Number of blocks in the pool. Must be > 0 and
  // <= IREE_HAL_MEMORY_BLOCK_POOL_MAX_BLOCKS. The managed offset range is
  // [0, block_count * block_size).
  uint32_t block_count;

  // Maximum number of frontier entries per block. Set to 0 to use
  // IREE_HAL_MEMORY_BLOCK_POOL_DEFAULT_FRONTIER_CAPACITY.
  uint16_t frontier_capacity;
} iree_hal_memory_block_pool_options_t;

// Describes the result of a successful allocation from the block pool.
typedef struct iree_hal_memory_block_pool_allocation_t {
  // Start offset of the allocated block within [0, block_count * block_size).
  iree_device_size_t offset;

  // Block index (0-based). Pass to iree_hal_memory_block_pool_release() to
  ...
  // IREE_HAL_MEMORY_BLOCK_POOL_BLOCK_FLAG_TAINTED to determine whether the
  // death frontier is trustworthy for dominance checking.
  iree_hal_memory_block_pool_block_flags_t block_flags;
} iree_hal_memory_block_pool_allocation_t;

// Running statistics for a block pool. All values are atomic snapshots —
// they may be momentarily inconsistent under concurrent modifications.
typedef struct iree_hal_memory_block_pool_stats_t {
  // Total blocks in the pool (immutable after init).
  uint32_t block_count;

  // Number of currently allocated blocks.
  uint32_t allocation_count;
} iree_hal_memory_block_pool_stats_t;

// The block pool instance. Allocated with iree_hal_memory_block_pool_allocate()
// and freed with iree_hal_memory_block_pool_free(). All fields are
// implementation details and must not be accessed directly by callers.
//
...
typedef struct iree_hal_memory_block_pool_t {
  ...
  // Host allocator used for the pool allocation itself.
  ...
  iree_alignas(iree_hardware_destructive_interference_size)
      iree_atomic_uint64_t bitmap[IREE_HAL_MEMORY_BLOCK_POOL_BITMAP_WORDS];
  ...
  iree_alignas(iree_hardware_destructive_interference_size) uint8_t
      block_storage[];
} iree_hal_memory_block_pool_t;

//===----------------------------------------------------------------------===//
// API
//===----------------------------------------------------------------------===//

// Allocates a block pool managing block_count blocks of block_size bytes.
// The managed offset range is [0, block_count * block_size). All blocks start
// free with empty death frontiers.
//
// The pool is a single cache-line-aligned allocation (struct + trailing
// per-block metadata). |host_allocator| is used for this allocation and stored
// for iree_hal_memory_block_pool_free().
//
// Returns IREE_STATUS_INVALID_ARGUMENT if options are invalid (zero block_size,
// zero block_count, block_count exceeds IREE_HAL_MEMORY_BLOCK_POOL_MAX_BLOCKS).
//
// Thread-safety: NOT thread-safe. Must be called before any concurrent access.
iree_status_t iree_hal_memory_block_pool_allocate(
    iree_hal_memory_block_pool_options_t options,
    iree_allocator_t host_allocator, iree_hal_memory_block_pool_t** out_pool);

// Frees a block pool, releasing all memory.
...
// has ceased.
void iree_hal_memory_block_pool_free(iree_hal_memory_block_pool_t* pool);
...
iree_status_t iree_hal_memory_block_pool_acquire(
    iree_hal_memory_block_pool_t* pool,
    iree_hal_memory_block_pool_allocation_t* out_allocation);

// Releases a previously acquired block, returning it to the pool.
//
// |block_index| is the index returned by iree_hal_memory_block_pool_acquire().
// |death_frontier| is the causal snapshot to attach to the released block. May
// be NULL for an empty frontier. The frontier entries are copied into the
...
// tainted (IREE_HAL_MEMORY_BLOCK_POOL_BLOCK_FLAG_TAINTED).
//
// Thread-safety: Lock-free. Safe to call concurrently from any thread.
void iree_hal_memory_block_pool_release(
    iree_hal_memory_block_pool_t* pool, uint32_t block_index,
    const iree_async_frontier_t* death_frontier);

// Copies the pool's running statistics into |out_stats|. Values are atomic
...
void iree_hal_memory_block_pool_query_stats(
    const iree_hal_memory_block_pool_t* pool,
    iree_hal_memory_block_pool_stats_t* out_stats);
...
const iree_async_frontier_t* iree_hal_memory_block_pool_block_death_frontier(
    const iree_hal_memory_block_pool_t* pool, uint32_t block_index);

// Returns the flags of a block. Useful for inspecting TAINTED status.
//
// Thread-safety: Safe to call on allocated blocks. The caller must ensure the
// block is not concurrently freed.
iree_hal_memory_block_pool_block_flags_t iree_hal_memory_block_pool_block_flags(
    const iree_hal_memory_block_pool_t* pool, uint32_t block_index);
...
#endif  // IREE_HAL_MEMORY_BLOCK_POOL_H_
