...
static void iree_hal_transfer_operation_notify_completion(
  ...
  if (!iree_status_is_ok(operation->error_status)) {
    for (iree_host_size_t i = 0; i < signal_semaphore_list.count; ++i) {
      // TODO(benvanik): this value comes from
      // IREE_HAL_VULKAN_SEMAPHORE_MAX_VALUE - it's unfortunate that we can't
      // use the whole 64-bit range but some implementations have an int32_t
      // max increment value.
      // DO NOT SUBMIT this is not failure everywhere :(
      signal_semaphore_list.payload_values[i] = (2147483647ull - 1);
    }
  }
