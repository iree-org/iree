...
#ifndef IREE_HAL_UTILS_TIMEPOINT_POOL_H_
#define IREE_HAL_UTILS_TIMEPOINT_POOL_H_
...
// to represent wait/signal of a timepoint on a timeline.
//
// iree_hal_timepoint_t objects cannot be directly created; it should be
...
typedef struct iree_hal_timepoint_t {
  ...
  iree_hal_timepoint_pool_t* pool;

  iree_hal_timepoint_kind_t kind;
  union {
    iree_event_t host_wait;
    iree_hal_wrapped_event_t* device_signal;
    // The device event to wait. NULL means no device event available to wait
    // for this timepoint at the moment.
    iree_hal_wrapped_event_t* device_wait;
  } timepoint;
} iree_hal_timepoint_t;
...
// A simple pool of iree_hal_timepoint_t objects to recycle.
//
...
#endif  // IREE_HAL_UTILS_TIMEPOINT_POOL_H_
