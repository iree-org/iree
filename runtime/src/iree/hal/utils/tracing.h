...
#ifndef IREE_HAL_UTILS_TRACING_H_
#define IREE_HAL_UTILS_TRACING_H_
...
//
// Usage:
//   IREE_STREAM_TRACE_ZONE_BEGIN(queue->tracing_context, stream);
//   *LaunchKernel(..., stream);
//   IREE_STREAM_TRACE_ZONE_END(queue->tracing_context, stream);
//   ...
//   iree_hal_tracing_context_collect(queue->tracing_context);
//
...
typedef struct iree_hal_tracing_context_event_t
    iree_hal_tracing_context_event_t;
...
typedef struct iree_hal_tracing_context_event_list_t {
  iree_hal_tracing_context_event_t* head;
  iree_hal_tracing_context_event_t* tail;
} iree_hal_tracing_context_event_list_t;

typedef enum iree_hal_tracing_verbosity_e {
...
} iree_hal_tracing_verbosity_t;

typedef struct iree_hal_tracing_device_interface_vtable_t
    iree_hal_tracing_device_interface_vtable_t;

typedef struct iree_hal_tracing_device_interface_t {
  const iree_hal_tracing_device_interface_vtable_t* vtable;
} iree_hal_tracing_device_interface_t;

typedef void* iree_hal_tracing_native_event_t;
typedef void* iree_hal_tracing_native_graph_node_t;
typedef void* iree_hal_tracing_native_graph_t;

typedef struct iree_hal_tracing_device_interface_vtable_t {
  // Destroys the iree_hal_tracing_device_interface_t.
  // Called when the iree_hal_tracing_context_t is done with it.
  void(IREE_API_PTR* destroy)(
      iree_hal_tracing_device_interface_t* device_interface);
  // Causes the CPU to wait on the |event| to complete.
  iree_status_t(IREE_API_PTR* synchronize_native_event)(
      iree_hal_tracing_device_interface_t* device_interface,
      iree_hal_tracing_native_event_t event);
...
iree_status_t iree_hal_tracing_context_allocate(
    iree_hal_tracing_device_interface_t* interface,
    iree_string_view_t queue_name,
    iree_hal_tracing_verbosity_t stream_tracing_verbosity,
    iree_arena_block_pool_t* block_pool, iree_allocator_t host_allocator,
    iree_hal_tracing_context_t** out_context);
...
void iree_hal_tracing_context_free(iree_hal_tracing_context_t* context);
...
void iree_hal_tracing_context_collect(iree_hal_tracing_context_t* context);

// Notifies that the given list of events has been dispached on to the gpu.
void iree_hal_tracing_notify_submitted(
    iree_hal_tracing_context_t* context,
    iree_hal_tracing_context_event_list_t* event_list);

// Frees the events and returns them back into the tracing context.
void iree_hal_tracing_free(iree_hal_tracing_context_t* context,
                           iree_hal_tracing_context_event_list_t* event_list);
...
#define IREE_STREAM_TRACE_ZONE_BEGIN(context, event_list, verbosity)      \
...
#define IREE_STREAM_TRACE_ZONE_BEGIN_EXTERNAL(                           \
...
#define IREE_GRAPH_TRACE_ZONE_BEGIN_EXTERNAL(                                 \
...
#define IREE_STREAM_TRACE_ZONE_END(context, event_list, verbosity) \
  iree_hal_stream_tracing_zone_end_impl(context, event_list, verbosity)

#define IREE_GRAPH_TRACE_ZONE_END(context, event_list, out_node, graph,      \
...
#define IREE_GRAPH_TRACE_ZONE_BEGIN_EXTERNAL(                                 \
    ...
    dependency_nodes_count, file_name, file_name_length, line, function_name, \
    function_name_length, name, name_length)
#define IREE_STREAM_TRACE_ZONE_END(context, evnet_list, verbosity)
#define IREE_GRAPH_TRACE_ZONE_END(context, event_list, out_node, graph, \
                                  verbosity, dependency_nodes,          \
                                  dependency_nodes_count)
...
#endif  // IREE_HAL_UTILS_TRACING_H_
