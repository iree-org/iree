...
typedef struct iree_hal_pool_reservation_t {
  ...
  // Reserved for future pool-specific routing metadata. Must be zero.
  uint16_t reserved[3];
...
typedef struct iree_hal_pool_stats_t {
  ...
  // Reserves from fresh (never-used) space.
...
typedef struct iree_hal_pool_epoch_query_t {
  // Callback invoked to check whether an axis has reached an epoch.
...
typedef struct iree_hal_pool_vtable_t {
  // Destroys the concrete pool when the resource ref count reaches zero.
  void(IREE_API_PTR* destroy)(iree_hal_pool_t* pool);

  // Attempts to reserve bytes from the pool.
  ...
  // Releases a prior reservation back to the pool.
  void(IREE_API_PTR* release_reservation)(
      iree_hal_pool_t* pool, const iree_hal_pool_reservation_t* reservation,
      const iree_async_frontier_t* death_frontier);

  // Wraps a reservation as a HAL buffer.
  iree_status_t(IREE_API_PTR* materialize_reservation)(
      iree_hal_pool_t* pool, iree_hal_buffer_params_t params,
      const iree_hal_pool_reservation_t* reservation,
      iree_hal_pool_materialize_flags_t flags, iree_hal_buffer_t** out_buffer);

  // Returns cached pool capabilities.
  ...
  // Releases unused committed memory where possible.
  iree_status_t(IREE_API_PTR* trim)(iree_hal_pool_t* pool);

  // Returns the notification signaled after reservation releases.
