...
// call is responsible for signaling them once it has completed. Note that other
// independent work in the queue is allowed to progress while the host call is
// in-flight. Calls can be implemented asynchronously by cloning and retaining
// the signal semaphores they are provided and signaling them at any point in
// the future (from an async completion callback, another queue, etc).
//
