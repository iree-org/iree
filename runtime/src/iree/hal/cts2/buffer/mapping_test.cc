...
//
// Most tests write into a buffer then read the (possibly partial) contents
// back via iree_hal_buffer_map_read. Test failures may indicate issues in
// either the writing or the reading code path.
//
// Where applicable, tests are organized in increasing complexity:
...
TEST_P(BufferMappingTest, FillEmpty) {
  ...
  // Buffer should still be all zeroes.
  std::vector<uint8_t> actual_data(kDefaultAllocationSize);
  IREE_ASSERT_OK(iree_hal_buffer_map_read(
      buffer, /*source_offset=*/0, actual_data.data(), actual_data.size()));
  std::vector<uint8_t> reference_buffer(kDefaultAllocationSize, 0);
...
TEST_P(BufferMappingTest, FillWholeBuffer) {
  ...
  IREE_ASSERT_OK(iree_hal_buffer_map_read(
      buffer, /*source_offset=*/0, actual_data.data(), actual_data.size()));
  std::vector<uint8_t> reference_buffer(kDefaultAllocationSize, fill_value);
  EXPECT_THAT(actual_data, ContainerEq(reference_buffer));
...
TEST_P(BufferMappingTest, ReadDataSubspan) {
  ...
  // Read only a segment of the subspan.
