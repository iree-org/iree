...
static void iree_uk_reference_rowmajor_matmul(iree_uk_mmt4d_type_t type, int M,
                                              int K, int N, const void* lhs,
                                              const void* rhs, void* out,
                                              bool accumulate) {
  if (type == iree_uk_mmt4d_type_f32f32f32) {
    iree_uk_reference_rowmajor_matmul_f32f32f32(
        M, K, N, (const float*)lhs, (const float*)rhs, (float*)out, accumulate);
  } else if (type == iree_uk_mmt4d_type_i8i8i32) {
    iree_uk_reference_rowmajor_matmul_i8i8i32(
        M, K, N, (const iree_uk_int8_t*)lhs, (const iree_uk_int8_t*)rhs,
        (iree_uk_int32_t*)out, accumulate);
  } else {
    iree_abort();
  }
}
...
static iree_status_t iree_uk_benchmark_e2e_matmul(
  ...
  iree_uk_mmt4d_params_t mmt4d_params = {
      .type = params->type,
      .cpu_data = cpu_data,
      .flags = FLAG_accumulate ? IREE_UK_FLAG_ACCUMULATE : 0,
      .M = M1,
      .K = K1,
      .N = N1,
      .M0 = M0,
  ...
  iree_uk_reference_rowmajor_matmul(
      params->type, params->M, params->K, params->N, rowmajor_lhs_buffer,
      rowmajor_rhs_buffer, rowmajor_reference_out_buffer, FLAG_accumulate);

  int64_t batch_count = 1;
  int64_t total_iterations = 0;
  bool already_checked_results = false;
  while (iree_benchmark_keep_running(benchmark_state, batch_count)) {
    for (int i = 0; i < batch_count; ++i) {
      ...
      if (FLAG_accumulate) iree_uk_pack(&pack_out_params);
      iree_uk_mmt4d(&mmt4d_params);
      iree_uk_unpack(&unpack_out_params);
      if (!already_checked_results) {
        already_checked_results = true;
...
static void iree_uk_benchmark_register_e2e_matmul(
    const char* type_str, int M, int K, int N,
    iree_uk_cpu_features_list_t* cpu_features) {
  char name[128];
