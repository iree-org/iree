...
template <typename T>
static void write_random_buffer(T* buffer, iree_uk_ssize_t size_in_bytes,
...
void write_random_buffer(void* buffer, iree_uk_ssize_t size_in_bytes,
  ...
  switch (type) {
    ...
    case IREE_UK_TYPE_INT_32:
      write_random_buffer(static_cast<iree_uk_int32_t*>(buffer), size_in_bytes,
                          engine);
      return;
    case IREE_UK_TYPE_INT_8:
      write_random_buffer(static_cast<iree_uk_int8_t*>(buffer), size_in_bytes,
                          engine);
      return;
    default:
...
#define GET_MMT4D_TYPE_STR_CASE(x) \
  case x:                          \
    return #x;
    GET_MMT4D_TYPE_STR_CASE(iree_uk_mmt4d_type_f32f32f32);
    GET_MMT4D_TYPE_STR_CASE(iree_uk_mmt4d_type_i8i8i32);
    default:
      assert(false && "unknown type");
      return "unknown type";
  }
