...
TEST(ProcessTest, CancelSuspendedScopeFailButNoEnd) {
  iree_task_scope_t scope;
  iree_task_scope_initialize(iree_make_cstring_view("test"),
                             IREE_TASK_SCOPE_FLAG_NONE, &scope);

  // Simulate another active process in the scope (scope_begin was called for
  // it). Our test process was never activated (no scope_begin for it).
  ...
  // Cancel propagates error to scope...
  EXPECT_TRUE(iree_task_scope_has_failed(&scope));
  // ...but does NOT call scope_end (the SUSPENDED process was never activated,
  // so scope_begin was never called for it). The scope should still be
  // non-idle because the other process's scope_begin is outstanding.
  EXPECT_FALSE(iree_task_scope_is_idle(&scope));

  // Clean up: end the simulated other process and consume the scope error.
...
TEST(ProcessTest, CompletionSignalsScope) {
  iree_task_scope_t scope;
  iree_task_scope_initialize(iree_make_cstring_view("test"),
                             IREE_TASK_SCOPE_FLAG_NONE, &scope);
  ...
  // Simulate executor's scope_begin at activation.
  iree_task_scope_begin(&scope);
  EXPECT_FALSE(iree_task_scope_is_idle(&scope));
  ...
  EXPECT_TRUE(iree_task_scope_is_idle(&scope));
  EXPECT_FALSE(iree_task_scope_has_failed(&scope));

  iree_task_scope_deinitialize(&scope);
}

TEST(ProcessTest, ErrorPropagatedToScope) {
  iree_task_scope_t scope;
  iree_task_scope_initialize(iree_make_cstring_view("test"),
                             IREE_TASK_SCOPE_FLAG_NONE, &scope);
  ...
  iree_task_process_report_error(
      &process, iree_make_status(IREE_STATUS_DATA_LOSS, "data gone"));
