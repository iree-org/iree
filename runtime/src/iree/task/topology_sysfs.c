...
//===----------------------------------------------------------------------===//
// NUMA / package node identity (never cluster_id) — D1/D2 for #24761
//===----------------------------------------------------------------------===//

// Upper bound on NUMA / package nodes we map into a dense 0..N-1 space.
#define IREE_SYSFS_MAX_NODE_IDS 64

// True when using the live host sysfs tree (not a fixture / compile-time root).
// Fixture tests must not apply the process affinity mask from the host machine.
static bool iree_sysfs_should_constrain_to_host_affinity(void) {
  return strcmp(iree_sysfs_get_root_path(), "/sys/devices/system") == 0;
}
...
iree_status_t iree_task_topology_initialize_from_physical_cores(
  ...
  if (iree_sysfs_should_constrain_to_host_affinity() &&
      iree_sysfs_query_current_affinity(&current_affinity)) {
    current_affinity_ptr = &current_affinity;
  }
