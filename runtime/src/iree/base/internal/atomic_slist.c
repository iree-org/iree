...
void iree_atomic_slist_concat(iree_atomic_slist_t* list,
  ...
  while (true) {
    intptr_t list_head = iree_atomic_load_intptr(&list->head, iree_memory_order_relaxed);
    iree_atomic_store_intptr(&tail->next, list_head, iree_memory_order_release);
    if (iree_atomic_compare_exchange_weak_intptr(&list->head, &list_head, (intptr_t)head, iree_memory_order_release, iree_memory_order_relaxed)) {
      break;
    }
  }
}
...
void iree_atomic_slist_push_unsafe(iree_atomic_slist_t* list,
  ...
  iree_atomic_store_intptr(&list->head, (intptr_t)entry, iree_memory_order_relaxed); 
}
...
iree_atomic_slist_entry_t* iree_atomic_slist_pop(iree_atomic_slist_t* list) {
  ...
  intptr_t list_head = iree_atomic_load_intptr(&list->head, iree_memory_order_acquire);
  while (true) {
    if (!list_head) return NULL;
    intptr_t list_head_next = iree_atomic_load_intptr(&((iree_atomic_slist_entry_t*)list_head)->next, iree_memory_order_relaxed);
    if (iree_atomic_compare_exchange_weak_intptr(&list->head, &list_head, list_head_next, iree_memory_order_release, iree_memory_order_relaxed)) {
      break;
    }
  }
...
bool iree_atomic_slist_flush(iree_atomic_slist_t* list,
  ...
  while (true) {
    ...
    if (iree_atomic_compare_exchange_weak_intptr(&list->head, &list_head, 0, iree_memory_order_release, iree_memory_order_relaxed)) {
      break;
    }
  }
  ...
  switch (flush_order) {
    case IREE_ATOMIC_SLIST_FLUSH_ORDER_APPROXIMATE_LIFO: {
      ...
      *out_head = head;
      if (out_tail) {
        ...
        while (p->next) p = (iree_atomic_slist_entry_t*)iree_atomic_load_intptr(&p->next, iree_memory_order_relaxed);
        *out_tail = p;
      }
      break;
    }
    case IREE_ATOMIC_SLIST_FLUSH_ORDER_APPROXIMATE_FIFO: {
      ...
      iree_atomic_slist_entry_t* p = head;
      do {
        iree_atomic_slist_entry_t* next = (iree_atomic_slist_entry_t*)iree_atomic_load_intptr(&p->next, iree_memory_order_relaxed);
        iree_atomic_store_intptr(&p->next, (intptr_t)head, iree_memory_order_relaxed);
        head = p;
        p = next;
      } while (p != NULL);
