...
#ifndef IREE_BASE_THREADING_MUTEX_H_
#define IREE_BASE_THREADING_MUTEX_H_
...
#define iree_mutex_initialize(out_mutex)                                      \
  ...
  iree_mutex_initialize_impl(&TracyConcat(__tracy_source_location, __LINE__), \
                             out_mutex);
IREE_API_EXPORT void iree_mutex_initialize_impl(
    const iree_tracing_location_t* src_loc, iree_mutex_t* out_mutex);
#else
// Initializes |out_mutex| to the well-defined unlocked contents.
// Must be called prior to using any other iree_mutex_* method.
IREE_API_EXPORT void iree_mutex_initialize(iree_mutex_t* out_mutex);
#endif  // IREE_TRACING_FEATURE_SLOW_LOCKS

// Deinitializes |mutex| (after a prior call to iree_mutex_initialize).
// The mutex must not be held by any thread.
IREE_API_EXPORT void iree_mutex_deinitialize(iree_mutex_t* mutex)
    IREE_THREAD_ANNOTATION_ATTRIBUTE(locks_excluded(mutex));

// Locks the |mutex| and returns when held by the caller.
IREE_API_EXPORT void iree_mutex_lock(iree_mutex_t* mutex)
...
IREE_API_EXPORT bool iree_mutex_try_lock(iree_mutex_t* mutex)
    IREE_THREAD_ANNOTATION_ATTRIBUTE(try_acquire_capability(true, mutex));

// Unlocks the |mutex|, which must be held by the caller.
IREE_API_EXPORT void iree_mutex_unlock(iree_mutex_t* mutex)
...
// it means to be a mutex in the context of the C11/C++11 memory model:
...
IREE_API_EXPORT void iree_slim_mutex_initialize_impl(
    const iree_tracing_location_t* src_loc, iree_slim_mutex_t* out_mutex);
#else
// Initializes |out_mutex| to the well-defined unlocked contents.
// Must be called prior to using any other iree_slim_mutex_* method.
//
...
IREE_API_EXPORT void iree_slim_mutex_initialize(iree_slim_mutex_t* out_mutex);
...
// The mutex must not be held by any thread.
IREE_API_EXPORT void iree_slim_mutex_deinitialize(iree_slim_mutex_t* mutex)
    IREE_THREAD_ANNOTATION_ATTRIBUTE(locks_excluded(mutex));

// Locks the |mutex| and returns when held by the caller.
IREE_API_EXPORT void iree_slim_mutex_lock(iree_slim_mutex_t* mutex)
    IREE_THREAD_ANNOTATION_ATTRIBUTE(acquire_capability(mutex));
...
IREE_API_EXPORT bool iree_slim_mutex_try_lock(iree_slim_mutex_t* mutex)
    IREE_THREAD_ANNOTATION_ATTRIBUTE(try_acquire_capability(true, mutex));

// Unlocks the |mutex|, which must be held by the caller.
IREE_API_EXPORT void iree_slim_mutex_unlock(iree_slim_mutex_t* mutex)
    IREE_THREAD_ANNOTATION_ATTRIBUTE(release_capability(mutex));
...
#endif  // IREE_BASE_THREADING_MUTEX_H_
