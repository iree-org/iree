...
//
// References:
// - https://github.com/libuv/libuv/blob/master/src/timer.c

#ifndef IREE_ASYNC_PLATFORM_POSIX_TIMER_LIST_H_
#define IREE_ASYNC_PLATFORM_POSIX_TIMER_LIST_H_
...
// The list uses intrusive pointers stored in timer->platform.posix.{next,prev}.
typedef struct iree_async_posix_timer_list_t {
  iree_async_timer_operation_t* head;  // Earliest deadline.
  iree_async_timer_operation_t* tail;  // Most distant deadline.
} iree_async_posix_timer_list_t;

// Initializes an empty timer list.
static inline void iree_async_posix_timer_list_initialize(
    iree_async_posix_timer_list_t* list) {
  list->head = NULL;
  list->tail = NULL;
}
...
static inline bool iree_async_posix_timer_list_is_empty(
    const iree_async_posix_timer_list_t* list) {
...
#endif  // IREE_ASYNC_PLATFORM_POSIX_TIMER_LIST_H_
