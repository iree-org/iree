...
typedef struct iree_net_shm_connection_t {
  ...
  iree_net_shm_connection_drain_t drain;
...
static void iree_net_shm_connection_carrier_drained(void* user_data) {
  iree_net_shm_connection_drain_t* drain =
      (iree_net_shm_connection_drain_t*)user_data;
...
static void iree_net_shm_connection_deactivate(
  ...
  iree_net_shm_connection_drain_t* drain = &connection->drain;
  ...
  for (uint16_t i = 0; i < connection->max_endpoint_count; ++i) {
    ...
    if (slot->adapter && slot->adapter->activated) {
      ...
      iree_status_t status = iree_net_carrier_deactivate(
          slot->adapter->carrier, iree_net_shm_connection_carrier_drained,
          drain);
      if (!iree_status_is_ok(status)) {
        iree_status_ignore(status);
        iree_net_shm_connection_carrier_drained(drain);
      }
    }
  }
...
static iree_status_t iree_net_shm_connection_open_endpoint(
  ...
  status = iree_net_shm_endpoint_deferred_submit(connection->proactor, endpoint,
...
// Cross-process listener and connect implementations live in platform-specific
// files: factory_unix.c (POSIX) and factory_win32.c (Windows).
...
static iree_status_t iree_net_shm_factory_connect(
  ...
  // Dispatch cross-process addresses to platform-specific implementations.
  ...
  if (iree_string_view_starts_with(address, IREE_SV("pipe:"))) {
    return iree_make_status(IREE_STATUS_UNIMPLEMENTED,
                            "pipe: addresses are not supported on this "
                            "platform; use unix: for cross-process SHM");
  }
#else   // IREE_PLATFORM_WINDOWS
  if (iree_string_view_starts_with(address, IREE_SV("pipe:"))) {
    return iree_net_shm_factory_connect_win32(factory, address, proactor,
                                              recv_pool, callback, user_data);
  }
  if (iree_string_view_starts_with(address, IREE_SV("unix:"))) {
    return iree_make_status(IREE_STATUS_UNIMPLEMENTED,
                            "unix: addresses are not supported on Windows; "
                            "use pipe: for cross-process SHM");
  }
...
static iree_status_t iree_net_shm_factory_create_listener(
  ...
  if (iree_string_view_starts_with(bind_address, IREE_SV("pipe:"))) {
    return iree_make_status(IREE_STATUS_UNIMPLEMENTED,
                            "pipe: addresses are not supported on this "
                            "platform; use unix: for cross-process SHM");
  }
#else   // IREE_PLATFORM_WINDOWS
  if (iree_string_view_starts_with(bind_address, IREE_SV("pipe:"))) {
    return iree_net_shm_factory_create_listener_win32(
        factory, bind_address, proactor, recv_pool, accept_callback, user_data,
        host_allocator, out_listener);
  }
  if (iree_string_view_starts_with(bind_address, IREE_SV("unix:"))) {
  ...
  *out_listener = NULL;
  ...
  iree_host_size_t total_size = 0;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, IREE_STRUCT_LAYOUT(iree_sizeof_struct(iree_net_shm_listener_t),
                             &total_size,
                             IREE_STRUCT_FIELD_FAM(bind_address.size, char)));
