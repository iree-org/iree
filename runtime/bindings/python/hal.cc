...
void SetupHalBindings(nanobind::module_ m) {
  ...
  py::class_<HalSemaphore>(m, "HalSemaphore")
      ...
      .def("import_timepoint", [](HalSemaphore& self,  uint64_t value, 
        HalExternalTimepoint& external_timepoint) {
        CheckApiStatus(iree_hal_semaphore_import_timepoint(self.raw_ptr(), value, 
            IREE_HAL_QUEUE_AFFINITY_ANY, external_timepoint.timepoint()), "importing timepoint");
      }, py::arg("value"), py::arg("external_timepoint"))
      .def("export_timepoint", [](HalSemaphore& self, uint64_t value,
        iree_hal_external_timepoint_type_t requested_type, iree_hal_external_timepoint_flags_t requested_flags, 
          HalExternalTimepoint& out_external_timepoint) {
            CheckApiStatus(iree_hal_semaphore_export_timepoint(self.raw_ptr(), value, 
              IREE_HAL_QUEUE_AFFINITY_ANY, requested_type, requested_flags, &out_external_timepoint.timepoint()), 
              "exporting timepoint");
          }, 
          py::arg("value"),
          py::arg("requested_type"),
          py::arg("requested_flags"),
          py::arg("out_external_timepoint"));
  ...
  py::class_<HalExternalTimepoint>(m, "HalExternalTimepoint")
      .def("__init__", [](HalExternalTimepoint* self) {
        new (self) HalExternalTimepoint();
      })
      .def_prop_rw("type", [](HalExternalTimepoint& self) -> int {
        return static_cast<int>(self.type());
      }, [](HalExternalTimepoint& self, int type) {
          return self.type() = static_cast<iree_hal_external_timepoint_type_t>(type);
        })
      .def_prop_rw("flags", [](HalExternalTimepoint& self) -> int {
        return static_cast<int>(self.flags());
      }, [](HalExternalTimepoint& self, int flags) {
          return self.flags() = static_cast<iree_hal_external_timepoint_flags_t>(flags);
        })
      .def_prop_rw("compatibility", [](HalExternalTimepoint& self) -> int {
          return static_cast<int>(self.compatibility());
        }, [](HalExternalTimepoint& self, int compatibility) {
            return self.compatibility() = static_cast<iree_hal_semaphore_compatibility_t>(compatibility);
          })
      .def_prop_rw("cuda_event", [](HalExternalTimepoint& self) -> uint64_t {
        return static_cast<uint64_t>(reinterpret_cast<uintptr_t>(self.cuda_event()));
          ...
          if(self.type() != IREE_HAL_EXTERNAL_TIMEPOINT_TYPE_CUDA_EVENT) {
            throw RaiseValueError("Unexpected cuda event for non cuda timepoint");
          }
          self.cuda_event() = reinterpret_cast<void*>(static_cast<uintptr_t>(cuda_event));
        })
      .def_prop_rw("hip_event", [](HalExternalTimepoint& self) -> uint64_t {
          return static_cast<uint64_t>(reinterpret_cast<uintptr_t>(self.hip_event()));
            ...
            if(self.type() != IREE_HAL_EXTERNAL_TIMEPOINT_TYPE_HIP_EVENT) {
              throw RaiseValueError("Unexpected hip event for non hip timepoint");
            }
            self.hip_event() = reinterpret_cast<void*>(static_cast<uintptr_t>(hip_event));
          });
