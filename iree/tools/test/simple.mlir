...
// iree-run-mlir
// RUN: (iree-run-mlir --iree-hal-target-backends=vmla --function-value="i32=-2" %s) | IreeFileCheck %s

// RUN: [[ $IREE_VULKAN_DISABLE == 1 ]] || (iree-run-mlir -iree-hal-target-backends=vulkan-spirv --function-value="i32=-2" %s | IreeFileCheck %s)
