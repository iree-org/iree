...
template <>
Optional<SmallVector<Value, 4>> getLoopRange<OpBuilder>(
    OpBuilder &builder, FuncOp numWorkgroupsFn, Location loc,
    linalg::LinalgOp linalgOp) {
  ...
  builder.setInsertionPointToEnd(numWorkgroupsFn.addEntryBlock());
  ...
  Optional<SmallVector<Value, 4>> bounds =
      getLoopRanges(builder, clonedLinalgOp);
  clonedLinalgOp.erase();
  return bounds;
}

template <>
Optional<SmallVector<Value, 4>> getLoopRange<PatternRewriter>(
...
template <>
Optional<SmallVector<Value, 4>> getLoopRange<ConversionPatternRewriter>(
    ConversionPatternRewriter &rewriter, FuncOp numWorkgroupsFn, Location loc,
    linalg::LinalgOp linalgOp) {
  if (!numWorkgroupsFn.empty()) {
    numWorkgroupsFn.emitError("num workgroups fn expected to be empty");
    return {};
...
static LogicalResult createNumWorkgroupsFromResultShapeImpl(
  ...
  auto loopRange = getLoopRange(builder, numWorkgroupsFn, loc, linalgOp);
...
LogicalResult createNumWorkgroupsFromLinearizedResultShape(
  ...
  Optional<SmallVector<Value, 4>> loopRange =
      getLoopRange(rewriter, numWorkgroupsFn, loc, linalgOp);
  if (!loopRange) return failure();
