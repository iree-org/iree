...
module  {
// CHECK-LABEL: bufferize_transfer_op
  func @bufferize_transfer_op() {
    ...
    // CHECK-DAG: subview {{.*}} memref<2x3xf32> to memref<1x3xf32
    // CHECK-DAG: subview {{.*}} memref<3x4xf32> to memref<3x1xf32
    // CHECK-DAG: subview {{.*}} memref<2x4xf32> to memref<1x1xf32
    ...
    // CHECK-COUNT-6: vector.transfer_read {{.*}} : memref<1x3xf32
    ...
    // CHECK-COUNT-3: vector.transfer_read {{.*}} : memref<3x1xf32
    ...
    // CHECK-COUNT-2: vector.transfer_read {{.*}} : memref<1x1xf32
    ...
    // CHECK-COUNT-6: vector.contract {{.*}} : vector<1x1xf32>, vector<1x1xf32> into vector<1x1xf32>
    ...
    // CHECK: alloc() : memref<2x1xf32>
    // CHECK: vector.transfer_write {{.*}} : vector<1x1xf32>, memref<1x1xf32
    // CHECK: alloc() : memref<2x1xf32>
    // CHECK: vector.transfer_write {{.*}} : vector<1x1xf32>, memref<2x1xf32>
    // CHECK: subview %{{.*}}[0, 0] [1, 1] [1, 1] : memref<2x4xf32> to memref<1x1xf32
    // CHECK: linalg.copy(%{{.*}}, %{{.*}}) : memref<2x1xf32>, memref<1x1xf32
