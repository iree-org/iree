...
//===- LinalgBufferizePass.cpp - Pass for Linalg on tensors to buffers-===//
//
// Pass to convert from linalg on tensors to Linalg on buffers. This may be
// called after transformations on Linalg on tensors and includes special
...
// tensor update" pattern, by an inplace update at `binding` and `offset, using
...
//
// For now, the following patterns are supported.
//
...
namespace {
struct TestRewriteDestructiveUpdatePatternInPlace
    : public PassWrapper<TestRewriteDestructiveUpdatePatternInPlace,
                         FunctionPass> {
...
void TestRewriteDestructiveUpdatePatternInPlace::runOnFunction() {
...
createTestRewriteDestructiveUpdatePatternInPlace() {
  return std::make_unique<TestRewriteDestructiveUpdatePatternInPlace>();
}

static PassRegistration<TestRewriteDestructiveUpdatePatternInPlace> pass(
    "test-iree-codegen-linalg-rewrite-destructive-update-pattern-inplace",
