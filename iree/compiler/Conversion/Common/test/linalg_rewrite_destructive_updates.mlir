...
func @tile_from_tensor_load() {
  ...
  //  CHECK-DAG:     %[[LHS:.*]] = hal.interface.load.tensor.tile @io::@TENSOR_LHS, base_offset = %{{[0-9a-z]*}},
  // CHECK-SAME:       offsets = [%[[I]], 0], sizes = [1, 3], strides = [1, 1] : tensor<1x3xf32>
  //  CHECK-DAG:     %[[RHS:.*]] = hal.interface.load.tensor.tile @io::@TENSOR_RHS, base_offset = %{{[0-9a-z]*}},
  // CHECK-SAME:       offsets = [0, %[[J]]], sizes = [3, 1], strides = [1, 1] : tensor<3x1xf32>
  //  CHECK-DAG:     %[[OUT:.*]] = hal.interface.load.tensor.tile @io::@TENSOR_INIT, base_offset = %{{[0-9a-z]*}},
  ...
  //      CHECK:     hal.interface.store.tensor.tile %[[RES]], @io::@ret0, base_offset = %{{[0-9a-z]*}},
...
func @tile_from_pointwise_lhs() {
  ...
  //      CHECK: scf.for %[[I:.*]] = {{.*}} step %{{[0-9a-z]+}} {
  //      CHECK:   scf.for %[[J:.*]] = {{.*}} step %{{[0-9a-z]+}} {
  //      CHECK:     %[[LHS:.*]] = hal.interface.load.tensor.tile @io::@TENSOR_LHS, base_offset = %{{[0-9a-z]*}},
  // CHECK-SAME:       offsets = [%[[I]], 0], sizes = [1, 3], strides = [1, 1] : tensor<1x3xf32>
  //      CHECK:     %[[RHS:.*]] = hal.interface.load.tensor.tile @io::@TENSOR_RHS, base_offset = %{{[0-9a-z]*}},
  // CHECK-SAME:       offsets = [0, %[[J]]], sizes = [3, 1], strides = [1, 1] : tensor<3x1xf32>
  // Compute.
  //      CHECK:     %[[LHS2:.*]] = linalg.generic {{.*}} ins(%[[LHS]] : tensor<1x3xf32>)
  // TODO: should we reorder this load?
  //      CHECK:     %[[OUT:.*]] = hal.interface.load.tensor.tile @io::@TENSOR_INIT, base_offset = %{{[0-9a-z]*}},
  // CHECK-SAME:       offsets = [%[[I]], %[[J]]], sizes = [1, 1], strides = [1, 1] : tensor<1x1xf32>
  // Compute.
