// RUN: iree-opt -split-input-file -iree-codegen-canonicalize-hal-interface-load-store %s | IreeFileCheck %s
...
func @fold_reshape() {
  ...
  %3 = flow.dispatch.tensor.load %1 : !flow.dispatch.tensor<readonly:3x3x1x96xf32> -> tensor<3x3x1x96xf32>
  ...
  flow.dispatch.tensor.store %5, %2, offsets = [%c0, %c0, %c0, %c0], sizes = [%c1, %c1, %c1, %c1], strides = [%c1, %c1, %c1, %c1] : tensor<3x3x96xf32> -> !flow.dispatch.tensor<writeonly:3x3x96xf32>
  return
}
