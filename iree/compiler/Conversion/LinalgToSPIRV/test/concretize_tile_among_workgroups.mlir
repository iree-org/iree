  ...
  hal.executable.target @vulkan_spirv, filter="vulkan*" {
    ...
    module attributes {spv.target_env = #spv.target_env<#spv.vce<v1.3, [Shader], [SPV_KHR_storage_buffer_storage_class]>, ARM:IntegratedGPU, {}>}  {
      func @conv2d() {
        ...
        %3 = muli %workgroup_size_z, %workgroup_id_z : index
        %4 = muli %workgroup_size_z, %workgroup_count_z : index
...
// CHECK: %[[Z_MUL_4:.+]] = muli %[[ID_Z]], %c4 : index
// CHECK: %[[Y_MUL_4:.+]] = muli %[[ID_Y]], %c4 : index
// CHECK: %[[X_MUL_16:.+]] = muli %[[ID_X]], %c16 : index
