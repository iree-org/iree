...
static linalg::LinalgOp getRootLinalgOp(FuncOp funcOp) {
  ...
  // TODO(ravishankarm): These options should not be set here. Instead the
  // options used by the pass must be plumbed through.
...
static LogicalResult getTileSizeAndWorkgroupSize(
  ...
  // TODO(ravishankarm): These options should not be set here. Instead the
  // options from the pass need to be plumbed through
  SPIRVCodegenOptions options;
  options.usingLinalgOnTensors = true;
  options.enableVectorization = true;
...
class ConcretizeTileAmongWorkgroupsPass
 ...
 private:
  LogicalResult runOnFunction(FuncOp funcOp) {
    ...
    size_t numParallelDims = getNumOuterParallelLoops(rootOp);
    size_t numTiledDims =
        std::min<size_t>(numParallelDims, kMaxWorkgroupDimCount);

    // 2. Figure out the original problem size.
    SmallVector<Type> inputTypes, outputTypes;
    std::tie(inputTypes, outputTypes) = getInputOutputTypes(rootOp);

    SmallVector<int64_t, 4> opTileSizes, deviceWorkgroupSize;

    // TODO(ravishankarm): This should not be needed to be done here. THe core
    // `initGPULaunchConfig` already does this.
    opTileSizes.assign(options.tileSizes.begin(), options.tileSizes.end());
    deviceWorkgroupSize.assign(options.workgroupSize.begin(),
                               options.workgroupSize.end());
    if (opTileSizes.empty() || deviceWorkgroupSize.empty()) {
      if (failed(getTileSizeAndWorkgroupSize(rootOp, inputTypes, outputTypes,
                                             opTileSizes,
                                             deviceWorkgroupSize))) {
        return failure();
      }
    }
    if (opTileSizes.size() < numParallelDims) {
      return rootOp.emitError(
                 "invalid tile size configuration, expected at least as many "
                 "as the number of tiled loops : ")
             << numParallelDims;
    }
    ArrayRef<int64_t> tileSizes(opTileSizes);
    ...
    if (failed(updateWorkGroupSize(funcOp, deviceWorkgroupSize))) {
    ...
    if (inlineTripOneLoops) {
      ...
      ArrayRef<int64_t> outputShape =
          outputTypes.front().cast<ShapedType>().getShape();
      if (outputShape.size() < numParallelDims) {
        return success();
      }
      ...
      {
        OwningRewritePatternList workgroupCountPatterns(&getContext());
        workgroupCountPatterns.insert<ConcretizeWorkgroupCountOp>(
            &context, numWorkgroups);
        (void)applyPatternsAndFoldGreedily(funcOp,
                                           std::move(workgroupCountPatterns));
      }
      {
        OwningRewritePatternList removeTripOneLoopPatterns(&getContext());
        removeTripOneLoopPatterns.insert<RemoveTripOneLoop>(
            &context, workloadSize, halWorkgroupSize);
        (void)applyPatternsAndFoldGreedily(
            funcOp, std::move(removeTripOneLoopPatterns));
      }
    }
