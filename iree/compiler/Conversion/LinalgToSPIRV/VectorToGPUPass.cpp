...
struct ConvertVectorToGPUPass
 ...
 private:
  // Helper for linalg::Copy lowering.
  void tileAndVectorizeLinaglCopy(FuncOp funcOp, MLIRContext *context);
};
...
void ConvertVectorToGPUPass::tileAndVectorizeLinaglCopy(FuncOp funcOp,
                                                        MLIRContext *context) {
  ...
  target->addDynamicallyLegalOp<linalg::CopyOp>([&](linalg::CopyOp copy) {
    if (hasMarker(copy, getCopyToWorkgroupMemoryMarker())) return false;
    return true;
  });
  ...
  if (failed(applyPartialConversion(funcOp, *target, tileAndDistributePattern)))
...
void ConvertVectorToGPUPass::runOnOperation() {
  ...
  tileAndVectorizeLinaglCopy(funcOp, context);
