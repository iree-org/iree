...
// CHECK-DAG: %[[OUT:.+]] = iree.placeholder for "interface buffer" {binding = @io::@ret0} : memref<2x5xi32>
// CHECK-DAG: %[[IN0:.+]] = iree.placeholder for "interface buffer" {binding = @io::@arg0} : memref<2x2xi32>
// CHECK-DAG: %[[IN1:.+]] = iree.placeholder for "interface buffer" {binding = @io::@arg1} : memref<2x3xi32>
...
module  {
  func @concatenate() {
    ...
    hal.interface.store.tensor %5, @io::@ret0, offset = %c0 : tensor<2x5xi32>
    return
  }
...
// CHECK-DAG: %[[OUT:.+]] = iree.placeholder for "interface buffer" {binding = @io::@ret0} : memref<5x2xi32>
// CHECK-DAG: %[[IN:.+]] = iree.placeholder for "interface buffer" {binding = @io::@arg0} : memref<2x2xi32>
...
module  {
  func @concatenate() {
    ...
    hal.interface.store.tensor %4, @io::@ret0, offset = %c0 : tensor<5x2xi32>
    return
  }
  hal.interface @io attributes {sym_visibility = "private"} {
    hal.interface.binding @arg0, set=0, binding=0, type="StorageBuffer", access="Read"
