...
struct ConvertSelectOp : public OpConversionPattern<chlo::BroadcastSelectOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    // Reject if the true and false types are not statically equal. While the
    // HLO op spec says nothing about static vs dynamic equality, it is
    // assumed that a well formed program with shape inference done properly,
    // the types should be statically equal, at least agreeing on what their
    // dynamic elements are.
    // This lets us compute the broadcast between pred and just one of them
    // but apply it to both.
    // if (onTrueType != onFalseType) {
    //   return rewriter.notifyMatchFailure(
    //       op, "true and false types not statically identical");
    // }
