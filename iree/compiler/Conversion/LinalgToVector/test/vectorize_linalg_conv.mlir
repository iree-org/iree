...
func @vectorize_depthwise_conv(%input: memref<1x1x3x4xf32>, %filter: memref<1x1x4xf32>, %output: memref<1x1x2x4xf32>) {
  %0 = subview %input[0, 0, 0, 0] [1, 1, 3, 4] [1, 1, 1, 1]  : memref<1x1x3x4xf32> to memref<1x1x3x4xf32>
  %1 = subview %filter[0, 0, 0] [1, 1, 4] [1, 1, 1]  : memref<1x1x4xf32> to memref<1x1x4xf32>
  %2 = subview %output[0, 0, 0, 0] [1, 1, 2, 4] [1, 1, 1, 1]  : memref<1x1x2x4xf32> to memref<1x1x2x4xf32>
  linalg.depthwise_conv_2d_input_nhwc_filter_hwc {strides = dense<2> : tensor<2xi64>} ins(%0, %1 : memref<1x1x3x4xf32>, memref<1x1x4xf32>) outs(%2 : memref<1x1x2x4xf32>)
  return
}

// CHECK-LABEL: func @vectorize_depthwise_conv
// CHECK-SAME: %[[INPUT_ARG:.+]]: memref<1x1x3x4xf32>,
// CHECK-SAME: %[[FILTER_ARG:.+]]: memref<1x1x4xf32>,
// CHECK-SAME: %[[OUTPUT_ARG:.+]]: memref<1x1x2x4xf32>
...
// CHECK: %[[FILTER_0:.+]] = vector.transfer_read %[[FILTER]][%c0, %c0, %c0], %[[FLOAT_ZERO]] {masked = [false]} : memref<1x1x4xf32>, vector<4xf32>
// CHECK: %[[OUTPUT_0:.+]] = vector.transfer_read %[[OUTPUT]][%c0, %c0, %c0, %c0], %[[FLOAT_ZERO]] {masked = [false]} : memref<1x1x2x4xf32>, vector<4xf32>
// CHECK:  %[[INPUT_0:.+]] = vector.transfer_read %[[INPUT]][%c0, %c0, %c0, %c0], %[[FLOAT_ZERO]] {masked = [false]} : memref<1x1x3x4xf32>, vector<4xf32>
// CHECK:    %[[FMA_0:.+]] = vector.fma %[[INPUT_0]], %[[FILTER_0]], %[[OUTPUT_0]] : vector<4xf32>
// CHECK: vector.transfer_write %[[FMA_0]], %[[OUTPUT]][%c0, %c0, %c0, %c0] {masked = [false]} : vector<4xf32>, memref<1x1x2x4xf32>
// CHECK: %[[OUTPUT_1:.+]] = vector.transfer_read %[[OUTPUT]][%c0, %c0, %c1, %c0], %[[FLOAT_ZERO]] {masked = [false]} : memref<1x1x2x4xf32>, vector<4xf32>
// CHECK:  %[[INPUT_1:.+]] = vector.transfer_read %[[INPUT]][%c0, %c0, %c2, %c0], %[[FLOAT_ZERO]] {masked = [false]} : memref<1x1x3x4xf32>, vector<4xf32>
// CHECK:    %[[FMA_1:.+]] = vector.fma %[[INPUT_1]], %[[FILTER_0]], %[[OUTPUT_1]] : vector<4xf32>
// CHECK: vector.transfer_write %[[FMA_1]], %[[OUTPUT]][%c0, %c0, %c1, %c0] {masked = [false]} : vector<4xf32>, memref<1x1x2x4xf32>

// -----

// CHECK-LABEL: func @do_not_vectorize_depthwise_conv_withnon_1_batch
func @do_not_vectorize_depthwise_conv_withnon_1_batch(%input: memref<2x1x3x4xf32>, %filter: memref<1x1x4xf32>, %output: memref<2x1x2x4xf32>) {
  %0 = subview %input[0, 0, 0, 0] [2, 1, 3, 4] [1, 1, 1, 1]  : memref<2x1x3x4xf32> to memref<2x1x3x4xf32>
  %1 = subview %filter[0, 0, 0] [1, 1, 4] [1, 1, 1]  : memref<1x1x4xf32> to memref<1x1x4xf32>
  %2 = subview %output[0, 0, 0, 0] [2, 1, 2, 4] [1, 1, 1, 1]  : memref<2x1x2x4xf32> to memref<2x1x2x4xf32>
  // CHECK: linalg.depthwise_conv_2d_input_nhwc_filter_hwc
  linalg.depthwise_conv_2d_input_nhwc_filter_hwc {strides = dense<2> : tensor<2xi64>} ins(%0, %1 : memref<2x1x3x4xf32>, memref<1x1x4xf32>) outs(%2 : memref<2x1x2x4xf32>)
  return
}

// -----

// CHECK-LABEL: func @do_not_vectorize_depthwise_conv_with_non_1_output_height
func @do_not_vectorize_depthwise_conv_with_non_1_output_height(%input: memref<1x3x3x4xf32>, %filter: memref<1x1x4xf32>, %output: memref<1x2x2x4xf32>) {
  %0 = subview %input[0, 0, 0, 0] [1, 3, 3, 4] [1, 1, 1, 1]  : memref<1x3x3x4xf32> to memref<1x3x3x4xf32>
  %1 = subview %filter[0, 0, 0] [1, 1, 4] [1, 1, 1]  : memref<1x1x4xf32> to memref<1x1x4xf32>
  %2 = subview %output[0, 0, 0, 0] [1, 2, 2, 4] [1, 1, 1, 1]  : memref<1x2x2x4xf32> to memref<1x2x2x4xf32>
  // CHECK: linalg.depthwise_conv_2d_input_nhwc_filter_hwc
  linalg.depthwise_conv_2d_input_nhwc_filter_hwc {strides = dense<2> : tensor<2xi64>} ins(%0, %1 : memref<1x3x3x4xf32>, memref<1x1x4xf32>) outs(%2 : memref<1x2x2x4xf32>)
  return
}
