...
func @matmul_fill() {
  ...
  %lhs = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : !flow.dispatch.tensor<readonly:?x?xf32>{%m, %k}
  %rhs = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : !flow.dispatch.tensor<readonly:?x?xf32>{%k, %n}
  %result = hal.interface.binding.subspan type(storage_buffer) set(0) binding(2) : !flow.dispatch.tensor<writeonly:?x?xf32>{%m, %n}
  %wg_id_y = hal.interface.workgroup.id[1] : index
  %wg_count_y = hal.interface.workgroup.count[1] : index
  %wg_size_y = hal.interface.workgroup.size[1] : index
  %wg_id_x = hal.interface.workgroup.id[0] : index
...
//  CHECK-DAG:   %[[RET1:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(2)
//      CHECK:   %[[SOURCE:.+]] = flow.dispatch.tensor.load %[[ARG0]]
//      CHECK:   %[[RESHAPE:.+]] = tensor.expand_shape %[[SOURCE]]
//      CHECK:   %[[GENERIC:.+]] = linalg.generic
// CHECK-SAME:       ins(%[[RESHAPE]]
...
//  CHECK-DAG:   %[[LHS:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0)
//  CHECK-DAG:   %[[RHS:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1)
//  CHECK-DAG:   %[[RESULT0:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(2)
//  CHECK-DAG:   %[[RESULT1:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(3)
//      CHECK:   scf.for %[[IV0:.+]] =
//      CHECK:     scf.for %[[IV1:.+]] =
//  CHECK-DAG:       %[[LHS_TILE:.+]] = flow.dispatch.tensor.load %[[LHS]]
//  CHECK-DAG:       %[[RHS_TILE:.+]] = flow.dispatch.tensor.load %[[RHS]]
