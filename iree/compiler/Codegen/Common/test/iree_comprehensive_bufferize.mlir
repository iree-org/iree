...
func @matmul() {
  ...
  %lhs = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : !flow.dispatch.tensor<readonly:?x?xf32>{%m, %k}
  %rhs = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : !flow.dispatch.tensor<readonly:?x?xf32>{%k, %n}
  %init = hal.interface.binding.subspan type(storage_buffer) set(0) binding(2) : !flow.dispatch.tensor<readonly:?x?xf32>{%m, %n}
  %result = hal.interface.binding.subspan type(storage_buffer) set(0) binding(3) : !flow.dispatch.tensor<writeonly:?x?xf32>{%m, %n}
...
//  CHECK-DAG:   %[[LHS:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0)
//  CHECK-DAG:   %[[RHS:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1)
//  CHECK-DAG:   %[[INIT:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(2)
//  CHECK-DAG:   %[[RESULT:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(3)
...
func @matmul_fill() {
  ...
  %lhs = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : !flow.dispatch.tensor<readonly:?x?xf32>{%m, %k}
  %rhs = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : !flow.dispatch.tensor<readonly:?x?xf32>{%k, %n}
  %result = hal.interface.binding.subspan type(storage_buffer) set(0) binding(2) : !flow.dispatch.tensor<readwrite:?x?xf32>{%m, %n}
  %wg_id_y = hal.interface.workgroup.id[1] : index
  %wg_count_y = hal.interface.workgroup.count[1] : index
  %wg_size_y = hal.interface.workgroup.size[1] : index
  %wg_id_x = hal.interface.workgroup.id[0] : index
...
//  CHECK-DAG:   %[[LHS:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0)
//  CHECK-DAG:   %[[RHS:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1)
//  CHECK-DAG:   %[[RESULT:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(2)
//  CHECK-DAG:   %[[WG_ID_Y:.+]] = hal.interface.workgroup.id[1]
//  CHECK-DAG:   %[[WG_COUNT_Y:.+]] = hal.interface.workgroup.count[1]
//  CHECK-DAG:   %[[WG_SIZE_Y:.+]] = hal.interface.workgroup.size[1]
//  CHECK-DAG:   %[[WG_ID_X:.+]] = hal.interface.workgroup.id[0]
