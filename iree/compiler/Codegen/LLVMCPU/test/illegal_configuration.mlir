  ...
  hal.executable.variant @llvm, target = #hal.executable.target<"llvm", "embedded-elf-x86_64", {}> {
    ...
    builtin.module {
      func @illegal() {
        ...
        %lhs = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<4x8xf32>
        %rhs = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<8x16xf32>
        %result = hal.interface.binding.subspan type(storage_buffer) set(0) binding(2) : memref<4x16xf32>
