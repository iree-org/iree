...
//     CHECK: %[[A:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<4096x1024xvector<4xf32>>
//     CHECK: %[[B:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<4096x1024xvector<4xf32>>
...
func @resource_copy() {
  ...
  %0 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<4096x4096xf32>
  %1 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<4096x4096xf32>
...
//     CHECK: %[[A:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<4096x1024xvector<4xf16>>
//     CHECK: %[[B:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<4096x1024xvector<4xf16>>
...
func @resource_copy_f16() {
  ...
  %c0 = arith.constant 0 : index
  %0 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<4096x4096xf16>
  %1 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<4096x4096xf16>
...
//     CHECK: %[[A:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<4096x512xvector<4xf32>>
//     CHECK: %[[B:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<4096x512xvector<4xf32>>
...
func @resource_copy_dynamic_shape() {
  ...
  // CHECK: %[[INPUT:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<?x8x?x32xvector<4xf32>>{%[[DIM0]], %[[DIM1]]}
  // CHECK: %[[OUTPUT:.+]] = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<?x8x?x32xvector<4xf32>>{%[[DIM0]], %[[DIM1]]}
  %0 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<?x8x?x128xf32>{%dim0, %dim1}
  %1 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<?x8x?x128xf32>{%dim0, %dim1}
...
func @resource_copy_dynamic_last_dim() {
  ...
  // CHECK: hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<4096x?xf32>
  // CHECK: hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<4096x?xf32>
  %0 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<4096x?xf32>{%dim}
  %1 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<4096x?xf32>{%dim}
...
func @do_not_vectorize_odd_vector_size() {
  ...
  %0 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<4x3xf32>
  // CHECK: hal.interface.binding.subspan
  // CHECK-SAME: memref<4x3xf32>
  %1 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<4x3xf32>
...
func @vectorize_binding_subspan() {
  %cst = arith.constant 0.000000e+00 : f32
  %c0 = arith.constant 0 : index
  // CHECK: hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<4096x1024xvector<4xf32>>
  // CHECK: hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<4096x1024xvector<4xf32>>
  %0 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<4096x4096xf32>
  %1 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<4096x4096xf32>
  %mat = vector.transfer_read %0[%c0, %c0], %cst : memref<4096x4096xf32>, vector<32x8xf32>
  vector.transfer_write %mat, %1[%c0, %c0] : vector<32x8xf32>, memref<4096x4096xf32>
...
func @scalarize_vector_transfer_op(%arg: vector<3xf32>) -> (vector<3xf32>) {
  ...
  %0 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<20xf32>
  %2 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<20xf32>
