  ...
  scf.for %arg0 = %workgroup_id_z to %c112 step %workgroup_count_z {
    ...
    scf.for %arg1 = %4 to %c112 step %5 {
      ...
      scf.for %arg2 = %6 to %c32 step %7 {
        ...
        %21 = linalg.fill(%cst, %9) {lowering.config = #iree_codegen.lowering.config<tile_sizes = [[0, 1, 4, 32], [0, 1, 2, 4], [0, 0, 0, 0, 1, 1, 4]], native_vector_size = []>} : f32, tensor<1x1x4x32xf32> -> tensor<1x1x4x32xf32>
        %22 = linalg.conv_2d_nhwc_hwcf {dilations = dense<1> : tensor<2xi64>, lowering.config = #iree_codegen.lowering.config<tile_sizes = [[0, 1, 4, 32], [0, 1, 2, 4], [0, 0, 0, 0, 1, 1, 4]], native_vector_size = []>, strides = dense<2> : tensor<2xi64>} ins(%19, %20 : tensor<1x?x?x3xf32>, tensor<3x3x3x32xf32>) outs(%21 : tensor<1x1x4x32xf32>) -> tensor<1x1x4x32xf32>
        %23 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>, affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>], iterator_types = ["parallel", "parallel", "parallel", "parallel"]} ins(%22, %8 : tensor<1x1x4x32xf32>, tensor<1x1x4x32xf32>) outs(%9 : tensor<1x1x4x32xf32>) attrs =  {lowering.config = #iree_codegen.lowering.config<tile_sizes = [[0, 1, 4, 32], [0, 1, 2, 4], [0, 0, 0, 0, 1, 1, 4]], native_vector_size = []>} {
