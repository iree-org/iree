      ...
      builtin.func @tensor_insert_slice() {
        ...
        scf.for %arg0 = %2 to %d0 step %3 {
          ...
          scf.for %arg1 = %5 to %d1 step %6 {
            ...
            linalg.generic(%8, %11) : memref<?x?xi32, affine_map<(d0, d1)[s0, s1] -> (d0 * s1 + s0 + d1)>>, memref<?x?xi32, affine_map<(d0, d1)[s0, s1] -> (d0 * s1 + s0 + d1)>>
          }
        }
        return
      }
        ...
        scf.for %arg0 = %2 to %c224 step %3 {
          ...
          scf.for %arg1 = %5 to %c224 step %6 {
            ...
            scf.for %arg2 = %8 to %c3 step %9 {
              ...
              linalg.generic(%11, %12) : memref<1x?x?x?xf32, affine_map<(d0, d1, d2, d3)[s0] -> (d0 * 150528 + s0 + d1 * 672 + d2 * 3 + d3)>>, memref<1x?x?x?xf32, affine_map<(d0, d1, d2, d3)[s0] -> (d0 * 151875 + s0 + d1 * 675 + d2 * 3 + d3)>>
            }
          }
        }
        return
