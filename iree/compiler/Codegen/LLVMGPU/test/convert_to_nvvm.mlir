...
func @abs_ex_dispatch_0() {
  ...
  %0 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(4) offset(%c128) : memref<16xf32>
  %1 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<16xi32>
  %2 = hal.interface.binding.subspan type(storage_buffer) set(1) binding(2) : memref<16xf32>
...
func @abs_dynamic() {
  ...
  %0 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(4) offset(%c128) : memref<?xf32>{%s}
  %1 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) : memref<16xi32>
  %2 = hal.interface.binding.subspan type(storage_buffer) set(1) binding(2) : memref<16xf32>
  %3 = "gpu.block_id"() {dimension = "x"} : () -> index
  %4 = "gpu.block_dim"() {dimension = "x"} : () -> index
...
func @dead_symbol() {
  %c0 = arith.constant 0 : index
  %c128 = arith.constant 128 : index
  %1 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<16xi32>
  %2 = hal.interface.binding.subspan type(storage_buffer) set(1) binding(2) : memref<16xf32>
  %3 = "gpu.block_id"() {dimension = "x"} : () -> index
  %4 = "gpu.block_dim"() {dimension = "x"} : () -> index
  %5 = "gpu.thread_id"() {dimension = "x"} : () -> index
...
func @mixed_type() {
  %c0 = arith.constant 0 : index
  %c128 = arith.constant 128 : index
  %0 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) offset(%c128) : memref<16xf32>
  %1 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) offset(%c0) : memref<16xi32>
  %2 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(1) : memref<16xf32>
  %3 = "gpu.block_id"() {dimension = "x"} : () -> index
  %4 = "gpu.block_dim"() {dimension = "x"} : () -> index
  %5 = "gpu.thread_id"() {dimension = "x"} : () -> index
  %6 = arith.muli %3, %4 : index
