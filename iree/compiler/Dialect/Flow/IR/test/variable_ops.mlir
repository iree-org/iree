...
func @loaded_indirect() {
  ...
  %0 = flow.variable.address @v_loaded : !util.ptr<tensor<4xf32>>
...
func @stored_indirect() {
  ...
  %1 = flow.variable.address @v_stored : !util.ptr<tensor<4xf32>>
  // CHECK-NEXT: flow.variable.store.indirect %[[VALUE]], %[[ADDR]]
  flow.variable.store.indirect %0, %1 : tensor<4xf32> -> !util.ptr<tensor<4xf32>>
  return
}
