...
func @constants() {
  // CHECK-DAG: %[[W:.+]] = arith.constant 1 : index
...
func @dependencies_with_dispatch() {
  ...
  %w = arith.constant 1 : index
  %c2 = arith.constant 2 : index
  %ct3 = arith.constant dense<3> : tensor<i32>
  // CHECK: flow.dispatch
  %d0 = flow.dispatch @dispatch0::@dispatch0[%w]() : () -> tensor<i32>
  // CHECK: addi
  %add0 = arith.addi %d0, %ct3 : tensor<i32>
  return
}
