...
func @outsideTieShape(%arg0: tensor<?xi32> {iree.reflection = {}}, %arg1: !shapex.ranked_shape<[?]> {iree.reflection = {}}) -> (tensor<?xi32> {iree.reflection = {}}) {
  ...
  // CHECK-NEXT:   %[[WORKLOAD0:.+]] = arith.constant 0 : index
...
func @outerOps(%arg0: tensor<4xf32>) -> tensor<4xf32> {
  // CHECK: %0 = arith.addf %arg0, %arg0 : tensor<4xf32>
  ...
  // CHECK-NEXT:   %[[WORKLOAD:.+]] = arith.constant 4 : index
  ...
  // CHECK: %2 = arith.addf %1, %1 : tensor<4xf32>
  %2 = arith.addf %1, %1 : tensor<4xf32>
...
func @nondependentOuterOps(%arg0: tensor<4xf32>) -> tensor<4xf32> {
  %cst = arith.constant 4 : index
  // CHECK-NEXT: %[[ADD1:.+]] = arith.addf %arg0, %arg0 : tensor<4xf32>
  %add1 = arith.addf %arg0, %arg0 : tensor<4xf32>
  ...
  // CHECK-NEXT: }
  // CHECK-NEXT: %[[ADD2:.+]] = arith.addf %[[S]], %arg0 : tensor<4xf32>
  %add2 = arith.addf %d2, %arg0 : tensor<4xf32>
...
func @interleavedOuterOps(%arg0: tensor<4xf32>) -> tensor<4xf32> {
  ...
  // CHECK-NEXT:   %[[WORKLOAD1:.+]] = arith.constant 4 : index
  ...
  // CHECK-NEXT: %1 = arith.addf %0, %0 : tensor<4xf32>
  %1 = arith.addf %0, %0 : tensor<4xf32>
  // CHECK-NEXT: %2 = flow.ex.stream.fragment(%1) : (tensor<4xf32>) -> tensor<4xf32> =
  // CHECK-NEXT:     (%arg1: tensor<4xf32>) -> tensor<4xf32> {
  // CHECK-NEXT:   %[[WORKLOAD2:.+]] = arith.constant 4 : index
...
func @interleavedDot(%arg0: tensor<4x4xf32>) -> tensor<4x4xf32> {
  ...
  // CHECK-NEXT:   %[[WORKLOAD:.+]] = arith.constant 16 : index
...
func @interstream_readback() -> (tensor<i32>, tensor<f32>, tensor<2xf32>) {
  %w = arith.constant 1 : index
  //      CHECK: %[[S1:.+]]:2 = flow.ex.stream.fragment
  //      CHECK:   %[[W:.+]] = arith.constant 1 : index
  //      CHECK:   %[[D1:.+]] = flow.dispatch @dispatch_1::@dispatch_1
  //  CHECK-DAG:   %[[D2:.+]] = flow.dispatch @dispatch_2::@dispatch_2
