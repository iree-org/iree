...
static bool inlineConstantGlobalStores(GlobalTable &globalTable) {
  return globalTable.forEach([&](Global &global) {
    ...
    for (auto storeOp : global.storeOps) {
      // Ignore initializer stores - they are getting overridden here.
      if (storeOp->getParentOfType<IREE::Util::InitializerOp>()) continue;
 ...
 public:
  ...
  void runOnOperation() override {
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      bool didChange = false;
      didChange = inlineConstantGlobalStores(globalTable) || didChange;
      didChange = renameChainedGlobals(globalTable) || didChange;
      didChange = updateGlobalImmutability(globalTable) || didChange;
      didChange = inlineConstantGlobalLoads(globalTable) || didChange;
      didChange = eraseUnusedGlobals(globalTable) || didChange;
      didChange = deduplicateConstantGlobals(globalTable) || didChange;
      if (!didChange) break;
    }
  }
};
