...
class LLVMAOTTargetBackend final : public TargetBackend {
 public:
  ...
  LogicalResult serializeExecutable(IREE::HAL::ExecutableTargetOp targetOp,
    ...
    auto sharedLibDataStatus = linkLLVMAOTObjects(objData);
    if (!sharedLibDataStatus.ok()) {
      return targetOp.emitError(
          "Can't link executable and generate target dylib");
    }

    dyLibExecutableDef.library_embedded = {sharedLibDataStatus.value().begin(),
                                           sharedLibDataStatus.value().end()};
