...
flow.executable @static_tiled_dispatch {
  ...
  module  {
    ...
    func @entry(%arg: !flow.dispatch.tensor<readonly:8x4xf32>, %ret: !flow.dispatch.tensor<writeonly:4x8xf32>) {
      ...
      // CHECK-NEXT: %[[ARG:.+]] = hal.interface.binding.subspan @io::@ro0[%c0] : !flow.dispatch.tensor<readonly:8x4xf32>
      // CHECK-NEXT: %[[RET:.+]] = hal.interface.binding.subspan @io::@wo1[%c0] : !flow.dispatch.tensor<writeonly:4x8xf32>
...
flow.executable @dynamic_tiled_dispatch {
  ...
  module  {
    ...
    func @entry(
        // CHECK-NEXT: %c0 = constant 0 : index
        // CHECK-DAG: %[[ARG:.+]] = hal.interface.binding.subspan @io::@ro0[%c0] : !flow.dispatch.tensor<readonly:7x?x24x?xf32>
        %arg: !flow.dispatch.tensor<readonly:7x?x24x?xf32>,
        // CHECK-DAG: %[[RET:.+]] = hal.interface.binding.subspan @io::@wo1[%c0] : !flow.dispatch.tensor<writeonly:?x?x1024xf32>
