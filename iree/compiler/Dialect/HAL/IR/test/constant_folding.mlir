...
hal.constant_pool @pool attributes {
  ...
  hal.constant_pool.span @cst_span : tensor<3xi8> = @_storage1[#util.byte_range<0, 3>] -> @pool_storage1_buffer[#util.byte_range<0, 3>]
  hal.constant_pool.splat @cst_splat = dense<1.000000e+00> : tensor<1xf32> -> @pool_splats[#util.byte_range<0, 4>]
}

// CHECK-LABEL: func @pools_identified
func @pools_identified() -> (tensor<2x3xf32>, tensor<3x2xf32>) {
  // CHECK-NEXT: = hal.constant.subspan @pool_storage1_buffer[#util.byte_range<0, 3>] : tensor<2x3xf32>
  %cst0 = hal.constant_pool.load @pool::@cst_span : tensor<2x3xf32>
  // CHECK-NEXT: = hal.constant.subspan @pool_splats[#util.byte_range<0, 4>] : tensor<3x2xf32>
