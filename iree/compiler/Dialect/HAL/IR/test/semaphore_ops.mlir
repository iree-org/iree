...
func @semaphore_status(%arg0 : !hal.semaphore) -> index {
  // CHECK: = hal.semaphore.status %arg0 : index
  %0 = hal.semaphore.status %arg0 : index
  return %0 : index
}
...
func @semaphore_query(%arg0 : !hal.semaphore) {
  // CHECK: = hal.semaphore.query %arg0 : index, index
  %status, %value = hal.semaphore.query %arg0 : index, index
  return
}
...
func @semaphore_await(%arg0 : !hal.semaphore) {
  %c0 = std.constant 0 : index
  // CHECK: = hal.semaphore.await %arg0, min_value = %c0 : index
  %0 = hal.semaphore.await %arg0, min_value = %c0 : index
  return
}
