...
func @interface_io_subspan(%dim0: index, %dim2: index) {
  ...
  // CHECK: = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) offset(%c8) : memref<?x4x?x16xi8>{%[[DIM0]], %[[DIM2]]}
  %0 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) offset(%c8) : memref<?x4x?x16xi8>{%dim0, %dim2}

  // CHECK: = hal.interface.binding.subspan type(storage_buffer) set(1) binding(2) alignment(16) : memref<16xi8>
  %1 = hal.interface.binding.subspan type(storage_buffer) set(1) binding(2) alignment(16) : memref<16xi8>
...
func @interface_io_subspan_wrong_dynamic_dim(%dim: index) {
  ...
  %0 = hal.interface.binding.subspan type(storage_buffer) set(0) binding(0) offset(%c8) : memref<?x4x?x16xi8>{%dim}
