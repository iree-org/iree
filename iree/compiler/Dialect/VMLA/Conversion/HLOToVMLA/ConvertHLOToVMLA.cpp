...
struct FftOpConversion : public OpConversionPattern<IREE::VMLA::FftPseudoOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    auto real_input_type =
        srcOp.getOperand(0).getType().cast<ShapedType>().getElementType();
    auto imag_input_type =
        srcOp.getOperand(0).getType().cast<ShapedType>().getElementType();

    // The input type should match for the real and imag components.
    ...
    rewriter.createOrFold<IREE::VMLA::FftOp>(
        ...
        input_shape, real_out, imag_out, TypeAttr::get(real_input_type),
        TypeAttr::get(imag_input_type));
