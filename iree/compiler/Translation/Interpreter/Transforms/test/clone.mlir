...
func @necessary_clone_not_removed() -> (memref<i32>, memref<i32>) {
  // CHECK: [[ORIG:%.+]] = iree_interp.constant[dense<1> : tensor<i32>]
  %original = iree_interp.constant[dense<1> : tensor<i32>] : memref<i32>
  // CHECK: [[CLONE:%.+]] = "iree_hl_interp.clone"([[ORIG]])
  %cloned = "iree_hl_interp.clone"(%original) : (memref<i32>) -> memref<i32>
  %other = iree_interp.constant[dense<2> : tensor<i32>] : memref<i32>
  %empty = iree_interp.constant[dense<[]> : tensor<0xi32>] : memref<0xi32>
