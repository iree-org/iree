...
module {
  ...
  func @const_f32(%arg0: memref<2x3xf32>)
  attributes  {iree.executable.export, iree.executable.workload = dense<[2, 3]> : tensor<2xi32>, iree.executable.workgroup_size = dense<[32, 1, 1]> : tensor<3xi32>, iree.ordinal = 0 : i32} {
    // CHECK: [[CONST0:%.*]] = spv.constant dense<1.000000e+00> : tensor<2x3xf32> : !spv.array<2 x !spv.array<3 x f32 [4]> [12]>
    // CHECK-NEXT: [[VAR0:%.*]] = spv.Variable init([[CONST0]]) : !spv.ptr<!spv.array<2 x !spv.array<3 x f32 [4]> [12]>, Function>
    // CHECK: [[NUM0PTR:%.*]] = spv.AccessChain [[VAR0]][{{.*}}]
    // CHECK-NEXT: [[NUM0:%.*]] = spv.Load "Function" [[NUM0PTR]] : f32
    ...
    %0 = "xla_hlo.constant"() {value = dense<1.0> : tensor<2x3xf32>} : () -> (tensor<2x3xf32>)
    ...
    iree.store_output(%2 : tensor<2x3xf32>, %arg0 : memref<2x3xf32>)
    iree.return
  }
}
...
module {
  // CHECK: spv.globalVariable [[GLOBALIDVAR:@.*]] built_in("GlobalInvocationId") : !spv.ptr<vector<3xi32>, Input>
  // CHECK: spv.globalVariable [[ARG0VAR:@.*]] bind(0, 0)
  func @const_i32(%arg0: memref<2x3xi32>)
  attributes  {iree.executable.export, iree.executable.workload = dense<[2, 3]> : tensor<2xi32>, iree.executable.workgroup_size = dense<[32, 1, 1]> : tensor<3xi32>, iree.ordinal = 0 : i32} {
    // CHECK: [[CONST0:%.*]] = spv.constant dense<1> : tensor<2x3xi32> : !spv.array<2 x !spv.array<3 x i32 [4]> [12]>
    // CHECK-NEXT: [[VAR0:%.*]] = spv.Variable init([[CONST0]]) : !spv.ptr<!spv.array<2 x !spv.array<3 x i32 [4]> [12]>, Function>
    // CHECK: [[NUM0PTR:%.*]] = spv.AccessChain [[VAR0]][{{.*}}]
    // CHECK-NEXT: [[NUM0:%.*]] = spv.Load "Function" [[NUM0PTR]] : i32
    ...
    %0 = "xla_hlo.constant"() {value = dense<1> : tensor<2x3xi32>} : () -> (tensor<2x3xi32>)
