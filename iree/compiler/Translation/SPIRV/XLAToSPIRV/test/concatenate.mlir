...
module {
  ...
  // CHECK-SAME: %[[ARG0:[a-zA-Z0-9$._-]+]]: !spv.ptr<!spv.struct<!spv.array<64 x f32, stride=4> [0]>, StorageBuffer>
  // CHECK-SAME: %[[ARG1:[a-zA-Z0-9$._-]+]]: !spv.ptr<!spv.struct<!spv.array<10 x f32, stride=4> [0]>, StorageBuffer>
  func @concatenate(%arg0: memref<1x64xf32>, %arg1 : memref<1x10xf32>, %arg2 : memref<1x74xf32>)
  attributes {iree.dispatch_fn_name = "concatenate"} {
    // CHECK: spv.loop
    // CHECK: spv.loop
