...
// CHECK: [[MAP0:#[a-zA-Z0-9_]*]] = affine_map<([[DIM0:[a-zA-Z0-9_]*]], {{.*}}, {{.*}}) -> ([[DIM0]])>
module {
  // CHECK: func @reduction_entry
  // CHECK-SAME: [[ARG0:%[a-zA-Z0-9_]*]]: memref<5xi32> {iree.index_computation_info = {{\[\[}}[[MAP0]]{{\]\]}}}
  // CHECK-SAME: [[ARG2:%[a-zA-Z0-9_]*]]: memref<i32> {iree.index_computation_info = {{\[\[}}affine_map<(d0, d1, d2) -> (0)>{{\]\]}}}
...
// CHECK: [[MAP0:#[a-zA-Z0-9_]*]] = affine_map<([[DIM0:[a-zA-Z0-9_]*]], [[DIM1:[a-zA-Z0-9_]*]]) -> ([[DIM1]], [[DIM0]])>
module {
  // CHECK: func @reduction_2D_dim0_entry
  // CHECK-SAME: [[ARG0:%[a-zA-Z0-9_]*]]: memref<5x4xi32> {iree.index_computation_info = {{\[\[}}[[MAP0]]{{\]\]}}}
  // CHECK-SAME: [[ARG2:%[a-zA-Z0-9_]*]]: memref<4xi32> {iree.index_computation_info = {{\[\[}}affine_map<(d0, d1) -> (d0)>{{\]\]}}}
...
module {
  ...
  // CHECK-SAME: [[ARG0:%[a-zA-Z0-9_]*]]: memref<5x4xi32> {iree.index_computation_info = {{\[\[}}[[MAP0]]{{\]\]}}}
  // CHECK-SAME: [[ARG2:%[a-zA-Z0-9_]*]]: memref<5xi32> {iree.index_computation_info = {{\[\[}}affine_map<(d0, d1) -> (d1)>{{\]\]}}}
