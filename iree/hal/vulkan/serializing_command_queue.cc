...
#pragma clang diagnostic pop
  ...
  while (!deferred_submissions_.empty()) {
    ...
    ASSIGN_OR_RETURN(bool ready_to_submit,
                     TryToPrepareSemaphores(submission->batch.wait_semaphores,
                                            submission->batch.signal_semaphores,
                                            submission->fence, &wait_semaphores,
                                            &signal_semaphores));

    if (ready_to_submit) {
      submit_infos.emplace_back();
      PrepareSubmitInfo(wait_semaphores, submission->batch.command_buffers,
                        signal_semaphores, &submit_infos.back(), &arena);

      submit_fences.push_back(submission->fence->value());
      pending_fences_.emplace_back(std::move(submission->fence));
    } else {
