...
TEST(BufferTest, Allocate) {
  auto buffer = HeapBuffer::Allocate(
  ...
  EXPECT_EQ(IREE_HAL_MEMORY_TYPE_HOST_LOCAL, buffer->memory_type());
...
TEST(BufferTest, WrapExternal) {
  ...
  std::vector<uint8_t> src_data = {0, 1, 2, 3};
  auto buffer = HeapBuffer::Wrap(IREE_HAL_MEMORY_TYPE_DEVICE_LOCAL,
                                 IREE_HAL_BUFFER_USAGE_ALL,
                                 absl::MakeConstSpan(src_data));
  EXPECT_EQ(IREE_HAL_MEMORY_TYPE_DEVICE_LOCAL, buffer->memory_type());

  // Should fail (for now) as the buffer is not host visible.
  EXPECT_TRUE(IsPermissionDenied(buffer->Fill8(0, IREE_WHOLE_BUFFER, 0x99u)));
}
...
TEST(BufferTest, DoesOverlap) {
  ...
  auto other_buffer = HeapBuffer::Allocate(IREE_HAL_BUFFER_USAGE_ALL, 128);
  EXPECT_FALSE(Buffer::DoesOverlap(parent_buffer.get(), 0, IREE_WHOLE_BUFFER,
...
TEST(BufferTest, Subspan) {
  ...
  IREE_EXPECT_OK(subspan_buffer->Fill8(1, IREE_WHOLE_BUFFER, 0xFFu));
  std::vector<uint8_t> actual_data(src_data.size());
  IREE_EXPECT_OK(
  ...
  // Subspan with IREE_WHOLE_BUFFER should get the remaining size (or zero).
  IREE_ASSERT_OK_AND_ASSIGN(
      auto whole_subspan_buffer,
      Buffer::Subspan(parent_buffer, 1, IREE_WHOLE_BUFFER));
  ...
  IREE_ASSERT_OK(Buffer::Subspan(subspan_buffer, 2, IREE_WHOLE_BUFFER));
}
...
TEST(BufferTest, SubspanIdentity) {
  ...
  EXPECT_EQ(parent_buffer.get(),
            Buffer::Subspan(parent_buffer, 0, IREE_WHOLE_BUFFER).value().get());
  EXPECT_EQ(parent_buffer.get(),
...
TEST(BufferTest, SubspanOutOfRange) {
  ...
  EXPECT_TRUE(IsOutOfRange(
      Buffer::Subspan(parent_buffer, 5, IREE_WHOLE_BUFFER).status()));
...
TEST(BufferTest, Fill8) {
  auto buffer = HeapBuffer::Allocate(IREE_HAL_BUFFER_USAGE_MAPPING, 5);
  ASSERT_TRUE(buffer);
...
TEST(BufferTest, Fill8BadMode) {
  ...
  auto nonmapping_buffer =
      HeapBuffer::Allocate(IREE_HAL_BUFFER_USAGE_TRANSFER, 4);
  EXPECT_TRUE(IsPermissionDenied(
      nonmapping_buffer->Fill8(0, IREE_WHOLE_BUFFER, 0x99u)));

  // Fail to fill constant buffers.
  std::vector<uint8_t> const_data = {1, 2, 3};
  auto constant_buffer = HeapBuffer::Wrap(IREE_HAL_MEMORY_TYPE_HOST_LOCAL,
                                          IREE_HAL_BUFFER_USAGE_MAPPING,
                                          absl::MakeConstSpan(const_data));
  EXPECT_TRUE(
      IsPermissionDenied(constant_buffer->Fill8(0, IREE_WHOLE_BUFFER, 0x99u)));
}
...
TEST(BufferTest, Fill8Subspan) {
  ...
  IREE_EXPECT_OK(subspan_buffer->Fill8(2, IREE_WHOLE_BUFFER, 0xDDu));
  IREE_EXPECT_OK(buffer->ReadData(0, actual_data.data(), actual_data.size()));
  EXPECT_THAT(actual_data, ElementsAre(0, 0, 0, 0xDD, 0));
}

TEST(BufferTest, Fill16) {
  auto buffer = HeapBuffer::Allocate(IREE_HAL_BUFFER_USAGE_MAPPING, 9);
  ASSERT_TRUE(buffer);
  ...
  // Fill the remaining parts of the buffer by using IREE_WHOLE_BUFFER.
  auto aligned_buffer = HeapBuffer::Allocate(IREE_HAL_BUFFER_USAGE_MAPPING, 8);
...
TEST(BufferTest, Fill16BadMode) {
  ...
  auto nonmapping_buffer =
      HeapBuffer::Allocate(IREE_HAL_BUFFER_USAGE_TRANSFER, 4);
  EXPECT_TRUE(IsPermissionDenied(
      nonmapping_buffer->Fill16(0, IREE_WHOLE_BUFFER, 0x99AAu)));
  ...
  auto constant_buffer = HeapBuffer::Wrap(IREE_HAL_MEMORY_TYPE_HOST_LOCAL,
                                          ...
                                          absl::MakeConstSpan(const_data));
  EXPECT_TRUE(IsPermissionDenied(
      constant_buffer->Fill16(0, IREE_WHOLE_BUFFER, 0x99AAu)));
}
...
TEST(BufferTest, Fill16Subspan) {
  ...
  IREE_EXPECT_OK(subspan_buffer->Fill16(2, IREE_WHOLE_BUFFER, 0xAABBu));
  IREE_EXPECT_OK(buffer->ReadData(0, actual_data.data(), actual_data.size()));
...
TEST(BufferTest, Fill32BadMode) {
  ...
  auto nonmapping_buffer =
      HeapBuffer::Allocate(IREE_HAL_BUFFER_USAGE_TRANSFER, 4);
  EXPECT_TRUE(IsPermissionDenied(
      nonmapping_buffer->Fill32(0, IREE_WHOLE_BUFFER, 0x99AABBCCu)));
  ...
  auto constant_buffer = HeapBuffer::Wrap(IREE_HAL_MEMORY_TYPE_HOST_LOCAL,
                                          ...
                                          absl::MakeConstSpan(const_data));
  EXPECT_TRUE(IsPermissionDenied(
      constant_buffer->Fill32(0, IREE_WHOLE_BUFFER, 0x99AABBCCu)));
}
...
TEST(BufferTest, Fill32Subspan) {
  ...
  IREE_EXPECT_OK(subspan_buffer->Fill32(0, IREE_WHOLE_BUFFER, 0xAABBCCDDu));
  IREE_EXPECT_OK(buffer->ReadData(0, actual_data.data(), actual_data.size()));
  EXPECT_THAT(actual_data,
...
TEST(BufferTest, ReadDataOutOfRange) {
  ...
  EXPECT_TRUE(IsInvalidArgument(
      buffer->ReadData(0, partial_data.data(), IREE_WHOLE_BUFFER)));
}
...
TEST(BufferTest, WriteDataOutOfRange) {
  ...
  EXPECT_TRUE(IsInvalidArgument(
      buffer->WriteData(0, partial_data.data(), IREE_WHOLE_BUFFER)));
}
...
TEST(BufferTest, CopyDataBadMode) {
  ...
  auto nonmapping_src_buffer =
      HeapBuffer::Allocate(IREE_HAL_BUFFER_USAGE_TRANSFER, 4);
  auto nonmapping_dst_buffer =
      HeapBuffer::Allocate(IREE_HAL_BUFFER_USAGE_TRANSFER, 4);
  EXPECT_TRUE(IsPermissionDenied(nonmapping_dst_buffer->CopyData(
      0, nonmapping_src_buffer.get(), 0, IREE_WHOLE_BUFFER)));
  EXPECT_TRUE(IsPermissionDenied(nonmapping_src_buffer->CopyData(
      0, nonmapping_dst_buffer.get(), 0, IREE_WHOLE_BUFFER)));
  ...
  auto src_buffer = HeapBuffer::AllocateCopy(
      ...
      src_data.data(), src_data.size());
  EXPECT_TRUE(IsPermissionDenied(
      constant_buffer->CopyData(0, src_buffer.get(), 0, IREE_WHOLE_BUFFER)));
}
...
TEST(BufferTest, MapMemory) {
  ...
  auto buffer = HeapBuffer::AllocateCopy(
      IREE_HAL_BUFFER_USAGE_TRANSFER | IREE_HAL_BUFFER_USAGE_MAPPING,
      IREE_HAL_MEMORY_ACCESS_READ, src_data.data(), src_data.size());
  ASSERT_TRUE(buffer);
...
TEST(BufferTest, MapMemoryOutOfRange) {
  ...
  EXPECT_TRUE(IsOutOfRange(
      buffer->MapMemory<uint16_t>(IREE_HAL_MEMORY_ACCESS_READ, 0, 123)
          .status()));
  EXPECT_TRUE(IsOutOfRange(
      buffer->MapMemory<uint16_t>(IREE_HAL_MEMORY_ACCESS_READ, 5, 1231)
          .status()));
  EXPECT_TRUE(
      IsOutOfRange(buffer
                       ->MapMemory<uint16_t>(IREE_HAL_MEMORY_ACCESS_READ, 6,
                                             IREE_WHOLE_BUFFER)
                       .status()));
  EXPECT_TRUE(IsOutOfRange(
      buffer->MapMemory<uint16_t>(IREE_HAL_MEMORY_ACCESS_READ, 1236, 1)
          .status()));
}
...
TEST(BufferTest, MapMemoryBadMode) {
  ...
  EXPECT_TRUE(IsPermissionDenied(
      read_buffer->MapMemory<uint8_t>(IREE_HAL_MEMORY_ACCESS_WRITE).status()));
  EXPECT_TRUE(IsPermissionDenied(
      read_buffer->MapMemory<uint8_t>(IREE_HAL_MEMORY_ACCESS_DISCARD_WRITE)
          .status()));
  EXPECT_TRUE(IsPermissionDenied(
      read_buffer
          ...
          .status()));
  EXPECT_TRUE(IsInvalidArgument(
      read_buffer->MapMemory<uint8_t>(IREE_HAL_MEMORY_ACCESS_NONE).status()));
}
...
TEST(BufferTest, MapMemoryWrite) {
  ...
  auto buffer = HeapBuffer::AllocateCopy(
      IREE_HAL_BUFFER_USAGE_TRANSFER | IREE_HAL_BUFFER_USAGE_MAPPING,
      IREE_HAL_MEMORY_ACCESS_ALL, src_data.data(), src_data.size());
  ASSERT_TRUE(buffer);
...
TEST(BufferTest, MapMemorySubspan) {
  ...
  auto external_buffer = HeapBuffer::WrapMutable(
      ...
      absl::MakeSpan(external_data));
  IREE_ASSERT_OK_AND_ASSIGN(auto external_subspan_buffer,
                            Buffer::Subspan(external_buffer, 0, 1));
  IREE_ASSERT_OK_AND_ASSIGN(
      mapping,
      external_subspan_buffer->MapMemory<uint8_t>(IREE_HAL_MEMORY_ACCESS_ALL));
