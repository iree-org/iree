...
class MockCommandBuffer : public ::testing::StrictMock<CommandBuffer> {
 public:
  ...
  MOCK_METHOD(Status, SignalEvent,
              (Event*, ExecutionStageBitfield source_stage_mask), (override));
  ...
  MOCK_METHOD(Status, FillBuffer,
              (Buffer*, device_size_t target_offset, device_size_t length,
               const void*, size_t pattern_length),
              (override));

  MOCK_METHOD(Status, DiscardBuffer, (Buffer*), (override));

  MOCK_METHOD(Status, UpdateBuffer,
              (const void*, device_size_t source_offset, Buffer*,
               device_size_t target_offset, device_size_t length),
              (override));

  MOCK_METHOD(Status, CopyBuffer,
              (Buffer*, device_size_t source_offset, Buffer*,
               device_size_t target_offset, device_size_t length),
              (override));

  MOCK_METHOD(Status, PushConstants,
              (ExecutableLayout*, size_t offset,
               absl::Span<const uint32_t> values),
              (override));

  MOCK_METHOD(Status, PushDescriptorSet,
              (ExecutableLayout*, int32_t set,
               absl::Span<const DescriptorSet::Binding> bindings),
              (override));
  MOCK_METHOD(Status, BindDescriptorSet,
              (ExecutableLayout*, int32_t set, DescriptorSet*,
               absl::Span<const device_size_t> dynamic_offsets),
              (override));

  MOCK_METHOD(Status, Dispatch,
              (Executable*, int32_t entry_point,
               (std::array<uint32_t, 3> workgroups)),
              (override));

  MOCK_METHOD(Status, DispatchIndirect,
              (Executable*, int32_t entry_point, Buffer*,
