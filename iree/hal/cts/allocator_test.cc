...
TEST_P(AllocatorTest, QueryBufferCompatibility) {
  iree_hal_buffer_compatibility_t buffer_compatibility;

  buffer_compatibility = iree_hal_allocator_query_buffer_compatibility(
      device_allocator_,
      IREE_HAL_MEMORY_TYPE_HOST_LOCAL | IREE_HAL_MEMORY_TYPE_DEVICE_VISIBLE,
      /*allowed_usage=*/IREE_HAL_BUFFER_USAGE_MAPPING,
      /*intended_usage=*/IREE_HAL_BUFFER_USAGE_MAPPING, 1024);
  EXPECT_TRUE(iree_all_bits_set(buffer_compatibility,
  ...
  // TODO(scotttodd): expand coverage here with more buffer types
}
...
TEST_P(AllocatorTest, AllocateBuffer) {
  ...
  EXPECT_TRUE((iree_hal_buffer_memory_type(buffer) & memory_type) ==
              memory_type);
  EXPECT_TRUE((iree_hal_buffer_allowed_usage(buffer) & buffer_usage) ==
              buffer_usage);
