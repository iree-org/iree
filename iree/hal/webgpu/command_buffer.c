...
iree_status_t iree_hal_webgpu_command_buffer_create(
  ...
  *out_command_buffer = NULL;
  ...
  if (iree_status_is_ok(status)) {
    iree_hal_resource_initialize(&iree_hal_webgpu_command_buffer_vtable,
...
static void iree_hal_webgpu_command_buffer_reset(
  ...
  if (command_buffer->state.compute_pass) {
    wgpuComputePassEncoderEndPass(command_buffer->state.compute_pass);
  }
...
static iree_hal_command_buffer_mode_t iree_hal_webgpu_command_buffer_mode(
    const iree_hal_command_buffer_t* base_command_buffer) {
  const iree_hal_webgpu_command_buffer_t* command_buffer =
      (const iree_hal_webgpu_command_buffer_t*)(base_command_buffer);
  return command_buffer->mode;
}

static iree_hal_command_category_t
iree_hal_webgpu_command_buffer_allowed_categories(
    const iree_hal_command_buffer_t* base_command_buffer) {
  const iree_hal_webgpu_command_buffer_t* command_buffer =
      (const iree_hal_webgpu_command_buffer_t*)(base_command_buffer);
...
static iree_status_t iree_hal_webgpu_command_buffer_fill_buffer(
  ...
  // the fill.
  wgpuComputePassEncoderDispatch(compute_pass, length, 1, 1);
...
static iree_status_t iree_hal_webgpu_command_buffer_dispatch(
  ...
  wgpuComputePassEncoderDispatch(compute_pass, workgroup_x, workgroup_y,
                                 workgroup_z);
...
const iree_hal_command_buffer_vtable_t iree_hal_webgpu_command_buffer_vtable = {
    ...
    .mode = iree_hal_webgpu_command_buffer_mode,
    .allowed_categories = iree_hal_webgpu_command_buffer_allowed_categories,
