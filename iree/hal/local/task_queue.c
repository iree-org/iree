...
static iree_status_t iree_hal_semaphore_list_clone(
  ...
  memcpy(out_target_list->semaphores, source_list->semaphores, semaphores_size);
  memcpy(out_target_list->payload_values, source_list->payload_values,
         payload_values_size);
...
static iree_status_t iree_hal_task_queue_wait_cmd_allocate(
  ...
  // Clone the wait semaphores from the batch - we don't retain them but do
  // need to capture them and their payloads.
...
static iree_status_t iree_hal_task_queue_issue_cmd_allocate(
    ...
    const iree_hal_command_buffer_t** command_buffers,
...
static iree_status_t iree_hal_task_queue_retire_cmd_allocate(
  ...
  // Clone the signal semaphores from the batch - we don't retain them but do
  // need to capture them and their payloads.
  if (iree_status_is_ok(status)) {
