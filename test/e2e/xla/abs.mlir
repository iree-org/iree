...
func @tensor() -> tensor<4xf32> {
  ...
  %unf = iree.do_not_optimize(%input) : tensor<4xf32>
  %result = "xla_hlo.abs"(%unf) : (tensor<4xf32>) -> tensor<4xf32>
...
func @scalar() -> tensor<f32> {
  ...
  %unf = iree.do_not_optimize(%input) : tensor<f32>
  %result = "xla_hlo.abs"(%unf) : (tensor<f32>) -> tensor<f32>
