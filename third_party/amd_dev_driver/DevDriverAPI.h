...
typedef struct DevDriverFeatureRGP
{
    union
    {
        struct
        {
            uint32_t m_disableEtw : 1;              // disable Etw protocol when creating listener
            uint32_t m_enableInstructionTrace : 1;  // enable instruction tracing
            uint32_t m_reserved   : 30;             // reserve for future use
        };
        uint32_t m_u32All;
        uint32_t m_uReserved;                       // pad to 64-bit
    };
    union
    {
        struct
        {
            uint32_t m_dispatchStart;               // OpenCL dispatch range start
            uint32_t m_dispatchEnd;                 // OpenCL dispatch range end
        };
        uint32_t m_frameNumber;                     // DX/Vk frame number to capture
    };
    uint64_t m_apiPSOHash;                          // The API PSO hash for detailed instruction trace
