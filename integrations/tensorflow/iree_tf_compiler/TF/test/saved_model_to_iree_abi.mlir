...
// CHECK: %[[R:.*]]:2 = call @__inference_binary_func_70(%arg0, %arg1)
// CHECK: return %[[R]]#0, %[[R]]#1 : tensor<16xf32>, tensor<16xf32>
...
// CHECK: %[[R:.*]] = call @__inference_unary_func_240(%arg0)
...
module @unary_func attributes {tf.versions = {bad_consumers = [], min_consumer = 12 : i32, producer = 729 : i32}, tf_saved_model.semantics}  {
  func @__inference_unary_func_240(%arg0: tensor<16xf32> {tf._user_specified_name = "a", tf_saved_model.index_path = [0]}) -> (tensor<16xf32> {tf_saved_model.index_path = []}) attributes {tf._construction_context = "kEagerRuntime", tf._input_shapes = [#tf.shape<16>], tf_saved_model.exported_names = ["unary_func"]} {
    %0 = "tf.Identity"(%arg0) {device = ""} : (tensor<16xf32>) -> tensor<16xf32>
...
// CHECK-SAME{LITERAL}: iree.abi = "{\22a\22:[[\22ndarray\22,\22f32\22,1,16],[\22ndarray\22,\22f32\22,1,16]],\22r\22:[[\22ndarray\22,\22f32\22,1,16],[\22ndarray\22,\22f32\22,1,16]],\22v\22:1}"
// CHECK: %[[R:.+]]:2 = call @__inference_return_list_260(%arg0, %arg1)
// CHECK: return %[[R]]#0, %[[R]]#1 : tensor<16xf32>, tensor<16xf32>
...
// CHECK: %[[L1:.+]] = iree.list.get %[[L0]][%[[c0_0]]] : !iree.list<?> -> tensor<16xf32>
// CHECK: %[[c1:.+]] = constant 1 : index
// CHECK: %[[L2:.+]] = iree.list.get %[[L0]][%[[c1]]] : !iree.list<?> -> tensor<16xf32>
// CHECK: %[[c1_1:.+]] = constant 1 : index
// CHECK: %[[L3:.+]] = iree.list.get %arg0[%[[c1_1]]] : !iree.list<?> -> !iree.list<?>
// CHECK: %[[c0_2:.+]] = constant 0 : index
// CHECK: %[[L4:.+]] = iree.list.get %[[L3]][%[[c0_2]]] : !iree.list<?> -> tensor<16xf32>
// CHECK: %[[c1_3:.+]] = constant 1 : index
// CHECK: %[[L5:.+]] = iree.list.get %[[L3]][%[[c1_3]]] : !iree.list<?> -> tensor<16xf32>
// CHECK: %[[RESULT:.+]]:4 = call @__inference_dict_nest_190(%[[L1]], %[[L2]], %[[L4]], %[[L5]], %arg1) : (tensor<16xf32>, tensor<16xf32>, tensor<16xf32>, tensor<16xf32>, tensor<f32>) -> (tensor<16xf32>, tensor<16xf32>, tensor<16xf32>, tensor<16xf32>)
...
// CHECK: iree.list.set %[[R8]][%[[c0_5]]], %[[RESULT]]#0 : tensor<16xf32> -> !iree.list<?>
// CHECK: %[[c1_6:.+]] = constant 1 : index
// CHECK: iree.list.set %[[R8]][%[[c1_6]]], %[[RESULT]]#1 : tensor<16xf32> -> !iree.list<?>
...
// CHECK: iree.list.set %[[R9]][%[[c0_9]]], %[[RESULT]]#2 : tensor<16xf32> -> !iree.list<?>
// CHECK: %[[c1_10:.+]] = constant 1 : index
// CHECK: iree.list.set %[[R9]][%[[c1_10]]], %[[RESULT]]#3 : tensor<16xf32> -> !iree.list<?>
