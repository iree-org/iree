...
def _setup_test_debug_dir(test_name):
  ...
  test_debug_dir = os.path.join(parent, test_name)

  # Create the directory.
  try:
    os.makedirs(test_debug_dir)
  except IOError:
    logging.exception("Error creating debug dir for: %s", test_debug_dir)
  ...
  class MultiResults(named_tuple_class):
    ...
    def save(self):
      for i in range(len(self)):
        ...
        fname = os.path.join(test_debug_dir, "output_{}".format(field))
...
class SavedModelTestCase(tf.test.TestCase):
  ...
  def setUpClass(cls):
    ...
    if cls._module_ctor is not None:
      ...
      crash_reproducer_path = os.path.join(test_debug_dir, "reproducer.mlir")
