...
#include "third_party/llvm/llvm-project/mlir/test/mlir-cpu-runner/include/mlir_runner_utils.h"

#ifndef IREE_EXAMPLES_MODELBUILDER_MEMREFUTILS_H_
#define IREE_EXAMPLES_MODELBUILDER_MEMREFUTILS_H_
...
::UnrankedMemRefType<T> *allocUnrankedDescriptor(
  ...
  res->rank = shape.size();
  if (res->rank == 0)
    res->descriptor = detail::makeStridedMemRefDescriptor<T>(data, shape);
  else if (res->rank == 1)
    res->descriptor = detail::makeStridedMemRefDescriptor<T, 1>(data, shape);
  else if (res->rank == 2)
    res->descriptor = detail::makeStridedMemRefDescriptor<T, 2>(data, shape);
  else if (res->rank == 3)
    res->descriptor = detail::makeStridedMemRefDescriptor<T, 3>(data, shape);
  else if (res->rank == 4)
    res->descriptor = detail::makeStridedMemRefDescriptor<T, 4>(data, shape);
  else if (res->rank == 5)
    res->descriptor = detail::makeStridedMemRefDescriptor<T, 5>(data, shape);
  else if (res->rank == 6)
    res->descriptor = detail::makeStridedMemRefDescriptor<T, 6>(data, shape);
  else
...
#endif  // IREE_EXAMPLES_MODELBUILDER_MEMREFUTILS_H_
