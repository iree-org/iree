...
#include "third_party/iree/examples/ModelBuilder/MemRefUtils.h"
#include "third_party/iree/examples/ModelBuilder/ModelBuilder.h"
...
int main() {
  constexpr StringLiteral funcName = "test_mnist_jit";
  ...
  buildMNIST(modelBuilder, funcName, B, W0, W1, W2, W3);
  ...
  // 4. Call the funcOp name `funcName` with arguments.
  void *args[2] = {&inputBuffer->descriptor, &outputBuffer->descriptor};
  auto error =
      runner.engine->invoke(funcName, llvm::MutableArrayRef<void *>{args});
