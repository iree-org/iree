...
if __name__ == "__main__":
    ...
    b0 = 1
    b1 = 1
    q = torch.rand(b0,b1,m,n).to(torch.float32) * 2
    k = torch.rand(b0,b1,k2,n).to(torch.float32) * 3
    v = torch.rand(b0,b1,k2,n).to(torch.float32) * 4
    mask = torch.rand(b0,b1,m,k2).to(torch.float32)*0
