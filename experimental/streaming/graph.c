...
iree_status_t iree_hal_streaming_graph_create(
  ...
  *out_graph = NULL;
  ...
  iree_arena_initialize(&device->graph_block_pool, &graph->arena);
...
iree_status_t iree_hal_streaming_graph_add_kernel_node(
  ...
  // First, calculate the size needed for packed arguments.
  iree_host_size_t packed_args_size = 0;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0,
      iree_hal_streaming_pack_kernel_args(params, NULL, 0, &packed_args_size));

  // Allocate node with dependencies and packed arguments in a single
  // allocation.
  iree_hal_streaming_graph_node_t* node = NULL;
  uint8_t* extra_data = NULL;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_streaming_graph_allocate_node(
              graph->arena_allocator, dependency_count, packed_args_size, &node,
              &extra_data));
  ...
  // Pack arguments into the extra data area.
  if (packed_args_size > 0 && extra_data) {
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, iree_hal_streaming_pack_kernel_args(params, extra_data,
                                                packed_args_size, NULL));
    attrs->packed_args =
        iree_make_const_byte_span(extra_data, packed_args_size);
  } else {
    attrs->packed_args = iree_const_byte_span_empty();
  }
