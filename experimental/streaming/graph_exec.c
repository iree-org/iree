...
iree_status_t iree_hal_streaming_graph_exec_create(
  ...
  *out_exec = NULL;
  ...
  iree_arena_initialize(&context->device_entry->graph_block_pool,
  ...
  iree_status_t status = iree_hal_resource_set_allocate(
      &context->device_entry->graph_block_pool, &exec->resource_set);
    ...
    switch (node->type) {
      case IREE_HAL_STREAMING_GRAPH_NODE_TYPE_KERNEL: {
        ...
        status = iree_hal_command_buffer_dispatch(
            command_buffer, symbol->module->executable, symbol->entry_point,
            config, attrs->packed_args, iree_hal_buffer_ref_list_empty(),
            flags);
        break;
      }
...
iree_status_t iree_hal_streaming_graph_exec_instantiate_locked(
  ...
  for (iree_host_size_t p = 0; p < schedule.partition_count; p++) {
    ...
    if (p < schedule.partition_count - 1) {
      signal_semaphore_count =
          partition->stream_count ? partition->stream_count : 1;
    }
    ...
    if (partition->type == IREE_HAL_STREAMING_GRAPH_PARTITION_TYPE_RECORDABLE) {
      ...
      for (uint8_t s = 0; s < stream_count; s++) {
        ...
        // Each stream signals its own semaphore for next partition.
        ...
        if (signal_semaphore_count > 0) {
          // For single stream, signal at partition_signal_semaphore_start.
          // For multi-stream, each stream signals its own semaphore.
          ptrs.signal_semaphore_indices[0] =
              (stream_count > 1) ? (partition_signal_semaphore_start + s)
                                 : partition_signal_semaphore_start;
      ...
      if (signal_semaphore_count > 0) {
        ptrs.signal_semaphore_indices[0] = semaphore_index;
        ptrs.signal_payload_deltas[0] = 1;
        semaphore_index++;
      }
      exec->blocks[block_index++] = block;
...
iree_status_t iree_hal_streaming_graph_exec_launch(
  ...
  for (uint32_t block_index = 0;
    ...
    // Maximum semaphores per block is limited by design.
    iree_hal_semaphore_t* wait_sems[16];
    uint64_t wait_vals[16];
    iree_hal_semaphore_t* signal_sems[16];
    uint64_t signal_vals[16];
    ...
    for (uint16_t i = 0; i < block->signal_semaphore_count; i++) {
      ...
      signal_vals[signal_count] =
          exec->semaphore_base_values[semaphore_index] + delta;
      // Update base value for next launch.
      exec->semaphore_base_values[semaphore_index] = signal_vals[signal_count];
      signal_count++;
    }
  ...
  // Update stream state after submitting all blocks.
  if (iree_status_is_ok(status)) {
    stream->completed_value = stream_signal_value;
  }
