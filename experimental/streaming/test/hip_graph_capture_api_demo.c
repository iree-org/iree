...
int main(int argc, char** argv) {
  ...
  printf("HIP Graph Capture API Demo\n");
  printf("==========================\n");
  ...
  // Create a stream for graph capture and execution.
  printf("\n8. Creating stream...\n");
  hipStream_t stream;
  HIP_CHECK(hipStreamCreate(&stream));

  // Begin stream capture.
  printf("\n9. Beginning stream capture...\n");
  HIP_CHECK(hipStreamBeginCapture(stream, hipStreamCaptureModeGlobal));

  // Record kernel launch into the stream (this will be captured into the
  // graph).
  printf("\n10. Recording kernel launch (captured into graph)...\n");
  ...
  // Launch kernel (this gets captured into the graph).
  ...
  // End stream capture to get the graph.
  printf("\n11. Ending stream capture to get graph...\n");
  hipGraph_t graph;
  HIP_CHECK(hipStreamEndCapture(stream, &graph));

  // Get number of nodes in the captured graph.
  size_t num_nodes = 0;
  HIP_CHECK(hipGraphGetNodes(graph, NULL, &num_nodes));
  printf("    Captured graph contains %zu node(s)\n", num_nodes);

  // Instantiate the captured graph.
  printf("\n12. Instantiating captured graph...\n");
  ...
  printf("\n13. Launching graph on stream (first execution)...\n");
  ...
  printf("\n14. Modifying input data for second execution...\n");
  for (int i = 0; i < n; i++) {
  ...
  printf("\n15. Launching graph on stream (second execution)...\n");
  HIP_CHECK(hipGraphLaunch(graph_exec, stream));
  ...
  if (errors1 == 0 && errors2 == 0) {
    ...
    printf("    Second execution (A[i] = i*4, B[i] = i*2):\n");
    for (int i = 0; i < 3; i++) {
      printf("      C[%d] = %.2f\n", i, h_C2[i]);
    }
  }
