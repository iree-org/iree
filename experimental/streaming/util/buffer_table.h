...
// Represents a device-addressable pointer.
// May not be accessible on the host.
typedef uint64_t iree_hal_streaming_deviceptr_t;

// Represents either a host or device-addressable pointer.
// These are commonly used for mapped memory where the host and device address
// are the same or where the host address has been imported into the device
// address space.
typedef uint64_t iree_hal_streaming_any_ptr_t;
