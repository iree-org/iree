...
typedef struct iree_hal_streaming_symbol_info_t iree_hal_streaming_symbol_t;
...
typedef struct iree_hal_streaming_device_t {
  ...
  // Arena block pool for graph allocations.
  // Shared by all graphs created from this device.
  iree_arena_block_pool_t graph_block_pool;
...
typedef struct iree_hal_streaming_symbol_info_t {
...
} iree_hal_streaming_symbol_info_t;
...
typedef struct iree_hal_streaming_module_t {
  ...
  // Symbol metadata.
  iree_hal_streaming_symbol_info_t* symbols;
...
typedef struct iree_hal_streaming_graph_kernel_node_attrs_t {
  ...
  uint32_t block_dim[3];
  uint32_t shared_memory_bytes;
  iree_const_byte_span_t packed_args;  // in extra_data
...
// Helper to pack kernel arguments into a byte buffer.
