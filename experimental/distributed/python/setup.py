...
def prepare_installation():
    ...
    if not IS_CONFIGURED:
        ...
        cmake_args = [
            "-GNinja",
            "--log-level=VERBOSE",
            "-DIREE_BUILD_EXPERIMENTAL_DISTRIBUTED=ON",
        ...
        subprocess.check_call(
            [
                ...
                ".",
                "--",
                "iree_experimental_distributed_python_distributed",
            ],
            cwd=IREE_BINARY_DIR,
        )
    ...
    install_args = [
        ...
        f"-DCMAKE_INSTALL_COMPONENT=IreePythonPackage-distributed",
        "-P",
...
class CMakeBuildPy(_build_py):
    def run(self):
        ...
        shutil.copytree(
            os.path.join(CMAKE_INSTALL_DIR_ABS, "python_packages", "iree_distributed"),
            target_dir,
            symlinks=False,
        )
...
packages = find_namespace_packages(
    where=os.path.join(CMAKE_INSTALL_DIR_ABS, "python_packages", "iree_distributed"),
    include=[
        "iree.experimental.runtime.distributed",
        "iree.experimental.runtime.distributed.*",
    ],
)
print(f"Found distributed packages: {packages}")

with open(
    os.path.join(os.path.realpath(os.path.dirname(__file__)), "..", "README.md"),
    "rt",
) as f:
    README = f.read()
...
setup(
    name=f"{custom_package_prefix}iree-distributed{custom_package_suffix}{PACKAGE_SUFFIX}",
    version=f"{PACKAGE_VERSION}",
    author="IREE Authors",
    author_email="iree-discuss@googlegroups.com",
    description="Experimental IREE Python Distributed Components",
    ...
    package_dir={
        ...
        "": f"{CMAKE_INSTALL_DIR_REL}/python_packages/iree_distributed",
    },
    packages=packages,
