...
static constexpr unsigned wavefrontSize = 64;
...
static LaunchConfig getOpLaunchConfig(linalg::GenericOp op) {
  ...
  config.setWorkgroupSize({wavefrontSize, 1, 1});
...
static LaunchConfig getOpLaunchConfig(linalg::MatmulOp op) {
  LaunchConfig config;
  const int64_t numWavefronts = 2;
  std::array<int64_t, 3> workgroupSize = {numWavefronts * wavefrontSize, 1, 1};
...
Optional<LaunchConfig> getROCMLaunchConfig(
