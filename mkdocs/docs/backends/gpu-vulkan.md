...
<!-- TODO(??): support matrix of different GPUs -->
...
=== "Linux"
    ...
    SDK][vulkan-sdk]. For Ubuntu 18.04/20.04, installing via LuarG's package
...
corresponding framework' import tool to convert into a format (i.e.,
...
<!-- TODO(??): choose the suitable triple -->
...
The above assumes the exported function in the model is named as `call` and it
expects one 224x224 RGB image. We are feeding in an image with all 0 values here
for brevity, see `iree-run-module --help` for the format to specify concrete
values.
