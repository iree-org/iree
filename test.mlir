...
#layout = #iree_vector_ext.layout<<[VECTORY], [16]>, <[VECTORX], [16]>>
#layout1 = #iree_vector_ext.layout<<[VECTORY], [16]>, <[BATCHY, VECTORX], [2, 8]>>

// First propagate the layout, and then enforce it up, but there is a layout conflict on the write.
builtin.module attributes { transform.with_named_sequence } {
  func.func @propagate_and_enforce_conflict(%arr: memref<16x16xf16>, %arr2: memref<16x16xf16>, %a: vector<16x16xf16>, %b: vector<16x16xf16>) -> vector<16x16xf16> {
    %c0 = arith.constant 0 : index
    %cst_0 = arith.constant 0.0 : f16
    %cst0 = arith.constant dense<0.0> : vector<16x16xf16>
    %root = vector.transfer_read %arr[%c0, %c0], %cst_0 {in_bounds = [true, true], "__vector_layout_test_anchor_result_0" = #layout} : memref<16x16xf16>, vector<16x16xf16>
    ...
    %d = arith.addf %c, %a : vector<16x16xf16>
    %e = arith.divf %d, %root2 : vector<16x16xf16>
    vector.transfer_write %e, %arr[%c0, %c0] {in_bounds = [true, true], "__vector_layout_test_anchor_operand_0" = #layout1} : vector<16x16xf16>, memref<16x16xf16>
    func.return %e : vector<16x16xf16>
  }
